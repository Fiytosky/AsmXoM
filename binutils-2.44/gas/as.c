...
# endif
  ...
  /* fiytosky, add */
  fprintf (stream, _("\
  --dcollect          fiytosky add: collect data in .text secion by assemly frag\n"));

  fprintf (stream, _("\
  --dsplit            fiytosky add: split data in .text section by assemly frag\n"));
  ...
  enum option_values
    {
      ...
      OPTION_INFO,
      OPTION_NOINFO,
      OPTION_DCOLLECT, /* fiytosky, add. Collect Embedded Data Information */
      OPTION_DSPLIT /* fiytosky, add. Split Embedded Data */
...
# endif
    ...
    ,{"dcollect", no_argument, NULL, OPTION_DCOLLECT} /* fiytosky, add */
    ,{"dsplit", no_argument, NULL, OPTION_DSPLIT} /* fiytosky, add */
  };
...
gas_init (void)
{
  ...
  if (dsplit) {
    ...
    bfd_set_section_flags (xom_section,
			 applicable & (SEC_ALLOC | SEC_LOAD | SEC_RELOC
				       | SEC_DATA | SEC_READONLY));
    record_alignment(xom_section, 8); /* 64字节对齐 */
  }
