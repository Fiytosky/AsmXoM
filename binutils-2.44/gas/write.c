		...
		void *xxx ATTRIBUTE_UNUSED)
{
  ...
  // fiytosky, add
  bool is_xom_data_section = false;
  if (!strcmp(sec->name, ".xomdata")) {
	is_xom_data_section = true;
  }
  ...
  for (f = seginfo->frchainP->frch_root;
       ...
       f = f->fr_next)
    {
      ...
      gas_assert (f->fr_type == rs_fill || f->fr_type == rs_fill_nop || is_xom_data_section);
      if (f->fr_fix)
	{
...
static void parse_frags_for_test (void) {
	...
	for (; f != NULL; f = f->fr_next) {
		// The last frag is definitely not an embedded data frag, because subseg_finish performs the final alignment.
		if (f->fr_next == NULL) {
			break;
		}

		/*
		 * On mainstream architectures (except MIPS), the md_do_align macro is defined, and 
		 * the fill type of the .align directive is always rs_align_code.
		 */
		...
		if (!f->fr_flags.insn_frag) {
			...
			/*
			 * when the volume of embedded data is large, obstack might split it into multiple frags. 
	...
	subsegs_finish_section (xom_data_section);
	relax_segment (seg_info (xom_data_section)->frchainP->frch_root, xom_data_section, 0);
  	size_seg (stdoutput, xom_data_section, NULL);
}

// fiytosky, add. Perform code and data separation
static void split_text_frags (void) {
	// xom_section_init ();
	as_datascope (_("Split data begining"));
	segment_info_type *text_seginfo = seg_info(text_section);
	segment_info_type *xom_seginfo = seg_info(xom_section);
	...
	while (curr_frag != NULL) {
		...
		// subseg_finish appends an alignment frag to the end of the text segment's frag chain, so the last frag can be safely skipped.
		if (next_frag == NULL) {
	...
	// Separation complete, finalize the end of the .xom_data section.
	...
	// Update symbol relocation table entries
	// Merge .rela.xom into .rela.text
	fixS *fixP;
	for (fixP = text_seginfo->fix_root; fixP != NULL; fixP = fixP->fx_next) {
        if (fixP->fx_addsy != NULL && S_GET_SEGMENT(fixP->fx_addsy) == xom_section) {
            fixP->fx_pcrel = 1;
			// Tell the assembler: do not resolve locally; leave it for the linker.
            fixP->fx_done = 0; 
...
static void try_disasm_unknown_frag (void) {
	segment_info_type *info;
	info = seg_info (text_section);
	fragS *f = NULL;
	f = info->frchainP->frch_root;
...
static bool all_frags_has_checked (void) {
	segment_info_type *info;
	info = seg_info (text_section);
	fragS *f = NULL;
	f = info->frchainP->frch_root;
...
write_object_file (void)
{
  ...
  if (dsplit) {
	...
	// handle unknown frags
	try_disasm_unknown_frag ();

	// Fallback: Perform separation only if mixd_frag and unknown_frag do not exist.
	if (all_frags_has_checked ()) {
		split_text_frags ();
	}
  }
  ...
  // fiytosky, add. Retrieve frag information from the .text section
	if (dcollect) {
		// dump_text_frags (1);
		const char *filename;
