...
struct local_symbol
{
  ...
  // fiytosky, add 
  bool call_flow_symbol; /* Whether the symbol is referenced by a control flow transfer instruction (e.g., j<cc>, call) */
  bool memory_access_symbol; /* Whether the symbol is referenced by a memory access instruction */
  bool bind_bytes; /* Whether the symbol is bound to hardcoded bytes */
};
       ...
       /* We copy this string: OK to alter later.  */)
{
  ...
  // fiytosky, add
  // Check whether to switch frags
  if ((dcollect || dsplit) && now_seg == text_section && !is_in_func) {
    if (frag_now_fix () > 0) {
      frag_wane (frag_now);
      frag_new (0);
