...
#include "dis-asm.h" /* fiytosky, add */
...
// fiytosky, add. disasm frag
/* State shared by the libopcodes callbacks while one instruction is being
   decoded.  The text is retained because the i386 disassembler represents
   some invalid encodings with a normal positive length and a "(bad)" marker.  */

struct i386_frag_disasm_state
{
  char *text;
  size_t length;
  size_t capacity;
...
static void
i386_frag_disasm_reserve (struct i386_frag_disasm_state *state, size_t extra)
{
  size_t needed;
  size_t capacity;

  if (extra > SIZE_MAX - state->length - 1)
    xmalloc_failed (SIZE_MAX);

  needed = state->length + extra + 1;
  if (needed <= state->capacity)
    return;

  capacity = state->capacity == 0 ? 128 : state->capacity;
  ...
  state->text = XRESIZEVEC (char, state->text, capacity);
  state->capacity = capacity;
}

static int
i386_frag_disasm_vprintf (struct i386_frag_disasm_state *state,
			  const char *format, va_list args)
{
  ...
  i386_frag_disasm_reserve (state, (size_t) required);
  va_copy (copy, args);
  vsnprintf (state->text + state->length,
	     state->capacity - state->length, format, copy);
  va_end (copy);
  state->length += (size_t) required;
  return required;
}

static int ATTRIBUTE_PRINTF_2
i386_frag_disasm_printf (void *stream, const char *format, ...)
{
  struct i386_frag_disasm_state *state = stream;
  va_list args;
  int result;

  va_start (args, format);
  result = i386_frag_disasm_vprintf (state, format, args);
  va_end (args);
  return result;
}

static int ATTRIBUTE_PRINTF_3
i386_frag_disasm_styled_printf (void *stream,
				...
				const char *format, ...)
{
  ...
  if (style == dis_style_mnemonic)
    state->saw_mnemonic = true;
  else if (style == dis_style_assembler_directive)
    state->saw_directive = true;
...
static int
i386_frag_disasm_read_memory (bfd_vma address, bfd_byte *destination,
			      unsigned int length,
			      struct disassemble_info *info)
{
  struct i386_frag_disasm_state *state = info->application_data;
  bfd_vma offset;

  if (address < info->buffer_vma)
    goto failure;
  offset = address - info->buffer_vma;
  if (offset > info->buffer_length
      || length > info->buffer_length - (size_t) offset)
    goto failure;

  memcpy (destination, info->buffer + (size_t) offset, length);
  return 0;
...
static void
i386_frag_disasm_memory_error (int status ATTRIBUTE_UNUSED,
...
static void
i386_frag_disasm_print_address (bfd_vma address ATTRIBUTE_UNUSED,
				struct disassemble_info *info ATTRIBUTE_UNUSED)
{
}

static void
i386_frag_disasm_reset (struct i386_frag_disasm_state *state)
{
  state->length = 0;
  state->saw_mnemonic = false;
  state->saw_directive = false;
  state->read_failed = false;
  i386_frag_disasm_reserve (state, 0);
  state->text[0] = '\0';
}

/* Classify all fixed bytes in FRAGP with the x86-64 libopcodes decoder.

   This function must be called after the frag has been finalized, so fr_fix
   is the number of bytes in fr_literal.  Calling it for frag_now is also
   supported; in that case the current obstack size is used.  Only the fixed
   part is inspected: fr_var is target-specific metadata and does not reliably
   describe additional materialized bytes.

   A successful decode must consume every byte as a real instruction.  A
   truncated instruction, a decoder-generated data directive, or an invalid
   opcode marker makes the whole frag mixed.  */

enum i386_frag_byte_kind
i386_classify_hardcoded_frag (fragS *fragP)
{
  struct i386_frag_disasm_state state;
  struct disassemble_info info;
  disassembler_ftype decoder;
  enum i386_frag_byte_kind result = I386_FRAG_BYTES_MIXED;
  addressT fixed_octets;
  size_t byte_count;
  size_t offset;
  ...
  fragP->tc_frag_data.byte_kind = result;
  if (fragP->tc_frag_data.code != CODE_64BIT)
    return result;

  fixed_octets = (fragP == frag_now
		  ? frag_now_fix_octets () : fragP->fr_fix);
  byte_count = (size_t) fixed_octets;
  if ((addressT) byte_count != fixed_octets || byte_count == 0)
    return result;

  decoder = disassembler (bfd_arch_i386, false, bfd_mach_x86_64, NULL);
  if (decoder == NULL)
    return result;

  memset (&state, 0, sizeof (state));
  init_disassemble_info (&info, &state, i386_frag_disasm_printf,
			 i386_frag_disasm_styled_printf);
  info.application_data = &state;
  ...
  info.buffer = (bfd_byte *) fragP->fr_literal;
  info.buffer_vma = 0;
  info.buffer_length = byte_count;
  info.stop_vma = byte_count;
  info.stop_offset = byte_count;
  info.read_memory_func = i386_frag_disasm_read_memory;
  info.memory_error_func = i386_frag_disasm_memory_error;
  info.print_address_func = i386_frag_disasm_print_address;
  disassemble_init_for_target (&info);

  for (offset = 0; offset < byte_count;)
    {
      size_t remaining = byte_count - offset;
      int decoded_length;

      i386_frag_disasm_reset (&state);
      decoded_length = decoder ((bfd_vma) offset, &info);
      if (decoded_length <= 0
	  || (size_t) decoded_length > remaining
	  || state.read_failed
	  || !state.saw_mnemonic
	  || state.saw_directive
	  || strstr (state.text, "(bad)") != NULL
	  || strstr (state.text, "{bad") != NULL
	  || strstr (state.text, "/bad") != NULL)
	goto done;

      offset += (size_t) decoded_length;
    }

  result = I386_FRAG_BYTES_INSN;
...
// fiytosky, add
// Update symbols in control flow instructions
static void
  ...
  if (i.tm.opcode_modifier.jump == JUMP)
	      ...
	      || i.tm.base_opcode == 0xaef8))
	{
	  ...
	  if (flag_code == CODE_16BIT)
        ...
        // fiytosky, add
        // Mark all frags that are instructions
        if (dcollect || dsplit) {
          // The design ensures that insn_start_frag is always an insn frag.
          fragS* frag_tmp = NULL;
