...
frag_grow (size_t nchars)
{
  if (obstack_room (&frchain_now->frch_obstack) < nchars)
	...
	obstack_chunk_size (&frchain_now->frch_obstack) = newc;
      ...
      // fiytosky, add
      // There are cases in the assembly file where an insn_frag is forced to split due to insufficient room. 
      // If both before and after the split point are explicit instructions, this is fine.
      // However, there is a small probability that the split point occurs between an insn and a .byte. 
      // This will cause the .byte not to be marked as an insn_frag, leading to a false positive.
      // An insn_frag is not closed unless a control-flow transfer instruction occurs.
      // Therefore, when frag_now is marked as an insn_frag and is not closed, all frags 
      // generated by frag_grow should be considered insn_frags.
      ...
      /*
          ...
          .byte	0xf3,0xc3     <-- False positive point
        .cfi_endproc	
	     ...
	     variable_length part of frag.  */)
{
  ...
  frag_now->fr_flags.unknown_frag = 1; /* default unknown */
}

...
frag_align_code (int alignment, int max)
{
  ...
  /* fiytosky, add */
  // During the creation of an align frag, multiple frags with a size of 0
  // might be created sequentially due to insufficient obstack chunk size.
  // In this case, the alignment needs to be propagated to the frag before frag_now.
