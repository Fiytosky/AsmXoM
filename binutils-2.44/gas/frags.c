  ...
  if (obstack_room (&frchain_now->frch_obstack) < nchars)
	...
	obstack_chunk_size (&frchain_now->frch_obstack) = newc;
      ...
      */
      ...
      if ((dcollect || dsplit) && frag_now->fr_flags.insn_frag == 1 && frag_now_fix () != frag_now->fr_fix) {
	     ...
	     variable_length part of frag.  */)
{
  ...
  frag_now->frag_has_hardcode = false;
  frag_now->hardcode_size = 0;
}

...
frag_align_code (int alignment, int max)
{
  ...
  if ((dcollect || dsplit) && fr != frag_now && !is_finish_subseg) {
