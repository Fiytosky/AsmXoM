...
# 27 "../fuzz/pem.c"
    ...
    (void)(((size_t)BIO_write(in, buf + 1, len - 1) == len - 1) ? 0 : (OPENSSL_die("assertion failed: " "(size_t)BIO_write(in, buf + 1, len - 1) == len - 1", "../fuzz/pem.c", 34), 1));
 ...
 BIO_write(in, data, outlen);
    ...
    if (buf[0] & 0x1) {
        CRYPTO_secure_free(name, "../fuzz/pem.c", 42);
        CRYPTO_secure_free(header, "../fuzz/pem.c", 43);
        CRYPTO_secure_free(data, "../fuzz/pem.c", 44);
    } else {
        CRYPTO_free(name, "../fuzz/pem.c", 46);
        CRYPTO_free(header, "../fuzz/pem.c", 47);
        CRYPTO_free(data, "../fuzz/pem.c", 48);
    }
