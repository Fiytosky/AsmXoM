...
# 27 "../fuzz/smime.c"
        ...
        struct stack_st_PKCS7_SIGNER_INFO *p7si = PKCS7_get_signer_info(p7);
        int i;

        for (i = 0; i < OPENSSL_sk_num(ossl_check_const_PKCS7_SIGNER_INFO_sk_type(p7si)); i++) {
            struct stack_st_X509_ALGOR *algs;

            PKCS7_cert_from_signer_info(p7,
                                        ((PKCS7_SIGNER_INFO *)OPENSSL_sk_value(ossl_check_const_PKCS7_SIGNER_INFO_sk_type(p7si), (i))));
            algs = PKCS7_get_smimecap(((PKCS7_SIGNER_INFO *)OPENSSL_sk_value(ossl_check_const_PKCS7_SIGNER_INFO_sk_type(p7si), (i))));
            OPENSSL_sk_pop_free(ossl_check_X509_ALGOR_sk_type(algs),ossl_check_X509_ALGOR_freefunc_type(X509_ALGOR_free));
        }
        PKCS7_free(p7);
    }
