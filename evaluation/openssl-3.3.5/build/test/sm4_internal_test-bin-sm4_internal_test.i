# 0 "../test/sm4_internal_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/sm4_internal_test.c"
# 15 "../test/sm4_internal_test.c"
...
static int test_sm4_ecb(void)
{
    ...
    if (!test_mem_eq("../test/sm4_internal_test.c", 61, "block", "expected", block, 16, expected, 16))
        return 0;

    for (i = 0; i != 999999; ++i)
        ossl_sm4_encrypt(block, block, &key);

    if (!test_mem_eq("../test/sm4_internal_test.c", 67, "block", "expected_iter", block, 16, expected_iter, 16))
        return 0;

    for (i = 0; i != 1000000; ++i)
        ossl_sm4_decrypt(block, block, &key);

    if (!test_mem_eq("../test/sm4_internal_test.c", 73, "block", "input", block, 16, input, 16))
        return 0;
...
int setup_tests(void)
{
    ...
    add_test("test_sm4_ecb", test_sm4_ecb);
