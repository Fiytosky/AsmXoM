# 0 "../test/quic_stream_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/quic_stream_test.c"
# 9 "../test/quic_stream_test.c"
...
# 15 "../include/internal/quic_stream.h" 2
...
# 43 "../test/quic_stream_test.c" 3 4
                           ((void *)0)
# 43 "../test/quic_stream_test.c"
                               ;
    OSSL_QUIC_FRAME_STREAM hdr;
    OSSL_QTX_IOVEC iov[2];
    size_t num_iov = 0, wr = 0, i, init_size = 8192;

    if (!test_ptr("../test/quic_stream_test.c", 48, "sstream = ossl_quic_sstream_new(init_size)", sstream = ossl_quic_sstream_new(init_size)))
        goto err;


    if (!test_true("../test/quic_stream_test.c", 52, "ossl_quic_sstream_is_totally_acked(sstream)", (ossl_quic_sstream_is_totally_acked(sstream)) != 0))
        goto err;


    num_iov = (sizeof(iov)/sizeof((iov)[0]));
    if (!test_false("../test/quic_stream_test.c", 57, "ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)", (ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)) != 0)
                                                                 )
        goto err;


    if (!test_true("../test/quic_stream_test.c", 62, "ossl_quic_sstream_append(sstream, data_1, sizeof(data_1), &wr)", (ossl_quic_sstream_append(sstream, data_1, sizeof(data_1), &wr)) != 0)

        || !test_size_t_eq("../test/quic_stream_test.c", 64, "wr", "sizeof(data_1)", wr, sizeof(data_1)))
        goto err;


    if (!test_false("../test/quic_stream_test.c", 68, "ossl_quic_sstream_is_totally_acked(sstream)", (ossl_quic_sstream_is_totally_acked(sstream)) != 0))
        goto err;


    num_iov = (sizeof(iov)/sizeof((iov)[0]));
    if (!test_true("../test/quic_stream_test.c", 73, "ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)", (ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)) != 0)

        || !test_size_t_gt("../test/quic_stream_test.c", 75, "num_iov", "0", num_iov, 0)
        || !test_uint64_t_eq("../test/quic_stream_test.c", 76, "hdr.offset", "0", hdr.offset, 0)
        || !test_uint64_t_eq("../test/quic_stream_test.c", 77, "hdr.len", "sizeof(data_1)", hdr.len, sizeof(data_1))
        || !test_false("../test/quic_stream_test.c", 78, "hdr.is_fin", (hdr.is_fin) != 0))
        goto err;

    if (!test_true("../test/quic_stream_test.c", 81, "compare_iov(data_1, sizeof(data_1), iov, num_iov)", (compare_iov(data_1, sizeof(data_1), iov, num_iov)) != 0))
        goto err;


    if (!test_true("../test/quic_stream_test.c", 85, "ossl_quic_sstream_mark_transmitted(sstream, 0, 7)", (ossl_quic_sstream_mark_transmitted(sstream, 0, 7)) != 0))
        goto err;


    num_iov = (sizeof(iov)/sizeof((iov)[0]));
    if (!test_true("../test/quic_stream_test.c", 90, "ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)", (ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)) != 0)

        || !test_size_t_gt("../test/quic_stream_test.c", 92, "num_iov", "0", num_iov, 0)
        || !test_uint64_t_eq("../test/quic_stream_test.c", 93, "hdr.offset", "8", hdr.offset, 8)
        || !test_uint64_t_eq("../test/quic_stream_test.c", 94, "hdr.len", "sizeof(data_1) - 8", hdr.len, sizeof(data_1) - 8)
        || !test_false("../test/quic_stream_test.c", 95, "hdr.is_fin", (hdr.is_fin) != 0))
        goto err;

    if (!test_true("../test/quic_stream_test.c", 98, "compare_iov(data_1 + 8, sizeof(data_1) - 8, iov, num_iov)", (compare_iov(data_1 + 8, sizeof(data_1) - 8, iov, num_iov)) != 0))
        goto err;

    if (!test_true("../test/quic_stream_test.c", 101, "ossl_quic_sstream_mark_transmitted(sstream, 8, 15)", (ossl_quic_sstream_mark_transmitted(sstream, 8, 15)) != 0))
        goto err;


    num_iov = (sizeof(iov)/sizeof((iov)[0]));
    if (!test_false("../test/quic_stream_test.c", 106, "ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)", (ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)) != 0)
                                                                 )
        goto err;


    if (!test_true("../test/quic_stream_test.c", 111, "ossl_quic_sstream_mark_lost(sstream, 4, 6)", (ossl_quic_sstream_mark_lost(sstream, 4, 6)) != 0))
        goto err;


    num_iov = (sizeof(iov)/sizeof((iov)[0]));
    if (!test_true("../test/quic_stream_test.c", 116, "ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)", (ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)) != 0)

        || !test_size_t_gt("../test/quic_stream_test.c", 118, "num_iov", "0", num_iov, 0)
        || !test_uint64_t_eq("../test/quic_stream_test.c", 119, "hdr.offset", "4", hdr.offset, 4)
        || !test_uint64_t_eq("../test/quic_stream_test.c", 120, "hdr.len", "3", hdr.len, 3)
        || !test_false("../test/quic_stream_test.c", 121, "hdr.is_fin", (hdr.is_fin) != 0))
        goto err;

    if (!test_true("../test/quic_stream_test.c", 124, "compare_iov(data_1 + 4, 3, iov, num_iov)", (compare_iov(data_1 + 4, 3, iov, num_iov)) != 0))
        goto err;


    if (!test_true("../test/quic_stream_test.c", 128, "ossl_quic_sstream_mark_transmitted(sstream, 4, 6)", (ossl_quic_sstream_mark_transmitted(sstream, 4, 6)) != 0))
        goto err;


    num_iov = (sizeof(iov)/sizeof((iov)[0]));
    if (!test_false("../test/quic_stream_test.c", 133, "ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)", (ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)) != 0)
                                                                 )
        goto err;

    if (!test_size_t_eq("../test/quic_stream_test.c", 137, "ossl_quic_sstream_get_buffer_used(sstream)", "16", ossl_quic_sstream_get_buffer_used(sstream), 16))
        goto err;


    if (!test_true("../test/quic_stream_test.c", 141, "ossl_quic_sstream_mark_acked(sstream, 1, 7)", (ossl_quic_sstream_mark_acked(sstream, 1, 7)) != 0)
        || !test_size_t_eq("../test/quic_stream_test.c", 142, "ossl_quic_sstream_get_buffer_used(sstream)", "16", ossl_quic_sstream_get_buffer_used(sstream), 16))
        goto err;


    if (!test_true("../test/quic_stream_test.c", 146, "ossl_quic_sstream_mark_acked(sstream, 0, 0)", (ossl_quic_sstream_mark_acked(sstream, 0, 0)) != 0)
        || !test_size_t_eq("../test/quic_stream_test.c", 147, "ossl_quic_sstream_get_buffer_used(sstream)", "8", ossl_quic_sstream_get_buffer_used(sstream), 8))
        goto err;

    if (!test_true("../test/quic_stream_test.c", 150, "ossl_quic_sstream_mark_acked(sstream, 0, 15)", (ossl_quic_sstream_mark_acked(sstream, 0, 15)) != 0)
        || !test_size_t_eq("../test/quic_stream_test.c", 151, "ossl_quic_sstream_get_buffer_used(sstream)", "0", ossl_quic_sstream_get_buffer_used(sstream), 0))
        goto err;
    ...
    for (i = 0; i < 2; ++i) {
        num_iov = (sizeof(iov)/sizeof((iov)[0]));
        if (!test_true("../test/quic_stream_test.c", 160, "ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)", (ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)) != 0)

            || !test_uint64_t_eq("../test/quic_stream_test.c", 162, "hdr.offset", "16", hdr.offset, 16)
            || !test_uint64_t_eq("../test/quic_stream_test.c", 163, "hdr.len", "0", hdr.len, 0)
            || !test_true("../test/quic_stream_test.c", 164, "hdr.is_fin", (hdr.is_fin) != 0)
            || !test_size_t_eq("../test/quic_stream_test.c", 165, "num_iov", "0", num_iov, 0))
            goto err;
    }

    if (!test_true("../test/quic_stream_test.c", 169, "ossl_quic_sstream_mark_transmitted_fin(sstream, 16)", (ossl_quic_sstream_mark_transmitted_fin(sstream, 16)) != 0))
        goto err;


    num_iov = (sizeof(iov)/sizeof((iov)[0]));
    if (!test_false("../test/quic_stream_test.c", 174, "ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)", (ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)) != 0)
                                                                 )
        goto err;


    if (!test_true("../test/quic_stream_test.c", 179, "ossl_quic_sstream_mark_lost_fin(sstream)", (ossl_quic_sstream_mark_lost_fin(sstream)) != 0))
        goto err;


    for (i = 0; i < 2; ++i) {
        num_iov = (sizeof(iov)/sizeof((iov)[0]));
        if (!test_true("../test/quic_stream_test.c", 185, "ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)", (ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)) != 0)

            || !test_uint64_t_eq("../test/quic_stream_test.c", 187, "hdr.offset", "16", hdr.offset, 16)
            || !test_uint64_t_eq("../test/quic_stream_test.c", 188, "hdr.len", "0", hdr.len, 0)
            || !test_true("../test/quic_stream_test.c", 189, "hdr.is_fin", (hdr.is_fin) != 0)
            || !test_size_t_eq("../test/quic_stream_test.c", 190, "num_iov", "0", num_iov, 0))
            goto err;
    }

    if (!test_true("../test/quic_stream_test.c", 194, "ossl_quic_sstream_mark_transmitted_fin(sstream, 16)", (ossl_quic_sstream_mark_transmitted_fin(sstream, 16)) != 0))
        goto err;


    num_iov = (sizeof(iov)/sizeof((iov)[0]));
    if (!test_false("../test/quic_stream_test.c", 199, "ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)", (ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)) != 0)
                                                                 )
        goto err;


    if (!test_true("../test/quic_stream_test.c", 204, "ossl_quic_sstream_mark_acked_fin(sstream)", (ossl_quic_sstream_mark_acked_fin(sstream)) != 0))
        goto err;

    if (!test_true("../test/quic_stream_test.c", 207, "ossl_quic_sstream_is_totally_acked(sstream)", (ossl_quic_sstream_is_totally_acked(sstream)) != 0))
        goto err;
...
static int test_sstream_bulk(int idx)
{
    int testresult = 0;
    QUIC_SSTREAM *sstream = 
# 219 "../test/quic_stream_test.c" 3 4
                           ((void *)0)
# 219 "../test/quic_stream_test.c"
                               ;
    OSSL_QUIC_FRAME_STREAM hdr;
    OSSL_QTX_IOVEC iov[2];
...
# 224 "../test/quic_stream_test.c" 3 4
                            ((void *)0)
# 224 "../test/quic_stream_test.c"
                                , *dst_buf = 
# 224 "../test/quic_stream_test.c" 3 4
                                             ((void *)0)
# 224 "../test/quic_stream_test.c"
                                                 ;
    unsigned char *ref_src_buf = 
# 225 "../test/quic_stream_test.c" 3 4
                                ((void *)0)
# 225 "../test/quic_stream_test.c"
                                    , *ref_dst_buf = 
# 225 "../test/quic_stream_test.c" 3 4
                                                     ((void *)0)
# 225 "../test/quic_stream_test.c"
    ...
    if (!test_ptr("../test/quic_stream_test.c", 228, "sstream = ossl_quic_sstream_new(init_size)", sstream = ossl_quic_sstream_new(init_size)))
        goto err;

    if (!test_size_t_eq("../test/quic_stream_test.c", 231, "ossl_quic_sstream_get_buffer_size(sstream)", "init_size", ossl_quic_sstream_get_buffer_size(sstream), init_size))
        goto err;

    if (!test_ptr("../test/quic_stream_test.c", 234, "src_buf = OPENSSL_zalloc(init_size)", src_buf = CRYPTO_zalloc(init_size, "../test/quic_stream_test.c", 234)))
        goto err;

    if (!test_ptr("../test/quic_stream_test.c", 237, "dst_buf = OPENSSL_malloc(init_size)", dst_buf = CRYPTO_malloc(init_size, "../test/quic_stream_test.c", 237)))
        goto err;

    if (!test_ptr("../test/quic_stream_test.c", 240, "ref_src_buf = OPENSSL_malloc(init_size)", ref_src_buf = CRYPTO_malloc(init_size, "../test/quic_stream_test.c", 240)))
        goto err;

    if (!test_ptr("../test/quic_stream_test.c", 243, "ref_dst_buf = OPENSSL_malloc(init_size)", ref_dst_buf = CRYPTO_malloc(init_size, "../test/quic_stream_test.c", 243)))
        goto err;




    if (!test_true("../test/quic_stream_test.c", 249, "ossl_quic_sstream_append(sstream, src_buf, init_size / 2, &consumed)", (ossl_quic_sstream_append(sstream, src_buf, init_size / 2, &consumed)) != 0)

        || !test_size_t_eq("../test/quic_stream_test.c", 251, "consumed", "init_size / 2", consumed, init_size / 2)
        || !test_true("../test/quic_stream_test.c", 252, "ossl_quic_sstream_mark_transmitted(sstream, 0, init_size / 2 - 1)", (ossl_quic_sstream_mark_transmitted(sstream, 0, init_size / 2 - 1)) != 0)

        || !test_true("../test/quic_stream_test.c", 254, "ossl_quic_sstream_mark_acked(sstream, 0, init_size / 2 - 1)", (ossl_quic_sstream_mark_acked(sstream, 0, init_size / 2 - 1)) != 0)
                                                                      )
        goto err;
    ...
    do {
        ...
        if (!test_true("../test/quic_stream_test.c", 268, "ossl_quic_sstream_append(sstream, src_buf, l, &consumed)", (ossl_quic_sstream_append(sstream, src_buf, l, &consumed)) != 0))
            goto err;
    ...
    if (!test_size_t_eq("../test/quic_stream_test.c", 276, "ossl_quic_sstream_get_buffer_used(sstream)", "init_size", ossl_quic_sstream_get_buffer_used(sstream), init_size)
        || !test_size_t_eq("../test/quic_stream_test.c", 277, "ossl_quic_sstream_get_buffer_avail(sstream)", "0", ossl_quic_sstream_get_buffer_avail(sstream), 0))
        goto err;
    ...
    for (i = 0; i < total_written; ++i) {
        if ((test_random() & 1) != 0) {
            *ref_dst_cur++ = *ref_src_cur;
            ++expected;
        } else if (!test_true("../test/quic_stream_test.c", 291, "ossl_quic_sstream_mark_transmitted(sstream, start_at + i, start_at + i)", (ossl_quic_sstream_mark_transmitted(sstream, start_at + i, start_at + i)) != 0)
    ...
    if (!test_true("../test/quic_stream_test.c", 300, "ossl_quic_sstream_set_buffer_size(sstream, init_size * 2)", (ossl_quic_sstream_set_buffer_size(sstream, init_size * 2)) != 0)
        || !test_true("../test/quic_stream_test.c", 301, "ossl_quic_sstream_set_buffer_size(sstream, init_size)", (ossl_quic_sstream_set_buffer_size(sstream, init_size)) != 0))
        goto err;
    ...
    for (i = 0, rd = 0; rd < expected; ++i) {
        num_iov = (sizeof(iov)/sizeof((iov)[0]));
        if (!test_true("../test/quic_stream_test.c", 308, "ossl_quic_sstream_get_stream_frame(sstream, i, &hdr, iov, &num_iov)", (ossl_quic_sstream_get_stream_frame(sstream, i, &hdr, iov, &num_iov)) != 0)
                                                                    )
            goto err;

        cur_rd = 0;
        for (j = 0; j < num_iov; ++j) {
            if (!test_size_t_le("../test/quic_stream_test.c", 314, "iov[j].buf_len + rd", "expected", iov[j].buf_len + rd, expected))
                goto err;
        ...
        if (!test_uint64_t_eq("../test/quic_stream_test.c", 322, "cur_rd", "hdr.len", cur_rd, hdr.len))
            goto err;

        rd += cur_rd;
    }

    if (!test_mem_eq("../test/quic_stream_test.c", 328, "dst_buf", "ref_dst_buf", dst_buf, rd, ref_dst_buf, expected))
        goto err;

    testresult = 1;
 err:
    CRYPTO_free(src_buf, "../test/quic_stream_test.c", 333);
    CRYPTO_free(dst_buf, "../test/quic_stream_test.c", 334);
    CRYPTO_free(ref_src_buf, "../test/quic_stream_test.c", 335);
    CRYPTO_free(ref_dst_buf, "../test/quic_stream_test.c", 336);
    ossl_quic_sstream_free(sstream);
    return testresult;
}
...
# 378 "../test/quic_stream_test.c" 3 4
                           ((void *)0)
# 378 "../test/quic_stream_test.c"
                               ;
    int ret = 0;
    ...
    if (!test_ptr("../test/quic_stream_test.c", 389, "rstream = ossl_quic_rstream_new(NULL, NULL, 0)", rstream = ossl_quic_rstream_new(
# 389 "../test/quic_stream_test.c" 3 4
        ((void *)0)
# 389 "../test/quic_stream_test.c"
        , 
# 389 "../test/quic_stream_test.c" 3 4
        ((void *)0)
# 389 "../test/quic_stream_test.c"
    ...
    if (!test_true("../test/quic_stream_test.c", 392, "ossl_quic_rstream_queue_data(rstream, NULL, 5, simple_data + 5, 10, 0)", (ossl_quic_rstream_queue_data(rstream, 
# 392 "../test/quic_stream_test.c" 3 4
        ((void *)0)
# 392 "../test/quic_stream_test.c"
        , 5, simple_data + 5, 10, 0)) != 0)

        || !test_true("../test/quic_stream_test.c", 394, "ossl_quic_rstream_queue_data(rstream, NULL, sizeof(simple_data) - 1, simple_data + sizeof(simple_data) - 1, 1, 1)", (ossl_quic_rstream_queue_data(rstream, 
# 394 "../test/quic_stream_test.c" 3 4
           ((void *)0)
# 394 "../test/quic_stream_test.c"
           , sizeof(simple_data) - 1, simple_data + sizeof(simple_data) - 1, 1, 1)) != 0)



        || !test_true("../test/quic_stream_test.c", 398, "ossl_quic_rstream_peek(rstream, buf, sizeof(buf), &readbytes, &fin)", (ossl_quic_rstream_peek(rstream, buf, sizeof(buf), &readbytes, &fin)) != 0)

        || !test_false("../test/quic_stream_test.c", 400, "fin", (fin) != 0)
        || !test_size_t_eq("../test/quic_stream_test.c", 401, "readbytes", "0", readbytes, 0)
        || !test_true("../test/quic_stream_test.c", 402, "ossl_quic_rstream_queue_data(rstream, NULL, sizeof(simple_data) - 10, simple_data + sizeof(simple_data) - 10, 10, 1)", (ossl_quic_rstream_queue_data(rstream, 
# 402 "../test/quic_stream_test.c" 3 4
           ((void *)0)
# 402 "../test/quic_stream_test.c"
           , sizeof(simple_data) - 10, simple_data + sizeof(simple_data) - 10, 10, 1)) != 0)



        || !test_true("../test/quic_stream_test.c", 406, "ossl_quic_rstream_queue_data(rstream, NULL, 0, simple_data, 1, 0)", (ossl_quic_rstream_queue_data(rstream, 
# 406 "../test/quic_stream_test.c" 3 4
           ((void *)0)
# 406 "../test/quic_stream_test.c"
           , 0, simple_data, 1, 0)) != 0)

        || !test_true("../test/quic_stream_test.c", 408, "ossl_quic_rstream_peek(rstream, buf, sizeof(buf), &readbytes, &fin)", (ossl_quic_rstream_peek(rstream, buf, sizeof(buf), &readbytes, &fin)) != 0)

        || !test_false("../test/quic_stream_test.c", 410, "fin", (fin) != 0)
        || !test_size_t_eq("../test/quic_stream_test.c", 411, "readbytes", "1", readbytes, 1)
        || !test_mem_eq("../test/quic_stream_test.c", 412, "buf", "simple_data", buf, 1, simple_data, 1)
        || (use_rbuf && !test_false("../test/quic_stream_test.c", 413, "ossl_quic_rstream_move_to_rbuf(rstream)", (ossl_quic_rstream_move_to_rbuf(rstream)) != 0))
        || (use_rbuf
            && !test_true("../test/quic_stream_test.c", 415, "ossl_quic_rstream_resize_rbuf(rstream, sizeof(simple_data))", (ossl_quic_rstream_resize_rbuf(rstream, sizeof(simple_data))) != 0)
                                                                             )
        || (use_rbuf && !test_true("../test/quic_stream_test.c", 417, "ossl_quic_rstream_move_to_rbuf(rstream)", (ossl_quic_rstream_move_to_rbuf(rstream)) != 0))
        || !test_true("../test/quic_stream_test.c", 418, "ossl_quic_rstream_queue_data(rstream, NULL, 0, simple_data, 10, 0)", (ossl_quic_rstream_queue_data(rstream, 
# 418 "../test/quic_stream_test.c" 3 4
           ((void *)0)
# 418 "../test/quic_stream_test.c"
           , 0, simple_data, 10, 0)) != 0)


        || !test_true("../test/quic_stream_test.c", 421, "ossl_quic_rstream_queue_data(rstream, NULL, sizeof(simple_data), NULL, 0, 1)", (ossl_quic_rstream_queue_data(rstream, 
# 421 "../test/quic_stream_test.c" 3 4
           ((void *)0)
# 421 "../test/quic_stream_test.c"
           , sizeof(simple_data), 
# 421 "../test/quic_stream_test.c" 3 4
           ((void *)0)
# 421 "../test/quic_stream_test.c"
        ...
        || !test_true("../test/quic_stream_test.c", 425, "ossl_quic_rstream_peek(rstream, buf, sizeof(buf), &readbytes, &fin)", (ossl_quic_rstream_peek(rstream, buf, sizeof(buf), &readbytes, &fin)) != 0)

        || !test_false("../test/quic_stream_test.c", 427, "fin", (fin) != 0)
        || !test_size_t_eq("../test/quic_stream_test.c", 428, "readbytes", "15", readbytes, 15)
        || !test_mem_eq("../test/quic_stream_test.c", 429, "buf", "simple_data", buf, 15, simple_data, 15)
        || !test_true("../test/quic_stream_test.c", 430, "ossl_quic_rstream_queue_data(rstream, NULL, 15, simple_data + 15, sizeof(simple_data) - 15, 1)", (ossl_quic_rstream_queue_data(rstream, 
# 430 "../test/quic_stream_test.c" 3 4
           ((void *)0)
# 430 "../test/quic_stream_test.c"
           , 15, simple_data + 15, sizeof(simple_data) - 15, 1)) != 0)



        || !test_true("../test/quic_stream_test.c", 434, "ossl_quic_rstream_available(rstream, &avail, &fin)", (ossl_quic_rstream_available(rstream, &avail, &fin)) != 0)
        || !test_true("../test/quic_stream_test.c", 435, "fin", (fin) != 0)
        || !test_size_t_eq("../test/quic_stream_test.c", 436, "avail", "sizeof(simple_data)", avail, sizeof(simple_data))
        || !test_true("../test/quic_stream_test.c", 437, "read_fn(rstream, buf, 2, &readbytes, &fin)", (read_fn(rstream, buf, 2, &readbytes, &fin)) != 0)
        || !test_false("../test/quic_stream_test.c", 438, "fin", (fin) != 0)
        || !test_size_t_eq("../test/quic_stream_test.c", 439, "readbytes", "2", readbytes, 2)
        || !test_mem_eq("../test/quic_stream_test.c", 440, "buf", "simple_data", buf, 2, simple_data, 2)
        || !test_true("../test/quic_stream_test.c", 441, "read_fn(rstream, buf + 2, 12, &readbytes, &fin)", (read_fn(rstream, buf + 2, 12, &readbytes, &fin)) != 0)
        || !test_false("../test/quic_stream_test.c", 442, "fin", (fin) != 0)
        || !test_size_t_eq("../test/quic_stream_test.c", 443, "readbytes", "12", readbytes, 12)
        || !test_mem_eq("../test/quic_stream_test.c", 444, "buf + 2", "simple_data + 2", buf + 2, 12, simple_data + 2, 12)
        || !test_true("../test/quic_stream_test.c", 445, "ossl_quic_rstream_queue_data(rstream, NULL, sizeof(simple_data), NULL, 0, 1)", (ossl_quic_rstream_queue_data(rstream, 
# 445 "../test/quic_stream_test.c" 3 4
           ((void *)0)
# 445 "../test/quic_stream_test.c"
           , sizeof(simple_data), 
# 445 "../test/quic_stream_test.c" 3 4
           ((void *)0)
# 445 "../test/quic_stream_test.c"
        ...
        || (use_rbuf
            && !test_true("../test/quic_stream_test.c", 450, "ossl_quic_rstream_resize_rbuf(rstream, 2 * sizeof(simple_data))", (ossl_quic_rstream_resize_rbuf(rstream, 2 * sizeof(simple_data))) != 0)
                                                                                 )
        || (use_rbuf && !test_true("../test/quic_stream_test.c", 452, "ossl_quic_rstream_move_to_rbuf(rstream)", (ossl_quic_rstream_move_to_rbuf(rstream)) != 0))
        || !test_true("../test/quic_stream_test.c", 453, "read_fn(rstream, buf + 14, 5, &readbytes, &fin)", (read_fn(rstream, buf + 14, 5, &readbytes, &fin)) != 0)
        || !test_false("../test/quic_stream_test.c", 454, "fin", (fin) != 0)
        || !test_size_t_eq("../test/quic_stream_test.c", 455, "readbytes", "5", readbytes, 5)
        || !test_mem_eq("../test/quic_stream_test.c", 456, "buf", "simple_data", buf, 14 + 5, simple_data, 14 + 5)
        || !test_true("../test/quic_stream_test.c", 457, "read_fn(rstream, buf + 14 + 5, sizeof(buf) - 14 - 5, &readbytes, &fin)", (read_fn(rstream, buf + 14 + 5, sizeof(buf) - 14 - 5, &readbytes, &fin)) != 0)

        || !test_true("../test/quic_stream_test.c", 459, "fin", (fin) != 0)
        || !test_size_t_eq("../test/quic_stream_test.c", 460, "readbytes", "sizeof(buf) - 14 - 5", readbytes, sizeof(buf) - 14 - 5)
        || !test_mem_eq("../test/quic_stream_test.c", 461, "buf", "simple_data", buf, sizeof(buf), simple_data, sizeof(simple_data))
        || (use_rbuf && !test_true("../test/quic_stream_test.c", 462, "ossl_quic_rstream_move_to_rbuf(rstream)", (ossl_quic_rstream_move_to_rbuf(rstream)) != 0))
        || !test_true("../test/quic_stream_test.c", 463, "read_fn(rstream, buf, sizeof(buf), &readbytes, &fin)", (read_fn(rstream, buf, sizeof(buf), &readbytes, &fin)) != 0)
        || !test_true("../test/quic_stream_test.c", 464, "fin", (fin) != 0)
        || !test_size_t_eq("../test/quic_stream_test.c", 465, "readbytes", "0", readbytes, 0))
        goto err;
...
{
    unsigned char *bulk_data = 
# 477 "../test/quic_stream_test.c" 3 4
                              ((void *)0)
# 477 "../test/quic_stream_test.c"
                                  ;
    unsigned char *read_buf = 
# 478 "../test/quic_stream_test.c" 3 4
                             ((void *)0)
# 478 "../test/quic_stream_test.c"
                                 ;
    QUIC_RSTREAM *rstream = 
# 479 "../test/quic_stream_test.c" 3 4
                           ((void *)0)
# 479 "../test/quic_stream_test.c"
                               ;
    ...
    if (!test_ptr("../test/quic_stream_test.c", 486, "bulk_data = OPENSSL_malloc(data_size)", bulk_data = CRYPTO_malloc(data_size, "../test/quic_stream_test.c", 486))
        || !test_ptr("../test/quic_stream_test.c", 487, "read_buf = OPENSSL_malloc(data_size)", read_buf = CRYPTO_malloc(data_size, "../test/quic_stream_test.c", 487))
        || !test_ptr("../test/quic_stream_test.c", 488, "rstream = ossl_quic_rstream_new(NULL, NULL, 0)", rstream = ossl_quic_rstream_new(
# 488 "../test/quic_stream_test.c" 3 4
           ((void *)0)
# 488 "../test/quic_stream_test.c"
           , 
# 488 "../test/quic_stream_test.c" 3 4
           ((void *)0)
# 488 "../test/quic_stream_test.c"
    ...
    for (r = 0; r < 100; ++r) {
        for (s = 0; s < 10; ++s) {
            ...
            if (!test_true("../test/quic_stream_test.c", 509, "ossl_quic_rstream_queue_data(rstream, NULL, off, bulk_data + off, size, 0)", (ossl_quic_rstream_queue_data(rstream, 
# 509 "../test/quic_stream_test.c" 3 4
                ((void *)0)
# 509 "../test/quic_stream_test.c"
                , off, bulk_data + off, size, 0)) != 0)
            ...
            if (!test_true("../test/quic_stream_test.c", 529, "ossl_quic_rstream_queue_data(rstream, NULL, off, bulk_data + off, size, 0)", (ossl_quic_rstream_queue_data(rstream, 
# 529 "../test/quic_stream_test.c" 3 4
                ((void *)0)
# 529 "../test/quic_stream_test.c"
                , off, bulk_data + off, size, 0)) != 0)
        ...
        if (idx % 2 == 0) {
            if (!test_true("../test/quic_stream_test.c", 537, "test_single_copy_read(rstream, read_buf, data_size, &readbytes, &fin)", (test_single_copy_read(rstream, read_buf, data_size, &readbytes, &fin)) != 0)
                                                                   )
                goto err;
        } else if (!test_true("../test/quic_stream_test.c", 540, "ossl_quic_rstream_read(rstream, read_buf, data_size, &readbytes, &fin)", (ossl_quic_rstream_read(rstream, read_buf, data_size, &readbytes, &fin)) != 0)

                                                                       ) {
            goto err;
        }
        if (!test_size_t_ge("../test/quic_stream_test.c", 545, "readbytes", "queued_min - read_off", readbytes, queued_min - read_off)
            || !test_size_t_le("../test/quic_stream_test.c", 546, "readbytes + read_off", "data_size", readbytes + read_off, data_size)
            || (idx % 3 != 0
                && !test_mem_eq("../test/quic_stream_test.c", 548, "read_buf", "bulk_data + read_off", read_buf, readbytes, bulk_data + read_off, readbytes)
                                          ))
            goto err;
        read_off += readbytes;
        queued_min = read_off;
        if (test_random() % 50 == 0)
            if (!test_true("../test/quic_stream_test.c", 554, "ossl_quic_rstream_resize_rbuf(rstream, queued_max - read_off + 1)", (ossl_quic_rstream_resize_rbuf(rstream, queued_max - read_off + 1)) != 0)

                || !test_true("../test/quic_stream_test.c", 556, "ossl_quic_rstream_move_to_rbuf(rstream)", (ossl_quic_rstream_move_to_rbuf(rstream)) != 0))
                goto err;
        if (!fin_set && queued_max >= data_size - test_random() % 200) {
            fin_set = 1;

            if (!test_true("../test/quic_stream_test.c", 561, "ossl_quic_rstream_queue_data(rstream, NULL, data_size, NULL, 0, 1)", (ossl_quic_rstream_queue_data(rstream, 
# 561 "../test/quic_stream_test.c" 3 4
                ((void *)0)
# 561 "../test/quic_stream_test.c"
                , data_size, 
# 561 "../test/quic_stream_test.c" 3 4
                ((void *)0)
# 561 "../test/quic_stream_test.c"
    ...
    test_info("../test/quic_stream_test.c", 567, "Total read bytes: %zu Fin rcvd: %d", read_off, fin);

    if (idx % 3 == 0)
        for (i = 0; i < read_off; i++)
            if (!test_uchar_eq("../test/quic_stream_test.c", 571, "bulk_data[i]", "0", bulk_data[i], 0))
                goto err;

    if (read_off == data_size && fin_set && !fin) {

        if (idx % 2 == 0) {
            if (!test_true("../test/quic_stream_test.c", 577, "test_single_copy_read(rstream, read_buf, data_size, &readbytes, &fin)", (test_single_copy_read(rstream, read_buf, data_size, &readbytes, &fin)) != 0)
                                                                   )
                goto err;
        } else if (!test_true("../test/quic_stream_test.c", 580, "ossl_quic_rstream_read(rstream, read_buf, data_size, &readbytes, &fin)", (ossl_quic_rstream_read(rstream, read_buf, data_size, &readbytes, &fin)) != 0)

                                                                       ) {
            goto err;
        }
        if (!test_size_t_eq("../test/quic_stream_test.c", 585, "readbytes", "0", readbytes, 0) || !test_true("../test/quic_stream_test.c", 585, "fin", (fin) != 0))
            goto err;
    }
 ...
 err:
    ossl_quic_rstream_free(rstream);
    CRYPTO_free(bulk_data, "../test/quic_stream_test.c", 593);
    CRYPTO_free(read_buf, "../test/quic_stream_test.c", 594);
    return ret;
}
