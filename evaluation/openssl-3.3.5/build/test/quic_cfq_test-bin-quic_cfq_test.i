# 0 "../test/quic_cfq_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/quic_cfq_test.c"
# 10 "../test/quic_cfq_test.c"
...
# 14 "../include/internal/quic_cfq.h" 2
...
# 15 "../include/internal/quic_cfq.h" 2
...
static const uint32_t expect[QUIC_PN_SPACE_NUM][11] = {
    { 4, 6, 0, 3, 5, 7, 
# 50 "../test/quic_cfq_test.c" 3 4
                       (4294967295U) 
# 50 "../test/quic_cfq_test.c"
                                  },
    { 1, 2, 
# 51 "../test/quic_cfq_test.c" 3 4
           (4294967295U) 
# 51 "../test/quic_cfq_test.c"
                      },
    { 8, 9, 
# 52 "../test/quic_cfq_test.c" 3 4
           (4294967295U) 
# 52 "../test/quic_cfq_test.c"
                      },
};
...
static int check(QUIC_CFQ *cfq)
{
    ...
    for (pn_space = QUIC_PN_SPACE_INITIAL; pn_space < QUIC_PN_SPACE_NUM; ++pn_space)
        for (i = 0, item = ossl_quic_cfq_get_priority_head(cfq, pn_space);;
            ...
            if (expect[pn_space][i] == 
# 77 "../test/quic_cfq_test.c" 3 4
                                      (4294967295U)
# 77 "../test/quic_cfq_test.c"
                                                ) {
                if (!test_ptr_null("../test/quic_cfq_test.c", 78, "item", item))
                    goto err;
            ...
            if (!test_ptr("../test/quic_cfq_test.c", 86, "item", item)
                || !test_ptr_eq("../test/quic_cfq_test.c", 87, "ossl_quic_cfq_item_get_encoded(item)", "ref_buf + expect[pn_space][i]", ossl_quic_cfq_item_get_encoded(item), ref_buf + expect[pn_space][i])

                || !test_int_eq("../test/quic_cfq_test.c", 89, "ossl_quic_cfq_item_get_pn_space(item)", "pn_space", ossl_quic_cfq_item_get_pn_space(item), pn_space)
                || !test_int_eq("../test/quic_cfq_test.c", 90, "ossl_quic_cfq_item_get_state(item)", "QUIC_CFQ_STATE_NEW", ossl_quic_cfq_item_get_state(item), 0)
                                                   )
                goto err;
        }
...
static int test_cfq(void)
{
    int testresult = 0;
    QUIC_CFQ *cfq = 
# 103 "../test/quic_cfq_test.c" 3 4
                   ((void *)0)
# 103 "../test/quic_cfq_test.c"
                       ;
    QUIC_CFQ_ITEM *item, *inext;
    size_t i;
    uint32_t pn_space;

    if (!test_ptr("../test/quic_cfq_test.c", 108, "cfq = ossl_quic_cfq_new()", cfq = ossl_quic_cfq_new()))
        goto err;

    g_free = 
# 111 "../test/quic_cfq_test.c" 3 4
                 ((void *)0)
# 111 "../test/quic_cfq_test.c"
                     ;
    g_free_len = 0;

    for (i = 0; i < (sizeof(ref_buf)/sizeof((ref_buf)[0])); ++i) {
        if (!test_ptr("../test/quic_cfq_test.c", 115, "item = ossl_quic_cfq_add_frame(cfq, ref_priority[i], ref_pn_space[i], ref_frame_type[i], 0, ref_buf + i, 1, free_cb, NULL)", item = ossl_quic_cfq_add_frame(cfq, ref_priority[i], ref_pn_space[i], ref_frame_type[i], 0, ref_buf + i, 1, free_cb, 
# 115 "../test/quic_cfq_test.c" 3 4
            ((void *)0)
# 115 "../test/quic_cfq_test.c"
            ))
            ...
            || !test_int_eq("../test/quic_cfq_test.c", 122, "ossl_quic_cfq_item_get_state(item)", "QUIC_CFQ_STATE_NEW", ossl_quic_cfq_item_get_state(item), 0)

            || !test_uint_eq("../test/quic_cfq_test.c", 124, "ossl_quic_cfq_item_get_pn_space(item)", "ref_pn_space[i]", ossl_quic_cfq_item_get_pn_space(item), ref_pn_space[i])

            || !test_uint64_t_eq("../test/quic_cfq_test.c", 126, "ossl_quic_cfq_item_get_frame_type(item)", "ref_frame_type[i]", ossl_quic_cfq_item_get_frame_type(item), ref_frame_type[i])

            || !test_ptr_eq("../test/quic_cfq_test.c", 128, "ossl_quic_cfq_item_get_encoded(item)", "ref_buf + i", ossl_quic_cfq_item_get_encoded(item), ref_buf + i)

            || !test_size_t_eq("../test/quic_cfq_test.c", 130, "ossl_quic_cfq_item_get_encoded_len(item)", "1", ossl_quic_cfq_item_get_encoded_len(item), 1)
                                 )
            goto err;
    }
    ...
    for (pn_space = QUIC_PN_SPACE_INITIAL; pn_space < QUIC_PN_SPACE_NUM; ++pn_space)
        for (item = ossl_quic_cfq_get_priority_head(cfq, pn_space);
             item != 
# 140 "../test/quic_cfq_test.c" 3 4
                    ((void *)0)
# 140 "../test/quic_cfq_test.c"
                        ; item = inext) {
    ...
    for (pn_space = QUIC_PN_SPACE_INITIAL; pn_space < QUIC_PN_SPACE_NUM; ++pn_space)
        if (!test_ptr_null("../test/quic_cfq_test.c", 147, "ossl_quic_cfq_get_priority_head(cfq, pn_space)", ossl_quic_cfq_get_priority_head(cfq, pn_space)))
            goto err;

    for (pn_space = QUIC_PN_SPACE_INITIAL; pn_space < QUIC_PN_SPACE_NUM; ++pn_space)
        for (i = 0; i < (sizeof(items[0])/sizeof((items[0])[0])); ++i)
            if (items[pn_space][i] != 
# 152 "../test/quic_cfq_test.c" 3 4
                                     ((void *)0)
# 152 "../test/quic_cfq_test.c"
                                         )
                ossl_quic_cfq_mark_lost(cfq, items[pn_space][i], 
# 153 "../test/quic_cfq_test.c" 3 4
                                                                (4294967295U)
# 153 "../test/quic_cfq_test.c"
                                                                          );
    ...
    for (pn_space = QUIC_PN_SPACE_INITIAL; pn_space < QUIC_PN_SPACE_NUM; ++pn_space)
        for (i = 0; i < (sizeof(items[0])/sizeof((items[0])[0])); ++i)
            if (items[pn_space][i] != 
# 160 "../test/quic_cfq_test.c" 3 4
                                     ((void *)0)
# 160 "../test/quic_cfq_test.c"
                                         )
                ossl_quic_cfq_release(cfq, items[pn_space][i]);

    for (pn_space = QUIC_PN_SPACE_INITIAL; pn_space < QUIC_PN_SPACE_NUM; ++pn_space)
        if (!test_ptr_null("../test/quic_cfq_test.c", 164, "ossl_quic_cfq_get_priority_head(cfq, pn_space)", ossl_quic_cfq_get_priority_head(cfq, pn_space)))
            goto err;
