# 0 "../test/punycode_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/punycode_test.c"
# 10 "../test/punycode_test.c"
...
# 14 "../test/punycode_test.c" 2
# 1 "../include/internal/nelem.h" 1
# 12 "../include/internal/nelem.h"
        
# 15 "../test/punycode_test.c" 2
...
static int test_punycode(int n)
{
    ...
    unsigned int bsize = (sizeof(buffer)/sizeof((buffer)[0]));
    size_t i;

    if (!test_true("../test/punycode_test.c", 160, "ossl_punycode_decode(tc->encoded, strlen(tc->encoded), buffer, &bsize)", (ossl_punycode_decode(tc->encoded, strlen(tc->encoded), buffer, &bsize)) != 0)
                                                        )
        return 0;
    for (i = 0; i < (sizeof(tc->raw)/sizeof((tc->raw)[0])); i++)
        if (tc->raw[i] == 0)
            break;
    if (!test_mem_eq("../test/punycode_test.c", 166, "buffer", "tc->raw", buffer, bsize * sizeof(*buffer), tc->raw, i * sizeof(*tc->raw))
                                                   )
        return 0;
    return 1;
}
...
static int test_a2ulabel_bad_decode(int tst)
{
    ...
    return test_int_eq("../test/punycode_test.c", 187, "ossl_a2ulabel(bad_decode_tests[tst].input, out, bad_decode_tests[tst].outlen)", "-1", ossl_a2ulabel(bad_decode_tests[tst].input, out, bad_decode_tests[tst].outlen), -1);
}
...
static int test_a2ulabel(void)
{
    ...
    if (!test_int_eq("../test/punycode_test.c", 199, "ossl_a2ulabel(\"xn--a.b.c\", out, 1)", "0", ossl_a2ulabel("xn--a.b.c", out, 1), 0)
            || !test_int_eq("../test/punycode_test.c", 200, "ossl_a2ulabel(\"xn--a.b.c\", out, 7)", "1", ossl_a2ulabel("xn--a.b.c", out, 7), 1))
        return 0;

    if (!test_int_eq("../test/punycode_test.c", 203, "ossl_a2ulabel(\"xn--a.b.c\", out, 6)", "0", ossl_a2ulabel("xn--a.b.c", out, 6), 0)
            || !test_int_eq("../test/punycode_test.c", 204, "ossl_a2ulabel(\"xn--a.b.c\", out, 7)", "1", ossl_a2ulabel("xn--a.b.c", out, 7), 1)
            || !test_str_eq("../test/punycode_test.c", 205, "out", "\"\\xc2\\x80.b.c\"", out, "\xc2\x80.b.c"))
        return 0;


    if (!test_int_eq("../test/punycode_test.c", 209, "ossl_a2ulabel(\"xn--e28h.com\", out, 10)", "1", ossl_a2ulabel("xn--e28h.com", out, 10), 1))
        return 0;
    ...
    memset(in + 4, 'e', 513);
    memcpy(in + 517, "-3ya", 4);
    if (!test_int_eq("../test/punycode_test.c", 216, "ossl_a2ulabel(in, out, 50)", "-1", ossl_a2ulabel(in, out, 50), -1))
        return 0;
...
static int test_puny_overrun(void)
{
    ...
    unsigned int buf[(sizeof(out)/sizeof((out)[0]))];
    unsigned int bsize = (sizeof(buf)/sizeof((buf)[0])) - 1;

    if (!test_false("../test/punycode_test.c", 231, "ossl_punycode_decode(in, strlen(in), buf, &bsize)", (ossl_punycode_decode(in, strlen(in), buf, &bsize)) != 0)) {
        if (test_mem_eq("../test/punycode_test.c", 232, "buf", "out", buf, bsize * sizeof(*buf), out, sizeof(out)))
            test_error("../test/punycode_test.c", 233, "CRITICAL: buffer overrun detected!");
        return 0;
    }
    return 1;
}
...
static int test_dotted_overflow(void)
{
    ...
    const size_t num_reps = (sizeof(string)/sizeof((string)[0])) / 2;
    WPACKET p;
    BUF_MEM *in;
    char *out = 
# 245 "../test/punycode_test.c" 3 4
               ((void *)0)
# 245 "../test/punycode_test.c"
                   ;
    size_t i;
    int res = 0;


    if (!test_ptr("../test/punycode_test.c", 250, "in = BUF_MEM_new()", in = BUF_MEM_new()))
        return 0;
    if (!test_true("../test/punycode_test.c", 252, "WPACKET_init_len(&p, in, 0)", (WPACKET_init_len(&p, in, 0)) != 0)) {
        BUF_MEM_free(in);
        return 0;
    }
    for (i = 0; i < num_reps; i++) {
        if (i > 1 && !test_true("../test/punycode_test.c", 257, "WPACKET_put_bytes_u8(&p, '.')", (WPACKET_put_bytes__((&p), ('.'), 1)) != 0))
            goto err;
        if (!test_true("../test/punycode_test.c", 259, "WPACKET_memcpy(&p, \"xn--a\", sizeof(\"xn--a\") - 1)", (WPACKET_memcpy(&p, "xn--a", sizeof("xn--a") - 1)) != 0))
            goto err;
    }
    if (!test_true("../test/punycode_test.c", 262, "WPACKET_put_bytes_u8(&p, '\\0')", (WPACKET_put_bytes__((&p), ('\0'), 1)) != 0))
            goto err;
    if (!test_ptr("../test/punycode_test.c", 264, "out = OPENSSL_malloc(in->length)", out = CRYPTO_malloc(in->length, "../test/punycode_test.c", 264)))
        goto err;


    memset(out, 0x7f, in->length - 1);
    if (!test_int_le("../test/punycode_test.c", 269, "ossl_a2ulabel(in->data, out, num_reps)", "0", ossl_a2ulabel(in->data, out, num_reps), 0)
            || !test_int_eq("../test/punycode_test.c", 270, "out[num_reps]", "0x7f", out[num_reps], 0x7f))
        goto err;


    if (!test_int_gt("../test/punycode_test.c", 274, "ossl_a2ulabel(in->data, out, in->length)", "0", ossl_a2ulabel(in->data, out, in->length), 0)
            || !test_size_t_eq("../test/punycode_test.c", 275, "strlen(out)", "num_reps * 3", strlen(out), num_reps * 3))
        goto err;
 ...
 err:
    WPACKET_cleanup(&p);
    BUF_MEM_free(in);
    CRYPTO_free(out, "../test/punycode_test.c", 282);
    return res;
}

int setup_tests(void)
{
    add_all_tests("test_punycode", test_punycode, (sizeof(puny_cases)/sizeof((puny_cases)[0])), 1);
    ...
    add_all_tests("test_a2ulabel_bad_decode", test_a2ulabel_bad_decode, (sizeof(bad_decode_tests)/sizeof((bad_decode_tests)[0])), 1);
    return 1;
}
