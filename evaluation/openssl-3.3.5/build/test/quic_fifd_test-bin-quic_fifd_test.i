# 0 "../test/quic_fifd_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/quic_fifd_test.c"
# 10 "../test/quic_fifd_test.c"
...
# 11 "../test/quic_fifd_test.c" 2
# 1 "../include/internal/quic_txpim.h" 1
# 13 "../include/internal/quic_txpim.h"
...
# 14 "../include/internal/quic_txpim.h" 2
...
# 15 "../include/internal/quic_txpim.h" 2
...
# 16 "../include/internal/quic_txpim.h" 2
...
# 12 "../test/quic_fifd_test.c" 2
# 1 "../include/internal/quic_fifd.h" 1
# 18 "../include/internal/quic_fifd.h"
...
# 19 "../include/internal/quic_fifd.h" 2
# 27 "../include/internal/quic_fifd.h"
struct quic_fifd_st {
...
static void step_time(uint64_t ms) {
    cur_time = ossl_time_add(cur_time, ossl_ticks2time((ms) * (((uint64_t)1000000000) / 1000)));
}
                                    ...
                                    void *arg)
{
    ...
    cb_fail = 1;
    return 
# 78 "../test/quic_fifd_test.c" 3 4
          ((void *)0)
# 78 "../test/quic_fifd_test.c"
              ;
}
...
static int test_generic(INFO *info, int kind)
{
    ...
    size_t i, consumed = 0;
    QUIC_TXPIM_PKT *pkt = 
# 110 "../test/quic_fifd_test.c" 3 4
                         ((void *)0)
# 110 "../test/quic_fifd_test.c"
                             , *pkt2 = 
# 110 "../test/quic_fifd_test.c" 3 4
                                       ((void *)0)
# 110 "../test/quic_fifd_test.c"
...
# 117 "../test/quic_fifd_test.c" 3 4
                             ((void *)0)
# 117 "../test/quic_fifd_test.c"
                                 ;
    uint32_t pn_space = (kind == 2)
        ? QUIC_PN_SPACE_HANDSHAKE : QUIC_PN_SPACE_APP;

    cur_time = ossl_ticks2time((1000) * ((uint64_t)1000000000));
    regen_count = 0;

    get_sstream_by_id_p = sstream_expect;
    regen_frame_p = regen_expect;

    if (!test_ptr("../test/quic_fifd_test.c", 127, "pkt = ossl_quic_txpim_pkt_alloc(info->txpim)", pkt = ossl_quic_txpim_pkt_alloc(info->txpim)))
        goto err;

    for (i = 0; i < 2; ++i) {
        num_iov = (sizeof(iov)/sizeof((iov)[0]));
        if (!test_true("../test/quic_fifd_test.c", 132, "ossl_quic_sstream_append(info->sstream[i], (unsigned char *)\"Test message\", 12, &consumed)", (ossl_quic_sstream_append(info->sstream[i], (unsigned char *)"Test message", 12, &consumed)) != 0)


            || !test_size_t_eq("../test/quic_fifd_test.c", 135, "consumed", "12", consumed, 12))
            goto err;

        if (i == 1)
            ossl_quic_sstream_fin(info->sstream[i]);

        if (!test_true("../test/quic_fifd_test.c", 141, "ossl_quic_sstream_get_stream_frame(info->sstream[i], 0, &hdr, iov, &num_iov)", (ossl_quic_sstream_get_stream_frame(info->sstream[i], 0, &hdr, iov, &num_iov)) != 0)

            || !test_int_eq("../test/quic_fifd_test.c", 143, "hdr.is_fin", "i == 1", hdr.is_fin, i == 1)
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 144, "hdr.offset", "0", hdr.offset, 0)
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 145, "hdr.len", "12", hdr.len, 12)
            || !test_size_t_eq("../test/quic_fifd_test.c", 146, "ossl_quic_sstream_get_buffer_used(info->sstream[i])", "12", ossl_quic_sstream_get_buffer_used(info->sstream[i]), 12)
            || !test_true("../test/quic_fifd_test.c", 147, "ossl_quic_sstream_mark_transmitted(info->sstream[i], hdr.offset, hdr.offset + hdr.len - 1)", (ossl_quic_sstream_mark_transmitted(info->sstream[i], hdr.offset, hdr.offset + hdr.len - 1)) != 0)

                                                                                       )
            goto err;

        if (i == 1 && !test_true("../test/quic_fifd_test.c", 152, "ossl_quic_sstream_mark_transmitted_fin(info->sstream[i], hdr.offset + hdr.len)", (ossl_quic_sstream_mark_transmitted_fin(info->sstream[i], hdr.offset + hdr.len)) != 0)
                                                                                              )
            goto err;

        chunk.has_fin = hdr.is_fin;
        chunk.stream_id = 42 + i;
        if (!test_true("../test/quic_fifd_test.c", 158, "ossl_quic_txpim_pkt_append_chunk(pkt, &chunk)", (ossl_quic_txpim_pkt_append_chunk(pkt, &chunk)) != 0))
            goto err;
    }

    cfq_freed = 0;
    if (!test_ptr("../test/quic_fifd_test.c", 163, "cfq_item = ossl_quic_cfq_add_frame(info->cfq, 10, pn_space, OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID, 0, placeholder_data, sizeof(placeholder_data), cfq_free_cb_, NULL)", cfq_item = ossl_quic_cfq_add_frame(info->cfq, 10, pn_space, 0x18, 0, placeholder_data, sizeof(placeholder_data), cfq_free_cb_, 
# 163 "../test/quic_fifd_test.c" 3 4
        ((void *)0)
# 163 "../test/quic_fifd_test.c"
        ))
        ...
        || !test_ptr_eq("../test/quic_fifd_test.c", 169, "cfq_item", "ossl_quic_cfq_get_priority_head(info->cfq, pn_space)", cfq_item, ossl_quic_cfq_get_priority_head(info->cfq, pn_space)))
        goto err;
    ...
    pkt->ackm_pkt.largest_acked = 
# 176 "../test/quic_fifd_test.c" 3 4
                                     (18446744073709551615UL)
# 176 "../test/quic_fifd_test.c"
                                                    ;
    ...
    if (!test_true("../test/quic_fifd_test.c", 194, "ossl_quic_fifd_pkt_commit(&info->fifd, pkt)", (ossl_quic_fifd_pkt_commit(&info->fifd, pkt)) != 0))
        goto err;


    if (!test_ptr_null("../test/quic_fifd_test.c", 198, "ossl_quic_cfq_get_priority_head(info->cfq, pn_space)", ossl_quic_cfq_get_priority_head(info->cfq, pn_space)))
        goto err;

    switch (kind) {
    case 0:
        if (!test_true("../test/quic_fifd_test.c", 203, "ossl_ackm_on_rx_ack_frame(info->ackm, &ack, pn_space, cur_time)", (ossl_ackm_on_rx_ack_frame(info->ackm, &ack, pn_space, cur_time)) != 0)

                                                           )
            goto err;

        for (i = 0; i < 2; ++i)
            if (!test_size_t_eq("../test/quic_fifd_test.c", 209, "ossl_quic_sstream_get_buffer_used(info->sstream[i])", "0", ossl_quic_sstream_get_buffer_used(info->sstream[i]), 0))
                goto err;


        if (!test_false("../test/quic_fifd_test.c", 213, "ossl_quic_sstream_mark_lost_fin(info->sstream[1])", (ossl_quic_sstream_mark_lost_fin(info->sstream[1])) != 0))
            goto err;


        if (!test_true("../test/quic_fifd_test.c", 217, "cfq_freed", (cfq_freed) != 0))
            goto err;


        if (!test_size_t_eq("../test/quic_fifd_test.c", 221, "regen_count", "0", regen_count, 0))
            goto err;
    ...
    case 1:
        ...
        if (!test_ptr("../test/quic_fifd_test.c", 228, "pkt2 = ossl_quic_txpim_pkt_alloc(info->txpim)", pkt2 = ossl_quic_txpim_pkt_alloc(info->txpim)))
            goto err;

        step_time(10000);
        pkt2->ackm_pkt.pkt_num = 50;
        pkt2->ackm_pkt.pkt_space = pn_space;
        pkt2->ackm_pkt.largest_acked = 
# 234 "../test/quic_fifd_test.c" 3 4
                                         (18446744073709551615UL)
# 234 "../test/quic_fifd_test.c"
                                                        ;
        ...
        if (!test_true("../test/quic_fifd_test.c", 245, "ossl_quic_fifd_pkt_commit(&info->fifd, pkt2)", (ossl_quic_fifd_pkt_commit(&info->fifd, pkt2)) != 0)
            || !test_true("../test/quic_fifd_test.c", 246, "ossl_ackm_on_rx_ack_frame(info->ackm, &ack, pn_space, cur_time)", (ossl_ackm_on_rx_ack_frame(info->ackm, &ack, pn_space, cur_time)) != 0)
                                                                        )
            goto err;
        ...
        for (i = 0; i < 2; ++i) {
            ...
            if (!test_true("../test/quic_fifd_test.c", 256, "ossl_quic_sstream_get_stream_frame(info->sstream[i], 0, &hdr, iov, &num_iov)", (ossl_quic_sstream_get_stream_frame(info->sstream[i], 0, &hdr, iov, &num_iov)) != 0)

                || !test_uint64_t_eq("../test/quic_fifd_test.c", 258, "hdr.offset", "0", hdr.offset, 0)
                || !test_uint64_t_eq("../test/quic_fifd_test.c", 259, "hdr.len", "12", hdr.len, 12))
                goto err;
        }


        if (!test_size_t_eq("../test/quic_fifd_test.c", 264, "regen_count", "7", regen_count, 7)
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 265, "regen_stream_id[0]", "42", regen_stream_id[0], 42)
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 266, "regen_frame_type[0]", "OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA", regen_frame_type[0], 0x11)
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 267, "regen_stream_id[1]", "43", regen_stream_id[1], 43)
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 268, "regen_frame_type[1]", "OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA", regen_frame_type[1], 0x11)
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 269, "regen_frame_type[2]", "OSSL_QUIC_FRAME_TYPE_HANDSHAKE_DONE", regen_frame_type[2], 0x1E)
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 270, "regen_stream_id[2]", "UINT64_MAX", regen_stream_id[2], 
# 270 "../test/quic_fifd_test.c" 3 4
               (18446744073709551615UL)
# 270 "../test/quic_fifd_test.c"
               )
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 271, "regen_frame_type[3]", "OSSL_QUIC_FRAME_TYPE_MAX_DATA", regen_frame_type[3], 0x10)
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 272, "regen_stream_id[3]", "UINT64_MAX", regen_stream_id[3], 
# 272 "../test/quic_fifd_test.c" 3 4
               (18446744073709551615UL)
# 272 "../test/quic_fifd_test.c"
               )
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 273, "regen_frame_type[4]", "OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_BIDI", regen_frame_type[4], 0x12)
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 274, "regen_stream_id[4]", "UINT64_MAX", regen_stream_id[4], 
# 274 "../test/quic_fifd_test.c" 3 4
               (18446744073709551615UL)
# 274 "../test/quic_fifd_test.c"
               )
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 275, "regen_frame_type[5]", "OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_UNI", regen_frame_type[5], 0x13)
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 276, "regen_stream_id[5]", "UINT64_MAX", regen_stream_id[5], 
# 276 "../test/quic_fifd_test.c" 3 4
               (18446744073709551615UL)
# 276 "../test/quic_fifd_test.c"
               )
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 277, "regen_frame_type[6]", "OSSL_QUIC_FRAME_TYPE_ACK_WITH_ECN", regen_frame_type[6], 0x03)
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 278, "regen_stream_id[6]", "UINT64_MAX", regen_stream_id[6], 
# 278 "../test/quic_fifd_test.c" 3 4
               (18446744073709551615UL)
# 278 "../test/quic_fifd_test.c"
               ))
            goto err;


        if (!test_ptr_eq("../test/quic_fifd_test.c", 282, "cfq_item", "ossl_quic_cfq_get_priority_head(info->cfq, pn_space)", cfq_item, ossl_quic_cfq_get_priority_head(info->cfq, pn_space)))
            goto err;


        num_iov = (sizeof(iov)/sizeof((iov)[0]));
        if (!test_true("../test/quic_fifd_test.c", 287, "ossl_quic_sstream_get_stream_frame(info->sstream[1], 1, &hdr, iov, &num_iov)", (ossl_quic_sstream_get_stream_frame(info->sstream[1], 1, &hdr, iov, &num_iov)) != 0)

            || !test_true("../test/quic_fifd_test.c", 289, "hdr.is_fin", (hdr.is_fin) != 0)
            || !test_uint64_t_eq("../test/quic_fifd_test.c", 290, "hdr.len", "0", hdr.len, 0))
            goto err;

        break;

    case 2:
        if (!test_true("../test/quic_fifd_test.c", 296, "ossl_ackm_on_pkt_space_discarded(info->ackm, pn_space)", (ossl_ackm_on_pkt_space_discarded(info->ackm, pn_space)) != 0))
            goto err;


        if (!test_true("../test/quic_fifd_test.c", 300, "cfq_freed", (cfq_freed) != 0))
            goto err;
    ...
    if (!test_size_t_eq("../test/quic_fifd_test.c", 310, "ossl_quic_txpim_get_in_use(info->txpim)", "0", ossl_quic_txpim_get_in_use(info->txpim), 0))
        goto err;
...
static int test_fifd(int idx)
{
    ...
    if (!test_true("../test/quic_fifd_test.c", 327, "ossl_statm_init(&info.statm)", (ossl_statm_init(&info.statm)) != 0)
        || !test_ptr("../test/quic_fifd_test.c", 328, "info.ccdata = ossl_cc_dummy_method.new(fake_now, NULL)", info.ccdata = ossl_cc_dummy_method.new(fake_now, 
# 328 "../test/quic_fifd_test.c" 3 4
           ((void *)0)
# 328 "../test/quic_fifd_test.c"
           ))
        || !test_ptr("../test/quic_fifd_test.c", 329, "info.ackm = ossl_ackm_new(fake_now, NULL, &info.statm, &ossl_cc_dummy_method, info.ccdata, 0)", info.ackm = ossl_ackm_new(fake_now, 
# 329 "../test/quic_fifd_test.c" 3 4
           ((void *)0)
# 329 "../test/quic_fifd_test.c"
           , &info.statm, &ossl_cc_dummy_method, info.ccdata, 0))




        || !test_true("../test/quic_fifd_test.c", 334, "ossl_ackm_on_handshake_confirmed(info.ackm)", (ossl_ackm_on_handshake_confirmed(info.ackm)) != 0)
        || !test_ptr("../test/quic_fifd_test.c", 335, "info.cfq = ossl_quic_cfq_new()", info.cfq = ossl_quic_cfq_new())
        || !test_ptr("../test/quic_fifd_test.c", 336, "info.txpim = ossl_quic_txpim_new()", info.txpim = ossl_quic_txpim_new())
        || !test_true("../test/quic_fifd_test.c", 337, "ossl_quic_fifd_init(&info.fifd, info.cfq, info.ackm, info.txpim, get_sstream_by_id, NULL, regen_frame, NULL, confirm_frame, NULL, sstream_updated, NULL, NULL, NULL)", (ossl_quic_fifd_init(&info.fifd, info.cfq, info.ackm, info.txpim, get_sstream_by_id, 
# 337 "../test/quic_fifd_test.c" 3 4
           ((void *)0)
# 337 "../test/quic_fifd_test.c"
           , regen_frame, 
# 337 "../test/quic_fifd_test.c" 3 4
           ((void *)0)
# 337 "../test/quic_fifd_test.c"
...
# 337 "../test/quic_fifd_test.c"
    ...
    for (i = 0; i < (sizeof(info.sstream)/sizeof((info.sstream)[0])); ++i)
        if (!test_ptr("../test/quic_fifd_test.c", 347, "info.sstream[i] = ossl_quic_sstream_new(1024)", info.sstream[i] = ossl_quic_sstream_new(1024)))
            goto err;

    ossl_statm_update_rtt(&info.statm, ossl_time_zero(), ossl_ticks2time((1) * (((uint64_t)1000000000) / 1000)));

    if (!test_true("../test/quic_fifd_test.c", 352, "test_generic(&info, idx)", (test_generic(&info, idx)) != 0)
        || !test_false("../test/quic_fifd_test.c", 353, "cb_fail", (cb_fail) != 0))
        goto err;
...
err:
    ...
    if (info.ccdata != 
# 363 "../test/quic_fifd_test.c" 3 4
                      ((void *)0)
# 363 "../test/quic_fifd_test.c"
                          )
        ossl_cc_dummy_method.free(info.ccdata);
    for (i = 0; i < (sizeof(info.sstream)/sizeof((info.sstream)[0])); ++i)
        ossl_quic_sstream_free(info.sstream[i]);
    cur_info = 
# 367 "../test/quic_fifd_test.c" 3 4
              ((void *)0)
# 367 "../test/quic_fifd_test.c"
                  ;
    return testresult;
}
