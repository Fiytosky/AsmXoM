# 0 "../test/hexstr_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/hexstr_test.c"
# 15 "../test/hexstr_test.c"
# 1 "../include/internal/nelem.h" 1
# 12 "../include/internal/nelem.h"
        
# 16 "../test/hexstr_test.c" 2
...
static int test_hexstr_sep_to_from(int test_index)
{
    ...
    long len = 0;
    unsigned char *buf = 
# 67 "../test/hexstr_test.c" 3 4
                        ((void *)0)
# 67 "../test/hexstr_test.c"
                            ;
    char *out = 
# 68 "../test/hexstr_test.c" 3 4
               ((void *)0)
# 68 "../test/hexstr_test.c"
                   ;
    struct testdata *test = &tbl_testdata[test_index];

    if (!test_ptr("../test/hexstr_test.c", 71, "buf = ossl_hexstr2buf_sep(test->in, &len, test->sep)", buf = ossl_hexstr2buf_sep(test->in, &len, test->sep))
        || !test_mem_eq("../test/hexstr_test.c", 72, "buf", "test->expected", buf, len, test->expected, test->expected_len)
        || !test_ptr("../test/hexstr_test.c", 73, "out = ossl_buf2hexstr_sep(buf, len, test->sep)", out = ossl_buf2hexstr_sep(buf, len, test->sep))
        || !test_str_eq("../test/hexstr_test.c", 74, "out", "test->in", out, test->in))
       goto err;

    ret = 1;
err:
    CRYPTO_free(buf, "../test/hexstr_test.c", 79);
    CRYPTO_free(out, "../test/hexstr_test.c", 80);
    return ret;
}
...
static int test_hexstr_to_from(int test_index)
{
    ...
    long len = 0;
    unsigned char *buf = 
# 88 "../test/hexstr_test.c" 3 4
                        ((void *)0)
# 88 "../test/hexstr_test.c"
                            ;
    char *out = 
# 89 "../test/hexstr_test.c" 3 4
               ((void *)0)
# 89 "../test/hexstr_test.c"
                   ;
    struct testdata *test = &tbl_testdata[test_index];

    if (test->sep != '_') {
        if (!test_ptr("../test/hexstr_test.c", 93, "buf = OPENSSL_hexstr2buf(test->in, &len)", buf = OPENSSL_hexstr2buf(test->in, &len))
            || !test_mem_eq("../test/hexstr_test.c", 94, "buf", "test->expected", buf, len, test->expected, test->expected_len)
            || !test_ptr("../test/hexstr_test.c", 95, "out = OPENSSL_buf2hexstr(buf, len)", out = OPENSSL_buf2hexstr(buf, len)))
           goto err;
        if (test->sep == ':') {
            if (!test_str_eq("../test/hexstr_test.c", 98, "out", "test->in", out, test->in))
                goto err;
        } else if (!test_str_ne("../test/hexstr_test.c", 100, "out", "test->in", out, test->in)) {
            goto err;
        }
    } else {
        if (!test_ptr_null("../test/hexstr_test.c", 104, "buf = OPENSSL_hexstr2buf(test->in, &len)", buf = OPENSSL_hexstr2buf(test->in, &len)))
            goto err;
    }
    ret = 1;
err:
    CRYPTO_free(buf, "../test/hexstr_test.c", 109);
    CRYPTO_free(out, "../test/hexstr_test.c", 110);
    return ret;
}
...
static int test_hexstr_ex_to_from(int test_index)
{
    ...
    return test_true("../test/hexstr_test.c", 121, "OPENSSL_hexstr2buf_ex(buf, sizeof(buf), &len, test->in, ':')", (OPENSSL_hexstr2buf_ex(buf, sizeof(buf), &len, test->in, ':')) != 0)
           && test_mem_eq("../test/hexstr_test.c", 122, "buf", "test->expected", buf, len, test->expected, test->expected_len)
           && test_false("../test/hexstr_test.c", 123, "OPENSSL_buf2hexstr_ex(out, 3 * len - 1, NULL, buf, len, ':')", (OPENSSL_buf2hexstr_ex(out, 3 * len - 1, 
# 123 "../test/hexstr_test.c" 3 4
             ((void *)0)
# 123 "../test/hexstr_test.c"
             , buf, len, ':')) != 0)

           && test_true("../test/hexstr_test.c", 125, "OPENSSL_buf2hexstr_ex(out, sizeof(out), NULL, buf, len, ':')", (OPENSSL_buf2hexstr_ex(out, sizeof(out), 
# 125 "../test/hexstr_test.c" 3 4
             ((void *)0)
# 125 "../test/hexstr_test.c"
             , buf, len, ':')) != 0)

           && test_str_eq("../test/hexstr_test.c", 127, "out", "test->in", out, test->in)
           && test_true("../test/hexstr_test.c", 128, "OPENSSL_buf2hexstr_ex(out, sizeof(out), NULL, buf, 0, ':')", (OPENSSL_buf2hexstr_ex(out, sizeof(out), 
# 128 "../test/hexstr_test.c" 3 4
             ((void *)0)
# 128 "../test/hexstr_test.c"
             , buf, 0, ':')) != 0)

           && test_size_t_eq("../test/hexstr_test.c", 130, "strlen(out)", "0", strlen(out), 0);
}

int setup_tests(void)
{
    add_all_tests("test_hexstr_sep_to_from", test_hexstr_sep_to_from, (sizeof(tbl_testdata)/sizeof((tbl_testdata)[0])), 1);
    add_all_tests("test_hexstr_to_from", test_hexstr_to_from, (sizeof(tbl_testdata)/sizeof((tbl_testdata)[0])), 1);
    add_all_tests("test_hexstr_ex_to_from", test_hexstr_ex_to_from, 2, 1);
    return 1;
}
