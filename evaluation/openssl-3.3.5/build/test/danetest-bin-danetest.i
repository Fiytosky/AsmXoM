...
{
    int ret = 
# 50 "../test/danetest.c" 3 4
             (*__errno_location ())
# 50 "../test/danetest.c"
                  ;
    
# 51 "../test/danetest.c" 3 4
   (*__errno_location ()) 
# 51 "../test/danetest.c"
         = saved_errno;
    return ret;
...
static int verify_chain(SSL *ssl, struct stack_st_X509 *chain)
{
    X509_STORE_CTX *store_ctx = 
# 57 "../test/danetest.c" 3 4
                               ((void *)0)
# 57 "../test/danetest.c"
                                   ;
    SSL_CTX *ssl_ctx = 
# 58 "../test/danetest.c" 3 4
                      ((void *)0)
# 58 "../test/danetest.c"
                          ;
    X509_STORE *store = 
# 59 "../test/danetest.c" 3 4
                       ((void *)0)
# 59 "../test/danetest.c"
                           ;
    int ret = 0;
    ...
    if (!test_ptr("../test/danetest.c", 63, "store_ctx = X509_STORE_CTX_new()", store_ctx = X509_STORE_CTX_new())
            || !test_ptr("../test/danetest.c", 64, "ssl_ctx = SSL_get_SSL_CTX(ssl)", ssl_ctx = SSL_get_SSL_CTX(ssl))
            || !test_ptr("../test/danetest.c", 65, "store = SSL_CTX_get_cert_store(ssl_ctx)", store = SSL_CTX_get_cert_store(ssl_ctx))
            || !test_true("../test/danetest.c", 66, "X509_STORE_CTX_init(store_ctx, store, NULL, chain)", (X509_STORE_CTX_init(store_ctx, store, 
# 66 "../test/danetest.c" 3 4
               ((void *)0)
# 66 "../test/danetest.c"
               , chain)) != 0)
            || !test_true("../test/danetest.c", 67, "X509_STORE_CTX_set_ex_data(store_ctx, store_ctx_idx, ssl)", (X509_STORE_CTX_set_ex_data(store_ctx, store_ctx_idx, ssl)) != 0)
                                                          )
        goto end;
...
# 77 "../test/danetest.c"
    ...
    if (!test_int_ge("../test/danetest.c", 81, "ret = X509_STORE_CTX_verify(store_ctx)", "0", ret = X509_STORE_CTX_verify(store_ctx), 0))
        ret = 0;
...
static struct stack_st_X509 *load_chain(BIO *fp, int nelem)
{
    int count;
    char *name = 0;
    char *header = 0;
    ...
    if (!test_ptr("../test/danetest.c", 102, "chain = sk_X509_new_null()", chain = ((struct stack_st_X509 *)OPENSSL_sk_new_null())))
        goto err;
    ...
    for (count = 0;
        ...
        if (strcmp(name, "CERTIFICATE") == 0
            ...
            d2i_X509_t d = strcmp(name, "TRUSTED CERTIFICATE") != 0
                ? d2i_X509_AUX : d2i_X509;
            X509 *cert;
            const unsigned char *p = data;

            if (!test_ptr("../test/danetest.c", 117, "cert = d(0, &p, len)", cert = d(0, &p, len))
                    || !test_long_eq("../test/danetest.c", 118, "p - data", "len", p - data, len)) {
                test_info("../test/danetest.c", 119, "Certificate parsing error");
                goto err;
            }

            if (!test_true("../test/danetest.c", 123, "sk_X509_push(chain, cert)", (OPENSSL_sk_push(ossl_check_X509_sk_type(chain), ossl_check_X509_type(cert))) != 0))
                goto err;
        } else {
            test_info("../test/danetest.c", 126, "Unknown chain file object %s", name);
            goto err;
        }

        CRYPTO_free(name, "../test/danetest.c", 130);
        CRYPTO_free(header, "../test/danetest.c", 131);
        CRYPTO_free(data, "../test/danetest.c", 132);
        name = header = 
# 133 "../test/danetest.c" 3 4
                       ((void *)0)
# 133 "../test/danetest.c"
                           ;
        data = 
# 134 "../test/danetest.c" 3 4
              ((void *)0)
# 134 "../test/danetest.c"
                  ;
    }
...
err:
    CRYPTO_free(name, "../test/danetest.c", 143);
    CRYPTO_free(header, "../test/danetest.c", 144);
    CRYPTO_free(data, "../test/danetest.c", 145);
    OSSL_STACK_OF_X509_free(chain);
    return 
# 147 "../test/danetest.c" 3 4
          ((void *)0)
# 147 "../test/danetest.c"
              ;
}
...
# 156 "../test/danetest.c"
    ...
    if (buf[n - 1] != '\n') {
        if (n + 1 == sizeof(buf))
            test_error("../test/danetest.c", 161, "input too long");
        else
            test_error("../test/danetest.c", 163, "EOF before newline");
        return 
# 164 "../test/danetest.c" 3 4
              ((void *)0)
# 164 "../test/danetest.c"
                  ;
    }
...
static ssize_t hexdecode(const char *in, void *result)
{
    ...
    if (!test_ptr("../test/danetest.c", 185, "ret = OPENSSL_malloc(strlen(in) / 2)", ret = CRYPTO_malloc(strlen(in) / 2, "../test/danetest.c", 185)))
        return -1;
    cp = ret;
...
# 192 "../test/danetest.c"
        ...
        if (x < 0) {
            CRYPTO_free(ret, "../test/danetest.c", 196);
            return 0;
        }
        byte |= (char)x;
        if ((nibble ^= 1) == 0) {
    ...
    if (nibble != 0) {
        CRYPTO_free(ret, "../test/danetest.c", 208);
        return 0;
    }
...
static ssize_t checked_uint8(const char *in, void *out)
{
...
# 254 "../test/danetest.c"
    ...
    for (f = tlsa_fields; f->var; ++f) {
        if ((len = f->parser(cp += len, f->var)) <= 0) {
            test_info("../test/danetest.c", 263, "bad TLSA %s field in: %s", f->name, rrdata);
            return 0;
        }
    }

    ret = SSL_dane_tlsa_add(ssl, usage, selector, mtype, data, len);
    CRYPTO_free(data, "../test/danetest.c", 269);
    if (ret == 0) {
        test_info("../test/danetest.c", 271, "unusable TLSA rrdata: %s", rrdata);
        return 0;
    }
    if (ret < 0) {
        test_info("../test/danetest.c", 275, "error loading TLSA rrdata: %s", rrdata);
        return 0;
    }
                         ...
                         BIO *f, const char *path)
{
    ...
    while (ret > 0 && (line = read_to_eol(f)) != 
# 298 "../test/danetest.c" 3 4
                                                ((void *)0)
# 298 "../test/danetest.c"
                                                    ) {
        struct stack_st_X509 *chain;
        ...
        if (sscanf(line, "%d %d %d %d %d%n",
            ...
            || !allws(line + off)) {
            test_error("../test/danetest.c", 318, "Malformed line for test %d", testno);
            return 0;
        }

        if (!test_ptr("../test/danetest.c", 322, "ssl = SSL_new(ctx)", ssl = SSL_new(ctx)))
            return 0;
        ...
        if (noncheck)
            SSL_dane_set_flags(ssl, (1L << 0));

        for (i = 0; i < ntlsa; ++i) {
            if ((line = read_to_eol(f)) == 
# 333 "../test/danetest.c" 3 4
                                          ((void *)0) 
# 333 "../test/danetest.c"
        ...
        ERR_clear_error();
        if (!test_ptr("../test/danetest.c", 341, "chain = load_chain(f, ncert)", chain = load_chain(f, ncert))) {
            SSL_free(ssl);
            return 0;
        }
        ...
        mdpth = SSL_get0_dane_authority(ssl, 
# 355 "../test/danetest.c" 3 4
                                            ((void *)0)
# 355 "../test/danetest.c"
                                                , 
# 355 "../test/danetest.c" 3 4
...
# 355 "../test/danetest.c"
        ...
        if (!test_int_eq("../test/danetest.c", 360, "err", "want", err, want)) {
            if (want == 0)
                test_info("../test/danetest.c", 362, "Verification failure in test %d: %d=%s", testno, err, X509_verify_cert_error_string(err))
                                                                          ;
            else
                test_info("../test/danetest.c", 365, "Unexpected error in test %d", testno);
            ret = 0;
            continue;
        }
        if (!test_false("../test/danetest.c", 369, "want == 0 && ok == 0", (want == 0 && ok == 0) != 0)) {
            test_info("../test/danetest.c", 370, "Verification failure in test %d: ok=0", testno);
            ret = 0;
            continue;
        }
        if (!test_int_eq("../test/danetest.c", 374, "mdpth", "want_depth", mdpth, want_depth)) {
            test_info("../test/danetest.c", 375, "In test test %d", testno);
            ret = 0;
        }
    }
    ERR_clear_error();
...
{
    SSL_CTX *ctx = 
# 386 "../test/danetest.c" 3 4
                  ((void *)0)
# 386 "../test/danetest.c"
                      ;
    BIO *f = 
# 387 "../test/danetest.c" 3 4
            ((void *)0)
# 387 "../test/danetest.c"
                ;
    int ret = 0;

    if (!test_ptr("../test/danetest.c", 390, "f = BIO_new_file(tlsafile, \"r\")", f = BIO_new_file(tlsafile, "r"))
            || !test_ptr("../test/danetest.c", 391, "ctx = SSL_CTX_new(TLS_client_method())", ctx = SSL_CTX_new(TLS_client_method()))
            || !test_int_gt("../test/danetest.c", 392, "SSL_CTX_dane_enable(ctx)", "0", SSL_CTX_dane_enable(ctx), 0)
            || !test_true("../test/danetest.c", 393, "SSL_CTX_load_verify_file(ctx, CAfile)", (SSL_CTX_load_verify_file(ctx, CAfile)) != 0)
            || !test_int_gt("../test/danetest.c", 394, "SSL_CTX_dane_mtype_set(ctx, EVP_sha512(), 2, 1)", "0", SSL_CTX_dane_mtype_set(ctx, EVP_sha512(), 2, 1), 0)
            || !test_int_gt("../test/danetest.c", 395, "SSL_CTX_dane_mtype_set(ctx, EVP_sha256(), 1, 2)", "0", SSL_CTX_dane_mtype_set(ctx, EVP_sha256(), 1, 2), 0)
            || !test_int_gt("../test/danetest.c", 396, "test_tlsafile(ctx, basedomain, f, tlsafile)", "0", test_tlsafile(ctx, basedomain, f, tlsafile), 0))
        goto end;
    ret = 1;
...
int setup_tests(void)
{
    if (!test_skip_common_options()) {
        test_error("../test/danetest.c", 412, "Error parsing test options\n");
        return 0;
    }

    if (!test_ptr("../test/danetest.c", 416, "basedomain = test_get_argument(0)", basedomain = test_get_argument(0))
            || !test_ptr("../test/danetest.c", 417, "CAfile = test_get_argument(1)", CAfile = test_get_argument(1))
            || !test_ptr("../test/danetest.c", 418, "tlsafile = test_get_argument(2)", tlsafile = test_get_argument(2)))
        return 0;
