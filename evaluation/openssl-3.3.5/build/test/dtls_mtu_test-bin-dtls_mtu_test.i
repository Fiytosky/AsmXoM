# 0 "../test/dtls_mtu_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/dtls_mtu_test.c"
# 10 "../test/dtls_mtu_test.c"
...
# 20 "../include/openssl/dtls1.h" 2
# 14 "../test/dtls_mtu_test.c" 2
...
{
    SSL *srvr_ssl = 
# 51 "../test/dtls_mtu_test.c" 3 4
                   ((void *)0)
# 51 "../test/dtls_mtu_test.c"
                       , *clnt_ssl = 
# 51 "../test/dtls_mtu_test.c" 3 4
                                     ((void *)0)
# 51 "../test/dtls_mtu_test.c"
                                         ;
    BIO *sc_bio = 
# 52 "../test/dtls_mtu_test.c" 3 4
                 ((void *)0)
# 52 "../test/dtls_mtu_test.c"
                     ;
    int i;
    size_t s;
    size_t mtus[30];
    ...
    if (!test_true("../test/dtls_mtu_test.c", 62, "create_ssl_objects(ctx, ctx, &srvr_ssl, &clnt_ssl, NULL, NULL)", (create_ssl_objects(ctx, ctx, &srvr_ssl, &clnt_ssl, 
# 62 "../test/dtls_mtu_test.c" 3 4
        ((void *)0)
# 62 "../test/dtls_mtu_test.c"
        , 
# 62 "../test/dtls_mtu_test.c" 3 4
        ((void *)0)
# 62 "../test/dtls_mtu_test.c"
    ...
    if (no_etm)
        SSL_set_options(srvr_ssl, ((uint64_t)1 << (uint64_t)19));

    if (!test_true("../test/dtls_mtu_test.c", 69, "SSL_set_cipher_list(srvr_ssl, cs)", (SSL_set_cipher_list(srvr_ssl, cs)) != 0)
            || !test_true("../test/dtls_mtu_test.c", 70, "SSL_set_cipher_list(clnt_ssl, cs)", (SSL_set_cipher_list(clnt_ssl, cs)) != 0)
            || !test_ptr("../test/dtls_mtu_test.c", 71, "sc_bio = SSL_get_rbio(srvr_ssl)", sc_bio = SSL_get_rbio(srvr_ssl))
            || !test_true("../test/dtls_mtu_test.c", 72, "create_ssl_connection(clnt_ssl, srvr_ssl, SSL_ERROR_NONE)", (create_ssl_connection(clnt_ssl, srvr_ssl, 0)) != 0)
                                                                )
        goto end;

    if (debug)
        test_info("../test/dtls_mtu_test.c", 77, "Channel established");



    for (i = 0; i < 30; i++) {
        SSL_ctrl((clnt_ssl),17,(500 + i),
# 82 "../test/dtls_mtu_test.c" 3 4
       ((void *)0)
# 82 "../test/dtls_mtu_test.c"
       );
        mtus[i] = DTLS_get_data_mtu(clnt_ssl);
        if (debug)
            test_info("../test/dtls_mtu_test.c", 85, "%s%s MTU for record mtu %d = %lu", cs, no_etm ? "-noEtM" : "", 500 + i, (unsigned long)mtus[i])

                                                      ;
        if (!test_size_t_ne("../test/dtls_mtu_test.c", 88, "mtus[i]", "0", mtus[i], 0)) {
            test_info("../test/dtls_mtu_test.c", 89, "Cipher %s MTU %d", cs, 500 + i);
            goto end;
        }
    }


    SSL_ctrl((clnt_ssl),17,(1000),
# 95 "../test/dtls_mtu_test.c" 3 4
   ((void *)0)
# 95 "../test/dtls_mtu_test.c"
   );
    ...
    for (s = mtus[0]; s <= mtus[29]; s++) {
        ...
        if (!test_int_eq("../test/dtls_mtu_test.c", 104, "SSL_write(clnt_ssl, buf, s)", "(int)s", SSL_write(clnt_ssl, buf, s), (int)s))
            goto end;
        reclen = BIO_read(sc_bio, buf, sizeof(buf));
        if (debug)
            test_info("../test/dtls_mtu_test.c", 108, "record %zu for payload %zu", reclen, s);

        for (i = 0; i < 30; i++) {


            if (!test_false("../test/dtls_mtu_test.c", 113, "s <= mtus[i] && reclen > (size_t)(500 + i)", (s <= mtus[i] && reclen > (size_t)(500 + i)) != 0)) {




                test_error("../test/dtls_mtu_test.c", 118, "%s: s=%lu, mtus[i]=%lu, reclen=%lu, i=%d", cs, (unsigned long)s, (unsigned long)mtus[i], (unsigned long)reclen, 500 + i)

                                                          ;
                goto end;
            }
            if (!test_false("../test/dtls_mtu_test.c", 123, "s > mtus[i] && reclen <= (size_t)(500 + i)", (s > mtus[i] && reclen <= (size_t)(500 + i)) != 0)) {





                test_error("../test/dtls_mtu_test.c", 129, "%s: s=%lu, mtus[i]=%lu, reclen=%lu, i=%d", cs, (unsigned long)s, (unsigned long)mtus[i], (unsigned long)reclen, 500 + i)
    ...
    if (!test_ptr("../test/dtls_mtu_test.c", 136, "clnt_sc = SSL_CONNECTION_FROM_SSL_ONLY(clnt_ssl)", clnt_sc = ((clnt_ssl) == 
# 136 "../test/dtls_mtu_test.c" 3 4
        ((void *)0) 
# 136 "../test/dtls_mtu_test.c"
        ? 
# 136 "../test/dtls_mtu_test.c" 3 4
        ((void *)0) 
# 136 "../test/dtls_mtu_test.c"
        : ((clnt_ssl)->type == 0 ? ( SSL_CONNECTION *)(clnt_ssl) : 
# 136 "../test/dtls_mtu_test.c" 3 4
        ((void *)0)
# 136 "../test/dtls_mtu_test.c"
        ))))
...
{
    SSL_CTX *ctx = 
# 149 "../test/dtls_mtu_test.c" 3 4
                  ((void *)0)
# 149 "../test/dtls_mtu_test.c"
                      ;
    struct stack_st_SSL_CIPHER *ciphers;
    int i, ret = 0;

    if (!test_ptr("../test/dtls_mtu_test.c", 153, "ctx = SSL_CTX_new(DTLS_method())", ctx = SSL_CTX_new(DTLS_method())))
        goto end;
    ...
    if (!test_true("../test/dtls_mtu_test.c", 165, "SSL_CTX_set_cipher_list(ctx, \"PSK\")", (SSL_CTX_set_cipher_list(ctx, "PSK")) != 0))
        goto end;

    ciphers = SSL_CTX_get_ciphers(ctx);
    for (i = 0; i < OPENSSL_sk_num(ossl_check_const_SSL_CIPHER_sk_type(ciphers)); i++) {
        const SSL_CIPHER *cipher = ((const SSL_CIPHER *)OPENSSL_sk_value(ossl_check_const_SSL_CIPHER_sk_type(ciphers), (i)));
        const char *cipher_name = SSL_CIPHER_get_name(cipher);


        if (!(strncmp(cipher_name, "PSK-" "", sizeof("PSK-") - 1) == 0))
            continue;

        if (!test_int_gt("../test/dtls_mtu_test.c", 177, "ret = mtu_test(ctx, cipher_name, 0)", "0", ret = mtu_test(ctx, cipher_name, 0), 0))
            break;
        test_info("../test/dtls_mtu_test.c", 179, "%s OK", cipher_name);
        if (ret == 1)
            continue;


        if (!test_int_gt("../test/dtls_mtu_test.c", 184, "ret = mtu_test(ctx, cipher_name, 1)", "0", ret = mtu_test(ctx, cipher_name, 1), 0))
            break;
        test_info("../test/dtls_mtu_test.c", 186, "%s without EtM OK", cipher_name);
    }
...
{
    SSL_CTX *ctx = 
# 196 "../test/dtls_mtu_test.c" 3 4
                  ((void *)0)
# 196 "../test/dtls_mtu_test.c"
                      ;
    SSL *srvr_ssl = 
# 197 "../test/dtls_mtu_test.c" 3 4
                   ((void *)0)
# 197 "../test/dtls_mtu_test.c"
                       , *clnt_ssl = 
# 197 "../test/dtls_mtu_test.c" 3 4
                                     ((void *)0)
# 197 "../test/dtls_mtu_test.c"
    ...
    if (!test_ptr("../test/dtls_mtu_test.c", 200, "ctx = SSL_CTX_new(DTLS_method())", ctx = SSL_CTX_new(DTLS_method())))
        goto end;
    ...
    if (!test_true("../test/dtls_mtu_test.c", 207, "SSL_CTX_set_dh_auto(ctx, 1)", (SSL_CTX_ctrl(ctx,118,1,
# 207 "../test/dtls_mtu_test.c" 3 4
        ((void *)0)
# 207 "../test/dtls_mtu_test.c"
        )) != 0))
        goto end;


    if (!test_true("../test/dtls_mtu_test.c", 211, "create_ssl_objects(ctx, ctx, &srvr_ssl, &clnt_ssl, NULL, NULL)", (create_ssl_objects(ctx, ctx, &srvr_ssl, &clnt_ssl, 
# 211 "../test/dtls_mtu_test.c" 3 4
        ((void *)0)
# 211 "../test/dtls_mtu_test.c"
        , 
# 211 "../test/dtls_mtu_test.c" 3 4
        ((void *)0)
# 211 "../test/dtls_mtu_test.c"
    ...
    SSL_set_options(srvr_ssl, ((uint64_t)1 << (uint64_t)12));
    if (!test_true("../test/dtls_mtu_test.c", 216, "DTLS_set_link_mtu(srvr_ssl, 1500)", (SSL_ctrl((srvr_ssl),120,(1500),
# 216 "../test/dtls_mtu_test.c" 3 4
        ((void *)0)
# 216 "../test/dtls_mtu_test.c"
        )) != 0))
        goto end;
    ...
    if (!test_true("../test/dtls_mtu_test.c", 222, "create_ssl_connection(srvr_ssl, clnt_ssl, SSL_ERROR_NONE)", (create_ssl_connection(srvr_ssl, clnt_ssl, 0)) != 0)
                                                         )
        goto end;
...
int setup_tests(void)
{
    ...
    add_test("test_server_mtu_larger_than_max_fragment_length", test_server_mtu_larger_than_max_fragment_length);
    return 1;
}
