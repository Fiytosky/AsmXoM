...
static int test_rc2(const int n)
{
    ...
    RC2_ecb_encrypt(&RC2plain[n][0], buf, &key, 1);
    if (!test_mem_eq("../test/rc2test.c", 56, "&RC2cipher[n][0]", "buf", &RC2cipher[n][0], 8, buf, 8))
        testresult = 0;

    RC2_ecb_encrypt(buf, buf2, &key, 0);
    if (!test_mem_eq("../test/rc2test.c", 60, "&RC2plain[n][0]", "buf2", &RC2plain[n][0], 8, buf2, 8))
        testresult = 0;
...
int setup_tests(void)
{
    ...
    add_all_tests("test_rc2", test_rc2, (sizeof(RC2key)/sizeof((RC2key)[0])), 1);
