...
{
    UI_METHOD *ui_method = 
# 33 "../test/uitest.c" 3 4
                          ((void *)0)
# 33 "../test/uitest.c"
                              ;
    UI *ui = 
# 34 "../test/uitest.c" 3 4
            ((void *)0)
# 34 "../test/uitest.c"
                ;
    ...
    if (!test_ptr("../test/uitest.c", 39, "ui_method = UI_UTIL_wrap_read_pem_callback(test_pem_password_cb, 0)", ui_method = UI_UTIL_wrap_read_pem_callback(test_pem_password_cb, 0))

            || !test_ptr("../test/uitest.c", 41, "ui = UI_new_method(ui_method)", ui = UI_new_method(ui_method)))
        goto err;


    UI_add_user_data(ui, defpass);

    if (UI_add_input_string(ui, "prompt", 0x02,
                             pass, 0, sizeof(pass) - 1) <= 0)
        goto err;

    switch (UI_process(ui)) {
    case -2:
        test_info("../test/uitest.c", 53, "test_old: UI process interrupted or cancelled");
    ...
    if (test_str_eq("../test/uitest.c", 61, "pass", "defpass", pass, defpass))
        ok = 1;
...
static int test_new_ui(void)
{
    ...
    if (test_int_gt("../test/uitest.c", 82, "password_callback(pass, sizeof(pass), 0, &cb_data)", "0", password_callback(pass, sizeof(pass), 0, &cb_data), 0)
            && test_str_eq("../test/uitest.c", 83, "pass", "cb_data.password", pass, cb_data.password))
        ok = 1;
    destroy_ui_method();
    return ok;
}
...
int setup_tests(void)
{
    ...
    add_test("test_new_ui", test_new_ui);
    return 1;
}
