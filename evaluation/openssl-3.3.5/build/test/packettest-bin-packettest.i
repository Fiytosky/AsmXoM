...
# 12 "../test/packettest.c" 2



static unsigned char smbuf[255 + 1];

static int test_PACKET_remaining(void)
{
    PACKET pkt;

    if (!test_true("../test/packettest.c", 21, "PACKET_buf_init(&pkt, smbuf, BUF_LEN)", (PACKET_buf_init(&pkt, smbuf, 255)) != 0)
            || !test_size_t_eq("../test/packettest.c", 22, "PACKET_remaining(&pkt)", "BUF_LEN", PACKET_remaining(&pkt), 255)
            || !test_true("../test/packettest.c", 23, "PACKET_forward(&pkt, BUF_LEN - 1)", (PACKET_forward(&pkt, 255 - 1)) != 0)
            || !test_size_t_eq("../test/packettest.c", 24, "PACKET_remaining(&pkt)", "1", PACKET_remaining(&pkt), 1)
            || !test_true("../test/packettest.c", 25, "PACKET_forward(&pkt, 1)", (PACKET_forward(&pkt, 1)) != 0)
            || !test_size_t_eq("../test/packettest.c", 26, "PACKET_remaining(&pkt)", "0", PACKET_remaining(&pkt), 0))
        return 0;
...
static int test_PACKET_end(void)
{
    ...
    if (!test_true("../test/packettest.c", 36, "PACKET_buf_init(&pkt, smbuf, BUF_LEN)", (PACKET_buf_init(&pkt, smbuf, 255)) != 0)
            || !test_size_t_eq("../test/packettest.c", 37, "PACKET_remaining(&pkt)", "BUF_LEN", PACKET_remaining(&pkt), 255)
            || !test_ptr_eq("../test/packettest.c", 38, "PACKET_end(&pkt)", "smbuf + BUF_LEN", PACKET_end(&pkt), smbuf + 255)
            || !test_true("../test/packettest.c", 39, "PACKET_forward(&pkt, BUF_LEN - 1)", (PACKET_forward(&pkt, 255 - 1)) != 0)
            || !test_ptr_eq("../test/packettest.c", 40, "PACKET_end(&pkt)", "smbuf + BUF_LEN", PACKET_end(&pkt), smbuf + 255)
            || !test_true("../test/packettest.c", 41, "PACKET_forward(&pkt, 1)", (PACKET_forward(&pkt, 1)) != 0)
            || !test_ptr_eq("../test/packettest.c", 42, "PACKET_end(&pkt)", "smbuf + BUF_LEN", PACKET_end(&pkt), smbuf + 255))
        return 0;
...
static int test_PACKET_get_1(void)
{
    ...
    if (!test_true("../test/packettest.c", 53, "PACKET_buf_init(&pkt, smbuf, BUF_LEN)", (PACKET_buf_init(&pkt, smbuf, 255)) != 0)
            || !test_true("../test/packettest.c", 54, "PACKET_get_1(&pkt, &i)", (PACKET_get_1(&pkt, &i)) != 0)
            || !test_uint_eq("../test/packettest.c", 55, "i", "0x02", i, 0x02)
            || !test_true("../test/packettest.c", 56, "PACKET_forward(&pkt, BUF_LEN - 2)", (PACKET_forward(&pkt, 255 - 2)) != 0)
            || !test_true("../test/packettest.c", 57, "PACKET_get_1(&pkt, &i)", (PACKET_get_1(&pkt, &i)) != 0)
            || !test_uint_eq("../test/packettest.c", 58, "i", "0xfe", i, 0xfe)
            || !test_false("../test/packettest.c", 59, "PACKET_get_1(&pkt, &i)", (PACKET_get_1(&pkt, &i)) != 0))
        return 0;
...
static int test_PACKET_get_4(void)
{
    ...
    if (!test_true("../test/packettest.c", 70, "PACKET_buf_init(&pkt, smbuf, BUF_LEN)", (PACKET_buf_init(&pkt, smbuf, 255)) != 0)
            || !test_true("../test/packettest.c", 71, "PACKET_get_4(&pkt, &i)", (PACKET_get_4(&pkt, &i)) != 0)
            || !test_ulong_eq("../test/packettest.c", 72, "i", "0x08060402UL", i, 0x08060402UL)
            || !test_true("../test/packettest.c", 73, "PACKET_forward(&pkt, BUF_LEN - 8)", (PACKET_forward(&pkt, 255 - 8)) != 0)
            || !test_true("../test/packettest.c", 74, "PACKET_get_4(&pkt, &i)", (PACKET_get_4(&pkt, &i)) != 0)
            || !test_ulong_eq("../test/packettest.c", 75, "i", "0xfefcfaf8UL", i, 0xfefcfaf8UL)
            || !test_false("../test/packettest.c", 76, "PACKET_get_4(&pkt, &i)", (PACKET_get_4(&pkt, &i)) != 0))
        return 0;
...
static int test_PACKET_get_net_2(void)
{
    ...
    if (!test_true("../test/packettest.c", 87, "PACKET_buf_init(&pkt, smbuf, BUF_LEN)", (PACKET_buf_init(&pkt, smbuf, 255)) != 0)
            || !test_true("../test/packettest.c", 88, "PACKET_get_net_2(&pkt, &i)", (PACKET_get_net_2(&pkt, &i)) != 0)
            || !test_uint_eq("../test/packettest.c", 89, "i", "0x0204", i, 0x0204)
            || !test_true("../test/packettest.c", 90, "PACKET_forward(&pkt, BUF_LEN - 4)", (PACKET_forward(&pkt, 255 - 4)) != 0)
            || !test_true("../test/packettest.c", 91, "PACKET_get_net_2(&pkt, &i)", (PACKET_get_net_2(&pkt, &i)) != 0)
            || !test_uint_eq("../test/packettest.c", 92, "i", "0xfcfe", i, 0xfcfe)
            || !test_false("../test/packettest.c", 93, "PACKET_get_net_2(&pkt, &i)", (PACKET_get_net_2(&pkt, &i)) != 0))
        return 0;
...
static int test_PACKET_get_net_3(void)
{
    ...
    if (!test_true("../test/packettest.c", 104, "PACKET_buf_init(&pkt, smbuf, BUF_LEN)", (PACKET_buf_init(&pkt, smbuf, 255)) != 0)
            || !test_true("../test/packettest.c", 105, "PACKET_get_net_3(&pkt, &i)", (PACKET_get_net_3(&pkt, &i)) != 0)
            || !test_ulong_eq("../test/packettest.c", 106, "i", "0x020406UL", i, 0x020406UL)
            || !test_true("../test/packettest.c", 107, "PACKET_forward(&pkt, BUF_LEN - 6)", (PACKET_forward(&pkt, 255 - 6)) != 0)
            || !test_true("../test/packettest.c", 108, "PACKET_get_net_3(&pkt, &i)", (PACKET_get_net_3(&pkt, &i)) != 0)
            || !test_ulong_eq("../test/packettest.c", 109, "i", "0xfafcfeUL", i, 0xfafcfeUL)
            || !test_false("../test/packettest.c", 110, "PACKET_get_net_3(&pkt, &i)", (PACKET_get_net_3(&pkt, &i)) != 0))
        return 0;
...
static int test_PACKET_get_net_4(void)
{
    ...
    if (!test_true("../test/packettest.c", 121, "PACKET_buf_init(&pkt, smbuf, BUF_LEN)", (PACKET_buf_init(&pkt, smbuf, 255)) != 0)
            || !test_true("../test/packettest.c", 122, "PACKET_get_net_4(&pkt, &i)", (PACKET_get_net_4(&pkt, &i)) != 0)
            || !test_ulong_eq("../test/packettest.c", 123, "i", "0x02040608UL", i, 0x02040608UL)
            || !test_true("../test/packettest.c", 124, "PACKET_forward(&pkt, BUF_LEN - 8)", (PACKET_forward(&pkt, 255 - 8)) != 0)
            || !test_true("../test/packettest.c", 125, "PACKET_get_net_4(&pkt, &i)", (PACKET_get_net_4(&pkt, &i)) != 0)
            || !test_ulong_eq("../test/packettest.c", 126, "i", "0xf8fafcfeUL", i, 0xf8fafcfeUL)
            || !test_false("../test/packettest.c", 127, "PACKET_get_net_4(&pkt, &i)", (PACKET_get_net_4(&pkt, &i)) != 0))
        return 0;
...
static int test_PACKET_get_sub_packet(void)
{
    ...
    if (!test_true("../test/packettest.c", 138, "PACKET_buf_init(&pkt, smbuf, BUF_LEN)", (PACKET_buf_init(&pkt, smbuf, 255)) != 0)
            || !test_true("../test/packettest.c", 139, "PACKET_get_sub_packet(&pkt, &subpkt, 4)", (PACKET_get_sub_packet(&pkt, &subpkt, 4)) != 0)
            || !test_true("../test/packettest.c", 140, "PACKET_get_net_4(&subpkt, &i)", (PACKET_get_net_4(&subpkt, &i)) != 0)
            || !test_ulong_eq("../test/packettest.c", 141, "i", "0x02040608UL", i, 0x02040608UL)
            || !test_size_t_eq("../test/packettest.c", 142, "PACKET_remaining(&subpkt)", "0", PACKET_remaining(&subpkt), 0)
            || !test_true("../test/packettest.c", 143, "PACKET_forward(&pkt, BUF_LEN - 8)", (PACKET_forward(&pkt, 255 - 8)) != 0)
            || !test_true("../test/packettest.c", 144, "PACKET_get_sub_packet(&pkt, &subpkt, 4)", (PACKET_get_sub_packet(&pkt, &subpkt, 4)) != 0)
            || !test_true("../test/packettest.c", 145, "PACKET_get_net_4(&subpkt, &i)", (PACKET_get_net_4(&subpkt, &i)) != 0)
            || !test_ulong_eq("../test/packettest.c", 146, "i", "0xf8fafcfeUL", i, 0xf8fafcfeUL)
            || !test_size_t_eq("../test/packettest.c", 147, "PACKET_remaining(&subpkt)", "0", PACKET_remaining(&subpkt), 0)
            || !test_false("../test/packettest.c", 148, "PACKET_get_sub_packet(&pkt, &subpkt, 4)", (PACKET_get_sub_packet(&pkt, &subpkt, 4)) != 0))
        return 0;
...
{
    const unsigned char *bytes = 
# 156 "../test/packettest.c" 3 4
                                ((void *)0)
# 156 "../test/packettest.c"
                                    ;
    PACKET pkt;

    if (!test_true("../test/packettest.c", 159, "PACKET_buf_init(&pkt, smbuf, BUF_LEN)", (PACKET_buf_init(&pkt, smbuf, 255)) != 0)
            || !test_true("../test/packettest.c", 160, "PACKET_get_bytes(&pkt, &bytes, 4)", (PACKET_get_bytes(&pkt, &bytes, 4)) != 0)
            || !test_uchar_eq("../test/packettest.c", 161, "bytes[0]", "2", bytes[0], 2)
            || !test_uchar_eq("../test/packettest.c", 162, "bytes[1]", "4", bytes[1], 4)
            || !test_uchar_eq("../test/packettest.c", 163, "bytes[2]", "6", bytes[2], 6)
            || !test_uchar_eq("../test/packettest.c", 164, "bytes[3]", "8", bytes[3], 8)
            || !test_size_t_eq("../test/packettest.c", 165, "PACKET_remaining(&pkt)", "BUF_LEN -4", PACKET_remaining(&pkt), 255 -4)
            || !test_true("../test/packettest.c", 166, "PACKET_forward(&pkt, BUF_LEN - 8)", (PACKET_forward(&pkt, 255 - 8)) != 0)
            || !test_true("../test/packettest.c", 167, "PACKET_get_bytes(&pkt, &bytes, 4)", (PACKET_get_bytes(&pkt, &bytes, 4)) != 0)
            || !test_uchar_eq("../test/packettest.c", 168, "bytes[0]", "0xf8", bytes[0], 0xf8)
            || !test_uchar_eq("../test/packettest.c", 169, "bytes[1]", "0xfa", bytes[1], 0xfa)
            || !test_uchar_eq("../test/packettest.c", 170, "bytes[2]", "0xfc", bytes[2], 0xfc)
            || !test_uchar_eq("../test/packettest.c", 171, "bytes[3]", "0xfe", bytes[3], 0xfe)
            || !test_false("../test/packettest.c", 172, "PACKET_remaining(&pkt)", (PACKET_remaining(&pkt)) != 0))
        return 0;
...
static int test_PACKET_copy_bytes(void)
{
    ...
    if (!test_true("../test/packettest.c", 183, "PACKET_buf_init(&pkt, smbuf, BUF_LEN)", (PACKET_buf_init(&pkt, smbuf, 255)) != 0)
            || !test_true("../test/packettest.c", 184, "PACKET_copy_bytes(&pkt, bytes, 4)", (PACKET_copy_bytes(&pkt, bytes, 4)) != 0)
            || !test_char_eq("../test/packettest.c", 185, "bytes[0]", "2", bytes[0], 2)
            || !test_char_eq("../test/packettest.c", 186, "bytes[1]", "4", bytes[1], 4)
            || !test_char_eq("../test/packettest.c", 187, "bytes[2]", "6", bytes[2], 6)
            || !test_char_eq("../test/packettest.c", 188, "bytes[3]", "8", bytes[3], 8)
            || !test_size_t_eq("../test/packettest.c", 189, "PACKET_remaining(&pkt)", "BUF_LEN - 4", PACKET_remaining(&pkt), 255 - 4)
            || !test_true("../test/packettest.c", 190, "PACKET_forward(&pkt, BUF_LEN - 8)", (PACKET_forward(&pkt, 255 - 8)) != 0)
            || !test_true("../test/packettest.c", 191, "PACKET_copy_bytes(&pkt, bytes, 4)", (PACKET_copy_bytes(&pkt, bytes, 4)) != 0)
            || !test_uchar_eq("../test/packettest.c", 192, "bytes[0]", "0xf8", bytes[0], 0xf8)
            || !test_uchar_eq("../test/packettest.c", 193, "bytes[1]", "0xfa", bytes[1], 0xfa)
            || !test_uchar_eq("../test/packettest.c", 194, "bytes[2]", "0xfc", bytes[2], 0xfc)
            || !test_uchar_eq("../test/packettest.c", 195, "bytes[3]", "0xfe", bytes[3], 0xfe)
            || !test_false("../test/packettest.c", 196, "PACKET_remaining(&pkt)", (PACKET_remaining(&pkt)) != 0))
        return 0;
...
static int test_PACKET_copy_all(void)
{
    ...
    if (!test_true("../test/packettest.c", 208, "PACKET_buf_init(&pkt, smbuf, BUF_LEN)", (PACKET_buf_init(&pkt, smbuf, 255)) != 0)
            || !test_true("../test/packettest.c", 209, "PACKET_copy_all(&pkt, tmp, BUF_LEN, &len)", (PACKET_copy_all(&pkt, tmp, 255, &len)) != 0)
            || !test_size_t_eq("../test/packettest.c", 210, "len", "BUF_LEN", len, 255)
            || !test_mem_eq("../test/packettest.c", 211, "smbuf", "tmp", smbuf, 255, tmp, 255)
            || !test_size_t_eq("../test/packettest.c", 212, "PACKET_remaining(&pkt)", "BUF_LEN", PACKET_remaining(&pkt), 255)
            || !test_false("../test/packettest.c", 213, "PACKET_copy_all(&pkt, tmp, BUF_LEN - 1, &len)", (PACKET_copy_all(&pkt, tmp, 255 - 1, &len)) != 0))
        return 0;
...
{
    unsigned char *data = 
# 221 "../test/packettest.c" 3 4
                         ((void *)0)
# 221 "../test/packettest.c"
                             ;
    size_t len;
    PACKET pkt;
    int result = 0;

    if (!test_true("../test/packettest.c", 226, "PACKET_buf_init(&pkt, smbuf, BUF_LEN)", (PACKET_buf_init(&pkt, smbuf, 255)) != 0)
            || !test_true("../test/packettest.c", 227, "PACKET_memdup(&pkt, &data, &len)", (PACKET_memdup(&pkt, &data, &len)) != 0)
            || !test_size_t_eq("../test/packettest.c", 228, "len", "BUF_LEN", len, 255)
            || !test_mem_eq("../test/packettest.c", 229, "data", "PACKET_data(&pkt)", data, len, PACKET_data(&pkt), len)
            || !test_true("../test/packettest.c", 230, "PACKET_forward(&pkt, 10)", (PACKET_forward(&pkt, 10)) != 0)
            || !test_true("../test/packettest.c", 231, "PACKET_memdup(&pkt, &data, &len)", (PACKET_memdup(&pkt, &data, &len)) != 0)
            || !test_size_t_eq("../test/packettest.c", 232, "len", "BUF_LEN - 10", len, 255 - 10)
            || !test_mem_eq("../test/packettest.c", 233, "data", "PACKET_data(&pkt)", data, len, PACKET_data(&pkt), len))
        goto end;
    result = 1;
end:
    CRYPTO_free(data, "../test/packettest.c", 237);
    return result;
}
...
static int test_PACKET_strndup(void)
{
    ...
    char *data = 
# 244 "../test/packettest.c" 3 4
                ((void *)0)
# 244 "../test/packettest.c"
                    ;
    PACKET pkt;
    int result = 0;
    ...
    if (!test_true("../test/packettest.c", 252, "PACKET_buf_init(&pkt, (unsigned char*)buf1, 10)", (PACKET_buf_init(&pkt, (unsigned char*)buf1, 10)) != 0)
            || !test_true("../test/packettest.c", 253, "PACKET_strndup(&pkt, &data)", (PACKET_strndup(&pkt, &data)) != 0)
            || !test_size_t_eq("../test/packettest.c", 254, "strlen(data)", "10", strlen(data), 10)
            || !test_strn_eq("../test/packettest.c", 255, "data", "buf1", data, 10, buf1, 10)
            || !test_true("../test/packettest.c", 256, "PACKET_buf_init(&pkt, (unsigned char*)buf2, 10)", (PACKET_buf_init(&pkt, (unsigned char*)buf2, 10)) != 0)
            || !test_true("../test/packettest.c", 257, "PACKET_strndup(&pkt, &data)", (PACKET_strndup(&pkt, &data)) != 0)
            || !test_size_t_eq("../test/packettest.c", 258, "strlen(data)", "5", strlen(data), 5)
            || !test_str_eq("../test/packettest.c", 259, "data", "buf2", data, buf2))
        goto end;

    result = 1;
end:
    CRYPTO_free(data, "../test/packettest.c", 264);
    return result;
}
...
static int test_PACKET_contains_zero_byte(void)
{
    ...
    if (!test_true("../test/packettest.c", 277, "PACKET_buf_init(&pkt, (unsigned char*)buf1, 10)", (PACKET_buf_init(&pkt, (unsigned char*)buf1, 10)) != 0)
            || !test_false("../test/packettest.c", 278, "PACKET_contains_zero_byte(&pkt)", (PACKET_contains_zero_byte(&pkt)) != 0)
            || !test_true("../test/packettest.c", 279, "PACKET_buf_init(&pkt, (unsigned char*)buf2, 10)", (PACKET_buf_init(&pkt, (unsigned char*)buf2, 10)) != 0)
            || !test_true("../test/packettest.c", 280, "PACKET_contains_zero_byte(&pkt)", (PACKET_contains_zero_byte(&pkt)) != 0))
        return 0;
...
# 288 "../test/packettest.c" 3 4
                               ((void *)0)
# 288 "../test/packettest.c"
                                   ;
    PACKET pkt;

    if (!test_true("../test/packettest.c", 291, "PACKET_buf_init(&pkt, smbuf, BUF_LEN)", (PACKET_buf_init(&pkt, smbuf, 255)) != 0)
            || !test_true("../test/packettest.c", 292, "PACKET_forward(&pkt, 1)", (PACKET_forward(&pkt, 1)) != 0)
            || !test_true("../test/packettest.c", 293, "PACKET_get_bytes(&pkt, &byte, 1)", (PACKET_get_bytes(&pkt, &byte, 1)) != 0)
            || !test_uchar_eq("../test/packettest.c", 294, "byte[0]", "4", byte[0], 4)
            || !test_true("../test/packettest.c", 295, "PACKET_forward(&pkt, BUF_LEN - 3)", (PACKET_forward(&pkt, 255 - 3)) != 0)
            || !test_true("../test/packettest.c", 296, "PACKET_get_bytes(&pkt, &byte, 1)", (PACKET_get_bytes(&pkt, &byte, 1)) != 0)
            || !test_uchar_eq("../test/packettest.c", 297, "byte[0]", "0xfe", byte[0], 0xfe))
        return 0;
...
static int test_PACKET_buf_init(void)
{
    ...
    if (!test_true("../test/packettest.c", 309, "PACKET_buf_init(&pkt, buf1, 4)", (PACKET_buf_init(&pkt, buf1, 4)) != 0)
            || !test_size_t_eq("../test/packettest.c", 310, "PACKET_remaining(&pkt)", "4", PACKET_remaining(&pkt), 4)
            || !test_true("../test/packettest.c", 311, "PACKET_buf_init(&pkt, buf1, BUF_LEN)", (PACKET_buf_init(&pkt, buf1, 255)) != 0)
            || !test_size_t_eq("../test/packettest.c", 312, "PACKET_remaining(&pkt)", "BUF_LEN", PACKET_remaining(&pkt), 255)
            || !test_false("../test/packettest.c", 313, "PACKET_buf_init(&pkt, buf1, -1)", (PACKET_buf_init(&pkt, buf1, -1)) != 0))
        return 0;
...
static int test_PACKET_null_init(void)
{
    ...
    if (!test_size_t_eq("../test/packettest.c", 324, "PACKET_remaining(&pkt)", "0", PACKET_remaining(&pkt), 0)
            || !test_false("../test/packettest.c", 325, "PACKET_forward(&pkt, 1)", (PACKET_forward(&pkt, 1)) != 0))
        return 0;
...
static int test_PACKET_equal(void)
{
    ...
    if (!test_true("../test/packettest.c", 335, "PACKET_buf_init(&pkt, smbuf, 4)", (PACKET_buf_init(&pkt, smbuf, 4)) != 0)
            || !test_true("../test/packettest.c", 336, "PACKET_equal(&pkt, smbuf, 4)", (PACKET_equal(&pkt, smbuf, 4)) != 0)
            || !test_false("../test/packettest.c", 337, "PACKET_equal(&pkt, smbuf + 1, 4)", (PACKET_equal(&pkt, smbuf + 1, 4)) != 0)
            || !test_true("../test/packettest.c", 338, "PACKET_buf_init(&pkt, smbuf, BUF_LEN)", (PACKET_buf_init(&pkt, smbuf, 255)) != 0)
            || !test_true("../test/packettest.c", 339, "PACKET_equal(&pkt, smbuf, BUF_LEN)", (PACKET_equal(&pkt, smbuf, 255)) != 0)
            || !test_false("../test/packettest.c", 340, "PACKET_equal(&pkt, smbuf, BUF_LEN - 1)", (PACKET_equal(&pkt, smbuf, 255 - 1)) != 0)
            || !test_false("../test/packettest.c", 341, "PACKET_equal(&pkt, smbuf, BUF_LEN + 1)", (PACKET_equal(&pkt, smbuf, 255 + 1)) != 0)
            || !test_false("../test/packettest.c", 342, "PACKET_equal(&pkt, smbuf, 0)", (PACKET_equal(&pkt, smbuf, 0)) != 0))
        return 0;
...
static int test_PACKET_get_length_prefixed_1(void)
{
    ...
    if (!test_true("../test/packettest.c", 360, "PACKET_buf_init(&pkt, buf1, BUF_LEN)", (PACKET_buf_init(&pkt, buf1, 255)) != 0)
            || !test_true("../test/packettest.c", 361, "PACKET_buf_init(&short_pkt, buf1, len)", (PACKET_buf_init(&short_pkt, buf1, len)) != 0)
            || !test_true("../test/packettest.c", 362, "PACKET_get_length_prefixed_1(&pkt, &subpkt)", (PACKET_get_length_prefixed_1(&pkt, &subpkt)) != 0)
            || !test_size_t_eq("../test/packettest.c", 363, "PACKET_remaining(&subpkt)", "len", PACKET_remaining(&subpkt), len)
            || !test_true("../test/packettest.c", 364, "PACKET_get_net_2(&subpkt, &i)", (PACKET_get_net_2(&subpkt, &i)) != 0)
            || !test_uint_eq("../test/packettest.c", 365, "i", "0x0204", i, 0x0204)
            || !test_false("../test/packettest.c", 366, "PACKET_get_length_prefixed_1(&short_pkt, &subpkt)", (PACKET_get_length_prefixed_1(&short_pkt, &subpkt)) != 0)
            || !test_size_t_eq("../test/packettest.c", 367, "PACKET_remaining(&short_pkt)", "len", PACKET_remaining(&short_pkt), len))
        return 0;
...
static int test_PACKET_get_length_prefixed_2(void)
{
    ...
    if (!test_true("../test/packettest.c", 384, "PACKET_buf_init(&pkt, buf1, 1024)", (PACKET_buf_init(&pkt, buf1, 1024)) != 0)
            || !test_true("../test/packettest.c", 385, "PACKET_buf_init(&short_pkt, buf1, len)", (PACKET_buf_init(&short_pkt, buf1, len)) != 0)
            || !test_true("../test/packettest.c", 386, "PACKET_get_length_prefixed_2(&pkt, &subpkt)", (PACKET_get_length_prefixed_2(&pkt, &subpkt)) != 0)
            || !test_size_t_eq("../test/packettest.c", 387, "PACKET_remaining(&subpkt)", "len", PACKET_remaining(&subpkt), len)
            || !test_true("../test/packettest.c", 388, "PACKET_get_net_2(&subpkt, &i)", (PACKET_get_net_2(&subpkt, &i)) != 0)
            || !test_uint_eq("../test/packettest.c", 389, "i", "0x0608", i, 0x0608)
            || !test_false("../test/packettest.c", 390, "PACKET_get_length_prefixed_2(&short_pkt, &subpkt)", (PACKET_get_length_prefixed_2(&short_pkt, &subpkt)) != 0)
            || !test_size_t_eq("../test/packettest.c", 391, "PACKET_remaining(&short_pkt)", "len", PACKET_remaining(&short_pkt), len))
        return 0;
...
static int test_PACKET_get_length_prefixed_3(void)
{
    ...
    if (!test_true("../test/packettest.c", 408, "PACKET_buf_init(&pkt, buf1, 1024)", (PACKET_buf_init(&pkt, buf1, 1024)) != 0)
            || !test_true("../test/packettest.c", 409, "PACKET_buf_init(&short_pkt, buf1, len)", (PACKET_buf_init(&short_pkt, buf1, len)) != 0)
            || !test_true("../test/packettest.c", 410, "PACKET_get_length_prefixed_3(&pkt, &subpkt)", (PACKET_get_length_prefixed_3(&pkt, &subpkt)) != 0)
            || !test_size_t_eq("../test/packettest.c", 411, "PACKET_remaining(&subpkt)", "len", PACKET_remaining(&subpkt), len)
            || !test_true("../test/packettest.c", 412, "PACKET_get_net_2(&subpkt, &i)", (PACKET_get_net_2(&subpkt, &i)) != 0)
            || !test_uint_eq("../test/packettest.c", 413, "i", "0x0608", i, 0x0608)
            || !test_false("../test/packettest.c", 414, "PACKET_get_length_prefixed_3(&short_pkt, &subpkt)", (PACKET_get_length_prefixed_3(&short_pkt, &subpkt)) != 0)
            || !test_size_t_eq("../test/packettest.c", 415, "PACKET_remaining(&short_pkt)", "len", PACKET_remaining(&short_pkt), len))
        return 0;
...
static int test_PACKET_as_length_prefixed_1(void)
{
    ...
    if (!test_true("../test/packettest.c", 433, "PACKET_buf_init(&pkt, buf1, BUF_LEN)", (PACKET_buf_init(&pkt, buf1, 255)) != 0)
            || !test_true("../test/packettest.c", 434, "PACKET_buf_init(&exact_pkt, buf1, len + 1)", (PACKET_buf_init(&exact_pkt, buf1, len + 1)) != 0)
            || !test_false("../test/packettest.c", 435, "PACKET_as_length_prefixed_1(&pkt, &subpkt)", (PACKET_as_length_prefixed_1(&pkt, &subpkt)) != 0)
            || !test_size_t_eq("../test/packettest.c", 436, "PACKET_remaining(&pkt)", "BUF_LEN", PACKET_remaining(&pkt), 255)
            || !test_true("../test/packettest.c", 437, "PACKET_as_length_prefixed_1(&exact_pkt, &subpkt)", (PACKET_as_length_prefixed_1(&exact_pkt, &subpkt)) != 0)
            || !test_size_t_eq("../test/packettest.c", 438, "PACKET_remaining(&exact_pkt)", "0", PACKET_remaining(&exact_pkt), 0)
            || !test_size_t_eq("../test/packettest.c", 439, "PACKET_remaining(&subpkt)", "len", PACKET_remaining(&subpkt), len))
        return 0;
...
static int test_PACKET_as_length_prefixed_2(void)
{
    ...
    if (!test_true("../test/packettest.c", 456, "PACKET_buf_init(&pkt, buf, 1024)", (PACKET_buf_init(&pkt, buf, 1024)) != 0)
            || !test_true("../test/packettest.c", 457, "PACKET_buf_init(&exact_pkt, buf, len + 2)", (PACKET_buf_init(&exact_pkt, buf, len + 2)) != 0)
            || !test_false("../test/packettest.c", 458, "PACKET_as_length_prefixed_2(&pkt, &subpkt)", (PACKET_as_length_prefixed_2(&pkt, &subpkt)) != 0)
            || !test_size_t_eq("../test/packettest.c", 459, "PACKET_remaining(&pkt)", "1024", PACKET_remaining(&pkt), 1024)
            || !test_true("../test/packettest.c", 460, "PACKET_as_length_prefixed_2(&exact_pkt, &subpkt)", (PACKET_as_length_prefixed_2(&exact_pkt, &subpkt)) != 0)
            || !test_size_t_eq("../test/packettest.c", 461, "PACKET_remaining(&exact_pkt)", "0", PACKET_remaining(&exact_pkt), 0)
            || !test_size_t_eq("../test/packettest.c", 462, "PACKET_remaining(&subpkt)", "len", PACKET_remaining(&subpkt), len))
        return 0;
...
static int test_PACKET_get_quic_vlint(void)
{
    ...
    for (i = 0; i < (sizeof(cases)/sizeof((cases)[0])); ++i) {
        ...
        if (!test_true("../test/packettest.c", 509, "PACKET_buf_init(&pkt, cases[i].buf, sizeof(cases[i].buf))", (PACKET_buf_init(&pkt, cases[i].buf, sizeof(cases[i].buf))) != 0)
                || !test_true("../test/packettest.c", 510, "PACKET_get_quic_vlint(&pkt, &v)", (PACKET_get_quic_vlint(&pkt, &v)) != 0)
                || !test_uint64_t_eq("../test/packettest.c", 511, "v", "cases[i].value", v, cases[i].value)
                || !test_size_t_eq("../test/packettest.c", 512, "PACKET_remaining(&pkt)", "sizeof(cases[i].buf) - cases[i].expected_read_count", PACKET_remaining(&pkt), sizeof(cases[i].buf) - cases[i].expected_read_count)
...
static int test_PACKET_get_quic_length_prefixed(void)
{
    ...
    for (i = 0; i < (sizeof(cases)/sizeof((cases)[0])); ++i) {
        ...
        if (!test_true("../test/packettest.c", 549, "PACKET_buf_init(&pkt, cases[i].buf, cases[i].fail ? sizeof(cases[i].buf) : cases[i].enclen + cases[i].len)", (PACKET_buf_init(&pkt, cases[i].buf, cases[i].fail ? sizeof(cases[i].buf) : cases[i].enclen + cases[i].len)) != 0)


                                                                           )
            return 0;

        if (!test_int_eq("../test/packettest.c", 555, "PACKET_get_quic_length_prefixed(&pkt, &subpkt)", "!cases[i].fail", PACKET_get_quic_length_prefixed(&pkt, &subpkt), !cases[i].fail))
            return 0;

        if (cases[i].fail) {
            if (!test_ptr_eq("../test/packettest.c", 559, "pkt.curr", "cases[i].buf", pkt.curr, cases[i].buf))
                return 0;
            continue;
        }

        if (!test_ptr_eq("../test/packettest.c", 564, "subpkt.curr", "cases[i].buf + cases[i].enclen", subpkt.curr, cases[i].buf + cases[i].enclen))
            return 0;

        if (!test_size_t_eq("../test/packettest.c", 567, "subpkt.remaining", "cases[i].len", subpkt.remaining, cases[i].len))
            return 0;
    }
...
int setup_tests(void)
{
    ...
    add_test("test_PACKET_end", test_PACKET_end);
