...
static int test_static_sha_common(const char *input, size_t length,
                                  ...
                                  unsigned char *(*md)(const unsigned char *d,
                                                       ...
                                                       unsigned char *md))
{
    unsigned char buf[64], *sbuf;
    const unsigned char *in = (unsigned char *)input;
    const size_t in_len = strlen(input);

    sbuf = (*md)(in, in_len, buf);
    if (!test_ptr("../test/sha_test.c", 25, "sbuf", sbuf)
            || !test_ptr_eq("../test/sha_test.c", 26, "sbuf", "buf", sbuf, buf)
            || !test_mem_eq("../test/sha_test.c", 27, "sbuf", "out", sbuf, length, out, length))
        return 0;
    sbuf = (*md)(in, in_len, 
# 29 "../test/sha_test.c" 3 4
                            ((void *)0)
# 29 "../test/sha_test.c"
                                );
    if (!test_ptr("../test/sha_test.c", 30, "sbuf", sbuf)
            || !test_ptr_ne("../test/sha_test.c", 31, "sbuf", "buf", sbuf, buf)
            || !test_mem_eq("../test/sha_test.c", 32, "sbuf", "out", sbuf, length, out, length))
        return 0;
    return 1;
}
...
{
    static const unsigned char output[20] = {
        0xa9, 0x99, 0x3e, 0x36, 0x47, 0x06, 0x81, 0x6a,
        0xba, 0x3e, 0x25, 0x71, 0x78, 0x50, 0xc2, 0x6c,
        0x9c, 0xd0, 0xd8, 0x9d
    };

    return test_static_sha_common("abc", 20, output, &SHA1);
}

static int test_static_sha224(void)
{
    static const unsigned char output[28] = {
    ...
    return test_static_sha_common("abc", 28, output, &SHA224);
}

static int test_static_sha256(void)
{
    static const unsigned char output[32] = {
    ...
    return test_static_sha_common("abc", 32, output, &SHA256);
}

static int test_static_sha384(void)
{
    static const unsigned char output[48] = {
    ...
    return test_static_sha_common("abc", 48, output, &SHA384);
}

static int test_static_sha512(void)
{
    static const unsigned char output[64] = {
    ...
    return test_static_sha_common("abc", 64, output, &SHA512);
}
