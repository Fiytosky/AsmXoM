# 0 "../test/servername_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/servername_test.c"
# 11 "../test/servername_test.c"
...
static char *cert = 
# 31 "../test/servername_test.c" 3 4
                   ((void *)0)
# 31 "../test/servername_test.c"
                       ;
static char *privkey = 
# 32 "../test/servername_test.c" 3 4
                      ((void *)0)
# 32 "../test/servername_test.c"
                          ;
...
static int get_sni_from_client_hello(BIO *bio, char **sni)
{
    ...
    if (!test_long_ge("../test/servername_test.c", 55, "len = BIO_get_mem_data(bio, (char **)&data)", "0", len = BIO_ctrl(bio,3,0,(char *)((char **)&data)), 0)
            || !test_true("../test/servername_test.c", 56, "PACKET_buf_init(&pkt, data, len)", (PACKET_buf_init(&pkt, data, len)) != 0)

            || !PACKET_forward(&pkt, 5)

            || !test_true("../test/servername_test.c", 60, "PACKET_forward(&pkt, SSL3_HM_HEADER_LENGTH)", (PACKET_forward(&pkt, 4)) != 0)

            || !test_true("../test/servername_test.c", 62, "PACKET_forward(&pkt, CLIENT_VERSION_LEN + SSL3_RANDOM_SIZE)", (PACKET_forward(&pkt, 2 + 32)) != 0)


            || !test_true("../test/servername_test.c", 65, "PACKET_get_length_prefixed_1(&pkt, &pkt2)", (PACKET_get_length_prefixed_1(&pkt, &pkt2)) != 0)

            || !test_true("../test/servername_test.c", 67, "PACKET_get_length_prefixed_2(&pkt, &pkt2)", (PACKET_get_length_prefixed_2(&pkt, &pkt2)) != 0)

            || !test_true("../test/servername_test.c", 69, "PACKET_get_length_prefixed_1(&pkt, &pkt2)", (PACKET_get_length_prefixed_1(&pkt, &pkt2)) != 0)

            || !test_true("../test/servername_test.c", 71, "PACKET_as_length_prefixed_2(&pkt, &pkt2)", (PACKET_as_length_prefixed_2(&pkt, &pkt2)) != 0))
        goto end;


    while (PACKET_remaining(&pkt2)) {
        if (!test_true("../test/servername_test.c", 76, "PACKET_get_net_2(&pkt2, &type)", (PACKET_get_net_2(&pkt2, &type)) != 0)
                || !test_true("../test/servername_test.c", 77, "PACKET_get_length_prefixed_2(&pkt2, &pkt3)", (PACKET_get_length_prefixed_2(&pkt2, &pkt3)) != 0))
            goto end;
        if (type == 0) {
            if (!test_true("../test/servername_test.c", 80, "PACKET_get_length_prefixed_2(&pkt3, &pkt4)", (PACKET_get_length_prefixed_2(&pkt3, &pkt4)) != 0)
                    || !test_uint_ne("../test/servername_test.c", 81, "PACKET_remaining(&pkt4)", "0", PACKET_remaining(&pkt4), 0)
                    || !test_true("../test/servername_test.c", 82, "PACKET_get_1(&pkt4, &servname_type)", (PACKET_get_1(&pkt4, &servname_type)) != 0)
                    || !test_uint_eq("../test/servername_test.c", 83, "servname_type", "TLSEXT_NAMETYPE_host_name", servname_type, 0)
                    || !test_true("../test/servername_test.c", 84, "PACKET_get_length_prefixed_2(&pkt4, &pkt5)", (PACKET_get_length_prefixed_2(&pkt4, &pkt5)) != 0)
                    || !test_uint_le("../test/servername_test.c", 85, "PACKET_remaining(&pkt5)", "TLSEXT_MAXLEN_host_name", PACKET_remaining(&pkt5), 255)
                    || !test_false("../test/servername_test.c", 86, "PACKET_contains_zero_byte(&pkt5)", (PACKET_contains_zero_byte(&pkt5)) != 0)
                    || !test_true("../test/servername_test.c", 87, "PACKET_strndup(&pkt5, sni)", (PACKET_strndup(&pkt5, sni)) != 0))
                goto end;
            ret = 1;
            goto end;
        }
...
static int client_setup_sni_before_state(void)
{
    SSL_CTX *ctx;
    SSL *con = 
# 100 "../test/servername_test.c" 3 4
              ((void *)0)
# 100 "../test/servername_test.c"
                  ;
    BIO *rbio;
    BIO *wbio;
    char *hostname = 
# 103 "../test/servername_test.c" 3 4
                    ((void *)0)
# 103 "../test/servername_test.c"
                        ;
    int ret = 0;


    ctx = SSL_CTX_new(TLS_method());
    if (!test_ptr("../test/servername_test.c", 108, "ctx", ctx))
        goto end;

    if (maxversion > 0
            && !test_true("../test/servername_test.c", 112, "SSL_CTX_set_max_proto_version(ctx, maxversion)", (SSL_CTX_ctrl(ctx, 124, maxversion, 
# 112 "../test/servername_test.c" 3 4
               ((void *)0)
# 112 "../test/servername_test.c"
               )) != 0))
        goto end;

    con = SSL_new(ctx);
    if (!test_ptr("../test/servername_test.c", 116, "con", con))
        goto end;


    SSL_ctrl(con,55,0, (void *)host);

    rbio = BIO_new(BIO_s_mem());
    wbio = BIO_new(BIO_s_mem());
    if (!test_ptr("../test/servername_test.c", 124, "rbio", rbio)|| !test_ptr("../test/servername_test.c", 124, "wbio", wbio)) {
        BIO_free(rbio);
        BIO_free(wbio);
        goto end;
    }
    ...
    if (!test_int_le("../test/servername_test.c", 132, "SSL_connect(con)", "0", SSL_connect(con), 0))

        goto end;
    if (!test_true("../test/servername_test.c", 135, "get_sni_from_client_hello(wbio, &hostname)", (get_sni_from_client_hello(wbio, &hostname)) != 0))

        goto end;
    if (!test_str_eq("../test/servername_test.c", 138, "hostname", "host", hostname, host))

        goto end;
    ret = 1;
end:
    CRYPTO_free(hostname, "../test/servername_test.c", 143);
    SSL_free(con);
    SSL_CTX_free(ctx);
    return ret;
}
...
static int client_setup_sni_after_state(void)
{
    SSL_CTX *ctx;
    SSL *con = 
# 152 "../test/servername_test.c" 3 4
              ((void *)0)
# 152 "../test/servername_test.c"
                  ;
    BIO *rbio;
    BIO *wbio;
    char *hostname = 
# 155 "../test/servername_test.c" 3 4
                    ((void *)0)
# 155 "../test/servername_test.c"
                        ;
    int ret = 0;


    ctx = SSL_CTX_new(TLS_method());
    if (!test_ptr("../test/servername_test.c", 160, "ctx", ctx))
        goto end;

    if (maxversion > 0
            && !test_true("../test/servername_test.c", 164, "SSL_CTX_set_max_proto_version(ctx, maxversion)", (SSL_CTX_ctrl(ctx, 124, maxversion, 
# 164 "../test/servername_test.c" 3 4
               ((void *)0)
# 164 "../test/servername_test.c"
               )) != 0))
        goto end;

    con = SSL_new(ctx);
    if (!test_ptr("../test/servername_test.c", 168, "con", con))
        goto end;

    rbio = BIO_new(BIO_s_mem());
    wbio = BIO_new(BIO_s_mem());
    if (!test_ptr("../test/servername_test.c", 173, "rbio", rbio)|| !test_ptr("../test/servername_test.c", 173, "wbio", wbio)) {
        BIO_free(rbio);
        BIO_free(wbio);
        goto end;
    }
    ...
    if (!test_int_le("../test/servername_test.c", 185, "SSL_connect(con)", "0", SSL_connect(con), 0))

        goto end;
    if (!test_true("../test/servername_test.c", 188, "get_sni_from_client_hello(wbio, &hostname)", (get_sni_from_client_hello(wbio, &hostname)) != 0))

        goto end;
    if (!test_str_eq("../test/servername_test.c", 191, "hostname", "host", hostname, host))

        goto end;
    ret = 1;
end:
    CRYPTO_free(hostname, "../test/servername_test.c", 196);
    SSL_free(con);
    SSL_CTX_free(ctx);
    return ret;
}
...
{
    SSL_CTX *cctx = 
# 204 "../test/servername_test.c" 3 4
                   ((void *)0)
# 204 "../test/servername_test.c"
                       , *sctx = 
# 204 "../test/servername_test.c" 3 4
                                 ((void *)0)
# 204 "../test/servername_test.c"
                                     ;
    SSL *clientssl = 
# 205 "../test/servername_test.c" 3 4
                    ((void *)0)
# 205 "../test/servername_test.c"
                        , *serverssl = 
# 205 "../test/servername_test.c" 3 4
                                       ((void *)0)
# 205 "../test/servername_test.c"
    ...
    if (!test_true("../test/servername_test.c", 208, "create_ssl_ctx_pair(NULL, TLS_server_method(), TLS_client_method(), TLS1_VERSION, 0, &sctx, &cctx, cert, privkey)", (create_ssl_ctx_pair(
# 208 "../test/servername_test.c" 3 4
        ((void *)0)
# 208 "../test/servername_test.c"
        , TLS_server_method(), TLS_client_method(), 0x0301, 0, &sctx, &cctx, cert, privkey)) != 0)



            || !test_true("../test/servername_test.c", 212, "create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL, NULL)", (create_ssl_objects(sctx, cctx, &serverssl, &clientssl, 
# 212 "../test/servername_test.c" 3 4
               ((void *)0)
# 212 "../test/servername_test.c"
               , 
# 212 "../test/servername_test.c" 3 4
               ((void *)0)
# 212 "../test/servername_test.c"
    ...
    SSL_ctrl(serverssl,55,0, (void *)host);

    if (!test_true("../test/servername_test.c", 219, "create_ssl_connection(serverssl, clientssl, SSL_ERROR_NONE)", (create_ssl_connection(serverssl, clientssl, 0)) != 0))
        goto end;

    if (!test_ptr_null("../test/servername_test.c", 222, "SSL_get_servername(serverssl, TLSEXT_NAMETYPE_host_name)", SSL_get_servername(serverssl, 0))
                                                                     ) {
...
int setup_tests(void)
{
    if (!test_skip_common_options()) {
        test_error("../test/servername_test.c", 258, "Error parsing test options\n");
        return 0;
    }

    if (!test_ptr("../test/servername_test.c", 262, "cert = test_get_argument(0)", cert = test_get_argument(0))
            || !test_ptr("../test/servername_test.c", 263, "privkey = test_get_argument(1)", privkey = test_get_argument(1)))
        return 0;

    add_all_tests("test_servername", test_servername, (sizeof(sni_test_fns)/sizeof((sni_test_fns)[0])), 1);
    return 1;
}
