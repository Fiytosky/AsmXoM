# 0 "../test/quic_txpim_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/quic_txpim_test.c"
# 10 "../test/quic_txpim_test.c"
...
static int test_txpim(void)
{
    ...
    QUIC_TXPIM *txpim;
    size_t i, j;
    QUIC_TXPIM_PKT *pkts[10] = {
# 19 "../test/quic_txpim_test.c" 3 4
                               ((void *)0)
# 19 "../test/quic_txpim_test.c"
                                   };
    QUIC_TXPIM_CHUNK chunks[3];
    const QUIC_TXPIM_CHUNK *rchunks;

    if (!test_ptr("../test/quic_txpim_test.c", 23, "txpim = ossl_quic_txpim_new()", txpim = ossl_quic_txpim_new()))
        goto err;

    for (i = 0; i < (sizeof(pkts)/sizeof((pkts)[0])); ++i) {
        if (!test_ptr("../test/quic_txpim_test.c", 27, "pkts[i] = ossl_quic_txpim_pkt_alloc(txpim)", pkts[i] = ossl_quic_txpim_pkt_alloc(txpim)))
            goto err;

        if (!test_size_t_eq("../test/quic_txpim_test.c", 30, "ossl_quic_txpim_pkt_get_num_chunks(pkts[i])", "0", ossl_quic_txpim_pkt_get_num_chunks(pkts[i]), 0))
            goto err;
        ...
        for (j = 0; j < (sizeof(chunks)/sizeof((chunks)[0])); ++j) {
            ...
            if (!test_true("../test/quic_txpim_test.c", 38, "ossl_quic_txpim_pkt_append_chunk(pkts[i], chunks + j)", (ossl_quic_txpim_pkt_append_chunk(pkts[i], chunks + j)) != 0))
                goto err;
        }

        if (!test_size_t_eq("../test/quic_txpim_test.c", 42, "ossl_quic_txpim_pkt_get_num_chunks(pkts[i])", "OSSL_NELEM(chunks)", ossl_quic_txpim_pkt_get_num_chunks(pkts[i]), (sizeof(chunks)/sizeof((chunks)[0])))
                                               )
            goto err;

        rchunks = ossl_quic_txpim_pkt_get_chunks(pkts[i]);
        if (!test_uint64_t_eq("../test/quic_txpim_test.c", 47, "rchunks[0].stream_id", "98", rchunks[0].stream_id, 98)
            || !test_uint64_t_eq("../test/quic_txpim_test.c", 48, "rchunks[1].stream_id", "99", rchunks[1].stream_id, 99)
            || !test_uint64_t_eq("../test/quic_txpim_test.c", 49, "rchunks[2].stream_id", "100", rchunks[2].stream_id, 100))
            goto err;
    }
...
err:
    for (i = 0; i < (sizeof(pkts)/sizeof((pkts)[0])); ++i)
        if (txpim != 
# 56 "../test/quic_txpim_test.c" 3 4
                    ((void *)0) 
# 56 "../test/quic_txpim_test.c"
                         && pkts[i] != 
# 56 "../test/quic_txpim_test.c" 3 4
                                       ((void *)0)
# 56 "../test/quic_txpim_test.c"
...
int setup_tests(void)
{
    add_test("test_txpim", test_txpim);
    return 1;
}
