...
__attribute__((deprecated("Since OpenSSL " "3.0"))) const char *RC4_options(void);
__attribute__((deprecated("Since OpenSSL " "3.0"))) void RC4_set_key(RC4_KEY *key, int len,
                                       const unsigned char *data);
__attribute__((deprecated("Since OpenSSL " "3.0"))) void RC4(RC4_KEY *key, size_t len,
...
# 64 "../test/pbetest.c"
    ...
    if (!test_ptr("../test/pbetest.c", 69, "ctx", ctx))
        goto err;

    algor = X509_ALGOR_new();
    if (!test_ptr("../test/pbetest.c", 73, "algor", algor))
        goto err;

    if (!test_true("../test/pbetest.c", 76, "PKCS5_pbe_set0_algor(algor, EVP_CIPHER_nid(cipher), pbe_iter, pbe_salt, sizeof(pbe_salt))", (PKCS5_pbe_set0_algor(algor, EVP_CIPHER_get_nid(cipher), pbe_iter, pbe_salt, sizeof(pbe_salt))) != 0)

        || !test_true("../test/pbetest.c", 78, "PKCS5_PBE_keyivgen(ctx, pbe_password, strlen(pbe_password), algor->parameter, cipher, md, 1)", (PKCS5_PBE_keyivgen(ctx, pbe_password, strlen(pbe_password), algor->parameter, cipher, md, 1)) != 0)

        || !test_true("../test/pbetest.c", 80, "EVP_CipherUpdate(ctx, out, &i, pbe_plaintext, sizeof(pbe_plaintext))", (EVP_CipherUpdate(ctx, out, &i, pbe_plaintext, sizeof(pbe_plaintext))) != 0)
                                                              )
        goto err;
    outlen = i;

    if (!test_true("../test/pbetest.c", 85, "EVP_CipherFinal_ex(ctx, out + i, &i)", (EVP_CipherFinal_ex(ctx, out + i, &i)) != 0))
        goto err;
    outlen += i;

    if (!test_mem_eq("../test/pbetest.c", 89, "out", "exp", out, outlen, exp, exp_len))
        goto err;



    if (!test_true("../test/pbetest.c", 94, "PKCS5_PBE_keyivgen(ctx, pbe_password, strlen(pbe_password), algor->parameter, cipher, md, 0)", (PKCS5_PBE_keyivgen(ctx, pbe_password, strlen(pbe_password), algor->parameter, cipher, md, 0)) != 0)

        || !test_true("../test/pbetest.c", 96, "EVP_CipherUpdate(ctx, out, &i, exp, exp_len)", (EVP_CipherUpdate(ctx, out, &i, exp, exp_len)) != 0))
        goto err;

    outlen = i;
    if (!test_true("../test/pbetest.c", 100, "EVP_CipherFinal_ex(ctx, out + i, &i)", (EVP_CipherFinal_ex(ctx, out + i, &i)) != 0))
        goto err;

    if (!test_mem_eq("../test/pbetest.c", 103, "out", "pbe_plaintext", out, outlen, pbe_plaintext, sizeof(pbe_plaintext)))
        goto err;
