...
static int test_certs(int num)
{
    ...
    if (!test_ptr("../test/x509aux.c", 33, "fp", fp))
        return 0;

    for (c = 0; !err && PEM_read_bio(fp, &name, &header, &data, &len); ++c) {
        const int trusted = (strcmp(name, "TRUSTED CERTIFICATE") == 0);
        d2i_X509_t d2i = trusted ? d2i_X509_AUX : d2i_X509;
        i2d_X509_t i2d = trusted ? i2d_X509_AUX : i2d_X509;
        X509 *cert = 
# 40 "../test/x509aux.c" 3 4
                    ((void *)0)
# 40 "../test/x509aux.c"
                        ;
        X509 *reuse = 
# 41 "../test/x509aux.c" 3 4
                     ((void *)0)
# 41 "../test/x509aux.c"
                         ;
        const unsigned char *p = data;
        unsigned char *buf = 
# 43 "../test/x509aux.c" 3 4
                            ((void *)0)
# 43 "../test/x509aux.c"
                                ;
        unsigned char *bufp;
        ...
        if (!trusted
            ...
            test_error("../test/x509aux.c", 50, "unexpected PEM object: %s", name);
            err = 1;
            goto next;
        }
        cert = d2i(
# 54 "../test/x509aux.c" 3 4
                  ((void *)0)
# 54 "../test/x509aux.c"
                      , &p, len);

        if (cert == 
# 56 "../test/x509aux.c" 3 4
                   ((void *)0) 
# 56 "../test/x509aux.c"
                        || (p - data) != len) {
            test_error("../test/x509aux.c", 57, "error parsing input %s", name);
            err = 1;
            goto next;
        }
        ...
        enclen = i2d(cert, 
# 63 "../test/x509aux.c" 3 4
                          ((void *)0)
# 63 "../test/x509aux.c"
                              );
        if (len != enclen) {
            test_error("../test/x509aux.c", 65, "encoded length %ld of %s != input length %ld", enclen, name, len)
                                         ;
            err = 1;
            goto next;
        }
        if ((buf = bufp = CRYPTO_malloc(len, "../test/x509aux.c", 70)) == 
# 70 "../test/x509aux.c" 3 4
                                                 ((void *)0)
# 70 "../test/x509aux.c"
                                                     ) {
            test_perror("malloc");
        ...
        if (len != enclen) {
            test_error("../test/x509aux.c", 77, "encoded length %ld of %s != input length %ld", enclen, name, len)
                                         ;
            err = 1;
            goto next;
        }
        ...
        if (enclen != len) {
            test_error("../test/x509aux.c", 84, "unexpected buffer position after encoding %s", name);
            err = 1;
            goto next;
        }
        if (memcmp(buf, data, len) != 0) {
            test_error("../test/x509aux.c", 89, "encoded content of %s does not match input", name);
            err = 1;
            goto next;
        }
        p = buf;
        reuse = d2i(
# 94 "../test/x509aux.c" 3 4
                   ((void *)0)
# 94 "../test/x509aux.c"
                       , &p, enclen);
        if (reuse == 
# 95 "../test/x509aux.c" 3 4
                    ((void *)0)
# 95 "../test/x509aux.c"
                        ) {
            test_error("../test/x509aux.c", 96, "second d2i call failed for %s", name);
            err = 1;
            goto next;
        }
        err = X509_cmp(reuse, cert);
        if (err != 0) {
            test_error("../test/x509aux.c", 102, "X509_cmp for %s resulted in %d", name, err);
            err = 1;
            goto next;
        }
        CRYPTO_free(buf, "../test/x509aux.c", 106);
        buf = 
# 107 "../test/x509aux.c" 3 4
             ((void *)0)
# 107 "../test/x509aux.c"
                 ;
        ...
        enclen = i2d(cert, &buf);
        if (len != enclen) {
            test_error("../test/x509aux.c", 112, "encoded length %ld of %s != input length %ld", enclen, name, len)
                                         ;
            err = 1;
            goto next;
        }
        if (memcmp(buf, data, len) != 0) {
            test_error("../test/x509aux.c", 118, "encoded content of %s does not match input", name);
            err = 1;
            goto next;
        }
        ...
        if (trusted) {
            ...
            CRYPTO_free(buf, "../test/x509aux.c", 125);
            buf = 
# 126 "../test/x509aux.c" 3 4
                 ((void *)0)
# 126 "../test/x509aux.c"
                     ;
            ...
            enclen = i2d(cert, &buf);
            if (enclen > len) {
                test_error("../test/x509aux.c", 131, "encoded length %ld of %s > input length %ld", enclen, name, len)
                                             ;
                err = 1;
                goto next;
            }
            if (memcmp(buf, data, enclen) != 0) {
                test_error("../test/x509aux.c", 137, "encoded cert content does not match input");
                err = 1;
                goto next;
            }
        }
    ...
    next:
        X509_free(cert);
        X509_free(reuse);
        CRYPTO_free(buf, "../test/x509aux.c", 149);
        CRYPTO_free(name, "../test/x509aux.c", 150);
        CRYPTO_free(header, "../test/x509aux.c", 151);
        CRYPTO_free(data, "../test/x509aux.c", 152);
    }
    BIO_free(fp);

    if (ERR_GET_REASON(ERR_peek_last_error()) == 108) {
...
int setup_tests(void)
{
    ...
    if (!test_skip_common_options()) {
        test_error("../test/x509aux.c", 175, "Error parsing test options\n");
        return 0;
    }
    ...
    add_all_tests("test_certs", test_certs, (int)n, 1);
    return 1;
}
