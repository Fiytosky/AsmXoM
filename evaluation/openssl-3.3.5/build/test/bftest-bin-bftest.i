...
static unsigned int bf_plain[2][2] = {
    {0x424c4f57L, 0x46495348L},
    {0xfedcba98L, 0x76543210L}
};

static unsigned int bf_cipher[2][2] = {
...
static unsigned char ecb_data[34][8] = {
...
static unsigned char key_test[25] = {
    0xf0, 0xe1, 0xd2, 0xc3, 0xb4, 0xa5, 0x96, 0x87,
...
static unsigned char key_out[25][8] = {
...
static int test_bf_ecb_raw(int n)
{
    ...
    data[1] = bf_plain[n][1];
    BF_encrypt(data, &key);
    if (!test_mem_eq("../test/bftest.c", 304, "&(bf_cipher[n][0])", "&(data[0])", &(bf_cipher[n][0]), 8, &(data[0]), 8))
        ret = 0;

    BF_decrypt(&(data[0]), &key);
    if (!test_mem_eq("../test/bftest.c", 308, "&(bf_plain[n][0])", "&(data[0])", &(bf_plain[n][0]), 8, &(data[0]), 8))
        ret = 0;
...
static int test_bf_ecb(int n)
{
    ...
    BF_ecb_encrypt(&(plain_data[n][0]), out, &key, 1);
    if (!test_mem_eq("../test/bftest.c", 323, "&(cipher_data[n][0])", "out", &(cipher_data[n][0]), 8, out, 8))
        ret = 0;

    BF_ecb_encrypt(out, out, &key, 0);
    if (!test_mem_eq("../test/bftest.c", 327, "&(plain_data[n][0])", "out", &(plain_data[n][0]), 8, out, 8))
        ret = 0;
...
static int test_bf_set_key(int n)
{
    ...
    BF_ecb_encrypt(key_data, out, &key, 1);

    if (!test_mem_eq("../test/bftest.c", 342, "out", "&(key_out[n][0])", out, 8, &(key_out[n][0]), 8))
        ret = 0;
...
static int test_bf_cbc(void)
{
    ...
    BF_cbc_encrypt((unsigned char *)cbc_data, cbc_out, len,
                   &key, iv, 1);
    if (!test_mem_eq("../test/bftest.c", 363, "cbc_out", "cbc_ok", cbc_out, 32, cbc_ok, 32))
        ret = 0;

    memcpy(iv, cbc_iv, 8);
    BF_cbc_encrypt(cbc_out, cbc_in, len, &key, iv, 0);
    if (!test_mem_eq("../test/bftest.c", 368, "cbc_in", "cbc_data", cbc_in, len, cbc_data, strlen(cbc_data) + 1))
        ret = 0;
...
static int test_bf_cfb64(void)
{
    ...
    BF_cfb64_encrypt((unsigned char *)&(cbc_data[13]), &(cbc_out[13]),
                     len - 13, &key, iv, &n, 1);
    if (!test_mem_eq("../test/bftest.c", 392, "cbc_out", "cfb64_ok", cbc_out, (int)len, cfb64_ok, (int)len))
        ret = 0;

    n = 0;
    memcpy(iv, cbc_iv, 8);
    BF_cfb64_encrypt(cbc_out, cbc_in, 17, &key, iv, &n, 0);
    BF_cfb64_encrypt(&(cbc_out[17]), &(cbc_in[17]), len - 17,
                     &key, iv, &n, 0);
    if (!test_mem_eq("../test/bftest.c", 400, "cbc_in", "cbc_data", cbc_in, (int)len, cbc_data, (int)len))
        ret = 0;
...
static int test_bf_ofb64(void)
{
    ...
    BF_ofb64_encrypt((unsigned char *)&(cbc_data[13]), &(cbc_out[13]),
                     len - 13, &key, iv, &n);
    if (!test_mem_eq("../test/bftest.c", 424, "cbc_out", "ofb64_ok", cbc_out, (int)len, ofb64_ok, (int)len))
        ret = 0;
    ...
    if (!test_mem_eq("../test/bftest.c", 431, "cbc_in", "cbc_data", cbc_in, (int)len, cbc_data, (int)len))
        ret = 0;
...
# 467 "../test/bftest.c"
    ...
    add_all_tests("test_bf_ecb", test_bf_ecb, 34, 1);
    add_all_tests("test_bf_set_key", test_bf_set_key, 25 -1, 1);
