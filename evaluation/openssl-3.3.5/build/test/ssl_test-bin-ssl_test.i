...
# 17 "../test/ssl_test.c" 2

# 1 "../test/helpers/handshake.h" 1
# 13 "../test/helpers/handshake.h"
# 1 "../test/helpers/ssl_test_ctx.h" 1
# 16 "../test/helpers/ssl_test_ctx.h"
typedef enum {
    SSL_TEST_SUCCESS = 0,
...
typedef struct handshake_result {
    ...
    struct stack_st_X509_NAME *server_ca_names;
...
# 19 "../test/ssl_test.c" 2
# 1 "../test/helpers/ssl_test_ctx.h" 1
# 20 "../test/ssl_test.c" 2
# 1 "../test/testutil.h" 1
# 14 "../test/testutil.h"
...
# 21 "../test/ssl_test.c" 2

static CONF *conf = 
# 22 "../test/ssl_test.c" 3 4
                   ((void *)0)
# 22 "../test/ssl_test.c"
                       ;
static OSSL_PROVIDER *defctxnull = 
# 23 "../test/ssl_test.c" 3 4
                                  ((void *)0)
# 23 "../test/ssl_test.c"
                                      , *thisprov = 
# 23 "../test/ssl_test.c" 3 4
...
{
    if (!test_int_eq("../test/ssl_test.c", 36, "result->result", "test_ctx->expected_result", result->result, test_ctx->expected_result)) {
        test_info("../test/ssl_test.c", 37, "ExpectedResult mismatch: expected %s, got %s.", ssl_test_result_name(test_ctx->expected_result), ssl_test_result_name(result->result))
...
{
    if (!test_int_eq("../test/ssl_test.c", 47, "result->client_alert_sent", "result->client_alert_received", result->client_alert_sent, result->client_alert_received)
                                                   ) {
        test_info("../test/ssl_test.c", 49, "Client sent alert %s but server received %s.", print_alert(result->client_alert_sent), print_alert(result->client_alert_received))

                                                             ;
# 64 "../test/ssl_test.c"
    }

    if (!test_int_eq("../test/ssl_test.c", 66, "result->server_alert_sent", "result->server_alert_received", result->server_alert_sent, result->server_alert_received)
                                                   ) {
        test_info("../test/ssl_test.c", 68, "Server sent alert %s but client received %s.", print_alert(result->server_alert_sent), print_alert(result->server_alert_received))
    ...
    if (test_ctx->expected_client_alert
        ...
        test_error("../test/ssl_test.c", 82, "ClientAlert mismatch: expected %s, got %s.", print_alert(test_ctx->expected_client_alert), print_alert(result->client_alert_sent))
    ...
    if (test_ctx->expected_server_alert
        ...
        test_error("../test/ssl_test.c", 90, "ServerAlert mismatch: expected %s, got %s.", print_alert(test_ctx->expected_server_alert), print_alert(result->server_alert_sent))

                                                          ;
        return 0;
    }

    if (!test_int_le("../test/ssl_test.c", 96, "result->client_num_fatal_alerts_sent", "1", result->client_num_fatal_alerts_sent, 1))
        return 0;
    if (!test_int_le("../test/ssl_test.c", 98, "result->server_num_fatal_alerts_sent", "1", result->server_num_fatal_alerts_sent, 1))
        return 0;
    return 1;
}
...
{
    if (!test_int_eq("../test/ssl_test.c", 105, "result->client_protocol", "result->server_protocol", result->client_protocol, result->server_protocol)) {
        test_info("../test/ssl_test.c", 106, "Client has protocol %s but server has %s.", ssl_protocol_name(result->client_protocol), ssl_protocol_name(result->server_protocol))
    ...
    if (test_ctx->expected_protocol) {
        if (!test_int_eq("../test/ssl_test.c", 113, "result->client_protocol", "test_ctx->expected_protocol", result->client_protocol, test_ctx->expected_protocol)
                                                     ) {
            test_info("../test/ssl_test.c", 115, "Protocol mismatch: expected %s, got %s.\n", ssl_protocol_name(test_ctx->expected_protocol), ssl_protocol_name(result->client_protocol))
...
{
    if (!test_int_eq("../test/ssl_test.c", 126, "result->servername", "test_ctx->expected_servername", result->servername, test_ctx->expected_servername)) {
      test_info("../test/ssl_test.c", 127, "Client ServerName mismatch, expected %s, got %s.", ssl_servername_name(test_ctx->expected_servername), ssl_servername_name(result->servername))
...
static int check_session_ticket(HANDSHAKE_RESULT *result, SSL_TEST_CTX *test_ctx)
{
    if (test_ctx->session_ticket_expected == SSL_TEST_SESSION_TICKET_IGNORE)
        return 1;
    if (!test_int_eq("../test/ssl_test.c", 139, "result->session_ticket", "test_ctx->session_ticket_expected", result->session_ticket, test_ctx->session_ticket_expected)
                                                       ) {
        test_info("../test/ssl_test.c", 141, "Client SessionTicketExpected mismatch, expected %s, got %s.", ssl_session_ticket_name(test_ctx->session_ticket_expected), ssl_session_ticket_name(result->session_ticket))
...
static int check_session_id(HANDSHAKE_RESULT *result, SSL_TEST_CTX *test_ctx)
{
    if (test_ctx->session_id_expected == SSL_TEST_SESSION_ID_IGNORE)
        return 1;
    if (!test_int_eq("../test/ssl_test.c", 153, "result->session_id", "test_ctx->session_id_expected", result->session_id, test_ctx->session_id_expected)) {
        test_info("../test/ssl_test.c", 154, "Client SessionIdExpected mismatch, expected %s, got %s\n.", ssl_session_id_name(test_ctx->session_id_expected), ssl_session_id_name(result->session_id))
...
{
    if (!test_int_eq("../test/ssl_test.c", 164, "result->compression", "test_ctx->compression_expected", result->compression, test_ctx->compression_expected))
        return 0;
    return 1;
}
...
{
    int ret = 1;
    if (!test_str_eq("../test/ssl_test.c", 172, "result->client_npn_negotiated", "result->server_npn_negotiated", result->client_npn_negotiated, result->server_npn_negotiated)
                                                   )
        ret = 0;
    if (!test_str_eq("../test/ssl_test.c", 175, "test_ctx->expected_npn_protocol", "result->client_npn_negotiated", test_ctx->expected_npn_protocol, result->client_npn_negotiated)
                                                   )
        ret = 0;
    return ret;
}
...
{
    int ret = 1;
    if (!test_str_eq("../test/ssl_test.c", 185, "result->client_alpn_negotiated", "result->server_alpn_negotiated", result->client_alpn_negotiated, result->server_alpn_negotiated)
                                                    )
        ret = 0;
    if (!test_str_eq("../test/ssl_test.c", 188, "test_ctx->expected_alpn_protocol", "result->client_alpn_negotiated", test_ctx->expected_alpn_protocol, result->client_alpn_negotiated)
                                                    )
        ret = 0;
    return ret;
}
                                         ...
                                         SSL_TEST_CTX *test_ctx)
{
    ...
    if (result->result_session_ticket_app_data != 
# 201 "../test/ssl_test.c" 3 4
                                                 ((void *)0)
# 201 "../test/ssl_test.c"
                                                     )
        result_len = strlen(result->result_session_ticket_app_data);
    if (test_ctx->expected_session_ticket_app_data != 
# 203 "../test/ssl_test.c" 3 4
                                                     ((void *)0)
# 203 "../test/ssl_test.c"
                                                         )
    ...
    if (!test_str_eq("../test/ssl_test.c", 208, "result->result_session_ticket_app_data", "test_ctx->expected_session_ticket_app_data", result->result_session_ticket_app_data, test_ctx->expected_session_ticket_app_data)
                                                                )
        return 0;
...
{
    if (!test_int_eq("../test/ssl_test.c", 217, "result->client_resumed", "result->server_resumed", result->client_resumed, result->server_resumed))
        return 0;
    if (!test_int_eq("../test/ssl_test.c", 219, "result->client_resumed", "test_ctx->resumption_expected", result->client_resumed, test_ctx->resumption_expected))
        return 0;
    return 1;
}
...
static int check_nid(const char *name, int expected_nid, int nid)
{
    if (expected_nid == 0 || expected_nid == nid)
        return 1;
    test_error("../test/ssl_test.c", 228, "%s type mismatch, %s vs %s\n", name, OBJ_nid2ln(expected_nid), nid == 0 ? "absent" : OBJ_nid2ln(nid))

                                                             ;
    return 0;
}

static void print_ca_names(struct stack_st_X509_NAME *names)
{
    int i;

    if (names == 
# 238 "../test/ssl_test.c" 3 4
                ((void *)0) 
# 238 "../test/ssl_test.c"
                     || OPENSSL_sk_num(ossl_check_const_X509_NAME_sk_type(names)) == 0) {
        test_note("    <empty>");
        return;
    }
    for (i = 0; i < OPENSSL_sk_num(ossl_check_const_X509_NAME_sk_type(names)); i++) {
        X509_NAME_print_ex(bio_err, ((X509_NAME *)OPENSSL_sk_value(ossl_check_const_X509_NAME_sk_type(names), (i))), 4,
                           ((1 | 2 | 4 | 0x10 | 0x100 | 0x200) | 8 | (2 << 16) | (1 << 23) | 0));
        BIO_puts(bio_err, "\n");
    }
}

static int check_ca_names(const char *name,
                          struct stack_st_X509_NAME *expected_names,
                          struct stack_st_X509_NAME *names)
{
    int i;

    if (expected_names == 
# 255 "../test/ssl_test.c" 3 4
                         ((void *)0)
# 255 "../test/ssl_test.c"
                             )
        return 1;
    if (names == 
# 257 "../test/ssl_test.c" 3 4
                ((void *)0) 
# 257 "../test/ssl_test.c"
                     || OPENSSL_sk_num(ossl_check_const_X509_NAME_sk_type(names)) == 0) {
        if (test_int_eq("../test/ssl_test.c", 258, "sk_X509_NAME_num(expected_names)", "0", OPENSSL_sk_num(ossl_check_const_X509_NAME_sk_type(expected_names)), 0))
            return 1;
        goto err;
    }
    if (OPENSSL_sk_num(ossl_check_const_X509_NAME_sk_type(names)) != OPENSSL_sk_num(ossl_check_const_X509_NAME_sk_type(expected_names)))
        goto err;
    for (i = 0; i < OPENSSL_sk_num(ossl_check_const_X509_NAME_sk_type(names)); i++) {
        if (!test_int_eq("../test/ssl_test.c", 265, "X509_NAME_cmp(sk_X509_NAME_value(names, i), sk_X509_NAME_value(expected_names, i))", "0", X509_NAME_cmp(((X509_NAME *)OPENSSL_sk_value(ossl_check_const_X509_NAME_sk_type(names), (i))), ((X509_NAME *)OPENSSL_sk_value(ossl_check_const_X509_NAME_sk_type(expected_names), (i)))), 0)
    ...
    return 1;
err:
    test_info("../test/ssl_test.c", 273, "%s: list mismatch", name);
...
{
    if (test_ctx->expected_cipher == 
# 347 "../test/ssl_test.c" 3 4
                                    ((void *)0)
# 347 "../test/ssl_test.c"
                                        )
        return 1;
    if (!test_ptr("../test/ssl_test.c", 349, "result->cipher", result->cipher))
        return 0;
    if (!test_str_eq("../test/ssl_test.c", 351, "test_ctx->expected_cipher", "result->cipher", test_ctx->expected_cipher, result->cipher)
                                    )
        return 0;
    return 1;
}
...
# 398 "../test/ssl_test.c"
                                                           ;
    SSL_TEST_CTX *test_ctx = 
# 399 "../test/ssl_test.c" 3 4
                            ((void *)0)
# 399 "../test/ssl_test.c"
                                ;
    HANDSHAKE_RESULT *result = 
# 400 "../test/ssl_test.c" 3 4
                              ((void *)0)
# 400 "../test/ssl_test.c"
                                  ;
    char test_app[100];
    ...
    if (!test_ptr("../test/ssl_test.c", 406, "test_ctx", test_ctx))
        goto err;


    if (test_ctx->fips_version != 
# 410 "../test/ssl_test.c" 3 4
                                 ((void *)0)
                
# 411 "../test/ssl_test.c"
               && !fips_provider_version_match(libctx, test_ctx->fips_version)) {
            ret = test_skip("../test/ssl_test.c", 412, "FIPS provider unable to run this test");
            goto err;
    }


    if (test_ctx->method == SSL_TEST_METHOD_DTLS) {
        server_ctx = SSL_CTX_new_ex(libctx, 
# 418 "../test/ssl_test.c" 3 4
                                           ((void *)0)
# 418 "../test/ssl_test.c"
                                               , DTLS_server_method());
        if (!test_true("../test/ssl_test.c", 419, "SSL_CTX_set_options(server_ctx, SSL_OP_ALLOW_CLIENT_RENEGOTIATION)", (SSL_CTX_set_options(server_ctx, ((uint64_t)1 << (uint64_t)8))) != 0)

                || !test_true("../test/ssl_test.c", 421, "SSL_CTX_set_max_proto_version(server_ctx, 0)", (SSL_CTX_ctrl(server_ctx, 124, 0, 
# 421 "../test/ssl_test.c" 3 4
                   ((void *)0)
# 421 "../test/ssl_test.c"
                   )) != 0))
            goto err;
        if (test_ctx->extra.server.servername_callback !=
            ...
            if (!test_ptr("../test/ssl_test.c", 425, "server2_ctx = SSL_CTX_new_ex(libctx, NULL, DTLS_server_method())", server2_ctx = SSL_CTX_new_ex(libctx, 
# 425 "../test/ssl_test.c" 3 4
                ((void *)0)
# 425 "../test/ssl_test.c"
                , DTLS_server_method()))

                    || !test_true("../test/ssl_test.c", 427, "SSL_CTX_set_options(server2_ctx, SSL_OP_ALLOW_CLIENT_RENEGOTIATION)", (SSL_CTX_set_options(server2_ctx, ((uint64_t)1 << (uint64_t)8))) != 0)
                                                               )
                goto err;
        }
        client_ctx = SSL_CTX_new_ex(libctx, 
# 431 "../test/ssl_test.c" 3 4
                                           ((void *)0)
# 431 "../test/ssl_test.c"
                                               , DTLS_client_method());
        if (!test_true("../test/ssl_test.c", 432, "SSL_CTX_set_max_proto_version(client_ctx, 0)", (SSL_CTX_ctrl(client_ctx, 124, 0, 
# 432 "../test/ssl_test.c" 3 4
            ((void *)0)
# 432 "../test/ssl_test.c"
            )) != 0))
            goto err;
        if (test_ctx->handshake_mode == SSL_TEST_HANDSHAKE_RESUME) {
            resume_server_ctx = SSL_CTX_new_ex(libctx, 
# 435 "../test/ssl_test.c" 3 4
                                                      ((void *)0)
# 435 "../test/ssl_test.c"
                                                          ,
                                               DTLS_server_method());
            if (!test_true("../test/ssl_test.c", 437, "SSL_CTX_set_max_proto_version(resume_server_ctx, 0)", (SSL_CTX_ctrl(resume_server_ctx, 124, 0, 
# 437 "../test/ssl_test.c" 3 4
                ((void *)0)
# 437 "../test/ssl_test.c"
                )) != 0)
                    || !test_true("../test/ssl_test.c", 438, "SSL_CTX_set_options(resume_server_ctx, SSL_OP_ALLOW_CLIENT_RENEGOTIATION)", (SSL_CTX_set_options(resume_server_ctx, ((uint64_t)1 << (uint64_t)8))) != 0)
                                                               )
                goto err;
            resume_client_ctx = SSL_CTX_new_ex(libctx, 
# 441 "../test/ssl_test.c" 3 4
                                                      ((void *)0)
# 441 "../test/ssl_test.c"
                                                          ,
                                               DTLS_client_method());
            if (!test_true("../test/ssl_test.c", 443, "SSL_CTX_set_max_proto_version(resume_client_ctx, 0)", (SSL_CTX_ctrl(resume_client_ctx, 124, 0, 
# 443 "../test/ssl_test.c" 3 4
                ((void *)0)
# 443 "../test/ssl_test.c"
                )) != 0))
                goto err;
            if (!test_ptr("../test/ssl_test.c", 445, "resume_server_ctx", resume_server_ctx)
                    || !test_ptr("../test/ssl_test.c", 446, "resume_client_ctx", resume_client_ctx))
                goto err;
        }
    }
    ...
    if (test_ctx->method == SSL_TEST_METHOD_TLS) {
        ...
        server_ctx = SSL_CTX_new_ex(libctx, 
# 461 "../test/ssl_test.c" 3 4
                                           ((void *)0)
# 461 "../test/ssl_test.c"
                                               , TLS_server_method());
        if (!test_true("../test/ssl_test.c", 462, "SSL_CTX_set_max_proto_version(server_ctx, maxversion)", (SSL_CTX_ctrl(server_ctx, 124, maxversion, 
# 462 "../test/ssl_test.c" 3 4
            ((void *)0)
# 462 "../test/ssl_test.c"
            )) != 0)
                || !test_true("../test/ssl_test.c", 463, "SSL_CTX_set_options(server_ctx, SSL_OP_ALLOW_CLIENT_RENEGOTIATION)", (SSL_CTX_set_options(server_ctx, ((uint64_t)1 << (uint64_t)8))) != 0)
                                                               )
            goto err;

        if (test_ctx->extra.server.servername_callback !=
            SSL_TEST_SERVERNAME_CB_NONE) {
            if (!test_ptr("../test/ssl_test.c", 469, "server2_ctx = SSL_CTX_new_ex(libctx, NULL, TLS_server_method())", server2_ctx = SSL_CTX_new_ex(libctx, 
# 469 "../test/ssl_test.c" 3 4
                ((void *)0)
# 469 "../test/ssl_test.c"
                , TLS_server_method()))

                    || !test_true("../test/ssl_test.c", 471, "SSL_CTX_set_options(server2_ctx, SSL_OP_ALLOW_CLIENT_RENEGOTIATION)", (SSL_CTX_set_options(server2_ctx, ((uint64_t)1 << (uint64_t)8))) != 0)
                                                               )
                goto err;
            if (!test_true("../test/ssl_test.c", 474, "SSL_CTX_set_max_proto_version(server2_ctx, maxversion)", (SSL_CTX_ctrl(server2_ctx, 124, maxversion, 
# 474 "../test/ssl_test.c" 3 4
                ((void *)0)
# 474 "../test/ssl_test.c"
                )) != 0)
                                                                     )
        ...
        client_ctx = SSL_CTX_new_ex(libctx, 
# 478 "../test/ssl_test.c" 3 4
                                           ((void *)0)
# 478 "../test/ssl_test.c"
                                               , TLS_client_method());
        if (!test_true("../test/ssl_test.c", 479, "SSL_CTX_set_max_proto_version(client_ctx, maxversion)", (SSL_CTX_ctrl(client_ctx, 124, maxversion, 
# 479 "../test/ssl_test.c" 3 4
            ((void *)0)
# 479 "../test/ssl_test.c"
            )) != 0))
            goto err;
        ...
        if (test_ctx->handshake_mode == SSL_TEST_HANDSHAKE_RESUME) {
            resume_server_ctx = SSL_CTX_new_ex(libctx, 
# 483 "../test/ssl_test.c" 3 4
                                                      ((void *)0)
# 483 "../test/ssl_test.c"
                                                          ,
                                               TLS_server_method());
            if (!test_true("../test/ssl_test.c", 485, "SSL_CTX_set_max_proto_version(resume_server_ctx, maxversion)", (SSL_CTX_ctrl(resume_server_ctx, 124, maxversion, 
# 485 "../test/ssl_test.c" 3 4
                ((void *)0)
# 485 "../test/ssl_test.c"
                )) != 0)

                    || !test_true("../test/ssl_test.c", 487, "SSL_CTX_set_options(resume_server_ctx, SSL_OP_ALLOW_CLIENT_RENEGOTIATION)", (SSL_CTX_set_options(resume_server_ctx, ((uint64_t)1 << (uint64_t)8))) != 0)
                                                               )
                goto err;
            resume_client_ctx = SSL_CTX_new_ex(libctx, 
# 490 "../test/ssl_test.c" 3 4
                                                      ((void *)0)
# 490 "../test/ssl_test.c"
                                                          ,
                                               TLS_client_method());
            if (!test_true("../test/ssl_test.c", 492, "SSL_CTX_set_max_proto_version(resume_client_ctx, maxversion)", (SSL_CTX_ctrl(resume_client_ctx, 124, maxversion, 
# 492 "../test/ssl_test.c" 3 4
                ((void *)0)
# 492 "../test/ssl_test.c"
                )) != 0)
                                                                     )
                goto err;
            if (!test_ptr("../test/ssl_test.c", 495, "resume_server_ctx", resume_server_ctx)
                    || !test_ptr("../test/ssl_test.c", 496, "resume_client_ctx", resume_client_ctx))
                goto err;
        }
    }
    ...
    if (!test_ptr("../test/ssl_test.c", 506, "server_ctx", server_ctx)
            || !test_ptr("../test/ssl_test.c", 507, "client_ctx", client_ctx)
            || !test_int_gt("../test/ssl_test.c", 508, "CONF_modules_load(conf, test_app, 0)", "0", CONF_modules_load(conf, test_app, 0), 0))
        goto err;
    ...
    if (server2_ctx != 
# 516 "../test/ssl_test.c" 3 4
                      ((void *)0) 
# 516 "../test/ssl_test.c"
                           && !SSL_CTX_config(server2_ctx, "server2"))
        goto err;
    if (resume_server_ctx != 
# 518 "../test/ssl_test.c" 3 4
                            ((void *)0)
        
# 519 "../test/ssl_test.c"
       && !SSL_CTX_config(resume_server_ctx, "resume-server"))
        goto err;
    if (resume_client_ctx != 
# 521 "../test/ssl_test.c" 3 4
                            ((void *)0)
...
int setup_tests(void)
{
    ...
    if (!test_skip_common_options()) {
        test_error("../test/ssl_test.c", 551, "Error parsing test options\n");
        return 0;
    }

    if (!test_ptr("../test/ssl_test.c", 555, "conf = NCONF_new(NULL)", conf = NCONF_new(
# 555 "../test/ssl_test.c" 3 4
        ((void *)0)
# 555 "../test/ssl_test.c"
        ))

            || !test_int_gt("../test/ssl_test.c", 557, "NCONF_load(conf, test_get_argument(0), NULL)", "0", NCONF_load(conf, test_get_argument(0), 
# 557 "../test/ssl_test.c" 3 4
               ((void *)0)
# 557 "../test/ssl_test.c"
               ), 0)
            || !test_int_ne("../test/ssl_test.c", 558, "NCONF_get_number_e(conf, NULL, \"num_tests\", &num_tests)", "0", NCONF_get_number_e(conf, 
# 558 "../test/ssl_test.c" 3 4
               ((void *)0)
# 558 "../test/ssl_test.c"
               , "num_tests", &num_tests), 0)
                                                              ) {
        test_error("../test/ssl_test.c", 560, "usage: ssl_test %s", "conf_file module_name [module_conf_file]\n");
        return 0;
    }

    if (!test_arg_libctx(&libctx, &defctxnull, &thisprov, 1, "conf_file module_name [module_conf_file]\n"))
        return 0;

    add_all_tests("test_handshake", test_handshake, (int)num_tests, 1);
    return 1;
}
