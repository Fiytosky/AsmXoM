# 0 "../test/sslcorrupttest.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/sslcorrupttest.c"
# 10 "../test/sslcorrupttest.c"
...
static int tls_corrupt_write(BIO *bio, const char *in, int inl)
{
    ...
    if (docorrupt) {
        if (!test_ptr("../test/sslcorrupttest.c", 44, "copy = OPENSSL_memdup(in, inl)", copy = CRYPTO_memdup((in), inl, "../test/sslcorrupttest.c", 44)))
            return 0;

        copy[inl-1] ^= 1;
        ret = BIO_write(next, copy, inl);
        CRYPTO_free(copy, "../test/sslcorrupttest.c", 49);
    } else {
...
static long tls_corrupt_ctrl(BIO *bio, int cmd, long num, void *ptr)
{
    ...
    if (next == 
# 63 "../test/sslcorrupttest.c" 3 4
               ((void *)0)
# 63 "../test/sslcorrupttest.c"
                   )
        return 0;
...
static BIO_METHOD *method_tls_corrupt = 
# 105 "../test/sslcorrupttest.c" 3 4
                                       ((void *)0)
# 105 "../test/sslcorrupttest.c"
                                           ;


static const BIO_METHOD *bio_f_tls_corrupt_filter(void)
{
    if (method_tls_corrupt == 
# 110 "../test/sslcorrupttest.c" 3 4
                             ((void *)0)
# 110 "../test/sslcorrupttest.c"
                                 ) {
        method_tls_corrupt = BIO_meth_new((0x80 | 0x0200),
                                          "TLS corrupt filter");
        if (method_tls_corrupt == 
# 113 "../test/sslcorrupttest.c" 3 4
                                 ((void *)0)
            
# 114 "../test/sslcorrupttest.c"
           || !BIO_meth_set_write(method_tls_corrupt, tls_corrupt_write)
            ...
            return 
# 121 "../test/sslcorrupttest.c" 3 4
                  ((void *)0)
# 121 "../test/sslcorrupttest.c"
                      ;
    }
...
# 138 "../test/sslcorrupttest.c"
static const char **cipher_list = 
# 138 "../test/sslcorrupttest.c" 3 4
                                 ((void *)0)
# 138 "../test/sslcorrupttest.c"
                                     ;
...
{
    SSL_CTX *ctx = 
# 142 "../test/sslcorrupttest.c" 3 4
                  ((void *)0)
# 142 "../test/sslcorrupttest.c"
                      ;
    SSL *ssl = 
# 143 "../test/sslcorrupttest.c" 3 4
              ((void *)0)
# 143 "../test/sslcorrupttest.c"
                  ;
    struct stack_st_SSL_CIPHER *sk_ciphers = 
# 144 "../test/sslcorrupttest.c" 3 4
                                      ((void *)0)
# 144 "../test/sslcorrupttest.c"
                                          ;
    int i, j, numciphers = 0;

    if (!test_ptr("../test/sslcorrupttest.c", 147, "ctx = SSL_CTX_new(TLS_server_method())", ctx = SSL_CTX_new(TLS_server_method()))
            || !test_ptr("../test/sslcorrupttest.c", 148, "ssl = SSL_new(ctx)", ssl = SSL_new(ctx))
            || !test_ptr("../test/sslcorrupttest.c", 149, "sk_ciphers = SSL_get1_supported_ciphers(ssl)", sk_ciphers = SSL_get1_supported_ciphers(ssl)))
        goto err;
    ...
    cipher_list = CRYPTO_malloc(OPENSSL_sk_num(ossl_check_const_SSL_CIPHER_sk_type(sk_ciphers)) * sizeof(cipher_list[0]), "../test/sslcorrupttest.c", 157)
                                                        ;
    if (!test_ptr("../test/sslcorrupttest.c", 159, "cipher_list", cipher_list))
        goto err;

    for (j = 0, i = 0; i < OPENSSL_sk_num(ossl_check_const_SSL_CIPHER_sk_type(sk_ciphers)); i++) {
        const SSL_CIPHER *cipher = ((const SSL_CIPHER *)OPENSSL_sk_value(ossl_check_const_SSL_CIPHER_sk_type(sk_ciphers), (i)));

        if (SSL_CIPHER_get_auth_nid(cipher) == 1046)
            cipher_list[j++] = SSL_CIPHER_get_name(cipher);
    }
    if (test_int_ne("../test/sslcorrupttest.c", 168, "j", "0", j, 0))
        numciphers = j;

err:
    OPENSSL_sk_free(ossl_check_SSL_CIPHER_sk_type(sk_ciphers));
    SSL_free(ssl);
    SSL_CTX_free(ctx);
...
static char *cert = 
# 179 "../test/sslcorrupttest.c" 3 4
                   ((void *)0)
# 179 "../test/sslcorrupttest.c"
                       ;
static char *privkey = 
# 180 "../test/sslcorrupttest.c" 3 4
                      ((void *)0)
# 180 "../test/sslcorrupttest.c"
                          ;
...
static int test_ssl_corrupt(int testidx)
{
    ...
    SSL_CTX *sctx = 
# 185 "../test/sslcorrupttest.c" 3 4
                   ((void *)0)
# 185 "../test/sslcorrupttest.c"
                       , *cctx = 
# 185 "../test/sslcorrupttest.c" 3 4
                                 ((void *)0)
# 185 "../test/sslcorrupttest.c"
                                     ;
    SSL *server = 
# 186 "../test/sslcorrupttest.c" 3 4
                 ((void *)0)
# 186 "../test/sslcorrupttest.c"
                     , *client = 
# 186 "../test/sslcorrupttest.c" 3 4
                                 ((void *)0)
# 186 "../test/sslcorrupttest.c"
    ...
    test_info("../test/sslcorrupttest.c", 195, "Starting #%d, %s", testidx, cipher_list[testidx]);

    if (!test_true("../test/sslcorrupttest.c", 197, "create_ssl_ctx_pair(NULL, TLS_server_method(), TLS_client_method(), TLS1_VERSION, 0, &sctx, &cctx, cert, privkey)", (create_ssl_ctx_pair(
# 197 "../test/sslcorrupttest.c" 3 4
        ((void *)0)
# 197 "../test/sslcorrupttest.c"
    ...
    if (!test_true("../test/sslcorrupttest.c", 203, "SSL_CTX_set_dh_auto(sctx, 1)", (SSL_CTX_ctrl(sctx,118,1,
# 203 "../test/sslcorrupttest.c" 3 4
        ((void *)0)
# 203 "../test/sslcorrupttest.c"
        )) != 0)
            || !test_true("../test/sslcorrupttest.c", 204, "SSL_CTX_set_cipher_list(cctx, cipher_list[testidx])", (SSL_CTX_set_cipher_list(cctx, cipher_list[testidx])) != 0)
            || !test_true("../test/sslcorrupttest.c", 205, "SSL_CTX_set_ciphersuites(cctx, \"\")", (SSL_CTX_set_ciphersuites(cctx, "")) != 0)
            || !test_ptr("../test/sslcorrupttest.c", 206, "ciphers = SSL_CTX_get_ciphers(cctx)", ciphers = SSL_CTX_get_ciphers(cctx))
            || !test_int_eq("../test/sslcorrupttest.c", 207, "sk_SSL_CIPHER_num(ciphers)", "1", OPENSSL_sk_num(ossl_check_const_SSL_CIPHER_sk_type(ciphers)), 1)
            || !test_ptr("../test/sslcorrupttest.c", 208, "currcipher = sk_SSL_CIPHER_value(ciphers, 0)", currcipher = ((const SSL_CIPHER *)OPENSSL_sk_value(ossl_check_const_SSL_CIPHER_sk_type(ciphers), (0)))))
        goto end;
    ...
    if (!test_true("../test/sslcorrupttest.c", 215, "SSL_CTX_set_max_proto_version(cctx, TLS1_2_VERSION)", (SSL_CTX_ctrl(cctx, 124, 0x0303, 
# 215 "../test/sslcorrupttest.c" 3 4
        ((void *)0)
# 215 "../test/sslcorrupttest.c"
        )) != 0))
        goto end;

    if (!test_ptr("../test/sslcorrupttest.c", 218, "c_to_s_fbio = BIO_new(bio_f_tls_corrupt_filter())", c_to_s_fbio = BIO_new(bio_f_tls_corrupt_filter())))
        goto end;


    if (!test_true("../test/sslcorrupttest.c", 222, "create_ssl_objects(sctx, cctx, &server, &client, NULL, c_to_s_fbio)", (create_ssl_objects(sctx, cctx, &server, &client, 
# 222 "../test/sslcorrupttest.c" 3 4
        ((void *)0)
# 222 "../test/sslcorrupttest.c"
        , c_to_s_fbio)) != 0)
                                                   )
        goto end;

    if (!test_true("../test/sslcorrupttest.c", 226, "create_ssl_connection(server, client, SSL_ERROR_NONE)", (create_ssl_connection(server, client, 0)) != 0))
        goto end;

    docorrupt = 1;

    if (!test_int_ge("../test/sslcorrupttest.c", 231, "SSL_write(client, junk, sizeof(junk))", "0", SSL_write(client, junk, sizeof(junk)), 0))
        goto end;

    if (!test_int_lt("../test/sslcorrupttest.c", 234, "SSL_read(server, junk, sizeof(junk))", "0", SSL_read(server, junk, sizeof(junk)), 0))
        goto end;
    ...
    do {
        ...
        if (err == 0) {
            test_error("../test/sslcorrupttest.c", 241, "Decryption failed or bad record MAC not seen");
            goto end;
        }
    } while (ERR_GET_REASON(err) != 281);
...
# 255 "../test/sslcorrupttest.c" 3 4
((void *)0) 
# 255 "../test/sslcorrupttest.c"
} }; return options; }
...
int setup_tests(void)
{
    ...
    if (!test_skip_common_options()) {
        test_error("../test/sslcorrupttest.c", 262, "Error parsing test options\n");
        return 0;
    }

    if (!test_ptr("../test/sslcorrupttest.c", 266, "cert = test_get_argument(0)", cert = test_get_argument(0))
            || !test_ptr("../test/sslcorrupttest.c", 267, "privkey = test_get_argument(1)", privkey = test_get_argument(1)))
        return 0;

    n = setup_cipher_list();
    if (n > 0)
        add_all_tests("test_ssl_corrupt", test_ssl_corrupt, n, 1);
    return 1;
}
...
void cleanup_tests(void)
{
    ...
    CRYPTO_free(cipher_list, "../test/sslcorrupttest.c", 279);
}
