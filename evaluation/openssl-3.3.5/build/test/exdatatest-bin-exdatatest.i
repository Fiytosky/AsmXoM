...
static void exnew(void *parent, void *ptr, CRYPTO_EX_DATA *ad,
          int idx, long argl, void *argp)
{
    if (!test_int_eq("../test/exdatatest.c", 32, "idx", "saved_idx", idx, saved_idx)
        || !test_long_eq("../test/exdatatest.c", 33, "argl", "saved_argl", argl, saved_argl)
        || !test_ptr_eq("../test/exdatatest.c", 34, "argp", "saved_argp", argp, saved_argp)
        || !test_ptr_null("../test/exdatatest.c", 35, "ptr", ptr))
        gbl_result = 0;
}
...
static int exdup(CRYPTO_EX_DATA *to, const CRYPTO_EX_DATA *from,
          void **from_d, int idx, long argl, void *argp)
{
    if (!test_int_eq("../test/exdatatest.c", 42, "idx", "saved_idx", idx, saved_idx)
        || !test_long_eq("../test/exdatatest.c", 43, "argl", "saved_argl", argl, saved_argl)
        || !test_ptr_eq("../test/exdatatest.c", 44, "argp", "saved_argp", argp, saved_argp)
        || !test_ptr("../test/exdatatest.c", 45, "from_d", from_d))
        gbl_result = 0;
    return 1;
}
...
static void exfree(void *parent, void *ptr, CRYPTO_EX_DATA *ad,
            int idx, long argl, void *argp)
{
    if (!test_int_eq("../test/exdatatest.c", 53, "idx", "saved_idx", idx, saved_idx)
        || !test_long_eq("../test/exdatatest.c", 54, "argl", "saved_argl", argl, saved_argl)
        || !test_ptr_eq("../test/exdatatest.c", 55, "argp", "saved_argp", argp, saved_argp))
        gbl_result = 0;
}
...
static void exnew2(void *parent, void *ptr, CRYPTO_EX_DATA *ad,
          int idx, long argl, void *argp)
{
    MYOBJ_EX_DATA *ex_data = CRYPTO_zalloc(sizeof(*ex_data), "../test/exdatatest.c", 74);

    if (!test_true("../test/exdatatest.c", 76, "idx == saved_idx2 || idx == saved_idx3", (idx == saved_idx2 || idx == saved_idx3) != 0)
        || !test_long_eq("../test/exdatatest.c", 77, "argl", "saved_argl", argl, saved_argl)
        || !test_ptr_eq("../test/exdatatest.c", 78, "argp", "saved_argp", argp, saved_argp)
        || !test_ptr_null("../test/exdatatest.c", 79, "ptr", ptr)
        || !test_ptr("../test/exdatatest.c", 80, "ex_data", ex_data)
        || !test_true("../test/exdatatest.c", 81, "CRYPTO_set_ex_data(ad, idx, ex_data)", (CRYPTO_set_ex_data(ad, idx, ex_data)) != 0)) {
        gbl_result = 0;
        CRYPTO_free(ex_data, "../test/exdatatest.c", 83);
    } else {
        ex_data->new = 1;
    }
}
...
# 93 "../test/exdatatest.c" 3 4
                            ((void *)0)
# 93 "../test/exdatatest.c"
                                ;

    if (!test_true("../test/exdatatest.c", 95, "idx == saved_idx2 || idx == saved_idx3", (idx == saved_idx2 || idx == saved_idx3) != 0)
        || !test_long_eq("../test/exdatatest.c", 96, "argl", "saved_argl", argl, saved_argl)
        || !test_ptr_eq("../test/exdatatest.c", 97, "argp", "saved_argp", argp, saved_argp)
        || !test_ptr("../test/exdatatest.c", 98, "from_d", from_d)
        || !test_ptr("../test/exdatatest.c", 99, "*update_ex_data", *update_ex_data)
        || !test_ptr("../test/exdatatest.c", 100, "ex_data = CRYPTO_get_ex_data(to, idx)", ex_data = CRYPTO_get_ex_data(to, idx))
        || !test_true("../test/exdatatest.c", 101, "ex_data->new", (ex_data->new) != 0)) {
        gbl_result = 0;
    } else {
            ...
            int idx, long argl, void *argp)
{
    ...
    if (!test_true("../test/exdatatest.c", 119, "idx == saved_idx2 || idx == saved_idx3", (idx == saved_idx2 || idx == saved_idx3) != 0)
        || !test_long_eq("../test/exdatatest.c", 120, "argl", "saved_argl", argl, saved_argl)
        || !test_ptr_eq("../test/exdatatest.c", 121, "argp", "saved_argp", argp, saved_argp)
        || !test_true("../test/exdatatest.c", 122, "CRYPTO_set_ex_data(ad, idx, NULL)", (CRYPTO_set_ex_data(ad, idx, 
# 122 "../test/exdatatest.c" 3 4
           ((void *)0)
# 122 "../test/exdatatest.c"
           )) != 0))
        gbl_result = 0;
    CRYPTO_free(ex_data, "../test/exdatatest.c", 124);
}
...
{
    static int count = 0;
    MYOBJ *obj = CRYPTO_malloc(sizeof(*obj), "../test/exdatatest.c", 136);

    if (obj != 
# 138 "../test/exdatatest.c" 3 4
              ((void *)0)
# 138 "../test/exdatatest.c"
                  ) {
        obj->id = ++count;
        obj->st = CRYPTO_new_ex_data(13, obj, &obj->ex_data);
    }
    return obj;
}
...
static void MYOBJ_sethello(MYOBJ *obj, char *cp)
{
    ...
    if (!test_int_eq("../test/exdatatest.c", 148, "obj->st", "1", obj->st, 1))
        gbl_result = 0;
}
...
static void MYOBJ_sethello2(MYOBJ *obj, char *cp)
{
    ...
    if (test_ptr("../test/exdatatest.c", 161, "ex_data", ex_data))
        ex_data->hello = cp;
    else
        obj->st = gbl_result = 0;
}
...
static char *MYOBJ_gethello2(MYOBJ *obj)
{
    ...
    if (test_ptr("../test/exdatatest.c", 171, "ex_data", ex_data))
        return ex_data->hello;

    obj->st = gbl_result = 0;
    return 
# 175 "../test/exdatatest.c" 3 4
          ((void *)0)
# 175 "../test/exdatatest.c"
              ;
}
...
{
    MYOBJ_EX_DATA* ex_data = 
# 180 "../test/exdatatest.c" 3 4
                            ((void *)0)
# 180 "../test/exdatatest.c"
                                ;

    if (test_ptr_null("../test/exdatatest.c", 182, "ex_data = CRYPTO_get_ex_data(&obj->ex_data, saved_idx3)", ex_data = CRYPTO_get_ex_data(&obj->ex_data, saved_idx3))
        && test_true("../test/exdatatest.c", 183, "CRYPTO_alloc_ex_data(CRYPTO_EX_INDEX_APP, obj, &obj->ex_data, saved_idx3)", (CRYPTO_alloc_ex_data(13, obj, &obj->ex_data, saved_idx3)) != 0)

        && test_ptr("../test/exdatatest.c", 185, "ex_data = CRYPTO_get_ex_data(&obj->ex_data, saved_idx3)", ex_data = CRYPTO_get_ex_data(&obj->ex_data, saved_idx3)))
        ex_data->hello = cp;
    else
        obj->st = gbl_result = 0;
}
...
static char *MYOBJ_gethello3(MYOBJ *obj)
{
    ...
    if (test_ptr("../test/exdatatest.c", 195, "ex_data", ex_data))
        return ex_data->hello;

    obj->st = gbl_result = 0;
    return 
# 199 "../test/exdatatest.c" 3 4
          ((void *)0)
# 199 "../test/exdatatest.c"
              ;
}
...
{
    if (obj != 
# 204 "../test/exdatatest.c" 3 4
              ((void *)0)
# 204 "../test/exdatatest.c"
                  ) {
        CRYPTO_free_ex_data(13, obj, &obj->ex_data);
        CRYPTO_free(obj, "../test/exdatatest.c", 206);
    }
}
...
static MYOBJ *MYOBJ_dup(MYOBJ *in)
{
    ...
    if (obj != 
# 214 "../test/exdatatest.c" 3 4
              ((void *)0)
# 214 "../test/exdatatest.c"
                  )
        obj->st |= CRYPTO_dup_ex_data(13, &obj->ex_data,
                                     &in->ex_data);
    return obj;
}
...
{
    MYOBJ *t1 = 
# 222 "../test/exdatatest.c" 3 4
               ((void *)0)
# 222 "../test/exdatatest.c"
                   , *t2 = 
# 222 "../test/exdatatest.c" 3 4
                           ((void *)0)
# 222 "../test/exdatatest.c"
...
# 222 "../test/exdatatest.c"
                                           ;
    MYOBJ_EX_DATA *ex_data = 
# 223 "../test/exdatatest.c" 3 4
                            ((void *)0)
# 223 "../test/exdatatest.c"
                                ;
    const char *cp;
    char *p;
    int res = 0;
    ...
    if (!test_ptr("../test/exdatatest.c", 230, "p = OPENSSL_strdup(\"hello world\")", p = CRYPTO_strdup("hello world", "../test/exdatatest.c", 230)))
        return 0;
    saved_argl = 21;
    if (!test_ptr("../test/exdatatest.c", 233, "saved_argp = OPENSSL_malloc(1)", saved_argp = CRYPTO_malloc(1, "../test/exdatatest.c", 233)))
        goto err;
    saved_idx = CRYPTO_get_ex_new_index(13,
                                        saved_argl, saved_argp,
                                        exnew, exdup, exfree);
    saved_idx2 = CRYPTO_get_ex_new_index(13,
                                         saved_argl, saved_argp,
                                         exnew2, exdup2, exfree2);
    t1 = MYOBJ_new();
    t2 = MYOBJ_new();
    if (!test_int_eq("../test/exdatatest.c", 243, "t1->st", "1", t1->st, 1) || !test_int_eq("../test/exdatatest.c", 243, "t2->st", "1", t2->st, 1))
        goto err;
    if (!test_ptr("../test/exdatatest.c", 245, "CRYPTO_get_ex_data(&t1->ex_data, saved_idx2)", CRYPTO_get_ex_data(&t1->ex_data, saved_idx2)))
        goto err;
    ...
    saved_idx3 = CRYPTO_get_ex_new_index(13,
                                         saved_argl, saved_argp,
                                         exnew2, exdup2, exfree2);
    if (!test_ptr_null("../test/exdatatest.c", 255, "CRYPTO_get_ex_data(&t1->ex_data, saved_idx3)", CRYPTO_get_ex_data(&t1->ex_data, saved_idx3)))
        goto err;

    MYOBJ_sethello(t1, p);
    cp = MYOBJ_gethello(t1);
    if (!test_ptr_eq("../test/exdatatest.c", 260, "cp", "p", cp, p))
        goto err;

    MYOBJ_sethello2(t1, p);
    cp = MYOBJ_gethello2(t1);
    if (!test_ptr_eq("../test/exdatatest.c", 265, "cp", "p", cp, p))
        goto err;

    MYOBJ_allochello3(t1, p);
    cp = MYOBJ_gethello3(t1);
    if (!test_ptr_eq("../test/exdatatest.c", 270, "cp", "p", cp, p))
        goto err;

    cp = MYOBJ_gethello(t2);
    if (!test_ptr_null("../test/exdatatest.c", 274, "cp", cp))
        goto err;

    cp = MYOBJ_gethello2(t2);
    if (!test_ptr_null("../test/exdatatest.c", 278, "cp", cp))
        goto err;

    t3 = MYOBJ_dup(t1);
    if (!test_int_eq("../test/exdatatest.c", 282, "t3->st", "1", t3->st, 1))
        goto err;

    ex_data = CRYPTO_get_ex_data(&t3->ex_data, saved_idx2);
    if (!test_ptr("../test/exdatatest.c", 286, "ex_data", ex_data))
        goto err;
    if (!test_int_eq("../test/exdatatest.c", 288, "ex_data->dup", "1", ex_data->dup, 1))
        goto err;

    cp = MYOBJ_gethello(t3);
    if (!test_ptr_eq("../test/exdatatest.c", 292, "cp", "p", cp, p))
        goto err;

    cp = MYOBJ_gethello2(t3);
    if (!test_ptr_eq("../test/exdatatest.c", 296, "cp", "p", cp, p))
        goto err;

    cp = MYOBJ_gethello3(t3);
    if (!test_ptr_eq("../test/exdatatest.c", 300, "cp", "p", cp, p))
        goto err;
 ...
 err:
    ...
    MYOBJ_free(t2);
    MYOBJ_free(t3);
    CRYPTO_free(saved_argp, "../test/exdatatest.c", 309);
    saved_argp = 
# 310 "../test/exdatatest.c" 3 4
                ((void *)0)
# 310 "../test/exdatatest.c"
                    ;
    CRYPTO_free(p, "../test/exdatatest.c", 311);
    return res;
}
