...
# 37 "../test/pemtest.c"
    ...
    if (!test_ptr("../test/pemtest.c", 43, "b", b)
        || !test_true("../test/pemtest.c", 44, "BIO_printf(b, \"-----BEGIN %s-----\\n\", pemtype)", (BIO_printf(b, "-----BEGIN %s-----\n", pemtype)) != 0)
        || !test_true("../test/pemtest.c", 45, "BIO_printf(b, \"%s\\n\", encoded)", (BIO_printf(b, "%s\n", encoded)) != 0)
        || !test_true("../test/pemtest.c", 46, "BIO_printf(b, \"-----END %s-----\\n\", pemtype)", (BIO_printf(b, "-----END %s-----\n", pemtype)) != 0)
        || !test_true("../test/pemtest.c", 47, "PEM_read_bio_ex(b, &name, &header, &data, &len, PEM_FLAG_ONLY_B64)", (PEM_read_bio_ex(b, &name, &header, &data, &len, 0x4)) != 0)
                                                         )
        goto err;
    if (!test_int_eq("../test/pemtest.c", 50, "memcmp(pemtype, name, strlen(pemtype))", "0", memcmp(pemtype, name, strlen(pemtype)), 0)
        || !test_int_eq("../test/pemtest.c", 51, "len", "strlen(raw)", len, strlen(raw))
        || !test_int_eq("../test/pemtest.c", 52, "memcmp(data, raw, strlen(raw))", "0", memcmp(data, raw, strlen(raw)), 0))
        goto err;
    ret = 1;
 err:
    BIO_free(b);
    CRYPTO_free(name, "../test/pemtest.c", 57);
    CRYPTO_free(header, "../test/pemtest.c", 58);
    CRYPTO_free(data, "../test/pemtest.c", 59);
    return ret;
}
...
# 67 "../test/pemtest.c"
    ...
    if (!test_ptr("../test/pemtest.c", 71, "b", b)
        || !test_true("../test/pemtest.c", 72, "BIO_printf(b, \"-----BEGIN %s-----\\n\", pemtype)", (BIO_printf(b, "-----BEGIN %s-----\n", pemtype)) != 0)
        || !test_true("../test/pemtest.c", 73, "BIO_printf(b, \"%c%s\\n\", '\\t', encoded)", (BIO_printf(b, "%c%s\n", '\t', encoded)) != 0)
        || !test_true("../test/pemtest.c", 74, "BIO_printf(b, \"-----END %s-----\\n\", pemtype)", (BIO_printf(b, "-----END %s-----\n", pemtype)) != 0)

        || test_true("../test/pemtest.c", 76, "PEM_read_bio_ex(b, &name, &header, &data, &len, PEM_FLAG_ONLY_B64)", (PEM_read_bio_ex(b, &name, &header, &data, &len, 0x4)) != 0)
                                                        ) {
        BIO_free(b);
        return 0;
    }
    BIO_free(b);
    CRYPTO_free(name, "../test/pemtest.c", 82);
    CRYPTO_free(header, "../test/pemtest.c", 83);
    CRYPTO_free(data, "../test/pemtest.c", 84);
    return 1;
}
...
static int test_cert_key_cert(void)
{
    ...
    if (!test_ptr("../test/pemtest.c", 92, "key = load_pkey_pem(pemfile, NULL)", key = load_pkey_pem(pemfile, 
# 92 "../test/pemtest.c" 3 4
        ((void *)0)
# 92 "../test/pemtest.c"
        )))
        return 0;
...
# 107 "../test/pemtest.c"
    ...
    if (!test_ptr("../test/pemtest.c", 112, "b", b))
        return 0;


    if (!test_false("../test/pemtest.c", 116, "PEM_read_bio_ex(b, &name, &header, &data, &len, 0)", (PEM_read_bio_ex(b, &name, &header, &data, &len, 0)) != 0))
        goto err;

    ret = 1;
 err:
    CRYPTO_free(name, "../test/pemtest.c", 121);
    CRYPTO_free(header, "../test/pemtest.c", 122);
    CRYPTO_free(data, "../test/pemtest.c", 123);
    BIO_free(b);
    return ret;
}
...
# 138 "../test/pemtest.c"
    ...
    if (!test_ptr("../test/pemtest.c", 142, "b", b))
        return 0;


    pkey = PEM_read_bio_Parameters(b, 
# 146 "../test/pemtest.c" 3 4
                                     ((void *)0)
# 146 "../test/pemtest.c"
                                         );
    if (!test_ptr_null("../test/pemtest.c", 147, "pkey", pkey))
        goto err;
...
int setup_tests(void)
{
    if (!test_ptr("../test/pemtest.c", 159, "pemfile = test_get_argument(0)", pemfile = test_get_argument(0)))
        return 0;
    add_all_tests("test_b64", test_b64, (sizeof(b64_pem_data)/sizeof((b64_pem_data)[0])), 1);
    ...
    add_test("test_protected_params", test_protected_params);
    return 1;
}
