...
# 19 "../test/casttest.c" 2


# 1 "../include/internal/nelem.h" 1
# 12 "../include/internal/nelem.h"
        
# 22 "../test/casttest.c" 2
# 1 "../test/testutil.h" 1
# 13 "../test/testutil.h"
# 1 "/root/AsmXoM/gcc-15.2.0/build/lib/gcc/x86_64-pc-linux-gnu/15.2.0/include/stdarg.h" 1 3 4
...
static int cast_test_vector(int z)
{
    ...
    CAST_ecb_encrypt(in, out, &key, 1);
    if (!test_mem_eq("../test/casttest.c", 71, "out", "c[z]", out, sizeof(c[z]), c[z], sizeof(c[z]))) {
        test_info("../test/casttest.c", 72, "CAST_ENCRYPT iteration %d failed (len=%d)", z, k_len[z]);
        testresult = 0;
    }

    CAST_ecb_encrypt(out, out, &key, 0);
    if (!test_mem_eq("../test/casttest.c", 77, "out", "in", out, sizeof(in), in, sizeof(in))) {
        test_info("../test/casttest.c", 78, "CAST_DECRYPT iteration %d failed (len=%d)", z, k_len[z]);
        testresult = 0;
    }
    return testresult;
}
...
static int cast_test_iterations(void)
{
    ...
    for (l = 0; l < 1000000L; l++) {
        ...
        CAST_ecb_encrypt(&(out_a[0]), &(out_a[0]), &key_b, 1);
        CAST_ecb_encrypt(&(out_a[8]), &(out_a[8]), &key_b, 1);
        CAST_set_key(&key, 16, out_a);
        CAST_ecb_encrypt(&(out_b[0]), &(out_b[0]), &key, 1);
        CAST_ecb_encrypt(&(out_b[8]), &(out_b[8]), &key, 1);
    }

    if (!test_mem_eq("../test/casttest.c", 103, "out_a", "c_a", out_a, sizeof(c_a), c_a, sizeof(c_a))
            || !test_mem_eq("../test/casttest.c", 104, "out_b", "c_b", out_b, sizeof(c_b), c_b, sizeof(c_b)))
        testresult = 0;
...
int setup_tests(void)
{
    ...
    add_all_tests("cast_test_vector", cast_test_vector, (sizeof(k_len)/sizeof((k_len)[0])), 1);
    add_test("cast_test_iterations", cast_test_iterations);
