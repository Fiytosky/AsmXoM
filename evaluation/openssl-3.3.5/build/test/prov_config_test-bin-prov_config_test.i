# 0 "../test/prov_config_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/prov_config_test.c"
# 10 "../test/prov_config_test.c"
...
# 14 "../test/prov_config_test.c" 2

static char *configfile = 
# 15 "../test/prov_config_test.c" 3 4
                         ((void *)0)
# 15 "../test/prov_config_test.c"
                             ;
static char *recurseconfigfile = 
# 16 "../test/prov_config_test.c" 3 4
                                ((void *)0)
# 16 "../test/prov_config_test.c"
                                    ;
static char *pathedconfig = 
# 17 "../test/prov_config_test.c" 3 4
                           ((void *)0)
# 17 "../test/prov_config_test.c"
                               ;
...
static int test_double_config(void)
{
    ...
    int testresult = 0;
    EVP_MD *sha256 = 
# 27 "../test/prov_config_test.c" 3 4
                    ((void *)0)
# 27 "../test/prov_config_test.c"
                        ;

    if (!test_ptr("../test/prov_config_test.c", 29, "ctx", ctx))
        return 0;

    if (!test_true("../test/prov_config_test.c", 32, "OSSL_LIB_CTX_load_config(ctx, configfile)", (OSSL_LIB_CTX_load_config(ctx, configfile)) != 0))
        goto err;
    if (!test_true("../test/prov_config_test.c", 34, "OSSL_LIB_CTX_load_config(ctx, configfile)", (OSSL_LIB_CTX_load_config(ctx, configfile)) != 0))
        goto err;


    sha256 = EVP_MD_fetch(ctx, "SHA2-256", 
# 38 "../test/prov_config_test.c" 3 4
                                          ((void *)0)
# 38 "../test/prov_config_test.c"
                                              );
    if (!test_ptr("../test/prov_config_test.c", 39, "sha256", sha256))
        goto err;
...
static int test_recursive_config(void)
{
    ...
    if (!test_ptr("../test/prov_config_test.c", 55, "ctx", ctx))
        goto err;

    if (!test_false("../test/prov_config_test.c", 58, "OSSL_LIB_CTX_load_config(ctx, recurseconfigfile)", (OSSL_LIB_CTX_load_config(ctx, recurseconfigfile)) != 0))
        goto err;

    err = ERR_peek_error();

    if (ERR_GET_REASON(err) == 126)
        testresult = 1;
 err:
    OSSL_LIB_CTX_free(ctx);
    return testresult;
...
{
    OSSL_LIB_CTX *ctx = 
# 73 "../test/prov_config_test.c" 3 4
                       ((void *)0)
# 73 "../test/prov_config_test.c"
                           ;
    OSSL_PROVIDER *prov;
    int testresult = 0;
    struct stat sbuf;
    ...
    char *full_path = 
# 78 "../test/prov_config_test.c" 3 4
                     ((void *)0)
# 78 "../test/prov_config_test.c"
                         ;
    int rc;

    if (!test_ptr("../test/prov_config_test.c", 81, "module_path", module_path))
        return 0;

    full_path = CRYPTO_zalloc(strlen(module_path) + strlen("/../test/p_test.so") + 1, "../test/prov_config_test.c", 84);
    if (!test_ptr("../test/prov_config_test.c", 85, "full_path", full_path))
        return 0;

    strcpy(full_path, module_path);
    full_path = strcat(full_path, "/../test/p_test.so");
    test_info("../test/prov_config_test.c", 90, "full path is %s", full_path);
    rc = stat(full_path, &sbuf);
    CRYPTO_free(full_path, "../test/prov_config_test.c", 92);
    if (rc == -1)
        return test_skip("../test/prov_config_test.c", 94, "Skipping modulepath test as provider not present");

    if (!test_ptr("../test/prov_config_test.c", 96, "pathedconfig", pathedconfig))
        return 0;

    ctx = OSSL_LIB_CTX_new();
    if (!test_ptr("../test/prov_config_test.c", 100, "ctx", ctx))
        return 0;

    if (!test_true("../test/prov_config_test.c", 103, "OSSL_LIB_CTX_load_config(ctx, pathedconfig)", (OSSL_LIB_CTX_load_config(ctx, pathedconfig)) != 0))
        goto err;


    if (!test_ptr("../test/prov_config_test.c", 107, "prov = OSSL_PROVIDER_load(ctx, \"test\")", prov = OSSL_PROVIDER_load(ctx, "test")))
        goto err;
...
const OPTIONS *test_get_options(void) { enum { OPT_TEST_HELP = 500, OPT_TEST_LIST, OPT_TEST_SINGLE, OPT_TEST_ITERATION, OPT_TEST_INDENT, OPT_TEST_SEED }; static const OPTIONS options[] = { { OPT_HELP_STR, 1, '-', "Usage: %s [options] " "configfile\n" }, { OPT_HELP_STR, 1, '-', "Valid options are:\n" }, { "help", OPT_TEST_HELP, '-', "Display this summary" }, { "list", OPT_TEST_LIST, '-', "Display the list of tests available" }, { "test", OPT_TEST_SINGLE, 's', "Run a single test by id or name" }, { "iter", OPT_TEST_ITERATION, 'n', "Run a single iteration of a test" }, { "indent", OPT_TEST_INDENT,'p', "Number of tabs added to output" }, { "seed", OPT_TEST_SEED, 'n', "Seed value to randomize tests with" }, { 
# 118 "../test/prov_config_test.c" 3 4
((void *)0) 
# 118 "../test/prov_config_test.c"
} }; return options; }

int setup_tests(void)
{
    if (!test_skip_common_options()) {
        test_error("../test/prov_config_test.c", 123, "Error parsing test options\n");
        return 0;
    }

    if (!test_ptr("../test/prov_config_test.c", 127, "configfile = test_get_argument(0)", configfile = test_get_argument(0)))
        return 0;

    if (!test_ptr("../test/prov_config_test.c", 130, "recurseconfigfile = test_get_argument(1)", recurseconfigfile = test_get_argument(1)))
        return 0;

    if (!test_ptr("../test/prov_config_test.c", 133, "pathedconfig = test_get_argument(2)", pathedconfig = test_get_argument(2)))
        return 0;

    add_test("test_recursive_config", test_recursive_config);
    add_test("test_double_config", test_double_config);
    add_test("test_path_config", test_path_config);
    return 1;
}
