# 0 "../test/siphash_internal_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/siphash_internal_test.c"
# 12 "../test/siphash_internal_test.c"
...
# 13 "../test/siphash_internal_test.c" 2
...
# 14 "../test/siphash_internal_test.c" 2
...
int BIO_meth_set_callback_ctrl(BIO_METHOD *biom,
                               long (*callback_ctrl) (BIO *, int,
                                                      BIO_info_cb *));
# 16 "../test/siphash_internal_test.c" 2
# 1 "../test/testutil.h" 1
# 14 "../test/testutil.h"
...
# 17 "../test/siphash_internal_test.c" 2
# 1 "../include/crypto/siphash.h" 1
# 12 "../include/crypto/siphash.h"
        

# 1 "/root/AsmXoM/gcc-15.2.0/build/lib/gcc/x86_64-pc-linux-gnu/15.2.0/include/stddef.h" 1 3 4
# 15 "../include/crypto/siphash.h" 2
...
# 18 "../test/siphash_internal_test.c" 2
...
# 38 "../test/siphash_internal_test.c"
...
static int test_siphash(int idx)
{
    ...
    if (expectedlen != 8 &&
        expectedlen != 16) {
        test_info("../test/siphash_internal_test.c", 183, "size %zu vs %d and %d", expectedlen, 8, 16)
                                                                   ;
        return 0;
    }

    if (!test_int_le("../test/siphash_internal_test.c", 188, "inlen", "sizeof(in)", inlen, sizeof(in)))
        return 0;
    ...
    if (!test_true("../test/siphash_internal_test.c", 198, "SipHash_set_hash_size(&siphash, expectedlen)", (SipHash_set_hash_size(&siphash, expectedlen)) != 0)
        || !test_true("../test/siphash_internal_test.c", 199, "SipHash_Init(&siphash, key, 0, 0)", (SipHash_Init(&siphash, key, 0, 0)) != 0))
        return 0;
    SipHash_Update(&siphash, in, inlen);
    if (!test_true("../test/siphash_internal_test.c", 202, "SipHash_Final(&siphash, out, expectedlen)", (SipHash_Final(&siphash, out, expectedlen)) != 0)
        || !test_mem_eq("../test/siphash_internal_test.c", 203, "out", "expected", out, expectedlen, expected, expectedlen))
        return 0;

    if (inlen > 16) {
        if (!test_true("../test/siphash_internal_test.c", 207, "SipHash_set_hash_size(&siphash, expectedlen)", (SipHash_set_hash_size(&siphash, expectedlen)) != 0)
            || !test_true("../test/siphash_internal_test.c", 208, "SipHash_Init(&siphash, key, 0, 0)", (SipHash_Init(&siphash, key, 0, 0)) != 0))
            return 0;
        SipHash_Update(&siphash, in, 1);
        SipHash_Update(&siphash, in+1, inlen-1);
        if (!test_true("../test/siphash_internal_test.c", 212, "SipHash_Final(&siphash, out, expectedlen)", (SipHash_Final(&siphash, out, expectedlen)) != 0))
            return 0;

        if (!test_mem_eq("../test/siphash_internal_test.c", 215, "out", "expected", out, expectedlen, expected, expectedlen)) {
            test_info("../test/siphash_internal_test.c", 216, "SipHash test #%d/1+(N-1) failed.", idx);
            return 0;
        }
    }
    ...
    if (inlen > 32) {
        ...
        if (!test_true("../test/siphash_internal_test.c", 224, "SipHash_set_hash_size(&siphash, expectedlen)", (SipHash_set_hash_size(&siphash, expectedlen)) != 0)
            || !test_true("../test/siphash_internal_test.c", 225, "SipHash_Init(&siphash, key, 0, 0)", (SipHash_Init(&siphash, key, 0, 0)) != 0))
            return 0;
        SipHash_Update(&siphash, in, half);
        SipHash_Update(&siphash, in+half, inlen-half);
        if (!test_true("../test/siphash_internal_test.c", 229, "SipHash_Final(&siphash, out, expectedlen)", (SipHash_Final(&siphash, out, expectedlen)) != 0))
            return 0;

        if (!test_mem_eq("../test/siphash_internal_test.c", 232, "out", "expected", out, expectedlen, expected, expectedlen)) {
            test_info("../test/siphash_internal_test.c", 233, "SipHash test #%d/2 failed.", idx);
            return 0;
        }

        for (half = 16; half < inlen; half += 16) {
            if (!test_true("../test/siphash_internal_test.c", 238, "SipHash_set_hash_size(&siphash, expectedlen)", (SipHash_set_hash_size(&siphash, expectedlen)) != 0)
                || !test_true("../test/siphash_internal_test.c", 239, "SipHash_Init(&siphash, key, 0, 0)", (SipHash_Init(&siphash, key, 0, 0)) != 0))
                return 0;
            SipHash_Update(&siphash, in, half);
            SipHash_Update(&siphash, in+half, inlen-half);
            if (!test_true("../test/siphash_internal_test.c", 243, "SipHash_Final(&siphash, out, expectedlen)", (SipHash_Final(&siphash, out, expectedlen)) != 0))
                return 0;

            if (!test_mem_eq("../test/siphash_internal_test.c", 246, "out", "expected", out, expectedlen, expected, expectedlen)) {
                test_info("../test/siphash_internal_test.c", 247, "SipHash test #%d/%zu+%zu failed.", idx, half, inlen-half)
                                                ;
                return 0;
            }
        }
...
static int test_siphash_basic(void)
{
    ...
    return test_int_eq("../test/siphash_internal_test.c", 264, "SipHash_set_hash_size(&siphash, 4)", "0", SipHash_set_hash_size(&siphash, 4), 0)
           && test_false("../test/siphash_internal_test.c", 265, "SipHash_Final(&siphash, output, 0)", (SipHash_Final(&siphash, output, 0)) != 0)

           && test_true("../test/siphash_internal_test.c", 267, "SipHash_set_hash_size(&siphash, 8)", (SipHash_set_hash_size(&siphash, 8)) != 0)
           && test_false("../test/siphash_internal_test.c", 268, "SipHash_Final(&siphash, output, 8)", (SipHash_Final(&siphash, output, 8)) != 0)
           && test_true("../test/siphash_internal_test.c", 269, "SipHash_Init(&siphash, key, 0, 0)", (SipHash_Init(&siphash, key, 0, 0)) != 0)
           && test_true("../test/siphash_internal_test.c", 270, "SipHash_Final(&siphash, output, 8)", (SipHash_Final(&siphash, output, 8)) != 0)
           && test_int_eq("../test/siphash_internal_test.c", 271, "SipHash_Final(&siphash, output, 16)", "0", SipHash_Final(&siphash, output, 16), 0)


           && test_true("../test/siphash_internal_test.c", 274, "SipHash_set_hash_size(&siphash, 16)", (SipHash_set_hash_size(&siphash, 16)) != 0)
           && test_true("../test/siphash_internal_test.c", 275, "SipHash_Init(&siphash, key, 0, 0)", (SipHash_Init(&siphash, key, 0, 0)) != 0)
           && test_int_eq("../test/siphash_internal_test.c", 276, "SipHash_Final(&siphash, output, 8)", "0", SipHash_Final(&siphash, output, 8), 0)
           && test_true("../test/siphash_internal_test.c", 277, "SipHash_Final(&siphash, output, 16)", (SipHash_Final(&siphash, output, 16)) != 0)


           && test_true("../test/siphash_internal_test.c", 280, "SipHash_set_hash_size(&siphash, 0)", (SipHash_set_hash_size(&siphash, 0)) != 0)
           && test_true("../test/siphash_internal_test.c", 281, "SipHash_Init(&siphash, key, 0, 0)", (SipHash_Init(&siphash, key, 0, 0)) != 0)
           && test_int_eq("../test/siphash_internal_test.c", 282, "SipHash_Final(&siphash, output, 8)", "0", SipHash_Final(&siphash, output, 8), 0)
           && test_true("../test/siphash_internal_test.c", 283, "SipHash_Final(&siphash, output, 16)", (SipHash_Final(&siphash, output, 16)) != 0);
}

int setup_tests(void)
{
    add_test("test_siphash_basic", test_siphash_basic);
    add_all_tests("test_siphash", test_siphash, (sizeof(tests)/sizeof((tests)[0])), 1);
    return 1;
}
