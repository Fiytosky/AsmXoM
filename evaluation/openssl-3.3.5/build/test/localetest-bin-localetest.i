...
# 20 "../test/localetest.c" 2





# 24 "../test/localetest.c"
int setup_tests(void)
{
  ...
  0x5f
    ...
    int res;
    X509 *cert = 
# 97 "../test/localetest.c" 3 4
                ((void *)0)
# 97 "../test/localetest.c"
                    ;
    X509_PUBKEY *cert_pubkey = 
# 98 "../test/localetest.c" 3 4
                              ((void *)0)
# 98 "../test/localetest.c"
                                  ;
    const unsigned char *p = der_bytes;

    if (setlocale(
# 101 "../test/localetest.c" 3 4
                 6
# 101 "../test/localetest.c"
                       , "") == 
# 101 "../test/localetest.c" 3 4
                                ((void *)0)
# 101 "../test/localetest.c"
                                    )
        return test_skip("../test/localetest.c", 102, "Cannot set the locale necessary for test");

    res = strcasecmp(str1, str2);
    test_note("Case-insensitive comparison via strcasecmp in current locale %s\n", res ? "failed" : "succeeded");

    if (!test_false("../test/localetest.c", 107, "OPENSSL_strcasecmp(str1, str2)", (OPENSSL_strcasecmp(str1, str2)) != 0))
        return 0;

    cert = d2i_X509(
# 110 "../test/localetest.c" 3 4
                   ((void *)0)
# 110 "../test/localetest.c"
                       , &p, sizeof(der_bytes));
    if (!test_ptr("../test/localetest.c", 111, "cert", cert))
        return 0;

    cert_pubkey = X509_get_X509_PUBKEY(cert);
    if (!test_ptr("../test/localetest.c", 115, "cert_pubkey", cert_pubkey)) {
        X509_free(cert);
        return 0;
    }

    if (!test_ptr("../test/localetest.c", 120, "X509_PUBKEY_get0(cert_pubkey)", X509_PUBKEY_get0(cert_pubkey))) {
        X509_free(cert);
        return 0;
    }
