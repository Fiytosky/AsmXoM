# 0 "../test/x509_load_cert_file_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/x509_load_cert_file_test.c"
...
# 9 "../test/x509_load_cert_file_test.c" 2
...
# 10 "../test/x509_load_cert_file_test.c" 2
...
# 29 "include/openssl/x509_vfy.h" 2
...
# 11 "../test/x509_load_cert_file_test.c" 2
...
# 13 "../test/x509_load_cert_file_test.c" 2
...
static int test_load_cert_file(void)
{
    int ret = 0, i;
    X509_STORE *store = 
# 20 "../test/x509_load_cert_file_test.c" 3 4
                       ((void *)0)
# 20 "../test/x509_load_cert_file_test.c"
                           ;
    X509_LOOKUP *lookup = 
# 21 "../test/x509_load_cert_file_test.c" 3 4
                         ((void *)0)
# 21 "../test/x509_load_cert_file_test.c"
                             ;
    struct stack_st_X509 *certs = 
# 22 "../test/x509_load_cert_file_test.c" 3 4
                           ((void *)0)
# 22 "../test/x509_load_cert_file_test.c"
                               ;
    struct stack_st_X509_OBJECT *objs = 
# 23 "../test/x509_load_cert_file_test.c" 3 4
                                 ((void *)0)
# 23 "../test/x509_load_cert_file_test.c"
                                     ;

    if (!test_ptr("../test/x509_load_cert_file_test.c", 25, "store = X509_STORE_new()", store = X509_STORE_new())
        || !test_ptr("../test/x509_load_cert_file_test.c", 26, "lookup = X509_STORE_add_lookup(store, X509_LOOKUP_file())", lookup = X509_STORE_add_lookup(store, X509_LOOKUP_file()))
        || !test_true("../test/x509_load_cert_file_test.c", 27, "X509_load_cert_file(lookup, chain, X509_FILETYPE_PEM)", (X509_load_cert_file(lookup, chain, 1)) != 0)
        || !test_ptr("../test/x509_load_cert_file_test.c", 28, "certs = X509_STORE_get1_all_certs(store)", certs = X509_STORE_get1_all_certs(store))
        || !test_int_eq("../test/x509_load_cert_file_test.c", 29, "sk_X509_num(certs)", "4", OPENSSL_sk_num(ossl_check_const_X509_sk_type(certs)), 4)
        || !test_ptr("../test/x509_load_cert_file_test.c", 30, "objs = X509_STORE_get1_objects(store)", objs = X509_STORE_get1_objects(store))
        || !test_int_eq("../test/x509_load_cert_file_test.c", 31, "sk_X509_OBJECT_num(objs)", "4", OPENSSL_sk_num(ossl_check_const_X509_OBJECT_sk_type(objs)), 4))
        goto err;

    for (i = 0; i < OPENSSL_sk_num(ossl_check_const_X509_OBJECT_sk_type(objs)); i++) {
        const X509_OBJECT *obj = ((X509_OBJECT *)OPENSSL_sk_value(ossl_check_const_X509_OBJECT_sk_type(objs), (i)));
        if (!test_int_eq("../test/x509_load_cert_file_test.c", 36, "X509_OBJECT_get_type(obj)", "X509_LU_X509", X509_OBJECT_get_type(obj), X509_LU_X509))
            goto err;
    }

    if (crl != 
# 40 "../test/x509_load_cert_file_test.c" 3 4
              ((void *)0) 
# 40 "../test/x509_load_cert_file_test.c"
                   && !test_true("../test/x509_load_cert_file_test.c", 40, "X509_load_crl_file(lookup, crl, X509_FILETYPE_PEM)", (X509_load_crl_file(lookup, crl, 1)) != 0))
        goto err;
...
err:
    ...
    OPENSSL_sk_pop_free(ossl_check_X509_OBJECT_sk_type(objs),ossl_check_X509_OBJECT_freefunc_type(X509_OBJECT_free));
    X509_STORE_free(store);
    return ret;
}

const OPTIONS *test_get_options(void) { enum { OPT_TEST_HELP = 500, OPT_TEST_LIST, OPT_TEST_SINGLE, OPT_TEST_ITERATION, OPT_TEST_INDENT, OPT_TEST_SEED }; static const OPTIONS options[] = { { OPT_HELP_STR, 1, '-', "Usage: %s [options] " "cert.pem [crl.pem]\n" }, { OPT_HELP_STR, 1, '-', "Valid options are:\n" }, { "help", OPT_TEST_HELP, '-', "Display this summary" }, { "list", OPT_TEST_LIST, '-', "Display the list of tests available" }, { "test", OPT_TEST_SINGLE, 's', "Run a single test by id or name" }, { "iter", OPT_TEST_ITERATION, 'n', "Run a single iteration of a test" }, { "indent", OPT_TEST_INDENT,'p', "Number of tabs added to output" }, { "seed", OPT_TEST_SEED, 'n', "Seed value to randomize tests with" }, { 
# 52 "../test/x509_load_cert_file_test.c" 3 4
((void *)0) 
# 52 "../test/x509_load_cert_file_test.c"
} }; return options; }

int setup_tests(void)
{
    if (!test_skip_common_options()) {
        test_error("../test/x509_load_cert_file_test.c", 57, "Error parsing test options\n");
        return 0;
    }

    chain = test_get_argument(0);
    if (chain == 
# 62 "../test/x509_load_cert_file_test.c" 3 4
                ((void *)0)
# 62 "../test/x509_load_cert_file_test.c"
                    )
        return 0;

    crl = test_get_argument(1);

    add_test("test_load_cert_file", test_load_cert_file);
    return 1;
}
