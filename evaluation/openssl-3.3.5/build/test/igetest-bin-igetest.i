...
static unsigned char plaintext[10240];
static unsigned char saved_iv[16 * 4];
...
static struct bi_ige_test const bi_ige_test_vectors[] = {
    {{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
     ...
     {0x14, 0x40, 0x6f, 0xae, 0xa2, 0x79, 0xf2, 0x56,
      ...
      0x6a, 0x9c, 0x20, 0x33, 0x9d, 0x33, 0xfe, 0x12},
     16, 32, 1},
    {{0x58, 0x0a, 0x06, 0xe9, 0x97, 0x07, 0x59, 0x5c,
     ...
     {0xd1, 0x80, 0xa0, 0x31, 0x47, 0xa3, 0x11, 0x13,
      ...
      0x67, 0x7b, 0x58, 0xa8, 0x18, 0xf9, 0x72, 0xe4},
     {0x80, 0x3d, 0xbd, 0x4c, 0xe6, 0x7b, 0x06, 0xa9,
      ...
      0x48, 0x79, 0x37, 0x99, 0xff, 0x27, 0x93, 0xa3},
     {0xf1, 0x54, 0x3d, 0xca, 0xfe, 0xb5, 0xef, 0x1c,
      ...
      0x55, 0xa4, 0xb3, 0xa8, 0x40, 0x81, 0xa5, 0x0b},
     {0x42, 0xe5, 0x28, 0x30, 0x31, 0xc2, 0xa0, 0x23,
      ...
      0x5e, 0x28, 0xdc, 0x7f, 0x01, 0xc5, 0xa5, 0x4c},
     32, 64, 1},
...
static int test_ige_vectors(int n)
{
    ...
    if (!test_int_le("../test/igetest.c", 157, "v->length", "MAX_VECTOR_SIZE", v->length, 64))
        return 0;
    ...
    if (!test_mem_eq("../test/igetest.c", 167, "v->out", "buf", v->out, v->length, buf, v->length)) {
        test_info("../test/igetest.c", 168, "IGE test vector %d failed", n);
    ...
    if (!test_mem_eq("../test/igetest.c", 180, "v->out", "buf", v->out, v->length, buf, v->length)) {
        test_info("../test/igetest.c", 181, "IGE test vector %d failed (with in == out)", n);
        test_output_memory("key", v->key, sizeof(v->key));
        test_output_memory("iv", v->iv, sizeof(v->iv));
        test_output_memory("in", v->in, v->length);
        testresult = 0;
...
static int test_bi_ige_vectors(int n)
{
    ...
    unsigned char buf[64];
        ...
        if (!test_int_le("../test/igetest.c", 198, "v->length", "MAX_VECTOR_SIZE", v->length, 64))
            return 0;
    ...
    if (!test_mem_eq("../test/igetest.c", 212, "v->out", "buf", v->out, v->length, buf, v->length)) {
...
static int test_ige_enc_dec(void)
{
    ...
    AES_ige_encrypt(plaintext, ciphertext, 128, &key, iv, 1);

    AES_set_decrypt_key(rkey, 8 * sizeof(rkey), &key);
    memcpy(iv, saved_iv, sizeof(iv));
    AES_ige_encrypt(ciphertext, checktext, 128, &key, iv, 0);

    return test_mem_eq("../test/igetest.c", 238, "checktext", "plaintext", checktext, 128, plaintext, 128);
}
...
static int test_ige_enc_chaining(void)
{
    ...
    AES_set_encrypt_key(rkey, 8 * sizeof(rkey), &key);
    memcpy(iv, saved_iv, sizeof(iv));
    AES_ige_encrypt(plaintext, ciphertext, 128 / 2, &key, iv,
                    1);
    AES_ige_encrypt(plaintext + 128 / 2,
                    ciphertext + 128 / 2, 128 / 2,
                    &key, iv, 1);
    ...
    return test_mem_eq("../test/igetest.c", 260, "checktext", "plaintext", checktext, 128, plaintext, 128);
}
...
static int test_ige_dec_chaining(void)
{
    ...
    AES_set_decrypt_key(rkey, 8 * sizeof(rkey), &key);
    memcpy(iv, saved_iv, sizeof(iv));
    AES_ige_encrypt(ciphertext, checktext, 128 / 2, &key, iv,
                    0);
    AES_ige_encrypt(ciphertext + 128 / 2,
                    checktext + 128 / 2, 128 / 2, &key, iv,
                    0);

    return test_mem_eq("../test/igetest.c", 286, "checktext", "plaintext", checktext, 128, plaintext, 128);
}
...
static int test_ige_garble_forwards(void)
{
    ...
    if (!test_size_t_le("../test/igetest.c", 318, "matches", "ctsize / 2 + ctsize / 100", matches, ctsize / 2 + ctsize / 100))
        testresult = 0;


    if (!test_size_t_gt("../test/igetest.c", 322, "matches", "ctsize / 2", matches, ctsize / 2))
        testresult = 0;
    return testresult;
}
...
static int test_bi_ige_enc_dec(void)
{
    ...
    AES_bi_ige_encrypt(plaintext, ciphertext, 128, &key, &key2, iv,
                       1);

    AES_set_decrypt_key(rkey, 8 * sizeof(rkey), &key);
    AES_set_decrypt_key(rkey2, 8 * sizeof(rkey2), &key2);
    AES_bi_ige_encrypt(ciphertext, checktext, 128, &key, &key2, iv,
                       0);

    return test_mem_eq("../test/igetest.c", 345, "checktext", "plaintext", checktext, 128, plaintext, 128);
}
...
static int test_bi_ige_garble1(void)
{
    ...
    return test_size_t_le("../test/igetest.c", 376, "matches", "sizeof(checktext) / 100", matches, sizeof(checktext) / 100);
}
...
static int test_bi_ige_garble2(void)
{
    ...
    return test_size_t_le("../test/igetest.c", 407, "matches", "sizeof(checktext) / 100", matches, sizeof(checktext) / 100);
}
...
static int test_bi_ige_garble3(void)
{
    ...
    return test_size_t_le("../test/igetest.c", 438, "matches", "sizeof(checktext) / 100", matches, sizeof(checktext) / 100);
}
...
int setup_tests(void)
{
    ...
    add_test("test_bi_ige_enc_dec", test_bi_ige_enc_dec);
    ...
    add_all_tests("test_ige_vectors", test_ige_vectors, (sizeof(ige_test_vectors)/sizeof((ige_test_vectors)[0])), 1);
    add_all_tests("test_bi_ige_vectors", test_bi_ige_vectors, (sizeof(bi_ige_test_vectors)/sizeof((bi_ige_test_vectors)[0])), 1);
