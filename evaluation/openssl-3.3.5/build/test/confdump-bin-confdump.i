...
# 16 "../test/confdump.c" 2

static void dump_section(const char *name, const CONF *cnf)
{
    struct stack_st_CONF_VALUE *sect = NCONF_get_section(cnf, name);
    int i;

    printf("[ %s ]\n", name);
    for (i = 0; i < OPENSSL_sk_num(ossl_check_const_CONF_VALUE_sk_type(sect)); i++) {
        CONF_VALUE *cv = ((CONF_VALUE *)OPENSSL_sk_value(ossl_check_const_CONF_VALUE_sk_type(sect), (i)));
...
int main(int argc, char **argv)
{
    ...
    int ret = 1;
    struct stack_st_OPENSSL_CSTRING *section_names = 
# 35 "../test/confdump.c" 3 4
                                              ((void *)0)
# 35 "../test/confdump.c"
                                                  ;

    if (conf != 
# 37 "../test/confdump.c" 3 4
               ((void *)0) 
# 37 "../test/confdump.c"
        ...
        for (i = 0; i < OPENSSL_sk_num(ossl_check_const_OPENSSL_CSTRING_sk_type(section_names)); i++) {
            dump_section(((const char *)OPENSSL_sk_value(ossl_check_const_OPENSSL_CSTRING_sk_type(section_names), (i))), conf);
        }
        OPENSSL_sk_free(ossl_check_OPENSSL_CSTRING_sk_type(section_names));
        ret = 0;
    } else {
        ERR_print_errors_fp(
# 47 "../test/confdump.c" 3 4
                           stderr
# 47 "../test/confdump.c"
                                 );
    }
