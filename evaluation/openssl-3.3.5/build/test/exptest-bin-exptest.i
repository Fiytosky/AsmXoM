...
{
    if (!BN_is_zero(r)) {
        test_error("../test/exptest.c", 35, "%s failed: a ** 0 mod 1 = r (should be 0)", method);
        test_output_bignum("a", a);
        test_output_bignum("r", r);
        return 0;
    }
    return 1;
}
...
# 48 "../test/exptest.c"
                                         ;
    BIGNUM *r = 
# 49 "../test/exptest.c" 3 4
               ((void *)0)
# 49 "../test/exptest.c"
                   ;
    unsigned long one_word = 1;
    BN_CTX *ctx = BN_CTX_new();
    int ret = 0, failed = 0;
    BN_MONT_CTX *mont = 
# 53 "../test/exptest.c" 3 4
                       ((void *)0)
# 53 "../test/exptest.c"
                           ;

    if (!test_ptr("../test/exptest.c", 55, "m = BN_new()", m = BN_new())
        || !test_ptr("../test/exptest.c", 56, "a = BN_new()", a = BN_new())
        || !test_ptr("../test/exptest.c", 57, "p = BN_new()", p = BN_new())
        || !test_ptr("../test/exptest.c", 58, "r = BN_new()", r = BN_new()))
        goto err;
    ...
    if (!test_true("../test/exptest.c", 65, "BN_rand(a, 1024, BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ANY)", (BN_rand(a, 1024, 0, 0)) != 0))
        goto err;

    if (!test_true("../test/exptest.c", 68, "BN_mod_exp(r, a, p, m, ctx)", (BN_mod_exp(r, a, p, m, ctx)) != 0))
        goto err;

    if (!test_true("../test/exptest.c", 71, "a_is_zero_mod_one(\"BN_mod_exp\", r, a)", (a_is_zero_mod_one("BN_mod_exp", r, a)) != 0))
        failed = 1;

    if (!test_true("../test/exptest.c", 74, "BN_mod_exp_recp(r, a, p, m, ctx)", (BN_mod_exp_recp(r, a, p, m, ctx)) != 0))
        goto err;

    if (!test_true("../test/exptest.c", 77, "a_is_zero_mod_one(\"BN_mod_exp_recp\", r, a)", (a_is_zero_mod_one("BN_mod_exp_recp", r, a)) != 0))
        failed = 1;

    if (!test_true("../test/exptest.c", 80, "BN_mod_exp_simple(r, a, p, m, ctx)", (BN_mod_exp_simple(r, a, p, m, ctx)) != 0))
        goto err;

    if (!test_true("../test/exptest.c", 83, "a_is_zero_mod_one(\"BN_mod_exp_simple\", r, a)", (a_is_zero_mod_one("BN_mod_exp_simple", r, a)) != 0))
        failed = 1;

    if (!test_true("../test/exptest.c", 86, "BN_mod_exp_mont(r, a, p, m, ctx, NULL)", (BN_mod_exp_mont(r, a, p, m, ctx, 
# 86 "../test/exptest.c" 3 4
        ((void *)0)
# 86 "../test/exptest.c"
        )) != 0))
        goto err;

    if (!test_true("../test/exptest.c", 89, "a_is_zero_mod_one(\"BN_mod_exp_mont\", r, a)", (a_is_zero_mod_one("BN_mod_exp_mont", r, a)) != 0))
        failed = 1;

    if (!test_true("../test/exptest.c", 92, "BN_mod_exp_mont_consttime(r, a, p, m, ctx, NULL)", (BN_mod_exp_mont_consttime(r, a, p, m, ctx, 
# 92 "../test/exptest.c" 3 4
        ((void *)0)
# 92 "../test/exptest.c"
        )) != 0))
        goto err;

    if (!test_true("../test/exptest.c", 95, "a_is_zero_mod_one(\"BN_mod_exp_mont_consttime\", r, a)", (a_is_zero_mod_one("BN_mod_exp_mont_consttime", r, a)) != 0))
        failed = 1;

    if (!test_ptr("../test/exptest.c", 98, "mont = BN_MONT_CTX_new()", mont = BN_MONT_CTX_new()))
        goto err;

    ERR_set_mark();

    if (!test_false("../test/exptest.c", 103, "BN_mod_exp_mont_consttime(r, p, a, m, ctx, mont)", (BN_mod_exp_mont_consttime(r, p, a, m, ctx, mont)) != 0))
        goto err;
    if (!test_false("../test/exptest.c", 105, "BN_mod_exp_mont(r, p, a, m, ctx, mont)", (BN_mod_exp_mont(r, p, a, m, ctx, mont)) != 0))
        goto err;
    ERR_pop_to_mark();

    if (!test_true("../test/exptest.c", 109, "BN_MONT_CTX_set(mont, m, ctx)", (BN_MONT_CTX_set(mont, m, ctx)) != 0))
        goto err;


    if (!test_true("../test/exptest.c", 113, "BN_mod_exp_mont_consttime(r, p, a, m, ctx, mont)", (BN_mod_exp_mont_consttime(r, p, a, m, ctx, mont)) != 0))
        goto err;

    if (!test_true("../test/exptest.c", 116, "a_is_zero_mod_one(\"BN_mod_exp_mont_consttime\", r, a)", (a_is_zero_mod_one("BN_mod_exp_mont_consttime", r, a)) != 0))
        failed = 1;

    if (!test_true("../test/exptest.c", 119, "BN_mod_exp_mont(r, p, a, m, ctx, mont)", (BN_mod_exp_mont(r, p, a, m, ctx, mont)) != 0))
        goto err;

    if (!test_true("../test/exptest.c", 122, "a_is_zero_mod_one(\"BN_mod_exp_mont\", r, a)", (a_is_zero_mod_one("BN_mod_exp_mont", r, a)) != 0))
        failed = 1;
    ...
    if (!test_true("../test/exptest.c", 129, "BN_mod_exp_mont_word(r, one_word, p, m, ctx, NULL)", (BN_mod_exp_mont_word(r, one_word, p, m, ctx, 
# 129 "../test/exptest.c" 3 4
        ((void *)0)
# 129 "../test/exptest.c"
        )) != 0))
        goto err;

    if (!test_BN_eq_zero("../test/exptest.c", 132, "r", r)) {
        test_error("../test/exptest.c", 133, "BN_mod_exp_mont_word failed: " "1 ** 0 mod 1 = r (should be 0)")
                                                    ;
        test_output_bignum("r", r);
        goto err;
    }
...
static int test_mod_exp(int round)
{
    ...
    int ret = 0;
    BIGNUM *r_mont = 
# 156 "../test/exptest.c" 3 4
                    ((void *)0)
# 156 "../test/exptest.c"
                        ;
    BIGNUM *r_mont_const = 
# 157 "../test/exptest.c" 3 4
                          ((void *)0)
# 157 "../test/exptest.c"
                              ;
    BIGNUM *r_recp = 
# 158 "../test/exptest.c" 3 4
                    ((void *)0)
# 158 "../test/exptest.c"
                        ;
    BIGNUM *r_simple = 
# 159 "../test/exptest.c" 3 4
                      ((void *)0)
# 159 "../test/exptest.c"
                          ;
    BIGNUM *a = 
# 160 "../test/exptest.c" 3 4
               ((void *)0)
# 160 "../test/exptest.c"
                   ;
    BIGNUM *b = 
# 161 "../test/exptest.c" 3 4
               ((void *)0)
# 161 "../test/exptest.c"
                   ;
    BIGNUM *m = 
# 162 "../test/exptest.c" 3 4
               ((void *)0)
# 162 "../test/exptest.c"
                   ;

    if (!test_ptr("../test/exptest.c", 164, "ctx = BN_CTX_new()", ctx = BN_CTX_new()))
        goto err;

    if (!test_ptr("../test/exptest.c", 167, "r_mont = BN_new()", r_mont = BN_new())
        || !test_ptr("../test/exptest.c", 168, "r_mont_const = BN_new()", r_mont_const = BN_new())
        || !test_ptr("../test/exptest.c", 169, "r_recp = BN_new()", r_recp = BN_new())
        || !test_ptr("../test/exptest.c", 170, "r_simple = BN_new()", r_simple = BN_new())
        || !test_ptr("../test/exptest.c", 171, "a = BN_new()", a = BN_new())
        || !test_ptr("../test/exptest.c", 172, "b = BN_new()", b = BN_new())
        || !test_ptr("../test/exptest.c", 173, "m = BN_new()", m = BN_new()))
        goto err;

    if (!test_int_gt("../test/exptest.c", 176, "RAND_bytes(&c, 1)", "0", RAND_bytes(&c, 1), 0))
        goto err;
    c = (c % ((8 * 8) * 2)) - (8 * 8);
    if (!test_true("../test/exptest.c", 179, "BN_rand(a, NUM_BITS + c, BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ANY)", (BN_rand(a, ((8 * 8) * 4) + c, 0, 0)) != 0)
                                               )
        goto err;

    if (!test_int_gt("../test/exptest.c", 183, "RAND_bytes(&c, 1)", "0", RAND_bytes(&c, 1), 0))
        goto err;
    c = (c % ((8 * 8) * 2)) - (8 * 8);
    if (!test_true("../test/exptest.c", 186, "BN_rand(b, NUM_BITS + c, BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ANY)", (BN_rand(b, ((8 * 8) * 4) + c, 0, 0)) != 0)
                                               )
        goto err;

    if (!test_int_gt("../test/exptest.c", 190, "RAND_bytes(&c, 1)", "0", RAND_bytes(&c, 1), 0))
        goto err;
    c = (c % ((8 * 8) * 2)) - (8 * 8);
    if (!test_true("../test/exptest.c", 193, "BN_rand(m, NUM_BITS + c, BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ODD)", (BN_rand(m, ((8 * 8) * 4) + c, 0, 1)) != 0)
                                               )
        goto err;

    if (!test_true("../test/exptest.c", 197, "BN_mod(a, a, m, ctx)", (BN_div(
# 197 "../test/exptest.c" 3 4
        ((void *)0)
# 197 "../test/exptest.c"
        ,(a),(a),(m),(ctx))) != 0)
        || !test_true("../test/exptest.c", 198, "BN_mod(b, b, m, ctx)", (BN_div(
# 198 "../test/exptest.c" 3 4
           ((void *)0)
# 198 "../test/exptest.c"
           ,(b),(b),(m),(ctx))) != 0)
        || !test_true("../test/exptest.c", 199, "BN_mod_exp_mont(r_mont, a, b, m, ctx, NULL)", (BN_mod_exp_mont(r_mont, a, b, m, ctx, 
# 199 "../test/exptest.c" 3 4
           ((void *)0)
# 199 "../test/exptest.c"
           )) != 0)
        || !test_true("../test/exptest.c", 200, "BN_mod_exp_recp(r_recp, a, b, m, ctx)", (BN_mod_exp_recp(r_recp, a, b, m, ctx)) != 0)
        || !test_true("../test/exptest.c", 201, "BN_mod_exp_simple(r_simple, a, b, m, ctx)", (BN_mod_exp_simple(r_simple, a, b, m, ctx)) != 0)
        || !test_true("../test/exptest.c", 202, "BN_mod_exp_mont_consttime(r_mont_const, a, b, m, ctx, NULL)", (BN_mod_exp_mont_consttime(r_mont_const, a, b, m, ctx, 
# 202 "../test/exptest.c" 3 4
           ((void *)0)
# 202 "../test/exptest.c"
           )) != 0))
        goto err;

    if (!test_BN_eq("../test/exptest.c", 205, "r_simple", "r_mont", r_simple, r_mont)
        || !test_BN_eq("../test/exptest.c", 206, "r_simple", "r_recp", r_simple, r_recp)
        || !test_BN_eq("../test/exptest.c", 207, "r_simple", "r_mont_const", r_simple, r_mont_const)) {
        if (BN_cmp(r_simple, r_mont) != 0)
            test_info("../test/exptest.c", 209, "simple and mont results differ");
        if (BN_cmp(r_simple, r_mont_const) != 0)
            test_info("../test/exptest.c", 211, "simple and mont const time results differ");
        if (BN_cmp(r_simple, r_recp) != 0)
            test_info("../test/exptest.c", 213, "simple and recp results differ");

        test_output_bignum("a", a);
        test_output_bignum("b", b);
        test_output_bignum("m", m);
        test_output_bignum("r_simple", r_simple);
        test_output_bignum("r_recp", r_recp);
        test_output_bignum("r_mont", r_mont);
        test_output_bignum("r_mont_const", r_mont_const);
        goto err;
    }
...
static int test_mod_exp_x2(int idx)
{
    BN_CTX *ctx;
    int ret = 0;
    BIGNUM *r_mont_const_x2_1 = 
# 243 "../test/exptest.c" 3 4
                               ((void *)0)
# 243 "../test/exptest.c"
                                   ;
    BIGNUM *r_mont_const_x2_2 = 
# 244 "../test/exptest.c" 3 4
                               ((void *)0)
# 244 "../test/exptest.c"
                                   ;
    BIGNUM *r_simple1 = 
# 245 "../test/exptest.c" 3 4
                       ((void *)0)
# 245 "../test/exptest.c"
                           ;
    BIGNUM *r_simple2 = 
# 246 "../test/exptest.c" 3 4
                       ((void *)0)
# 246 "../test/exptest.c"
                           ;
    BIGNUM *a1 = 
# 247 "../test/exptest.c" 3 4
                ((void *)0)
# 247 "../test/exptest.c"
                    ;
    BIGNUM *b1 = 
# 248 "../test/exptest.c" 3 4
                ((void *)0)
# 248 "../test/exptest.c"
                    ;
    BIGNUM *m1 = 
# 249 "../test/exptest.c" 3 4
                ((void *)0)
# 249 "../test/exptest.c"
                    ;
    BIGNUM *a2 = 
# 250 "../test/exptest.c" 3 4
                ((void *)0)
# 250 "../test/exptest.c"
                    ;
    BIGNUM *b2 = 
# 251 "../test/exptest.c" 3 4
                ((void *)0)
# 251 "../test/exptest.c"
                    ;
    BIGNUM *m2 = 
# 252 "../test/exptest.c" 3 4
                ((void *)0)
# 252 "../test/exptest.c"
                    ;
    int factor_size = 0;
    ...
    if (!test_ptr("../test/exptest.c", 262, "ctx = BN_CTX_new()", ctx = BN_CTX_new()))
        goto err;

    if (!test_ptr("../test/exptest.c", 265, "r_mont_const_x2_1 = BN_new()", r_mont_const_x2_1 = BN_new())
        || !test_ptr("../test/exptest.c", 266, "r_mont_const_x2_2 = BN_new()", r_mont_const_x2_2 = BN_new())
        || !test_ptr("../test/exptest.c", 267, "r_simple1 = BN_new()", r_simple1 = BN_new())
        || !test_ptr("../test/exptest.c", 268, "r_simple2 = BN_new()", r_simple2 = BN_new())
        || !test_ptr("../test/exptest.c", 269, "a1 = BN_new()", a1 = BN_new())
        || !test_ptr("../test/exptest.c", 270, "b1 = BN_new()", b1 = BN_new())
        || !test_ptr("../test/exptest.c", 271, "m1 = BN_new()", m1 = BN_new())
        || !test_ptr("../test/exptest.c", 272, "a2 = BN_new()", a2 = BN_new())
        || !test_ptr("../test/exptest.c", 273, "b2 = BN_new()", b2 = BN_new())
        || !test_ptr("../test/exptest.c", 274, "m2 = BN_new()", m2 = BN_new()))
        goto err;

    BN_rand(a1, factor_size, 0, 0);
    BN_rand(b1, factor_size, 0, 0);
    BN_rand(m1, factor_size, 0, 1);
    BN_rand(a2, factor_size, 0, 0);
    BN_rand(b2, factor_size, 0, 0);
    BN_rand(m2, factor_size, 0, 1);

    if (!test_true("../test/exptest.c", 284, "BN_mod(a1, a1, m1, ctx)", (BN_div(
# 284 "../test/exptest.c" 3 4
        ((void *)0)
# 284 "../test/exptest.c"
        ,(a1),(a1),(m1),(ctx))) != 0)
        || !test_true("../test/exptest.c", 285, "BN_mod(b1, b1, m1, ctx)", (BN_div(
# 285 "../test/exptest.c" 3 4
           ((void *)0)
# 285 "../test/exptest.c"
           ,(b1),(b1),(m1),(ctx))) != 0)
        || !test_true("../test/exptest.c", 286, "BN_mod(a2, a2, m2, ctx)", (BN_div(
# 286 "../test/exptest.c" 3 4
           ((void *)0)
# 286 "../test/exptest.c"
           ,(a2),(a2),(m2),(ctx))) != 0)
        || !test_true("../test/exptest.c", 287, "BN_mod(b2, b2, m2, ctx)", (BN_div(
# 287 "../test/exptest.c" 3 4
           ((void *)0)
# 287 "../test/exptest.c"
           ,(b2),(b2),(m2),(ctx))) != 0)
        || !test_true("../test/exptest.c", 288, "BN_mod_exp_simple(r_simple1, a1, b1, m1, ctx)", (BN_mod_exp_simple(r_simple1, a1, b1, m1, ctx)) != 0)
        || !test_true("../test/exptest.c", 289, "BN_mod_exp_simple(r_simple2, a2, b2, m2, ctx)", (BN_mod_exp_simple(r_simple2, a2, b2, m2, ctx)) != 0)
        || !test_true("../test/exptest.c", 290, "BN_mod_exp_mont_consttime_x2(r_mont_const_x2_1, a1, b1, m1, NULL, r_mont_const_x2_2, a2, b2, m2, NULL, ctx)", (BN_mod_exp_mont_consttime_x2(r_mont_const_x2_1, a1, b1, m1, 
# 290 "../test/exptest.c" 3 4
           ((void *)0)
# 290 "../test/exptest.c"
           , r_mont_const_x2_2, a2, b2, m2, 
# 290 "../test/exptest.c" 3 4
           ((void *)0)
# 290 "../test/exptest.c"
           , ctx)) != 0)
    ...
    if (!test_BN_eq("../test/exptest.c", 295, "r_simple1", "r_mont_const_x2_1", r_simple1, r_mont_const_x2_1)
        || !test_BN_eq("../test/exptest.c", 296, "r_simple2", "r_mont_const_x2_2", r_simple2, r_mont_const_x2_2)) {
        if (BN_cmp(r_simple1, r_mont_const_x2_1) != 0)
            test_info("../test/exptest.c", 298, "simple and mont const time x2 (#1) results differ");
        if (BN_cmp(r_simple2, r_mont_const_x2_2) != 0)
            test_info("../test/exptest.c", 300, "simple and mont const time x2 (#2) results differ");

        test_output_bignum("a1", a1);
        test_output_bignum("b1", b1);
        test_output_bignum("m1", m1);
        test_output_bignum("a2", a2);
        test_output_bignum("b2", b2);
        test_output_bignum("m2", m2);
        test_output_bignum("r_simple1", r_simple1);
        test_output_bignum("r_simple2", r_simple2);
        test_output_bignum("r_mont_const_x2_1", r_mont_const_x2_1);
        test_output_bignum("r_mont_const_x2_2", r_mont_const_x2_2);
        goto err;
    }
...
int setup_tests(void)
{
    ...
    add_all_tests("test_mod_exp_x2", test_mod_exp_x2, 300, 1);
    return 1;
}
