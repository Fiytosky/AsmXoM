# 0 "../test/quic_client_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/quic_client_test.c"
# 9 "../test/quic_client_test.c"
...
static int test_quic_client_ex(int fd_arg)
{
    ...
    int c_fd;
    BIO *c_net_bio = 
# 35 "../test/quic_client_test.c" 3 4
                    ((void *)0)
# 35 "../test/quic_client_test.c"
                        , *c_net_bio_own = 
# 35 "../test/quic_client_test.c" 3 4
                                           ((void *)0)
# 35 "../test/quic_client_test.c"
                                               ;
    BIO_ADDR *s_addr_ = 
# 36 "../test/quic_client_test.c" 3 4
                       ((void *)0)
# 36 "../test/quic_client_test.c"
                           ;
    struct in_addr ina = {0};
    SSL_CTX *c_ctx = 
# 38 "../test/quic_client_test.c" 3 4
                    ((void *)0)
# 38 "../test/quic_client_test.c"
                        ;
    SSL *c_ssl = 
# 39 "../test/quic_client_test.c" 3 4
                ((void *)0)
# 39 "../test/quic_client_test.c"
                    ;
    short port = 4433;
    ...
    if (fd_arg == (-1)) {
        ...
        c_fd = BIO_socket(
# 49 "../test/quic_client_test.c" 3 4
                         2
# 49 "../test/quic_client_test.c"
                                , 
# 49 "../test/quic_client_test.c" 3 4
                                  SOCK_DGRAM
# 49 "../test/quic_client_test.c"
...
# 49 "../test/quic_client_test.c"
                                                         , 0);
        if (!test_int_ne("../test/quic_client_test.c", 50, "c_fd", "INVALID_SOCKET", c_fd, (-1)))
            goto err;

        if (!test_true("../test/quic_client_test.c", 53, "BIO_socket_nbio(c_fd, 1)", (BIO_socket_nbio(c_fd, 1)) != 0))
            goto err;

        if (!test_ptr("../test/quic_client_test.c", 56, "s_addr_ = BIO_ADDR_new()", s_addr_ = BIO_ADDR_new()))
            goto err;

        ina.s_addr = htonl(0x7f000001UL);
        if (!test_true("../test/quic_client_test.c", 60, "BIO_ADDR_rawmake(s_addr_, AF_INET, &ina, sizeof(ina), htons(port))", (BIO_ADDR_rawmake(s_addr_, 
# 60 "../test/quic_client_test.c" 3 4
            2
# 60 "../test/quic_client_test.c"
            , &ina, sizeof(ina), htons(port))) != 0)
                                                     )
            goto err;
    } else {
        c_fd = fd_arg;
    ...
    if (!test_ptr("../test/quic_client_test.c", 67, "c_net_bio = c_net_bio_own = BIO_new_dgram(c_fd, 0)", c_net_bio = c_net_bio_own = BIO_new_dgram(c_fd, 0)))
        goto err;


    if (s_addr_ != 
# 71 "../test/quic_client_test.c" 3 4
                  ((void *)0) 
# 71 "../test/quic_client_test.c"
                       && !(int)BIO_ctrl(c_net_bio, 44, 0, (char *)(s_addr_)))
        goto err;

    if (!test_ptr("../test/quic_client_test.c", 74, "c_ctx = SSL_CTX_new(OSSL_QUIC_client_method())", c_ctx = SSL_CTX_new(OSSL_QUIC_client_method())))
        goto err;

    if (!test_ptr("../test/quic_client_test.c", 77, "c_ssl = SSL_new(c_ctx)", c_ssl = SSL_new(c_ctx)))
        goto err;


    if (!test_false("../test/quic_client_test.c", 81, "SSL_set_alpn_protos(c_ssl, alpn, sizeof(alpn))", (SSL_set_alpn_protos(c_ssl, alpn, sizeof(alpn))) != 0))
        goto err;
    ...
    if (!test_true("../test/quic_client_test.c", 88, "BIO_up_ref(c_net_bio)", (BIO_up_ref(c_net_bio)) != 0)) {
        c_net_bio_own = 
# 89 "../test/quic_client_test.c" 3 4
                       ((void *)0)
# 89 "../test/quic_client_test.c"
                           ;
        goto err;
    }
    ...
    c_net_bio_own = 
# 94 "../test/quic_client_test.c" 3 4
                   ((void *)0)
# 94 "../test/quic_client_test.c"
                       ;

    if (!test_true("../test/quic_client_test.c", 96, "SSL_set_blocking_mode(c_ssl, 0)", (SSL_set_blocking_mode(c_ssl, 0)) != 0))
        goto err;
    ...
    for (;;) {
        if (ossl_time_compare(ossl_time_subtract(ossl_time_now(), start_time),
                              ossl_ticks2time((10000) * (((uint64_t)1000000000) / 1000))) >= 0) {
            test_error("../test/quic_client_test.c", 104, "timeout while attempting QUIC client test");
            goto err;
        }

        if (!c_connected) {
            ret = SSL_connect(c_ssl);
            if (!test_true("../test/quic_client_test.c", 110, "ret == 1 || is_want(c_ssl, ret)", (ret == 1 || is_want(c_ssl, ret)) != 0))
                goto err;

            if (ret == 1) {
                c_connected = 1;
                test_info("../test/quic_client_test.c", 115, "Connected!");
            }
        }

        if (c_connected && !c_write_done) {
            if (!test_int_eq("../test/quic_client_test.c", 120, "SSL_write(c_ssl, msg1, sizeof(msg1) - 1)", "(int)sizeof(msg1) - 1", SSL_write(c_ssl, msg1, sizeof(msg1) - 1), (int)sizeof(msg1) - 1)
                                                   )
                goto err;

            if (!test_true("../test/quic_client_test.c", 124, "SSL_stream_conclude(c_ssl, 0)", (SSL_stream_conclude(c_ssl, 0)) != 0))
                goto err;
        ...
        if (c_write_done && !c_shutdown && c_total_read < sizeof(msg2) - 1) {
            ...
            if (ret != 1) {
                if (SSL_get_error(c_ssl, ret) == 6) {
                    c_shutdown = 1;
                    test_info("../test/quic_client_test.c", 136, "Message: \n%s\n", msg2);
                } else if (!test_true("../test/quic_client_test.c", 137, "is_want(c_ssl, ret)", (is_want(c_ssl, ret)) != 0)) {
                    goto err;
                }
            } else {
                c_total_read += l;

                if (!test_size_t_lt("../test/quic_client_test.c", 143, "c_total_read", "sizeof(msg2) - 1", c_total_read, sizeof(msg2) - 1))
                    goto err;
            }
        }
...
static int test_quic_client_connect_first(void)
{
    ...
    sin.sin_family = 
# 187 "../test/quic_client_test.c" 3 4
                    2
# 187 "../test/quic_client_test.c"
                           ;
    sin.sin_port = htons(4433);
    sin.sin_addr.s_addr = htonl(0x7f000001UL);

    c_fd = socket(
# 191 "../test/quic_client_test.c" 3 4
                 2
# 191 "../test/quic_client_test.c"
                        , 
# 191 "../test/quic_client_test.c" 3 4
                          SOCK_DGRAM
# 191 "../test/quic_client_test.c"
...
# 191 "../test/quic_client_test.c"
    ...
    if (!test_int_ne("../test/quic_client_test.c", 192, "c_fd", "INVALID_SOCKET", c_fd, (-1)))
        goto err;

    if (!test_int_eq("../test/quic_client_test.c", 195, "connect(c_fd, (const struct sockaddr *)&sin, sizeof(sin))", "0", connect(c_fd, (const struct sockaddr *)&sin, sizeof(sin)), 0))
        goto err;

    if (!test_true("../test/quic_client_test.c", 198, "BIO_socket_nbio(c_fd, 1)", (BIO_socket_nbio(c_fd, 1)) != 0))
        goto err;
...
# 213 "../test/quic_client_test.c" 3 4
((void *)0) 
# 213 "../test/quic_client_test.c"
} }; return options; }

int setup_tests(void)
{
    if (!test_skip_common_options()) {
        test_error("../test/quic_client_test.c", 218, "Error parsing test options\n");
        return 0;
    }
