# 0 "../test/testutil/stanza.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/testutil/stanza.c"
# 9 "../test/testutil/stanza.c"
...
# 14 "../test/testutil/stanza.c" 2

# 1 "../include/internal/nelem.h" 1
# 12 "../include/internal/nelem.h"
        
# 16 "../test/testutil/stanza.c" 2
...
# 18 "../test/testutil/stanza.c" 2

int test_start_file(STANZA *s, const char *testfile)
{
    test_info("../test/testutil/stanza.c", 21, "Reading %s", testfile);
    set_test_title(testfile);
    memset(s, 0, sizeof(*s));
    if (!test_ptr("../test/testutil/stanza.c", 24, "s->fp = BIO_new_file(testfile, \"r\")", s->fp = BIO_new_file(testfile, "r")))
        return 0;
...
int test_end_file(STANZA *s)
{
    test_info("../test/testutil/stanza.c", 32, "Completed %d tests with %d errors and %d skipped", s->numtests, s->errors, s->numskip)
                                                 ;
    BIO_free(s->fp);
    return 1;
}
...
static int read_key(STANZA *s)
{
    ...
    if (s->key == 
# 45 "../test/testutil/stanza.c" 3 4
                 ((void *)0)
# 45 "../test/testutil/stanza.c"
                     ) {
        if (!test_ptr("../test/testutil/stanza.c", 46, "s->key = BIO_new(BIO_s_mem())", s->key = BIO_new(BIO_s_mem())))
            return 0;
    } else if (!test_int_gt("../test/testutil/stanza.c", 48, "BIO_reset(s->key)", "0", (int)BIO_ctrl(s->key,1,0,
# 48 "../test/testutil/stanza.c" 3 4
               ((void *)0)
# 48 "../test/testutil/stanza.c"
               ), 0)) {
        return 0;
    }
    ...
    while (BIO_gets(s->fp, tmpbuf, sizeof(tmpbuf))) {
        s->curr++;
        if (!test_int_gt("../test/testutil/stanza.c", 55, "BIO_puts(s->key, tmpbuf)", "0", BIO_puts(s->key, tmpbuf), 0))
            return 0;
        if ((strncmp(tmpbuf, "-----END" "", sizeof("-----END") - 1) == 0))
            return 1;
    }
    test_error("../test/testutil/stanza.c", 60, "Can't find key end");
    return 0;
}
...
static char *strip_spaces(char *p)
{
    ...
    while (*p && 
# 73 "../test/testutil/stanza.c" 3 4
                ((*__ctype_b_loc ())[(int) ((
# 73 "../test/testutil/stanza.c"
                (unsigned char)*p
# 73 "../test/testutil/stanza.c" 3 4
...
# 73 "../test/testutil/stanza.c"
    ...
    if (*p == '\0')
        return 
# 76 "../test/testutil/stanza.c" 3 4
              ((void *)0)
# 76 "../test/testutil/stanza.c"
                  ;

    for (q = p + strlen(p) - 1; q != p && 
# 78 "../test/testutil/stanza.c" 3 4
                                         ((*__ctype_b_loc ())[(int) ((
# 78 "../test/testutil/stanza.c"
                                         (unsigned char)*q
# 78 "../test/testutil/stanza.c" 3 4
                                         ))] & (unsigned short int) _ISspace)
# 78 "../test/testutil/stanza.c"
        ...
        *q-- = '\0';
    return *p ? p : 
# 80 "../test/testutil/stanza.c" 3 4
                   ((void *)0)
# 80 "../test/testutil/stanza.c"
                       ;
}
...
int test_readstanza(STANZA *s)
{
    ...
    for (s->numpairs = 0; BIO_gets(s->fp, s->buff, sizeof(s->buff)); ) {
        s->curr++;
        if (!test_ptr("../test/testutil/stanza.c", 94, "p = strchr(s->buff, '\\n')", p = strchr(s->buff, '\n'))) {
            test_info("../test/testutil/stanza.c", 95, "Line %d too long", s->curr);
            return 0;
        }
        *p = '\0';
        ...
        if (!test_ptr("../test/testutil/stanza.c", 109, "equals = strchr(s->buff, '=')", equals = strchr(s->buff, '='))) {
            test_info("../test/testutil/stanza.c", 110, "Missing = at line %d\n", s->curr);
            return 0;
        }
        *equals++ = '\0';
        if (!test_ptr("../test/testutil/stanza.c", 114, "key = strip_spaces(s->buff)", key = strip_spaces(s->buff))) {
            test_info("../test/testutil/stanza.c", 115, "Empty field at line %d\n", s->curr);
            return 0;
        }
        if ((value = strip_spaces(equals)) == 
# 118 "../test/testutil/stanza.c" 3 4
                                             ((void *)0)
# 118 "../test/testutil/stanza.c"
                                                 )
            value = "";

        if (strcmp(key, "Title") == 0) {
            test_info("../test/testutil/stanza.c", 122, "Starting \"%s\" tests at line %d", value, s->curr);
            continue;
        }
        ...
        if (!test_int_lt("../test/testutil/stanza.c", 138, "s->numpairs++", "TESTMAXPAIRS", s->numpairs++, 150)
                || !test_ptr("../test/testutil/stanza.c", 139, "pp->key = OPENSSL_strdup(key)", pp->key = CRYPTO_strdup(key, "../test/testutil/stanza.c", 139))
                || !test_ptr("../test/testutil/stanza.c", 140, "pp->value = OPENSSL_strdup(value)", pp->value = CRYPTO_strdup(value, "../test/testutil/stanza.c", 140)))
            return 0;
        pp++;
    }
...
void test_clearstanza(STANZA *s)
{
    ...
    for ( ; --i >= 0; pp++) {
        CRYPTO_free(pp->key, "../test/testutil/stanza.c", 155);
        CRYPTO_free(pp->value, "../test/testutil/stanza.c", 156);
    }
    s->numpairs = 0;
}
