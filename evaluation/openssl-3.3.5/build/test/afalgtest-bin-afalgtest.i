...
static int test_afalg_aes_cbc(int keysize_idx)
{
    EVP_CIPHER_CTX *ctx;
    const EVP_CIPHER *cipher;
    unsigned char ebuf[17 + 32];
    unsigned char dbuf[17 + 32];
    const unsigned char *enc_result = 
# 34 "../test/afalgtest.c" 3 4
                                     ((void *)0)
# 34 "../test/afalgtest.c"
                                         ;
...
# 66 "../test/afalgtest.c"
    switch (keysize_idx) {
        ...
        default:
            cipher = 
# 80 "../test/afalgtest.c" 3 4
                    ((void *)0)
# 80 "../test/afalgtest.c"
                        ;
    }
    if (!test_ptr("../test/afalgtest.c", 82, "ctx = EVP_CIPHER_CTX_new()", ctx = EVP_CIPHER_CTX_new()))
            return 0;

    if (!test_true("../test/afalgtest.c", 85, "EVP_CipherInit_ex(ctx, cipher, e, key, iv, 1)", (EVP_CipherInit_ex(ctx, cipher, e, key, iv, 1)) != 0)
            || !test_true("../test/afalgtest.c", 86, "EVP_CipherUpdate(ctx, ebuf, &encl, in, BUFFER_SIZE)", (EVP_CipherUpdate(ctx, ebuf, &encl, in, 17)) != 0)
            || !test_true("../test/afalgtest.c", 87, "EVP_CipherFinal_ex(ctx, ebuf + encl, &encf)", (EVP_CipherFinal_ex(ctx, ebuf + encl, &encf)) != 0))
        goto end;
    encl += encf;

    if (!test_mem_eq("../test/afalgtest.c", 91, "enc_result", "ebuf", enc_result, 17, ebuf, 17))
        goto end;

    if (!test_true("../test/afalgtest.c", 94, "EVP_CIPHER_CTX_reset(ctx)", (EVP_CIPHER_CTX_reset(ctx)) != 0)
            || !test_true("../test/afalgtest.c", 95, "EVP_CipherInit_ex(ctx, cipher, e, key, iv, 0)", (EVP_CipherInit_ex(ctx, cipher, e, key, iv, 0)) != 0)
            || !test_true("../test/afalgtest.c", 96, "EVP_CipherUpdate(ctx, dbuf, &decl, ebuf, encl)", (EVP_CipherUpdate(ctx, dbuf, &decl, ebuf, encl)) != 0)
            || !test_true("../test/afalgtest.c", 97, "EVP_CipherFinal_ex(ctx, dbuf + decl, &decf)", (EVP_CipherFinal_ex(ctx, dbuf + decl, &decf)) != 0))
        goto end;
    decl += decf;

    if (!test_int_eq("../test/afalgtest.c", 101, "decl", "BUFFER_SIZE", decl, 17)
            || !test_mem_eq("../test/afalgtest.c", 102, "dbuf", "in", dbuf, 17, in, 17))
        goto end;
...
static int test_pr16743(void)
{
    ...
    if (!test_true("../test/afalgtest.c", 118, "ENGINE_init(e)", (ENGINE_init(e)) != 0))
        return 0;
    cipher = ENGINE_get_cipher(e, 419);
    ctx = EVP_CIPHER_CTX_new();
    if (cipher != 
# 122 "../test/afalgtest.c" 3 4
                 ((void *)0) 
# 122 "../test/afalgtest.c"
                      && ctx != 
# 122 "../test/afalgtest.c" 3 4
                                ((void *)0)
# 122 "../test/afalgtest.c"
                                    )
        ret = EVP_EncryptInit_ex(ctx, cipher, e, 
# 123 "../test/afalgtest.c" 3 4
                                                ((void *)0)
# 123 "../test/afalgtest.c"
                                                    , 
# 123 "../test/afalgtest.c" 3 4
                                                      ((void *)0)
# 123 "../test/afalgtest.c"
                                                          );
    test_true("../test/afalgtest.c", 124, "ret", (ret) != 0);
    EVP_CIPHER_CTX_free(ctx);
    ENGINE_finish(e);
    return ret;
}
...
int global_init(void)
{
    ...
    OPENSSL_init_crypto(0x00008000L, 
# 134 "../test/afalgtest.c" 3 4
                                                  ((void *)0)
# 134 "../test/afalgtest.c"
                                                      );
...
int setup_tests(void)
{
    ...
    if ((e = ENGINE_by_id("afalg")) == 
# 143 "../test/afalgtest.c" 3 4
                                      ((void *)0)
# 143 "../test/afalgtest.c"
                                          ) {

        test_info("../test/afalgtest.c", 145, "Can't load AFALG engine");
    } else {
        add_all_tests("test_afalg_aes_cbc", test_afalg_aes_cbc, 3, 1);
