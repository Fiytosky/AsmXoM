# 0 "../test/cipher_overhead_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/cipher_overhead_test.c"
# 10 "../test/cipher_overhead_test.c"
# 1 "../include/internal/nelem.h" 1
# 12 "../include/internal/nelem.h"
        
# 11 "../test/cipher_overhead_test.c" 2
# 1 "../test/testutil.h" 1
# 13 "../test/testutil.h"
...
# 12 "../test/cipher_overhead_test.c" 2
# 1 "../test/../ssl/ssl_local.h" 1
# 23 "../test/../ssl/ssl_local.h"
...
# 13 "../test/cipher_overhead_test.c" 2
...
static int cipher_enabled(const SSL_CIPHER *ciph)
{
    ...
    if ((ciph->algorithm_mac & 0x00000040U) != 0)
        return 1;

    if (ciph->algorithm_enc != 0x00000020U
            && EVP_get_cipherbyname(OBJ_nid2sn(SSL_CIPHER_get_cipher_nid(ciph))) == 
# 24 "../test/cipher_overhead_test.c" 3 4
                                                                      ((void *)0)
# 24 "../test/cipher_overhead_test.c"
                                                                          )
        return 0;

    if (EVP_get_digestbyname(OBJ_nid2sn(SSL_CIPHER_get_digest_nid(ciph))) == 
# 27 "../test/cipher_overhead_test.c" 3 4
                                                               ((void *)0)
# 27 "../test/cipher_overhead_test.c"
                                                                   )
        return 0;
...
static int cipher_overhead(void)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (!cipher_enabled(ciph)) {
            test_skip("../test/cipher_overhead_test.c", 44, "Skipping disabled cipher %s", ciph->name);
            continue;
        }
        if (!test_true("../test/cipher_overhead_test.c", 47, "ssl_cipher_get_overhead(ciph, &mac, &in, &blk, &ex)", (ssl_cipher_get_overhead(ciph, &mac, &in, &blk, &ex)) != 0)) {
            test_info("../test/cipher_overhead_test.c", 48, "Failed getting %s", ciph->name);
            ret = 0;
        } else {
            test_info("../test/cipher_overhead_test.c", 51, "Cipher %s: %zu %zu %zu %zu", ciph->name, mac, in, blk, ex)
                                                   ;
        }
    }
    return ret;
...
int setup_tests(void)
{
    add_test("cipher_overhead", cipher_overhead);
    return 1;
}
