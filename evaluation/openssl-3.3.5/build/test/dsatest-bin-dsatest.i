...
# 70 "../test/dsatest.c"
    ...
    if (!test_ptr("../test/dsatest.c", 82, "cb = BN_GENCB_new()", cb = BN_GENCB_new()))
        goto end;

    BN_GENCB_set(cb, dsa_cb, 
# 85 "../test/dsatest.c" 3 4
                            ((void *)0)
# 85 "../test/dsatest.c"
                                );
    if (!test_ptr("../test/dsatest.c", 86, "dsa = DSA_new()", dsa = DSA_new())
        || !test_true("../test/dsatest.c", 87, "DSA_generate_parameters_ex(dsa, 512, seed, 20, &counter, &h, cb)", (DSA_generate_parameters_ex(dsa, 512, seed, 20, &counter, &h, cb)) != 0)
                                                                  )
        goto end;

    if (!test_int_eq("../test/dsatest.c", 91, "counter", "105", counter, 105))
        goto end;
    if (!test_int_eq("../test/dsatest.c", 93, "h", "2", h, 2))
        goto end;

    DSA_get0_pqg(dsa, &p, &q, &g);
    i = BN_bn2bin(q, buf);
    j = sizeof(out_q);
    if (!test_int_eq("../test/dsatest.c", 99, "i", "j", i, j) || !test_mem_eq("../test/dsatest.c", 99, "buf", "out_q", buf, i, out_q, i))
        goto end;

    i = BN_bn2bin(p, buf);
    j = sizeof(out_p);
    if (!test_int_eq("../test/dsatest.c", 104, "i", "j", i, j) || !test_mem_eq("../test/dsatest.c", 104, "buf", "out_p", buf, i, out_p, i))
        goto end;

    i = BN_bn2bin(g, buf);
    j = sizeof(out_g);
    if (!test_int_eq("../test/dsatest.c", 109, "i", "j", i, j) || !test_mem_eq("../test/dsatest.c", 109, "buf", "out_g", buf, i, out_g, i))
        goto end;

    if (!test_true("../test/dsatest.c", 112, "DSA_generate_key(dsa)", (DSA_generate_key(dsa)) != 0))
        goto end;
    if (!test_true("../test/dsatest.c", 114, "DSA_sign(0, str1, 20, sig, &siglen, dsa)", (DSA_sign(0, str1, 20, sig, &siglen, dsa)) != 0))
        goto end;
    if (test_int_gt("../test/dsatest.c", 116, "DSA_verify(0, str1, 20, sig, siglen, dsa)", "0", DSA_verify(0, str1, 20, sig, siglen, dsa), 0))
        ret = 1;
 end:
    DSA_free(dsa);
    BN_GENCB_free(cb);
...
static int dsa_cb(int p, int n, BN_GENCB *arg)
{
    ...
    if (!ok && (p == 0) && (num > 1)) {
        test_error("../test/dsatest.c", 134, "dsa_cb error");
        return 0;
    }
    return 1;
}
...
# 160 "../test/dsatest.c"
    ...
    if (!test_ptr("../test/dsatest.c", 244, "p_in = BN_bin2bn(expected_p, sizeof(expected_p), NULL)", p_in = BN_bin2bn(expected_p, sizeof(expected_p), 
# 244 "../test/dsatest.c" 3 4
        ((void *)0)
# 244 "../test/dsatest.c"
        ))
        || !test_ptr("../test/dsatest.c", 245, "q_in = BN_bin2bn(expected_q, sizeof(expected_q), NULL)", q_in = BN_bin2bn(expected_q, sizeof(expected_q), 
# 245 "../test/dsatest.c" 3 4
           ((void *)0)
# 245 "../test/dsatest.c"
           ))
        || !test_ptr("../test/dsatest.c", 246, "g_in = BN_bin2bn(expected_g, sizeof(expected_g), NULL)", g_in = BN_bin2bn(expected_g, sizeof(expected_g), 
# 246 "../test/dsatest.c" 3 4
           ((void *)0)
# 246 "../test/dsatest.c"
           )))
        goto end;
    if (!test_ptr("../test/dsatest.c", 248, "pg_ctx = EVP_PKEY_CTX_new_from_name(NULL, \"DSA\", NULL)", pg_ctx = EVP_PKEY_CTX_new_from_name(
# 248 "../test/dsatest.c" 3 4
        ((void *)0)
# 248 "../test/dsatest.c"
        , "DSA", 
# 248 "../test/dsatest.c" 3 4
...
# 248 "../test/dsatest.c"
        ...
        || !test_int_gt("../test/dsatest.c", 249, "EVP_PKEY_paramgen_init(pg_ctx)", "0", EVP_PKEY_paramgen_init(pg_ctx), 0)
        || !test_ptr_null("../test/dsatest.c", 250, "EVP_PKEY_CTX_gettable_params(pg_ctx)", EVP_PKEY_CTX_gettable_params(pg_ctx))
        || !test_ptr("../test/dsatest.c", 251, "settables = EVP_PKEY_CTX_settable_params(pg_ctx)", settables = EVP_PKEY_CTX_settable_params(pg_ctx))
        || !test_ptr("../test/dsatest.c", 252, "OSSL_PARAM_locate_const(settables, OSSL_PKEY_PARAM_FFC_PBITS)", OSSL_PARAM_locate_const(settables, "pbits"))

        || !test_true("../test/dsatest.c", 254, "EVP_PKEY_CTX_set_dsa_paramgen_type(pg_ctx, \"fips186_4\")", (EVP_PKEY_CTX_set_dsa_paramgen_type(pg_ctx, "fips186_4")) != 0)
        || !test_true("../test/dsatest.c", 255, "EVP_PKEY_CTX_set_dsa_paramgen_bits(pg_ctx, 2048)", (EVP_PKEY_CTX_set_dsa_paramgen_bits(pg_ctx, 2048)) != 0)
        || !test_true("../test/dsatest.c", 256, "EVP_PKEY_CTX_set_dsa_paramgen_q_bits(pg_ctx, 224)", (EVP_PKEY_CTX_set_dsa_paramgen_q_bits(pg_ctx, 224)) != 0)
        || !test_true("../test/dsatest.c", 257, "EVP_PKEY_CTX_set_dsa_paramgen_seed(pg_ctx, seed_data, sizeof(seed_data))", (EVP_PKEY_CTX_set_dsa_paramgen_seed(pg_ctx, seed_data, sizeof(seed_data))) != 0)

        || !test_true("../test/dsatest.c", 259, "EVP_PKEY_CTX_set_dsa_paramgen_md_props(pg_ctx, \"SHA256\", \"\")", (EVP_PKEY_CTX_set_dsa_paramgen_md_props(pg_ctx, "SHA256", "")) != 0)

        || !test_int_gt("../test/dsatest.c", 261, "EVP_PKEY_generate(pg_ctx, &param_key)", "0", EVP_PKEY_generate(pg_ctx, &param_key), 0)
        || !test_ptr("../test/dsatest.c", 262, "kg_ctx = EVP_PKEY_CTX_new_from_pkey(NULL, param_key, NULL)", kg_ctx = EVP_PKEY_CTX_new_from_pkey(
# 262 "../test/dsatest.c" 3 4
           ((void *)0)
# 262 "../test/dsatest.c"
           , param_key, 
# 262 "../test/dsatest.c" 3 4
...
# 262 "../test/dsatest.c"
        ...
        || !test_int_gt("../test/dsatest.c", 263, "EVP_PKEY_keygen_init(kg_ctx)", "0", EVP_PKEY_keygen_init(kg_ctx), 0)
        || !test_int_gt("../test/dsatest.c", 264, "EVP_PKEY_generate(kg_ctx, &key)", "0", EVP_PKEY_generate(kg_ctx, &key), 0))
        goto end;

    if (!test_true("../test/dsatest.c", 267, "EVP_PKEY_get_bn_param(key, OSSL_PKEY_PARAM_FFC_P, &p_out)", (EVP_PKEY_get_bn_param(key, "p", &p_out)) != 0)
        || !test_BN_eq("../test/dsatest.c", 268, "p_in", "p_out", p_in, p_out)
        || !test_true("../test/dsatest.c", 269, "EVP_PKEY_get_bn_param(key, OSSL_PKEY_PARAM_FFC_Q, &q_out)", (EVP_PKEY_get_bn_param(key, "q", &q_out)) != 0)
        || !test_BN_eq("../test/dsatest.c", 270, "q_in", "q_out", q_in, q_out)
        || !test_true("../test/dsatest.c", 271, "EVP_PKEY_get_bn_param(key, OSSL_PKEY_PARAM_FFC_G, &g_out)", (EVP_PKEY_get_bn_param(key, "g", &g_out)) != 0)
        || !test_BN_eq("../test/dsatest.c", 272, "g_in", "g_out", g_in, g_out)
        || !test_true("../test/dsatest.c", 273, "EVP_PKEY_get_octet_string_param( key, OSSL_PKEY_PARAM_FFC_SEED, seed_out, sizeof(seed_out), &len)", (EVP_PKEY_get_octet_string_param( key, "seed", seed_out, sizeof(seed_out), &len)) != 0)


        || !test_mem_eq("../test/dsatest.c", 276, "seed_out", "seed_data", seed_out, len, seed_data, sizeof(seed_data))
        || !test_true("../test/dsatest.c", 277, "EVP_PKEY_get_int_param(key, OSSL_PKEY_PARAM_FFC_GINDEX, &gindex_out)", (EVP_PKEY_get_int_param(key, "gindex", &gindex_out)) != 0)

        || !test_int_eq("../test/dsatest.c", 279, "gindex_out", "-1", gindex_out, -1)
        || !test_true("../test/dsatest.c", 280, "EVP_PKEY_get_int_param(key, OSSL_PKEY_PARAM_FFC_H, &hcount_out)", (EVP_PKEY_get_int_param(key, "hindex", &hcount_out)) != 0)

        || !test_int_eq("../test/dsatest.c", 282, "hcount_out", "expected_h", hcount_out, expected_h)
        || !test_true("../test/dsatest.c", 283, "EVP_PKEY_get_int_param(key, OSSL_PKEY_PARAM_FFC_PCOUNTER, &pcount_out)", (EVP_PKEY_get_int_param(key, "pcounter", &pcount_out)) != 0)


        || !test_int_eq("../test/dsatest.c", 286, "pcount_out", "expected_c", pcount_out, expected_c)
        || !test_false("../test/dsatest.c", 287, "EVP_PKEY_get_utf8_string_param(key, OSSL_PKEY_PARAM_GROUP_NAME, group_out, sizeof(group_out), &len)", (EVP_PKEY_get_utf8_string_param(key, "group", group_out, sizeof(group_out), &len)) != 0)
...
# 310 "../test/dsatest.c" 3 4
                           ((void *)0)
# 310 "../test/dsatest.c"
                               ;
    EVP_PKEY_CTX *check_ctx = 
# 311 "../test/dsatest.c" 3 4
                             ((void *)0)
# 311 "../test/dsatest.c"
                                 ;
    EVP_PKEY *params = 
# 312 "../test/dsatest.c" 3 4
                      ((void *)0)
# 312 "../test/dsatest.c"
                          ;

    ret = test_ptr("../test/dsatest.c", 314, "gen_ctx = EVP_PKEY_CTX_new_from_name(NULL, \"DSA\", NULL)", gen_ctx = EVP_PKEY_CTX_new_from_name(
# 314 "../test/dsatest.c" 3 4
         ((void *)0)
# 314 "../test/dsatest.c"
         , "DSA", 
# 314 "../test/dsatest.c" 3 4
...
# 314 "../test/dsatest.c"
          ...
          && test_int_gt("../test/dsatest.c", 315, "EVP_PKEY_paramgen_init(gen_ctx)", "0", EVP_PKEY_paramgen_init(gen_ctx), 0)
          && (i == 0
              || test_true("../test/dsatest.c", 317, "EVP_PKEY_CTX_set_dsa_paramgen_bits(gen_ctx, 512)", (EVP_PKEY_CTX_set_dsa_paramgen_bits(gen_ctx, 512)) != 0))
          && test_int_gt("../test/dsatest.c", 318, "EVP_PKEY_generate(gen_ctx, &params)", "0", EVP_PKEY_generate(gen_ctx, &params), 0)
          && test_ptr("../test/dsatest.c", 319, "check_ctx = EVP_PKEY_CTX_new_from_pkey(NULL, params, NULL)", check_ctx = EVP_PKEY_CTX_new_from_pkey(
# 319 "../test/dsatest.c" 3 4
            ((void *)0)
# 319 "../test/dsatest.c"
            , params, 
# 319 "../test/dsatest.c" 3 4
...
# 319 "../test/dsatest.c"
          ...
          && test_int_gt("../test/dsatest.c", 320, "EVP_PKEY_param_check(check_ctx)", "0", EVP_PKEY_param_check(check_ctx), 0);
...
static int test_dsa_sig_infinite_loop(void)
{
    int ret = 0;
    DSA *dsa = 
# 331 "../test/dsatest.c" 3 4
              ((void *)0)
# 331 "../test/dsatest.c"
                  ;
    BIGNUM *p = 
# 332 "../test/dsatest.c" 3 4
               ((void *)0)
# 332 "../test/dsatest.c"
                   , *q = 
# 332 "../test/dsatest.c" 3 4
...
# 333 "../test/dsatest.c"
    ...
    if (!test_ptr("../test/dsatest.c", 354, "p = BN_bin2bn(out_p, sizeof(out_p), NULL)", p = BN_bin2bn(out_p, sizeof(out_p), 
# 354 "../test/dsatest.c" 3 4
        ((void *)0)
# 354 "../test/dsatest.c"
        ))
        || !test_ptr("../test/dsatest.c", 355, "q = BN_bin2bn(out_q, sizeof(out_q), NULL)", q = BN_bin2bn(out_q, sizeof(out_q), 
# 355 "../test/dsatest.c" 3 4
           ((void *)0)
# 355 "../test/dsatest.c"
           ))
        || !test_ptr("../test/dsatest.c", 356, "g = BN_bin2bn(out_g, sizeof(out_g), NULL)", g = BN_bin2bn(out_g, sizeof(out_g), 
# 356 "../test/dsatest.c" 3 4
           ((void *)0)
# 356 "../test/dsatest.c"
           ))
        || !test_ptr("../test/dsatest.c", 357, "pub = BN_bin2bn(out_pub, sizeof(out_pub), NULL)", pub = BN_bin2bn(out_pub, sizeof(out_pub), 
# 357 "../test/dsatest.c" 3 4
           ((void *)0)
# 357 "../test/dsatest.c"
           ))
        || !test_ptr("../test/dsatest.c", 358, "priv = BN_bin2bn(out_priv, sizeof(out_priv), NULL)", priv = BN_bin2bn(out_priv, sizeof(out_priv), 
# 358 "../test/dsatest.c" 3 4
           ((void *)0)
# 358 "../test/dsatest.c"
           ))
        || !test_ptr("../test/dsatest.c", 359, "priv2 = BN_dup(priv)", priv2 = BN_dup(priv))
        || !test_ptr("../test/dsatest.c", 360, "badq = BN_new()", badq = BN_new())
        || !test_true("../test/dsatest.c", 361, "BN_set_word(badq, 1)", (BN_set_word(badq, 1)) != 0)
        || !test_ptr("../test/dsatest.c", 362, "badpriv = BN_new()", badpriv = BN_new())
        || !test_true("../test/dsatest.c", 363, "BN_set_word(badpriv, 0)", (BN_set_word(badpriv, 0)) != 0)
        || !test_ptr("../test/dsatest.c", 364, "dsa = DSA_new()", dsa = DSA_new()))
        goto err;

    if (!test_true("../test/dsatest.c", 367, "DSA_set0_pqg(dsa, p, q, g)", (DSA_set0_pqg(dsa, p, q, g)) != 0))
        goto err;
    p = q = g = 
# 369 "../test/dsatest.c" 3 4
               ((void *)0)
# 369 "../test/dsatest.c"
                   ;

    if (!test_true("../test/dsatest.c", 371, "DSA_set0_key(dsa, pub, priv)", (DSA_set0_key(dsa, pub, priv)) != 0))
        goto err;
    pub = priv = 
# 373 "../test/dsatest.c" 3 4
                ((void *)0)
# 373 "../test/dsatest.c"
                    ;

    if (!test_int_le("../test/dsatest.c", 375, "DSA_size(dsa)", "sizeof(signature)", DSA_size(dsa), sizeof(signature)))
        goto err;


    if (!test_true("../test/dsatest.c", 379, "DSA_sign(0, msg, sizeof(msg), NULL, &signature_len0, dsa)", (DSA_sign(0, msg, sizeof(msg), 
# 379 "../test/dsatest.c" 3 4
        ((void *)0)
# 379 "../test/dsatest.c"
        , &signature_len0, dsa)) != 0)
        || !test_int_gt("../test/dsatest.c", 380, "signature_len0", "0", signature_len0, 0))
        goto err;

    if (!test_true("../test/dsatest.c", 383, "DSA_sign(0, msg, sizeof(msg), signature, &signature_len, dsa)", (DSA_sign(0, msg, sizeof(msg), signature, &signature_len, dsa)) != 0)
        || !test_int_gt("../test/dsatest.c", 384, "signature_len", "0", signature_len, 0)
        || !test_int_le("../test/dsatest.c", 385, "signature_len", "signature_len0", signature_len, signature_len0))
        goto err;


    if (!test_true("../test/dsatest.c", 389, "DSA_set0_key(dsa, NULL, badpriv)", (DSA_set0_key(dsa, 
# 389 "../test/dsatest.c" 3 4
        ((void *)0)
# 389 "../test/dsatest.c"
        , badpriv)) != 0))
        goto err;
    badpriv = 
# 391 "../test/dsatest.c" 3 4
             ((void *)0)
# 391 "../test/dsatest.c"
                 ;
    if (!test_false("../test/dsatest.c", 392, "DSA_sign(0, msg, sizeof(msg), signature, &signature_len, dsa)", (DSA_sign(0, msg, sizeof(msg), signature, &signature_len, dsa)) != 0))
        goto err;


    if (!test_true("../test/dsatest.c", 396, "DSA_set0_key(dsa, NULL, priv2)", (DSA_set0_key(dsa, 
# 396 "../test/dsatest.c" 3 4
        ((void *)0)
# 396 "../test/dsatest.c"
        , priv2)) != 0))
        goto err;
    priv2 = 
# 398 "../test/dsatest.c" 3 4
           ((void *)0)
# 398 "../test/dsatest.c"
               ;
    if (!test_true("../test/dsatest.c", 399, "DSA_set0_pqg(dsa, NULL, badq, NULL)", (DSA_set0_pqg(dsa, 
# 399 "../test/dsatest.c" 3 4
        ((void *)0)
# 399 "../test/dsatest.c"
        , badq, 
# 399 "../test/dsatest.c" 3 4
...
# 399 "../test/dsatest.c"
        ...
        goto err;
    badq = 
# 401 "../test/dsatest.c" 3 4
          ((void *)0)
# 401 "../test/dsatest.c"
              ;
    if (!test_false("../test/dsatest.c", 402, "DSA_sign(0, msg, sizeof(msg), signature, &signature_len, dsa)", (DSA_sign(0, msg, sizeof(msg), signature, &signature_len, dsa)) != 0))
        goto err;
...
static int test_dsa_sig_neg_param(void)
{
    int ret = 0, setpqg = 0;
    DSA *dsa = 
# 422 "../test/dsatest.c" 3 4
              ((void *)0)
# 422 "../test/dsatest.c"
                  ;
    BIGNUM *p = 
# 423 "../test/dsatest.c" 3 4
               ((void *)0)
# 423 "../test/dsatest.c"
                   , *q = 
# 423 "../test/dsatest.c" 3 4
...
# 423 "../test/dsatest.c"
    ...
    if (!test_ptr("../test/dsatest.c", 443, "p = BN_bin2bn(out_p, sizeof(out_p), NULL)", p = BN_bin2bn(out_p, sizeof(out_p), 
# 443 "../test/dsatest.c" 3 4
        ((void *)0)
# 443 "../test/dsatest.c"
        ))
        || !test_ptr("../test/dsatest.c", 444, "q = BN_bin2bn(out_q, sizeof(out_q), NULL)", q = BN_bin2bn(out_q, sizeof(out_q), 
# 444 "../test/dsatest.c" 3 4
           ((void *)0)
# 444 "../test/dsatest.c"
           ))
        || !test_ptr("../test/dsatest.c", 445, "g = BN_bin2bn(out_g, sizeof(out_g), NULL)", g = BN_bin2bn(out_g, sizeof(out_g), 
# 445 "../test/dsatest.c" 3 4
           ((void *)0)
# 445 "../test/dsatest.c"
           ))
        || !test_ptr("../test/dsatest.c", 446, "pub = BN_bin2bn(out_pub, sizeof(out_pub), NULL)", pub = BN_bin2bn(out_pub, sizeof(out_pub), 
# 446 "../test/dsatest.c" 3 4
           ((void *)0)
# 446 "../test/dsatest.c"
           ))
        || !test_ptr("../test/dsatest.c", 447, "priv = BN_bin2bn(out_priv, sizeof(out_priv), NULL)", priv = BN_bin2bn(out_priv, sizeof(out_priv), 
# 447 "../test/dsatest.c" 3 4
           ((void *)0)
# 447 "../test/dsatest.c"
           ))
        || !test_ptr("../test/dsatest.c", 448, "dsa = DSA_new()", dsa = DSA_new()))
        goto err;

    if (!test_true("../test/dsatest.c", 451, "DSA_set0_pqg(dsa, p, q, g)", (DSA_set0_pqg(dsa, p, q, g)) != 0))
        goto err;
    setpqg = 1;

    if (!test_true("../test/dsatest.c", 455, "DSA_set0_key(dsa, pub, priv)", (DSA_set0_key(dsa, pub, priv)) != 0))
        goto err;
    pub = priv = 
# 457 "../test/dsatest.c" 3 4
                ((void *)0)
# 457 "../test/dsatest.c"
                    ;

    BN_set_negative(p, 1);
    if (!test_false("../test/dsatest.c", 460, "DSA_sign(0, msg, sizeof(msg), signature, &signature_len, dsa)", (DSA_sign(0, msg, sizeof(msg), signature, &signature_len, dsa)) != 0))
        goto err;

    BN_set_negative(p, 0);
    BN_set_negative(q, 1);
    if (!test_false("../test/dsatest.c", 465, "DSA_sign(0, msg, sizeof(msg), signature, &signature_len, dsa)", (DSA_sign(0, msg, sizeof(msg), signature, &signature_len, dsa)) != 0))
        goto err;

    BN_set_negative(q, 0);
    BN_set_negative(g, 1);
    if (!test_false("../test/dsatest.c", 470, "DSA_sign(0, msg, sizeof(msg), signature, &signature_len, dsa)", (DSA_sign(0, msg, sizeof(msg), signature, &signature_len, dsa)) != 0))
        goto err;

    BN_set_negative(p, 1);
    BN_set_negative(q, 1);
    BN_set_negative(g, 1);
    if (!test_false("../test/dsatest.c", 476, "DSA_sign(0, msg, sizeof(msg), signature, &signature_len, dsa)", (DSA_sign(0, msg, sizeof(msg), signature, &signature_len, dsa)) != 0))
        goto err;
...
int setup_tests(void)
{
    ...
    add_all_tests("test_dsa_default_paramgen_validate", test_dsa_default_paramgen_validate, 2, 1);
