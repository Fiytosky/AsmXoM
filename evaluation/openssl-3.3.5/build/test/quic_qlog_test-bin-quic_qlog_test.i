# 0 "../test/quic_qlog_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/quic_qlog_test.c"
# 10 "../test/quic_qlog_test.c"
# 1 "../include/internal/qlog.h" 1
# 13 "../include/internal/qlog.h"
...
static OSSL_TIME now(void *arg)
{
    ...
    last_time = ossl_time_add(t, ossl_ticks2time((1000) * (((uint64_t)1000000000) / 1000)));
    return t;
}
...
static int test_qlog(void)
{
    ...
    BIO *bio;
    char *buf = 
# 83 "../test/quic_qlog_test.c" 3 4
               ((void *)0)
# 83 "../test/quic_qlog_test.c"
                   ;
    size_t buf_len = 0;
    ...
    if (!test_ptr("../test/quic_qlog_test.c", 97, "qlog = ossl_qlog_new(&qti)", qlog = ossl_qlog_new(&qti)))
        goto err;

    if (!test_true("../test/quic_qlog_test.c", 100, "ossl_qlog_set_event_type_enabled(qlog, QLOG_EVENT_TYPE_transport_packet_sent, 1)", (ossl_qlog_set_event_type_enabled(qlog, QLOG_EVENT_TYPE_transport_packet_sent, 1)) != 0))
        goto err;

    if (!test_ptr("../test/quic_qlog_test.c", 103, "bio = BIO_new(BIO_s_mem())", bio = BIO_new(BIO_s_mem())))
        goto err;

    if (!test_true("../test/quic_qlog_test.c", 106, "ossl_qlog_set_sink_bio(qlog, bio)", (ossl_qlog_set_sink_bio(qlog, bio)) != 0))
        goto err;
    ...
    { QLOG *qlog_instance = (qlog); uint32_t qlog_event_type = QLOG_EVENT_TYPE_transport_packet_sent; if (ossl_qlog_event_try_begin(qlog_instance, qlog_event_type, "transport", "packet_sent", "transport" ":" "packet_sent")) {
        ossl_qlog_str(qlog_instance, ("field1"), ("foo"));
        ossl_qlog_str_len(qlog_instance, ("field2"), ("bar"), (3));
        ossl_qlog_i64(qlog_instance, ("field3"), (42));
        ossl_qlog_i64(qlog_instance, ("field4"), (1ULL << 60));
        ossl_qlog_u64(qlog_instance, ("field5"), (
# 114 "../test/quic_qlog_test.c" 3 4
       (18446744073709551615UL)
# 114 "../test/quic_qlog_test.c"
       ));
        ossl_qlog_bool(qlog_instance, ("field6"), (0));
        ossl_qlog_bool(qlog_instance, ("field7"), (1));
        ossl_qlog_bin(qlog_instance, ("field8"), (bin_buf), (sizeof(bin_buf)));
        ossl_qlog_bin(qlog_instance, (("field9")), ((&qti.odcid)->id), ((&qti.odcid)->id_len));
        { ossl_qlog_group_begin(qlog_instance, ("subgroup"));
            ossl_qlog_str(qlog_instance, ("field10"), ("baz"));
        ossl_qlog_group_end(qlog_instance); }
        { ossl_qlog_array_begin(qlog_instance, ("array"));
            ossl_qlog_str(qlog_instance, (
# 123 "../test/quic_qlog_test.c" 3 4
           ((void *)0)
# 123 "../test/quic_qlog_test.c"
           ), ("a"));
            ossl_qlog_str(qlog_instance, (
# 124 "../test/quic_qlog_test.c" 3 4
           ((void *)0)
# 124 "../test/quic_qlog_test.c"
           ), ("b"));
    ...
    { QLOG *qlog_instance = (qlog); uint32_t qlog_event_type = QLOG_EVENT_TYPE_transport_packet_sent; if (ossl_qlog_event_try_begin(qlog_instance, qlog_event_type, "transport", "packet_sent", "transport" ":" "packet_sent")) {
        ossl_qlog_str(qlog_instance, ("field1"), ("bar"));
    ossl_qlog_event_end(qlog_instance); } }

    if (!test_true("../test/quic_qlog_test.c", 138, "ossl_qlog_flush(qlog)", (ossl_qlog_flush(qlog)) != 0))
        goto err;

    buf_len = BIO_ctrl(bio,3,0,(char *)(&buf));
    if (!test_size_t_gt("../test/quic_qlog_test.c", 142, "buf_len", "0", buf_len, 0))
        goto err;

    if (!test_mem_eq("../test/quic_qlog_test.c", 145, "buf", "expected", buf, buf_len, expected, sizeof(expected)))
        goto err;
...
static int test_qlog_filter(int idx)
{
    ...
    if (!test_ptr("../test/quic_qlog_test.c", 225, "qlog = ossl_qlog_new(&qti)", qlog = ossl_qlog_new(&qti)))
        goto err;

    if (!test_int_eq("../test/quic_qlog_test.c", 228, "ossl_qlog_set_filter(qlog, filters[idx].filter)", "filters[idx].expect_ok", ossl_qlog_set_filter(qlog, filters[idx].filter), filters[idx].expect_ok)
                                            )
        goto err;

    if (filters[idx].expect_event_type != QLOG_EVENT_TYPE_NONE)
        if (!test_int_eq("../test/quic_qlog_test.c", 233, "ossl_qlog_enabled(qlog, filters[idx].expect_event_type)", "filters[idx].expect_event_enable", ossl_qlog_enabled(qlog, filters[idx].expect_event_type), filters[idx].expect_event_enable)
                                                          )
            goto err;
...
int setup_tests(void)
{
    ...
    add_all_tests("test_qlog_filter", test_qlog_filter, (sizeof(filters)/sizeof((filters)[0])), 1);
    return 1;
}
