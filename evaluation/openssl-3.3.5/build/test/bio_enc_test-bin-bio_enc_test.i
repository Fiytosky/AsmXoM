# 0 "../test/bio_enc_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/bio_enc_test.c"
# 9 "../test/bio_enc_test.c"
...
# 16 "../test/bio_enc_test.c" 2
# 24 "../test/bio_enc_test.c"
...
{
    BIO *b, *mem;
    static unsigned char inp[(1024 + 32)] = { 0 };
    unsigned char out[(1024 + 32)], ref[(1024 + 32)];
    int i, lref, len;


    if (!test_int_gt("../test/bio_enc_test.c", 47, "RAND_bytes(inp, DATA_SIZE)", "0", RAND_bytes(inp, 1024), 0))
        return 0;
    ...
    if (!test_ptr("../test/bio_enc_test.c", 54, "b", b))
        return 0;
    if (!test_true("../test/bio_enc_test.c", 56, "BIO_set_cipher(b, cipher, key, iv, ENCRYPT)", (BIO_set_cipher(b, cipher, key, iv, 1)) != 0))
        goto err;
    mem = BIO_new_mem_buf(inp, 1024);
    if (!test_ptr("../test/bio_enc_test.c", 59, "mem", mem))
        goto err;
    BIO_push(b, mem);
    ...
    for (i = 1; i < lref; i++) {
        b = BIO_new(BIO_f_cipher());
        if (!test_ptr("../test/bio_enc_test.c", 68, "b", b))
            return 0;
        if (!test_true("../test/bio_enc_test.c", 70, "BIO_set_cipher(b, cipher, key, iv, ENCRYPT)", (BIO_set_cipher(b, cipher, key, iv, 1)) != 0)) {
            test_info("../test/bio_enc_test.c", 71, "Split encrypt failed @ operation %d", i);
            goto err;
        }
        mem = BIO_new_mem_buf(inp, 1024);
        if (!test_ptr("../test/bio_enc_test.c", 75, "mem", mem))
            goto err;
        BIO_push(b, mem);
        ...
        if (!test_uchar_eq("../test/bio_enc_test.c", 82, "out[i]", "(unsigned char)~ref[i]", out[i], (unsigned char)~ref[i])) {
            test_info("../test/bio_enc_test.c", 83, "Encrypt overstep check failed @ operation %d", i);
            goto err;
        }
        len += BIO_read(b, out + len, sizeof(out) - len);
        BIO_free_all(b);

        if (!test_mem_eq("../test/bio_enc_test.c", 89, "out", "ref", out, len, ref, lref)) {
            test_info("../test/bio_enc_test.c", 90, "Encrypt compare failed @ operation %d", i);
            return 0;
        }
    }
    ...
    for (i = 1; i < lref / 2; i++) {
        ...
        b = BIO_new(BIO_f_cipher());
        if (!test_ptr("../test/bio_enc_test.c", 100, "b", b))
            return 0;
        if (!test_true("../test/bio_enc_test.c", 102, "BIO_set_cipher(b, cipher, key, iv, ENCRYPT)", (BIO_set_cipher(b, cipher, key, iv, 1)) != 0)) {
            test_info("../test/bio_enc_test.c", 103, "Small chunk encrypt failed @ operation %d", i);
            goto err;
        }
        mem = BIO_new_mem_buf(inp, 1024);
        if (!test_ptr("../test/bio_enc_test.c", 107, "mem", mem))
            goto err;
        BIO_push(b, mem);
        memset(out, 0, sizeof(out));
        ...
        if (!test_mem_eq("../test/bio_enc_test.c", 116, "out", "ref", out, len, ref, lref)) {
            test_info("../test/bio_enc_test.c", 117, "Small chunk encrypt compare failed @ operation %d", i);
            return 0;
        }
    }
    ...
    b = BIO_new(BIO_f_cipher());
    if (!test_ptr("../test/bio_enc_test.c", 126, "b", b))
        return 0;
    if (!test_true("../test/bio_enc_test.c", 128, "BIO_set_cipher(b, cipher, key, iv, DECRYPT)", (BIO_set_cipher(b, cipher, key, iv, 0)) != 0))
        goto err;

    mem = BIO_new_mem_buf(ref, lref);
    if (!test_ptr("../test/bio_enc_test.c", 132, "mem", mem))
        goto err;
    BIO_push(b, mem);
# 146 "../test/bio_enc_test.c"
    memset(out, 0, sizeof(out));
    len = BIO_read(b, out, sizeof(out));
    BIO_free_all(b);

    if (!test_mem_eq("../test/bio_enc_test.c", 150, "inp", "out", inp, 1024, out, len))
        return 0;


    for (i = 1; i < lref; i++) {
        b = BIO_new(BIO_f_cipher());
        if (!test_ptr("../test/bio_enc_test.c", 156, "b", b))
            return 0;
        if (!test_true("../test/bio_enc_test.c", 158, "BIO_set_cipher(b, cipher, key, iv, DECRYPT)", (BIO_set_cipher(b, cipher, key, iv, 0)) != 0)) {
            test_info("../test/bio_enc_test.c", 159, "Split decrypt failed @ operation %d", i);
            goto err;
        }
        mem = BIO_new_mem_buf(ref, lref);
        if (!test_ptr("../test/bio_enc_test.c", 163, "mem", mem))
            goto err;
        BIO_push(b, mem);
        memset(out, 0, sizeof(out));
        out[i] = ~ref[i];
        ...
        if (!test_uchar_eq("../test/bio_enc_test.c", 170, "out[i]", "(unsigned char)~ref[i]", out[i], (unsigned char)~ref[i])) {
            test_info("../test/bio_enc_test.c", 171, "Decrypt overstep check failed @ operation %d", i);
            goto err;
        }
        len += BIO_read(b, out + len, sizeof(out) - len);
        BIO_free_all(b);

        if (!test_mem_eq("../test/bio_enc_test.c", 177, "inp", "out", inp, 1024, out, len)) {
            test_info("../test/bio_enc_test.c", 178, "Decrypt compare failed @ operation %d", i);
            return 0;
        }
    }
    ...
    for (i = 1; i < lref / 2; i++) {
        ...
        b = BIO_new(BIO_f_cipher());
        if (!test_ptr("../test/bio_enc_test.c", 188, "b", b))
            return 0;
        if (!test_true("../test/bio_enc_test.c", 190, "BIO_set_cipher(b, cipher, key, iv, DECRYPT)", (BIO_set_cipher(b, cipher, key, iv, 0)) != 0)) {
            test_info("../test/bio_enc_test.c", 191, "Small chunk decrypt failed @ operation %d", i);
            goto err;
        }
        mem = BIO_new_mem_buf(ref, lref);
        if (!test_ptr("../test/bio_enc_test.c", 195, "mem", mem))
            goto err;
        BIO_push(b, mem);
        memset(out, 0, sizeof(out));
        for (len = 0; (delta = BIO_read(b, out + len, i)); ) {
        ...
        if (!test_mem_eq("../test/bio_enc_test.c", 204, "inp", "out", inp, 1024, out, len)) {
            test_info("../test/bio_enc_test.c", 205, "Small chunk decrypt compare failed @ operation %d", i);
            return 0;
        }
    }
...
static int do_test_bio_cipher(const EVP_CIPHER* cipher, int idx)
{
    ...
    {
        case 0:
            return do_bio_cipher(cipher, KEY, 
# 222 "../test/bio_enc_test.c" 3 4
                                             ((void *)0)
# 222 "../test/bio_enc_test.c"
                                                 );
        case 1:
...
{
    return do_test_bio_cipher(EVP_aes_256_cfb128(), idx);
}
...
static int test_bio_enc_eof_read_flush(void)
{
    ...
    EVP_CIPHER_CTX *ctx = 
# 269 "../test/bio_enc_test.c" 3 4
                         ((void *)0)
# 269 "../test/bio_enc_test.c"
                             ;
    BIO *mem = 
# 270 "../test/bio_enc_test.c" 3 4
              ((void *)0)
# 270 "../test/bio_enc_test.c"
                  , *b64 = 
# 270 "../test/bio_enc_test.c" 3 4
                           ((void *)0)
# 270 "../test/bio_enc_test.c"
...
# 270 "../test/bio_enc_test.c"
    ...
    if (!test_ptr("../test/bio_enc_test.c", 276, "cipher", cipher)
        || !test_int_gt("../test/bio_enc_test.c", 277, "(key_size = EVP_CIPHER_key_length(cipher))", "0", (key_size = EVP_CIPHER_get_key_length(cipher)), 0)
        || !test_int_gt("../test/bio_enc_test.c", 278, "(iv_size = EVP_CIPHER_iv_length(cipher))", "0", (iv_size = EVP_CIPHER_get_iv_length(cipher)), 0)
        || !test_ptr("../test/bio_enc_test.c", 279, "mem = BIO_new(BIO_s_mem())", mem = BIO_new(BIO_s_mem()))
        || !test_ptr("../test/bio_enc_test.c", 280, "b64 = BIO_new(BIO_f_base64())", b64 = BIO_new(BIO_f_base64()))
        || !test_ptr("../test/bio_enc_test.c", 281, "cbio = BIO_new(BIO_f_cipher())", cbio = BIO_new(BIO_f_cipher()))
        || !test_ptr("../test/bio_enc_test.c", 282, "BIO_push(b64, mem)", BIO_push(b64, mem))
        || !test_ptr("../test/bio_enc_test.c", 283, "BIO_push(cbio, b64)", BIO_push(cbio, b64))
        || !test_int_gt("../test/bio_enc_test.c", 284, "BIO_get_cipher_ctx(cbio, &ctx)", "0", BIO_ctrl(cbio,129,0,(&ctx)), 0)
        || !test_true("../test/bio_enc_test.c", 285, "EVP_CipherInit_ex(ctx, cipher, NULL, KEY, IV, ENCRYPT)", (EVP_CipherInit_ex(ctx, cipher, 
# 285 "../test/bio_enc_test.c" 3 4
           ((void *)0)
# 285 "../test/bio_enc_test.c"
           , KEY, IV, 1)) != 0)
        || !test_int_gt("../test/bio_enc_test.c", 286, "BIO_write(cbio, pbuf, sizeof(pbuf) - 1)", "0", BIO_write(cbio, pbuf, sizeof(pbuf) - 1), 0)
        || !test_int_gt("../test/bio_enc_test.c", 287, "BIO_flush(cbio)", "0", (int)BIO_ctrl(cbio,11,0,
# 287 "../test/bio_enc_test.c" 3 4
           ((void *)0)
# 287 "../test/bio_enc_test.c"
           ), 0)
        || !test_int_gt("../test/bio_enc_test.c", 288, "EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, sizeof(tag), tag)", "0", EVP_CIPHER_CTX_ctrl(ctx, 0x10, sizeof(tag), tag), 0)
                                                                 )
        goto end;
    BIO_free(cbio);
    BIO_free(b64);
    b64 = cbio = 
# 293 "../test/bio_enc_test.c" 3 4
                ((void *)0)
# 293 "../test/bio_enc_test.c"
                    ;

    BIO_ctrl(mem,130,0,
# 295 "../test/bio_enc_test.c" 3 4
   ((void *)0)
# 295 "../test/bio_enc_test.c"
   );
    BIO_set_flags(mem, 0x400);
    if (!test_int_gt("../test/bio_enc_test.c", 297, "BIO_reset(mem)", "0", (int)BIO_ctrl(mem,1,0,
# 297 "../test/bio_enc_test.c" 3 4
        ((void *)0)
# 297 "../test/bio_enc_test.c"
        ), 0)
        || !test_ptr("../test/bio_enc_test.c", 298, "b64 = BIO_new(BIO_f_base64())", b64 = BIO_new(BIO_f_base64()))
        || !test_ptr("../test/bio_enc_test.c", 299, "cbio = BIO_new(BIO_f_cipher())", cbio = BIO_new(BIO_f_cipher()))
        || !test_ptr("../test/bio_enc_test.c", 300, "BIO_push(b64, mem)", BIO_push(b64, mem))
        || !test_ptr("../test/bio_enc_test.c", 301, "BIO_push(cbio, b64)", BIO_push(cbio, b64))
        || !test_int_gt("../test/bio_enc_test.c", 302, "BIO_get_cipher_ctx(cbio, &ctx)", "0", BIO_ctrl(cbio,129,0,(&ctx)), 0)
        || !test_true("../test/bio_enc_test.c", 303, "EVP_CipherInit_ex(ctx, cipher, NULL, KEY, IV, DECRYPT)", (EVP_CipherInit_ex(ctx, cipher, 
# 303 "../test/bio_enc_test.c" 3 4
           ((void *)0)
# 303 "../test/bio_enc_test.c"
           , KEY, IV, 0)) != 0)
        || !test_int_gt("../test/bio_enc_test.c", 304, "EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, sizeof(tag), tag)", "0", EVP_CIPHER_CTX_ctrl(ctx, 0x11, sizeof(tag), tag), 0)

        || !test_int_gt("../test/bio_enc_test.c", 306, "(n = BIO_read(cbio, cbuf, sizeof(cbuf)))", "0", (n = BIO_read(cbio, cbuf, sizeof(cbuf))), 0)
        || !test_true("../test/bio_enc_test.c", 307, "BIO_get_cipher_status(cbio)", (BIO_ctrl(cbio,113,0,
# 307 "../test/bio_enc_test.c" 3 4
           ((void *)0)
# 307 "../test/bio_enc_test.c"
           )) != 0)

        || (!test_int_gt("../test/bio_enc_test.c", 309, "BIO_flush(cbio)", "0", (int)BIO_ctrl(cbio,11,0,
# 309 "../test/bio_enc_test.c" 3 4
            ((void *)0)
# 309 "../test/bio_enc_test.c"
            ), 0) +
            !test_true("../test/bio_enc_test.c", 310, "BIO_get_cipher_status(cbio)", (BIO_ctrl(cbio,113,0,
# 310 "../test/bio_enc_test.c" 3 4
            ((void *)0)
# 310 "../test/bio_enc_test.c"
            )) != 0))
        || !test_mem_eq("../test/bio_enc_test.c", 311, "cbuf", "pbuf", cbuf, n, pbuf, sizeof(pbuf) - 1))
        goto end;
...
int setup_tests(void)
{
    ...
    add_test("test_bio_enc_eof_read_flush", test_bio_enc_eof_read_flush);
    return 1;
}
