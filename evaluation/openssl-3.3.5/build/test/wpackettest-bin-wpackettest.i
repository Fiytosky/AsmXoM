# 0 "../test/wpackettest.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/wpackettest.c"
# 10 "../test/wpackettest.c"
...
static int test_WPACKET_init(void)
{
    ...
    if (!test_true("../test/wpackettest.c", 72, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 73, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)

            || !test_false("../test/wpackettest.c", 75, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)

            || !test_true("../test/wpackettest.c", 77, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)




            || !test_false("../test/wpackettest.c", 82, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_false("../test/wpackettest.c", 83, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 84, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 85, "buf->data", "simple1", buf->data, written, simple1, sizeof(simple1)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 89, "WPACKET_init_len(&pkt, buf, 1)", (WPACKET_init_len(&pkt, buf, 1)) != 0)
            || !test_true("../test/wpackettest.c", 90, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)
            || !test_true("../test/wpackettest.c", 91, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 92, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 93, "buf->data", "simple2", buf->data, written, simple2, sizeof(simple2)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 97, "WPACKET_init_len(&pkt, buf, 4)", (WPACKET_init_len(&pkt, buf, 4)) != 0)
            || !test_true("../test/wpackettest.c", 98, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)
            || !test_true("../test/wpackettest.c", 99, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 100, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 101, "buf->data", "simple3", buf->data, written, simple3, sizeof(simple3)))
        return cleanup(&pkt);

    if (!test_true("../test/wpackettest.c", 104, "WPACKET_init_len(&pkt, buf, 1)", (WPACKET_init_len(&pkt, buf, 1)) != 0))
        return cleanup(&pkt);
    for (i = 1; i < 257; i++) {
        ...
        if (!test_int_eq("../test/wpackettest.c", 111, "WPACKET_put_bytes_u8(&pkt, 0xff)", "i < 256", WPACKET_put_bytes__((&pkt), (0xff), 1), i < 256))
            return cleanup(&pkt);
    }
    if (!test_true("../test/wpackettest.c", 114, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 118, "WPACKET_init_static_len(&pkt, sbuf, sizeof(sbuf), 0)", (WPACKET_init_static_len(&pkt, sbuf, sizeof(sbuf), 0)) != 0)

            || !test_true("../test/wpackettest.c", 120, "WPACKET_put_bytes_u24(&pkt, 0xffffff)", (WPACKET_put_bytes__((&pkt), (0xffffff), 3)) != 0)

            || !test_false("../test/wpackettest.c", 122, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)

            || !test_true("../test/wpackettest.c", 124, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 125, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 126, "sbuf", "fixed", sbuf, written, fixed, sizeof(sbuf))

            || !test_true("../test/wpackettest.c", 128, "WPACKET_init_static_len(&pkt, sbuf, sizeof(sbuf), 1)", (WPACKET_init_static_len(&pkt, sbuf, sizeof(sbuf), 1)) != 0)

            || !test_true("../test/wpackettest.c", 130, "WPACKET_put_bytes_u16(&pkt, 0xfeff)", (WPACKET_put_bytes__((&pkt), (0xfeff), 2)) != 0)

            || !test_false("../test/wpackettest.c", 132, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)
            || !test_true("../test/wpackettest.c", 133, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 134, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 135, "sbuf", "alloc", sbuf, written, alloc, sizeof(alloc)))
        return cleanup(&pkt);
...
static int test_WPACKET_set_max_size(void)
{
    ...
    if (!test_true("../test/wpackettest.c", 146, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)




            || !test_true("../test/wpackettest.c", 151, "WPACKET_set_max_size(&pkt, SIZE_MAX)", (WPACKET_set_max_size(&pkt, 
# 151 "../test/wpackettest.c" 3 4
               (18446744073709551615UL)
# 151 "../test/wpackettest.c"
               )) != 0)

            || !test_true("../test/wpackettest.c", 153, "WPACKET_set_max_size(&pkt, SIZE_MAX -1)", (WPACKET_set_max_size(&pkt, 
# 153 "../test/wpackettest.c" 3 4
               (18446744073709551615UL) 
# 153 "../test/wpackettest.c"
               -1)) != 0)

            || !test_true("../test/wpackettest.c", 155, "WPACKET_set_max_size(&pkt, SIZE_MAX)", (WPACKET_set_max_size(&pkt, 
# 155 "../test/wpackettest.c" 3 4
               (18446744073709551615UL)
# 155 "../test/wpackettest.c"
               )) != 0)
            || !test_true("../test/wpackettest.c", 156, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0))
        return cleanup(&pkt);

    if (!test_true("../test/wpackettest.c", 159, "WPACKET_init_len(&pkt, buf, 1)", (WPACKET_init_len(&pkt, buf, 1)) != 0)




            || !test_false("../test/wpackettest.c", 164, "WPACKET_set_max_size(&pkt, 0)", (WPACKET_set_max_size(&pkt, 0)) != 0)




            || !test_false("../test/wpackettest.c", 169, "WPACKET_set_max_size(&pkt, 0x0101)", (WPACKET_set_max_size(&pkt, 0x0101)) != 0)

            || !test_true("../test/wpackettest.c", 171, "WPACKET_set_max_size(&pkt, 0x0100)", (WPACKET_set_max_size(&pkt, 0x0100)) != 0)

            || !test_true("../test/wpackettest.c", 173, "WPACKET_set_max_size(&pkt, 0x01)", (WPACKET_set_max_size(&pkt, 0x01)) != 0)

            || !test_false("../test/wpackettest.c", 175, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)

            || !test_true("../test/wpackettest.c", 177, "WPACKET_set_max_size(&pkt, 0x02)", (WPACKET_set_max_size(&pkt, 0x02)) != 0)
            || !test_true("../test/wpackettest.c", 178, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)
            || !test_false("../test/wpackettest.c", 179, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)
            || !test_true("../test/wpackettest.c", 180, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 181, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 182, "buf->data", "simple2", buf->data, written, simple2, sizeof(simple2)))
        return cleanup(&pkt);
...
static int test_WPACKET_start_sub_packet(void)
{
    ...
    if (!test_true("../test/wpackettest.c", 194, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 195, "WPACKET_start_sub_packet(&pkt)", (WPACKET_start_sub_packet(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 196, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)

            || !test_false("../test/wpackettest.c", 198, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 199, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)

            || !test_false("../test/wpackettest.c", 201, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)

            || !test_true("../test/wpackettest.c", 203, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 204, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 205, "buf->data", "simple1", buf->data, written, simple1, sizeof(simple1)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 209, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 210, "WPACKET_start_sub_packet_u8(&pkt)", (WPACKET_start_sub_packet_len__((&pkt), 1)) != 0)
            || !test_true("../test/wpackettest.c", 211, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)
            || !test_true("../test/wpackettest.c", 212, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 213, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 214, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 215, "buf->data", "simple2", buf->data, written, simple2, sizeof(simple2)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 219, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 220, "WPACKET_start_sub_packet_u8(&pkt)", (WPACKET_start_sub_packet_len__((&pkt), 1)) != 0)
            || !test_true("../test/wpackettest.c", 221, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)
            || !test_true("../test/wpackettest.c", 222, "WPACKET_start_sub_packet_u8(&pkt)", (WPACKET_start_sub_packet_len__((&pkt), 1)) != 0)
            || !test_true("../test/wpackettest.c", 223, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)
            || !test_true("../test/wpackettest.c", 224, "WPACKET_get_length(&pkt, &len)", (WPACKET_get_length(&pkt, &len)) != 0)
            || !test_size_t_eq("../test/wpackettest.c", 225, "len", "1", len, 1)
            || !test_true("../test/wpackettest.c", 226, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 227, "WPACKET_get_length(&pkt, &len)", (WPACKET_get_length(&pkt, &len)) != 0)
            || !test_size_t_eq("../test/wpackettest.c", 228, "len", "3", len, 3)
            || !test_true("../test/wpackettest.c", 229, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 230, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 231, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 232, "buf->data", "nestedsub", buf->data, written, nestedsub, sizeof(nestedsub)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 236, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 237, "WPACKET_start_sub_packet_u8(&pkt)", (WPACKET_start_sub_packet_len__((&pkt), 1)) != 0)
            || !test_true("../test/wpackettest.c", 238, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)
            || !test_true("../test/wpackettest.c", 239, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 240, "WPACKET_start_sub_packet_u8(&pkt)", (WPACKET_start_sub_packet_len__((&pkt), 1)) != 0)
            || !test_true("../test/wpackettest.c", 241, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)
            || !test_true("../test/wpackettest.c", 242, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 243, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 244, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 245, "buf->data", "seqsub", buf->data, written, seqsub, sizeof(seqsub)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 249, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 250, "WPACKET_start_sub_packet_u8(&pkt)", (WPACKET_start_sub_packet_len__((&pkt), 1)) != 0)
            || !test_true("../test/wpackettest.c", 251, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)
            || !test_true("../test/wpackettest.c", 252, "WPACKET_start_sub_packet_u8(&pkt)", (WPACKET_start_sub_packet_len__((&pkt), 1)) != 0)
            || !test_true("../test/wpackettest.c", 253, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)
            || !test_true("../test/wpackettest.c", 254, "WPACKET_get_length(&pkt, &len)", (WPACKET_get_length(&pkt, &len)) != 0)
            || !test_size_t_eq("../test/wpackettest.c", 255, "len", "1", len, 1)
            || !test_true("../test/wpackettest.c", 256, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 257, "WPACKET_get_length(&pkt, &len)", (WPACKET_get_length(&pkt, &len)) != 0)
            || !test_size_t_eq("../test/wpackettest.c", 258, "len", "3", len, 3)
            || !test_true("../test/wpackettest.c", 259, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 260, "WPACKET_fill_lengths(&pkt)", (WPACKET_fill_lengths(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 261, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 262, "buf->data", "nestedsub", buf->data, written, nestedsub, sizeof(nestedsub))
            || !test_true("../test/wpackettest.c", 263, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0))
        return cleanup(&pkt);
...
static int test_WPACKET_set_flags(void)
{
    ...
    if (!test_true("../test/wpackettest.c", 276, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 277, "WPACKET_set_flags(&pkt, WPACKET_FLAGS_NON_ZERO_LENGTH)", (WPACKET_set_flags(&pkt, 1)) != 0)

            || !test_false("../test/wpackettest.c", 279, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 280, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)
            || !test_true("../test/wpackettest.c", 281, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 282, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 283, "buf->data", "simple1", buf->data, written, simple1, sizeof(simple1)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 287, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 288, "WPACKET_start_sub_packet(&pkt)", (WPACKET_start_sub_packet(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 289, "WPACKET_set_flags(&pkt, WPACKET_FLAGS_NON_ZERO_LENGTH)", (WPACKET_set_flags(&pkt, 1)) != 0)

            || !test_false("../test/wpackettest.c", 291, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 292, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)
            || !test_true("../test/wpackettest.c", 293, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 294, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 295, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 296, "buf->data", "simple1", buf->data, written, simple1, sizeof(simple1)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 300, "WPACKET_init_len(&pkt, buf, 1)", (WPACKET_init_len(&pkt, buf, 1)) != 0)
            || !test_true("../test/wpackettest.c", 301, "WPACKET_set_flags(&pkt, WPACKET_FLAGS_ABANDON_ON_ZERO_LENGTH)", (WPACKET_set_flags(&pkt, 2)) != 0)
            || !test_true("../test/wpackettest.c", 302, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 303, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_size_t_eq("../test/wpackettest.c", 304, "written", "0", written, 0))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 308, "WPACKET_init_len(&pkt, buf, 1)", (WPACKET_init_len(&pkt, buf, 1)) != 0)
            || !test_true("../test/wpackettest.c", 309, "WPACKET_start_sub_packet_u8(&pkt)", (WPACKET_start_sub_packet_len__((&pkt), 1)) != 0)
            || !test_true("../test/wpackettest.c", 310, "WPACKET_set_flags(&pkt, WPACKET_FLAGS_ABANDON_ON_ZERO_LENGTH)", (WPACKET_set_flags(&pkt, 2)) != 0)
            || !test_true("../test/wpackettest.c", 311, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 312, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 313, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 314, "buf->data", "empty", buf->data, written, empty, sizeof(empty)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 318, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 319, "WPACKET_start_sub_packet_u8(&pkt)", (WPACKET_start_sub_packet_len__((&pkt), 1)) != 0)
            || !test_true("../test/wpackettest.c", 320, "WPACKET_set_flags(&pkt, WPACKET_FLAGS_ABANDON_ON_ZERO_LENGTH)", (WPACKET_set_flags(&pkt, 2)) != 0)
            || !test_true("../test/wpackettest.c", 321, "WPACKET_put_bytes_u8(&pkt, 0xff)", (WPACKET_put_bytes__((&pkt), (0xff), 1)) != 0)
            || !test_true("../test/wpackettest.c", 322, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 323, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 324, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 325, "buf->data", "simple2", buf->data, written, simple2, sizeof(simple2)))
        return cleanup(&pkt);
    return 1;
}
...
static int test_WPACKET_allocate_bytes(void)
{
    ...
    if (!test_true("../test/wpackettest.c", 336, "WPACKET_init_len(&pkt, buf, 1)", (WPACKET_init_len(&pkt, buf, 1)) != 0)
            || !test_true("../test/wpackettest.c", 337, "WPACKET_allocate_bytes(&pkt, 2, &bytes)", (WPACKET_allocate_bytes(&pkt, 2, &bytes)) != 0))
        return cleanup(&pkt);
    bytes[0] = 0xfe;
    bytes[1] = 0xff;
    if (!test_true("../test/wpackettest.c", 341, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 342, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 343, "buf->data", "alloc", buf->data, written, alloc, sizeof(alloc)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 347, "WPACKET_init_len(&pkt, buf, 1)", (WPACKET_init_len(&pkt, buf, 1)) != 0)
            || !test_true("../test/wpackettest.c", 348, "WPACKET_sub_allocate_bytes_u8(&pkt, 2, &bytes)", (WPACKET_sub_allocate_bytes__((&pkt), (2), (&bytes), 1)) != 0))
        return cleanup(&pkt);
    bytes[0] = 0xfe;
    bytes[1] = 0xff;
    if (!test_true("../test/wpackettest.c", 352, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 353, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 354, "buf->data", "submem", buf->data, written, submem, sizeof(submem)))
        return cleanup(&pkt);
...
static int test_WPACKET_memcpy(void)
{
    ...
    if (!test_true("../test/wpackettest.c", 366, "WPACKET_init_len(&pkt, buf, 1)", (WPACKET_init_len(&pkt, buf, 1)) != 0)
            || !test_true("../test/wpackettest.c", 367, "WPACKET_memcpy(&pkt, bytes, sizeof(bytes))", (WPACKET_memcpy(&pkt, bytes, sizeof(bytes))) != 0)
            || !test_true("../test/wpackettest.c", 368, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 369, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 370, "buf->data", "alloc", buf->data, written, alloc, sizeof(alloc)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 374, "WPACKET_init_len(&pkt, buf, 1)", (WPACKET_init_len(&pkt, buf, 1)) != 0)
            || !test_true("../test/wpackettest.c", 375, "WPACKET_sub_memcpy_u8(&pkt, bytes, sizeof(bytes))", (WPACKET_sub_memcpy__((&pkt), (bytes), (sizeof(bytes)), 1)) != 0)
            || !test_true("../test/wpackettest.c", 376, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 377, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 378, "buf->data", "submem", buf->data, written, submem, sizeof(submem)))
        return cleanup(&pkt);
...
static int test_WPACKET_init_der(void)
{
    ...
    if (!test_true("../test/wpackettest.c", 396, "WPACKET_init_der(&pkt, sbuf, sizeof(sbuf))", (WPACKET_init_der(&pkt, sbuf, sizeof(sbuf))) != 0)
            || !test_true("../test/wpackettest.c", 397, "WPACKET_put_bytes_u24(&pkt, 0xfffefd)", (WPACKET_put_bytes__((&pkt), (0xfffefd), 3)) != 0)

            || !test_true("../test/wpackettest.c", 399, "WPACKET_start_sub_packet(&pkt)", (WPACKET_start_sub_packet(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 400, "WPACKET_memcpy(&pkt, testdata, sizeof(testdata))", (WPACKET_memcpy(&pkt, testdata, sizeof(testdata))) != 0)
            || !test_true("../test/wpackettest.c", 401, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 402, "WPACKET_put_bytes_u8(&pkt, 0xfc)", (WPACKET_put_bytes__((&pkt), (0xfc), 1)) != 0)

            || (!test_true("../test/wpackettest.c", 404, "WPACKET_start_sub_packet(&pkt)", (WPACKET_start_sub_packet(&pkt)) != 0)
                || !test_true("../test/wpackettest.c", 405, "WPACKET_set_flags(&pkt, flags)", (WPACKET_set_flags(&pkt, flags)) != 0)
                || !test_true("../test/wpackettest.c", 406, "WPACKET_get_total_written(&pkt, &size1)", (WPACKET_get_total_written(&pkt, &size1)) != 0)
                || !test_true("../test/wpackettest.c", 407, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
                || !test_true("../test/wpackettest.c", 408, "WPACKET_get_total_written(&pkt, &size2)", (WPACKET_get_total_written(&pkt, &size2)) != 0)
                || !test_size_t_eq("../test/wpackettest.c", 409, "size1", "size2", size1, size2))
            || !test_true("../test/wpackettest.c", 410, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 411, "WPACKET_get_total_written(&pkt, &written[0])", (WPACKET_get_total_written(&pkt, &written[0])) != 0)
            || !test_mem_eq("../test/wpackettest.c", 412, "WPACKET_get_curr(&pkt)", "simpleder", WPACKET_get_curr(&pkt), written[0], simpleder, sizeof(simpleder))
                                              )
        return cleanup(&pkt);


    if (!test_int_gt("../test/wpackettest.c", 417, "RAND_bytes(&testdata2[3], sizeof(testdata2) - 3)", "0", RAND_bytes(&testdata2[3], sizeof(testdata2) - 3), 0))
        return 0;
    ...
    for (i = 0; i < 2; i++) {
        if (i == 0) {
            if (!test_true("../test/wpackettest.c", 427, "WPACKET_init_null_der(&pkt)", (WPACKET_init_null_der(&pkt)) != 0))
                return 0;
        } else {
            if (!test_true("../test/wpackettest.c", 430, "WPACKET_init_der(&pkt, sbuf, sizeof(sbuf))", (WPACKET_init_der(&pkt, sbuf, sizeof(sbuf))) != 0))
                return 0;
        }
        if (!test_true("../test/wpackettest.c", 433, "WPACKET_start_sub_packet(&pkt)", (WPACKET_start_sub_packet(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 434, "WPACKET_memcpy(&pkt, &testdata2[3], sizeof(testdata2) - 3)", (WPACKET_memcpy(&pkt, &testdata2[3], sizeof(testdata2) - 3)) != 0)

            || !test_true("../test/wpackettest.c", 436, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 437, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 438, "WPACKET_get_total_written(&pkt, &written[i])", (WPACKET_get_total_written(&pkt, &written[i])) != 0))
        return cleanup(&pkt);
    }
    ...
    if (!test_size_t_eq("../test/wpackettest.c", 447, "written[0]", "written[1]", written[0], written[1])
            || !test_mem_eq("../test/wpackettest.c", 448, "WPACKET_get_curr(&pkt)", "testdata2", WPACKET_get_curr(&pkt), written[1], testdata2, sizeof(testdata2))
                                              )
        return 0;
...
static int test_WPACKET_quic(void)
{
    ...
    if (!test_true("../test/wpackettest.c", 464, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 465, "WPACKET_start_quic_sub_packet(&pkt)", (WPACKET_start_quic_sub_packet(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 466, "WPACKET_quic_write_vlint(&pkt, 0x09)", (WPACKET_quic_write_vlint(&pkt, 0x09)) != 0)

            || !test_false("../test/wpackettest.c", 468, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 469, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)

            || !test_false("../test/wpackettest.c", 471, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)

            || !test_true("../test/wpackettest.c", 473, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 474, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 475, "buf->data", "quic1", buf->data, written, quic1, sizeof(quic1)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 479, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 480, "WPACKET_start_quic_sub_packet_bound(&pkt, OSSL_QUIC_VLINT_1B_MAX)", (WPACKET_start_quic_sub_packet_bound(&pkt, (64 - 1))) != 0)
            || !test_true("../test/wpackettest.c", 481, "WPACKET_quic_write_vlint(&pkt, 0x09)", (WPACKET_quic_write_vlint(&pkt, 0x09)) != 0)
            || !test_false("../test/wpackettest.c", 482, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 483, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_false("../test/wpackettest.c", 484, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 485, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 486, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 487, "buf->data", "quic2", buf->data, written, quic2, sizeof(quic2)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 491, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 492, "WPACKET_start_quic_sub_packet_bound(&pkt, OSSL_QUIC_VLINT_2B_MIN)", (WPACKET_start_quic_sub_packet_bound(&pkt, 64)) != 0)
            || !test_true("../test/wpackettest.c", 493, "WPACKET_quic_write_vlint(&pkt, 0x41)", (WPACKET_quic_write_vlint(&pkt, 0x41)) != 0)
            || !test_false("../test/wpackettest.c", 494, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 495, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_false("../test/wpackettest.c", 496, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 497, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 498, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 499, "buf->data", "quic3", buf->data, written, quic3, sizeof(quic3)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 503, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 504, "WPACKET_start_quic_sub_packet_bound(&pkt, OSSL_QUIC_VLINT_8B_MIN)", (WPACKET_start_quic_sub_packet_bound(&pkt, 1073741824)) != 0)
            || !test_true("../test/wpackettest.c", 505, "WPACKET_quic_write_vlint(&pkt, 0x13c6a)", (WPACKET_quic_write_vlint(&pkt, 0x13c6a)) != 0)
            || !test_false("../test/wpackettest.c", 506, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 507, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_false("../test/wpackettest.c", 508, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 509, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 510, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 511, "buf->data", "quic4", buf->data, written, quic4, sizeof(quic4)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 515, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 516, "WPACKET_start_quic_sub_packet_bound(&pkt, OSSL_QUIC_VLINT_8B_MIN)", (WPACKET_start_quic_sub_packet_bound(&pkt, 1073741824)) != 0)
            || !test_true("../test/wpackettest.c", 517, "WPACKET_quic_write_vlint(&pkt, 0x2f77213f3f505ba5ULL)", (WPACKET_quic_write_vlint(&pkt, 0x2f77213f3f505ba5ULL)) != 0)
            || !test_false("../test/wpackettest.c", 518, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 519, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_false("../test/wpackettest.c", 520, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 521, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 522, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 523, "buf->data", "quic5", buf->data, written, quic5, sizeof(quic5)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 527, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 528, "WPACKET_quic_sub_allocate_bytes(&pkt, 3, &bytes)", (WPACKET_quic_sub_allocate_bytes(&pkt, 3, &bytes)) != 0))
        return cleanup(&pkt);
    ...
    if (!test_true("../test/wpackettest.c", 535, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 536, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 537, "buf->data", "quic6", buf->data, written, quic6, sizeof(quic6)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 541, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_true("../test/wpackettest.c", 542, "WPACKET_quic_write_vlint(&pkt, 0x07)", (WPACKET_quic_write_vlint(&pkt, 0x07)) != 0)
            || !test_true("../test/wpackettest.c", 543, "WPACKET_get_length(&pkt, &len)", (WPACKET_get_length(&pkt, &len)) != 0)
            || !test_size_t_eq("../test/wpackettest.c", 544, "len", "1", len, 1)
            || !test_true("../test/wpackettest.c", 545, "WPACKET_start_quic_sub_packet_bound(&pkt, OSSL_QUIC_VLINT_4B_MIN)", (WPACKET_start_quic_sub_packet_bound(&pkt, 16384)) != 0)
            || !test_true("../test/wpackettest.c", 546, "WPACKET_quic_write_vlint(&pkt, 0x2514)", (WPACKET_quic_write_vlint(&pkt, 0x2514)) != 0)
            || !test_true("../test/wpackettest.c", 547, "WPACKET_get_length(&pkt, &len)", (WPACKET_get_length(&pkt, &len)) != 0)
            || !test_size_t_eq("../test/wpackettest.c", 548, "len", "2", len, 2)
            || !test_true("../test/wpackettest.c", 549, "WPACKET_start_quic_sub_packet_bound(&pkt, OSSL_QUIC_VLINT_2B_MIN)", (WPACKET_start_quic_sub_packet_bound(&pkt, 64)) != 0)
            || !test_true("../test/wpackettest.c", 550, "WPACKET_quic_write_vlint(&pkt, 0x05)", (WPACKET_quic_write_vlint(&pkt, 0x05)) != 0)
            || !test_true("../test/wpackettest.c", 551, "WPACKET_get_length(&pkt, &len)", (WPACKET_get_length(&pkt, &len)) != 0)
            || !test_size_t_eq("../test/wpackettest.c", 552, "len", "1", len, 1)
            || !test_true("../test/wpackettest.c", 553, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 554, "WPACKET_start_quic_sub_packet_bound(&pkt, OSSL_QUIC_VLINT_2B_MIN)", (WPACKET_start_quic_sub_packet_bound(&pkt, 64)) != 0)
            || !test_true("../test/wpackettest.c", 555, "WPACKET_quic_write_vlint(&pkt, 0x11)", (WPACKET_quic_write_vlint(&pkt, 0x11)) != 0)
            || !test_true("../test/wpackettest.c", 556, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 557, "WPACKET_get_length(&pkt, &len)", (WPACKET_get_length(&pkt, &len)) != 0)
            || !test_size_t_eq("../test/wpackettest.c", 558, "len", "8", len, 8)
            || !test_true("../test/wpackettest.c", 559, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 560, "WPACKET_start_quic_sub_packet_bound(&pkt, OSSL_QUIC_VLINT_2B_MIN)", (WPACKET_start_quic_sub_packet_bound(&pkt, 64)) != 0)
            || !test_true("../test/wpackettest.c", 561, "WPACKET_quic_write_vlint(&pkt, 0x12)", (WPACKET_quic_write_vlint(&pkt, 0x12)) != 0)
            || !test_true("../test/wpackettest.c", 562, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 563, "WPACKET_start_quic_sub_packet_bound(&pkt, OSSL_QUIC_VLINT_2B_MIN)", (WPACKET_start_quic_sub_packet_bound(&pkt, 64)) != 0)
            || !test_true("../test/wpackettest.c", 564, "WPACKET_quic_write_vlint(&pkt, 0x13)", (WPACKET_quic_write_vlint(&pkt, 0x13)) != 0)
            || !test_true("../test/wpackettest.c", 565, "WPACKET_close(&pkt)", (WPACKET_close(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 566, "WPACKET_finish(&pkt)", (WPACKET_finish(&pkt)) != 0)
            || !test_true("../test/wpackettest.c", 567, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0)
            || !test_mem_eq("../test/wpackettest.c", 568, "buf->data", "quic7", buf->data, written, quic7, sizeof(quic7)))
        return cleanup(&pkt);


    if (!test_true("../test/wpackettest.c", 572, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
            || !test_false("../test/wpackettest.c", 573, "WPACKET_quic_write_vlint(&pkt, OSSL_QUIC_VLINT_MAX+1)", (WPACKET_quic_write_vlint(&pkt, (((uint64_t)1 << 62) - 1)+1)) != 0)
            || !test_true("../test/wpackettest.c", 574, "WPACKET_quic_write_vlint(&pkt, OSSL_QUIC_VLINT_MAX)", (WPACKET_quic_write_vlint(&pkt, (((uint64_t)1 << 62) - 1))) != 0))
            return cleanup(&pkt);
...
static int test_WPACKET_quic_vlint_random(void)
{
    ...
    for (i = 0; i < 10000; ++i) {
        if (!test_int_gt("../test/wpackettest.c", 590, "RAND_bytes(rand_data, sizeof(rand_data))", "0", RAND_bytes(rand_data, sizeof(rand_data)), 0))
            return 0;
        ...
        switch (rand_data[8] & 3) {
            ...
            case 1:
                ...
                break;
            case 2:
                expected &= (1073741824 - 1);
                break;
            case 3:
                expected &= (((uint64_t)1 << 62) - 1);
                break;
        }

        if (!test_true("../test/wpackettest.c", 613, "WPACKET_init(&pkt, buf)", (WPACKET_init(&pkt, buf)) != 0)
                || !test_true("../test/wpackettest.c", 614, "WPACKET_quic_write_vlint(&pkt, expected)", (WPACKET_quic_write_vlint(&pkt, expected)) != 0)
                || !test_true("../test/wpackettest.c", 615, "WPACKET_get_total_written(&pkt, &written)", (WPACKET_get_total_written(&pkt, &written)) != 0))
            return cleanup(&pkt);

        if (!test_true("../test/wpackettest.c", 618, "PACKET_buf_init(&read_pkt, (unsigned char *)buf->data, written)", (PACKET_buf_init(&read_pkt, (unsigned char *)buf->data, written)) != 0)
                || !test_true("../test/wpackettest.c", 619, "PACKET_get_quic_vlint(&read_pkt, &actual)", (PACKET_get_quic_vlint(&read_pkt, &actual)) != 0)
                || !test_uint64_t_eq("../test/wpackettest.c", 620, "expected", "actual", expected, actual))
            return cleanup(&pkt);
...
int setup_tests(void)
{
    if (!test_ptr("../test/wpackettest.c", 634, "buf = BUF_MEM_new()", buf = BUF_MEM_new()))
            return 0;
    ...
    add_test("test_WPACKET_quic_vlint_random", test_WPACKET_quic_vlint_random);
