...
{
    BIO *bio = 
# 46 "../test/d2i_test.c" 3 4
              ((void *)0)
# 46 "../test/d2i_test.c"
                  ;
    ASN1_VALUE *value = 
# 47 "../test/d2i_test.c" 3 4
                       ((void *)0)
# 47 "../test/d2i_test.c"
                           ;
    int ret = 0;
...
# 51 "../test/d2i_test.c"
    ...
    if (!test_ptr("../test/d2i_test.c", 56, "bio", bio))
        return 0;

    if (expected_error == ASN1_BIO) {
        if (test_ptr_null("../test/d2i_test.c", 60, "ASN1_item_d2i_bio(item_type, bio, NULL)", ASN1_item_d2i_bio(item_type, bio, 
# 60 "../test/d2i_test.c" 3 4
           ((void *)0)
# 60 "../test/d2i_test.c"
           )))
            ret = 1;
    ...
    if (!test_int_ge("../test/d2i_test.c", 71, "len", "0", len, 0))
        goto err;

    value = ASN1_item_d2i(
# 74 "../test/d2i_test.c" 3 4
                         ((void *)0)
# 74 "../test/d2i_test.c"
                             , &buf_ptr, len, item_type);
    if (value == 
# 75 "../test/d2i_test.c" 3 4
                ((void *)0)
# 75 "../test/d2i_test.c"
                    ) {
        if (test_int_eq("../test/d2i_test.c", 76, "expected_error", "ASN1_DECODE", expected_error, ASN1_DECODE))
            ret = 1;
        goto err;
    }
    ...
    if (der == 
# 83 "../test/d2i_test.c" 3 4
              ((void *)0) 
# 83 "../test/d2i_test.c"
                   || derlen < 0) {
        if (test_int_eq("../test/d2i_test.c", 84, "expected_error", "ASN1_ENCODE", expected_error, ASN1_ENCODE))
            ret = 1;
        goto err;
    }

    if (derlen != len || memcmp(der, buf, derlen) != 0) {
        if (test_int_eq("../test/d2i_test.c", 90, "expected_error", "ASN1_COMPARE", expected_error, ASN1_COMPARE))
            ret = 1;
        goto err;
    }

    if (test_int_eq("../test/d2i_test.c", 95, "expected_error", "ASN1_OK", expected_error, ASN1_OK))
        ret = 1;

 err:

    if (ret == 1
        && !test_false("../test/d2i_test.c", 101, "ERR_GET_REASON(ERR_peek_error()) == ERR_R_MALLOC_FAILURE", (ERR_GET_REASON(ERR_peek_error()) == (256|((0x1 << 18L)|(0x2 << 18L)))) != 0))
        ret = 0;
    BIO_free(bio);
    CRYPTO_free(der, "../test/d2i_test.c", 104);
    ASN1_item_free(value, item_type);
    return ret;
}

const OPTIONS *test_get_options(void) { enum { OPT_TEST_HELP = 500, OPT_TEST_LIST, OPT_TEST_SINGLE, OPT_TEST_ITERATION, OPT_TEST_INDENT, OPT_TEST_SEED }; static const OPTIONS options[] = { { OPT_HELP_STR, 1, '-', "Usage: %s [options] " "item_name expected_error test_file.der\n" }, { OPT_HELP_STR, 1, '-', "Valid options are:\n" }, { "help", OPT_TEST_HELP, '-', "Display this summary" }, { "list", OPT_TEST_LIST, '-', "Display the list of tests available" }, { "test", OPT_TEST_SINGLE, 's', "Run a single test by id or name" }, { "iter", OPT_TEST_ITERATION, 'n', "Run a single iteration of a test" }, { "indent", OPT_TEST_INDENT,'p', "Number of tabs added to output" }, { "seed", OPT_TEST_SEED, 'n', "Seed value to randomize tests with" }, { 
# 109 "../test/d2i_test.c" 3 4
((void *)0) 
# 109 "../test/d2i_test.c"
} }; return options; }
...
int setup_tests(void)
{
    ...
    if (!test_skip_common_options()) {
        test_error("../test/d2i_test.c", 131, "Error parsing test options\n");
        return 0;
    }

    if (!test_ptr("../test/d2i_test.c", 135, "test_type_name = test_get_argument(0)", test_type_name = test_get_argument(0))
            || !test_ptr("../test/d2i_test.c", 136, "expected_error_string = test_get_argument(1)", expected_error_string = test_get_argument(1))
            || !test_ptr("../test/d2i_test.c", 137, "test_file = test_get_argument(2)", test_file = test_get_argument(2)))
        return 0;

    item_type = ASN1_ITEM_lookup(test_type_name);

    if (item_type == 
# 142 "../test/d2i_test.c" 3 4
                    ((void *)0)
# 142 "../test/d2i_test.c"
                        ) {
        test_error("../test/d2i_test.c", 143, "Unknown type %s", test_type_name);
        test_note("Supported types:");
        for (i = 0;; i++) {
            const ASN1_ITEM *it = ASN1_ITEM_get(i);

            if (it == 
# 148 "../test/d2i_test.c" 3 4
                     ((void *)0)
# 148 "../test/d2i_test.c"
                         )
                break;
    ...
    for (i = 0; i < (sizeof(expected_errors)/sizeof((expected_errors)[0])); i++) {
    ...
    if (expected_error == ASN1_UNKNOWN) {
        test_error("../test/d2i_test.c", 163, "Unknown expected error %s\n", expected_error_string);
        return 0;
    }

    add_test("test_bad_asn1", test_bad_asn1);
    return 1;
}
