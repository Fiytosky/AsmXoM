# 0 "../test/quic_cc_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/quic_cc_test.c"
# 13 "../test/quic_cc_test.c"
# 1 "../test/testutil.h" 1
# 13 "../test/testutil.h"
...
# 14 "../include/internal/quic_cc.h" 2
...
# 15 "../include/internal/quic_cc.h" 2
...
{
    fake_time = ossl_time_add(fake_time, ossl_ticks2time((ms) * (((uint64_t)1000000000) / 1000)));
}
# 50 "../test/quic_cc_test.c"
...
typedef struct ossl_priority_queue_st_NET_PKT OSSL_PRIORITY_QUEUE_NET_PKT; static __attribute__((unused)) inline OSSL_PRIORITY_QUEUE_NET_PKT * ossl_pqueue_NET_PKT_new(int (*compare)(const NET_PKT *, const NET_PKT *)) { return (OSSL_PRIORITY_QUEUE_NET_PKT *)ossl_pqueue_new( (int (*)(const void *, const void *))compare); } static __attribute__((unused)) inline void ossl_pqueue_NET_PKT_free(OSSL_PRIORITY_QUEUE_NET_PKT *pq) { ossl_pqueue_free((OSSL_PQUEUE *)pq); } static __attribute__((unused)) inline void ossl_pqueue_NET_PKT_pop_free(OSSL_PRIORITY_QUEUE_NET_PKT *pq, void (*freefunc)(NET_PKT *)) { ossl_pqueue_pop_free((OSSL_PQUEUE *)pq, (void (*)(void *))freefunc); } static __attribute__((unused)) inline int ossl_pqueue_NET_PKT_reserve(OSSL_PRIORITY_QUEUE_NET_PKT *pq, size_t n) { return ossl_pqueue_reserve((OSSL_PQUEUE *)pq, n); } static __attribute__((unused)) inline size_t ossl_pqueue_NET_PKT_num(const OSSL_PRIORITY_QUEUE_NET_PKT *pq) { return ossl_pqueue_num((OSSL_PQUEUE *)pq); } static __attribute__((unused)) inline int ossl_pqueue_NET_PKT_push(OSSL_PRIORITY_QUEUE_NET_PKT *pq, NET_PKT *data, size_t *elem) { return ossl_pqueue_push((OSSL_PQUEUE *)pq, (void *)data, elem); } static __attribute__((unused)) inline NET_PKT * ossl_pqueue_NET_PKT_peek(const OSSL_PRIORITY_QUEUE_NET_PKT *pq) { return (NET_PKT *)ossl_pqueue_peek((OSSL_PQUEUE *)pq); } static __attribute__((unused)) inline NET_PKT * ossl_pqueue_NET_PKT_pop(OSSL_PRIORITY_QUEUE_NET_PKT *pq) { return (NET_PKT *)ossl_pqueue_pop((OSSL_PQUEUE *)pq); } static __attribute__((unused)) inline NET_PKT * ossl_pqueue_NET_PKT_remove(OSSL_PRIORITY_QUEUE_NET_PKT *pq, size_t elem) { return (NET_PKT *)ossl_pqueue_remove((OSSL_PQUEUE *)pq, elem); } struct ossl_priority_queue_st_NET_PKT;
...
struct net_sim {
    ...
    OSSL_PRIORITY_QUEUE_NET_PKT *pkts;
                        ...
                        uint64_t capacity, uint64_t latency)
{
    ...
    if (!test_ptr("../test/quic_cc_test.c", 122, "s->pkts = ossl_pqueue_NET_PKT_new(net_pkt_cmp)", s->pkts = ossl_pqueue_NET_PKT_new(net_pkt_cmp)))
        return 0;
...
{
    CRYPTO_free(pkt, "../test/quic_cc_test.c", 130);
}
...
{
    NET_PKT *pkt = CRYPTO_zalloc(sizeof(*pkt), "../test/quic_cc_test.c", 142);
    int success;

    if (!test_ptr("../test/quic_cc_test.c", 145, "pkt", pkt))
        return 0;
    ...
    if (!test_true("../test/quic_cc_test.c", 152, "net_sim_process(s, 0)", (net_sim_process(s, 0)) != 0))
        goto err;
    ...
    if (success) {
        ...
        pkt->arrive_time = ossl_time_add(pkt->tx_time,
                                                ossl_ticks2time((s->latency) * (((uint64_t)1000000000) / 1000)));
        ...
        pkt->determination_time = ossl_time_add(pkt->tx_time,
                                                ossl_ticks2time((3 * s->latency) * (((uint64_t)1000000000) / 1000)));
        pkt->next_time = pkt->determination_time;
    }

    pkt->size = sz;

    if (!test_true("../test/quic_cc_test.c", 187, "s->ccm->on_data_sent(s->cc, sz)", (s->ccm->on_data_sent(s->cc, sz)) != 0))
        goto err;

    if (!test_true("../test/quic_cc_test.c", 190, "ossl_pqueue_NET_PKT_push(s->pkts, pkt, &pkt->idx)", (ossl_pqueue_NET_PKT_push(s->pkts, pkt, &pkt->idx)) != 0))
        goto err;

    return 1;

err:
    CRYPTO_free(pkt, "../test/quic_cc_test.c", 196);
    return 0;
}
...
static int net_sim_process_one(struct net_sim *s, int skip_forward)
{
    ...
    if (pkt == 
# 204 "../test/quic_cc_test.c" 3 4
              ((void *)0)
# 204 "../test/quic_cc_test.c"
                  )
        return 3;
    ...
    if (!test_true("../test/quic_cc_test.c", 228, "!pkt->success || pkt->arrived", (!pkt->success || pkt->arrived) != 0))
        return 0;
    ...
    if (!pkt->success) {
        ...
        if (!test_true("../test/quic_cc_test.c", 237, "s->ccm->on_data_lost(s->cc, &loss_info)", (s->ccm->on_data_lost(s->cc, &loss_info)) != 0))
            return 0;

        if (!test_true("../test/quic_cc_test.c", 240, "s->ccm->on_data_lost_finished(s->cc, 0)", (s->ccm->on_data_lost_finished(s->cc, 0)) != 0))
            return 0;

        s->total_lost += pkt->size;
        ossl_pqueue_NET_PKT_pop(s->pkts);
        CRYPTO_free(pkt, "../test/quic_cc_test.c", 245);
    } else {
        ...
        if (!test_true("../test/quic_cc_test.c", 252, "s->ccm->on_data_acked(s->cc, &ack_info)", (s->ccm->on_data_acked(s->cc, &ack_info)) != 0))
            return 0;

        s->total_acked += pkt->size;
        ossl_pqueue_NET_PKT_pop(s->pkts);
        CRYPTO_free(pkt, "../test/quic_cc_test.c", 257);
    }
...
# 284 "../test/quic_cc_test.c"
static int dump_state(const OSSL_CC_METHOD *ccm, OSSL_CC_DATA *cc,
                                  struct net_sim *s)
{
# 317 "../test/quic_cc_test.c"
    return 1;
}
# 328 "../test/quic_cc_test.c"
static int test_simulate(void)
{
    ...
    OSSL_CC_DATA *cc = 
# 334 "../test/quic_cc_test.c" 3 4
                      ((void *)0)
# 334 "../test/quic_cc_test.c"
                          ;
    size_t mdpl = 1472;
    ...
    uint64_t diag_cur_bytes_in_flight = 
# 339 "../test/quic_cc_test.c" 3 4
                                       (18446744073709551615UL)
# 339 "../test/quic_cc_test.c"
                                                 ;
    uint64_t diag_cur_cwnd_size = 
# 340 "../test/quic_cc_test.c" 3 4
                                 (18446744073709551615UL)
# 340 "../test/quic_cc_test.c"
                                           ;
    struct net_sim sim;
    OSSL_PARAM params[3], *p = params;

    fake_time = (ossl_ticks2time(5 * ((uint64_t)1000000000)));

    if (!test_ptr("../test/quic_cc_test.c", 346, "cc = ccm->new(fake_now, NULL)", cc = ccm->new(fake_now, 
# 346 "../test/quic_cc_test.c" 3 4
        ((void *)0)
# 346 "../test/quic_cc_test.c"
        )))
        goto err;

    if (!test_true("../test/quic_cc_test.c", 349, "net_sim_init(&sim, ccm, cc, actual_capacity, 100)", (net_sim_init(&sim, ccm, cc, actual_capacity, 100)) != 0))
        goto err;

    have_sim = 1;

    *p++ = OSSL_PARAM_construct_size_t("max_dgram_payload_len",
                                       &mdpl);
    *p++ = OSSL_PARAM_construct_end();

    if (!test_true("../test/quic_cc_test.c", 358, "ccm->set_input_params(cc, params)", (ccm->set_input_params(cc, params)) != 0))
        goto err;

    p = params;
    *p++ = OSSL_PARAM_construct_uint64("bytes_in_flight",
                                       &diag_cur_bytes_in_flight);
    *p++ = OSSL_PARAM_construct_uint64("cur_cwnd_size",
                                       &diag_cur_cwnd_size);
    *p++ = OSSL_PARAM_construct_end();

    if (!test_true("../test/quic_cc_test.c", 368, "ccm->bind_diagnostics(cc, params)", (ccm->bind_diagnostics(cc, params)) != 0))
        goto err;

    ccm->reset(cc);

    if (!test_uint64_t_ge("../test/quic_cc_test.c", 373, "allowance = ccm->get_tx_allowance(cc)", "mdpl", allowance = ccm->get_tx_allowance(cc), mdpl))
        goto err;
    ...
    while (total_sent < total_to_send) {
        ...
        for (;;) {
            ...
            if (sz > 
# 394 "../test/quic_cc_test.c" 3 4
                    (18446744073709551615UL)
# 394 "../test/quic_cc_test.c"
                            )
                sz = 
# 395 "../test/quic_cc_test.c" 3 4
                    (18446744073709551615UL)
# 395 "../test/quic_cc_test.c"
                            ;
            ...
            if (!test_true("../test/quic_cc_test.c", 406, "net_sim_send(&sim, (size_t)sz)", (net_sim_send(&sim, (size_t)sz)) != 0))
                goto err;
        ...
        if (!test_int_gt("../test/quic_cc_test.c", 414, "rc", "0", rc, 0))
            goto err;
        ...
        if (rc == 3) {
            if (!test_uint64_t_eq("../test/quic_cc_test.c", 422, "diag_cur_bytes_in_flight", "0", diag_cur_bytes_in_flight, 0))
                goto err;

            if (!test_uint64_t_ge("../test/quic_cc_test.c", 425, "ccm->get_tx_allowance(cc)", "mdpl", ccm->get_tx_allowance(cc), mdpl))
                goto err;
        }
...

        {
            ...
            if (!test_uint64_t_ne("../test/quic_cc_test.c", 433, "diag_cur_bytes_in_flight", "UINT64_MAX", diag_cur_bytes_in_flight, 
# 433 "../test/quic_cc_test.c" 3 4
                (18446744073709551615UL)
# 433 "../test/quic_cc_test.c"
                )
                || !test_uint64_t_ne("../test/quic_cc_test.c", 434, "diag_cur_cwnd_size", "UINT64_MAX", diag_cur_cwnd_size, 
# 434 "../test/quic_cc_test.c" 3 4
                   (18446744073709551615UL)
# 434 "../test/quic_cc_test.c"
                   ))
                goto err;
...

    {
        ...
        test_info("../test/quic_cc_test.c", 451, "est = %6llu kB/s, act=%6llu kB/s (error=%.02f%%)\n", (unsigned long long)estimated_capacity, (unsigned long long)actual_capacity, error * 100.0)


                                ;


        if (!test_double_le("../test/quic_cc_test.c", 457, "error", "0.05", error, 0.05))
            goto err;
    }
...
err:
    ...
    if (cc != 
# 466 "../test/quic_cc_test.c" 3 4
             ((void *)0)
# 466 "../test/quic_cc_test.c"
                 )
        ccm->free(cc);
...
static int test_sanity(void)
{
    int testresult = 0;
    OSSL_CC_DATA *cc = 
# 486 "../test/quic_cc_test.c" 3 4
                      ((void *)0)
# 486 "../test/quic_cc_test.c"
                          ;
    const OSSL_CC_METHOD *ccm = &ossl_cc_newreno_method;
    OSSL_CC_LOSS_INFO loss_info = {0};
    OSSL_CC_ACK_INFO ack_info = {0};
    ...
    OSSL_PARAM params[3], *p = params;
    size_t mdpl = 1472, diag_mdpl = 
# 492 "../test/quic_cc_test.c" 3 4
                                   (18446744073709551615UL)
# 492 "../test/quic_cc_test.c"
                                           ;
    uint64_t diag_cur_bytes_in_flight = 
# 493 "../test/quic_cc_test.c" 3 4
                                       (18446744073709551615UL)
# 493 "../test/quic_cc_test.c"
                                                 ;

    fake_time = (ossl_ticks2time(5 * ((uint64_t)1000000000)));

    if (!test_ptr("../test/quic_cc_test.c", 497, "cc = ccm->new(fake_now, NULL)", cc = ccm->new(fake_now, 
# 497 "../test/quic_cc_test.c" 3 4
        ((void *)0)
# 497 "../test/quic_cc_test.c"
        )))
        goto err;
    ...
    *p++ = OSSL_PARAM_construct_end();
    ...
    if (!test_true("../test/quic_cc_test.c", 505, "ccm->set_input_params(cc, params)", (ccm->set_input_params(cc, params)) != 0))
        goto err;
    ...
    *p++ = OSSL_PARAM_construct_end();
    ...
    if (!test_true("../test/quic_cc_test.c", 517, "ccm->bind_diagnostics(cc, params)", (ccm->bind_diagnostics(cc, params)) != 0)
        || !test_size_t_eq("../test/quic_cc_test.c", 518, "diag_mdpl", "1472", diag_mdpl, 1472))
        goto err;

    if (!test_uint64_t_ge("../test/quic_cc_test.c", 521, "allowance = ccm->get_tx_allowance(cc)", "1472", allowance = ccm->get_tx_allowance(cc), 1472))
        goto err;


    if (!test_true("../test/quic_cc_test.c", 525, "ossl_time_is_zero(ccm->get_wakeup_deadline(cc))", (ossl_time_is_zero(ccm->get_wakeup_deadline(cc))) != 0))
        goto err;


    if (!test_uint64_t_eq("../test/quic_cc_test.c", 529, "diag_cur_bytes_in_flight", "0", diag_cur_bytes_in_flight, 0))
        goto err;


    if (!test_true("../test/quic_cc_test.c", 533, "ccm->on_data_sent(cc, 1200)", (ccm->on_data_sent(cc, 1200)) != 0))
        goto err;


    if (!test_uint64_t_eq("../test/quic_cc_test.c", 537, "ccm->get_tx_allowance(cc)", "allowance - 1200", ccm->get_tx_allowance(cc), allowance - 1200))
        goto err;
    ...
    step_time(100);
    if (!test_true("../test/quic_cc_test.c", 544, "ccm->on_data_acked(cc, &ack_info)", (ccm->on_data_acked(cc, &ack_info)) != 0))
        goto err;


    if (!test_uint64_t_ge("../test/quic_cc_test.c", 548, "allowance2 = ccm->get_tx_allowance(cc)", "allowance", allowance2 = ccm->get_tx_allowance(cc), allowance))
        goto err;


    if (!test_true("../test/quic_cc_test.c", 552, "ccm->on_data_sent(cc, 1200)", (ccm->on_data_sent(cc, 1200)) != 0))
        goto err;


    if (!test_uint64_t_eq("../test/quic_cc_test.c", 556, "ccm->get_tx_allowance(cc)", "allowance - 1200", ccm->get_tx_allowance(cc), allowance - 1200))
        goto err;

    if (!test_true("../test/quic_cc_test.c", 559, "ccm->on_data_invalidated(cc, 1200)", (ccm->on_data_invalidated(cc, 1200)) != 0))
        goto err;


    if (!test_uint64_t_eq("../test/quic_cc_test.c", 563, "ccm->get_tx_allowance(cc)", "allowance2", ccm->get_tx_allowance(cc), allowance2))
        goto err;


    if (!test_uint64_t_ge("../test/quic_cc_test.c", 567, "allowance = ccm->get_tx_allowance(cc)", "1200 + 1300", allowance = ccm->get_tx_allowance(cc), 1200 + 1300))
        goto err;

    if (!test_true("../test/quic_cc_test.c", 570, "ccm->on_data_sent(cc, 1200)", (ccm->on_data_sent(cc, 1200)) != 0))
        goto err;

    if (!test_true("../test/quic_cc_test.c", 573, "ccm->on_data_sent(cc, 1300)", (ccm->on_data_sent(cc, 1300)) != 0))
        goto err;

    if (!test_uint64_t_eq("../test/quic_cc_test.c", 576, "allowance2 = ccm->get_tx_allowance(cc)", "allowance - 1200 - 1300", allowance2 = ccm->get_tx_allowance(cc), allowance - 1200 - 1300)
                                                  )
        goto err;
    ...
    if (!test_true("../test/quic_cc_test.c", 584, "ccm->on_data_lost(cc, &loss_info)", (ccm->on_data_lost(cc, &loss_info)) != 0))
        goto err;

    loss_info.tx_size = 1300;
    if (!test_true("../test/quic_cc_test.c", 588, "ccm->on_data_lost(cc, &loss_info)", (ccm->on_data_lost(cc, &loss_info)) != 0))
        goto err;

    if (!test_true("../test/quic_cc_test.c", 591, "ccm->on_data_lost_finished(cc, 0)", (ccm->on_data_lost_finished(cc, 0)) != 0))
        goto err;


    if (!test_uint64_t_ne("../test/quic_cc_test.c", 595, "ccm->get_tx_allowance(cc)", "allowance2", ccm->get_tx_allowance(cc), allowance2))
        goto err;


    if (!test_uint64_t_lt("../test/quic_cc_test.c", 599, "ccm->get_tx_allowance(cc)", "allowance", ccm->get_tx_allowance(cc), allowance))
        goto err;
...
err:
    if (cc != 
# 605 "../test/quic_cc_test.c" 3 4
             ((void *)0)
# 605 "../test/quic_cc_test.c"
                 )
        ccm->free(cc);
