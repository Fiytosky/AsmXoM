# 0 "../test/ext_internal_test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/ext_internal_test.c"
# 10 "../test/ext_internal_test.c"
# 1 "../include/internal/nelem.h" 1
# 12 "../include/internal/nelem.h"
        
# 11 "../test/ext_internal_test.c" 2
...
static EXT_LIST ext_list[] = {
    ...
    { TLSEXT_IDX_renegotiate, 0xff01, "renegotiate" },
    { TLSEXT_IDX_server_name, 0, "server_name" },
    { TLSEXT_IDX_max_fragment_length, 1, "max_fragment_length" },

    { TLSEXT_IDX_srp, 12, "srp" },



    { TLSEXT_IDX_ec_point_formats, 11, "ec_point_formats" },
    { TLSEXT_IDX_supported_groups, 10, "supported_groups" },
    { TLSEXT_IDX_session_ticket, 35, "session_ticket" },

    { TLSEXT_IDX_status_request, 5, "status_request" },




    { TLSEXT_IDX_next_proto_neg, 13172, "next_proto_neg" },



    { TLSEXT_IDX_application_layer_protocol_negotiation, 16, "application_layer_protocol_negotiation" },

    { TLSEXT_IDX_use_srtp, 14, "use_srtp" },



    { TLSEXT_IDX_encrypt_then_mac, 22, "encrypt_then_mac" },

    { TLSEXT_IDX_signed_certificate_timestamp, 18, "signed_certificate_timestamp" },



    { TLSEXT_IDX_extended_master_secret, 23, "extended_master_secret" },
    { TLSEXT_IDX_signature_algorithms_cert, 50, "signature_algorithms_cert" },
    { TLSEXT_IDX_post_handshake_auth, 49, "post_handshake_auth" },
    { TLSEXT_IDX_client_cert_type, 19, "client_cert_type" },
    { TLSEXT_IDX_server_cert_type, 20, "server_cert_type" },
    { TLSEXT_IDX_signature_algorithms, 13, "signature_algorithms" },
    { TLSEXT_IDX_supported_versions, 43, "supported_versions" },
    { TLSEXT_IDX_psk_kex_modes, 45, "psk_kex_modes" },
    { TLSEXT_IDX_key_share, 51, "key_share" },
    { TLSEXT_IDX_cookie, 44, "cookie" },
    { TLSEXT_IDX_cryptopro_bug, 0xfde8, "cryptopro_bug" },
    { TLSEXT_IDX_compress_certificate, 27, "compress_certificate" },
    { TLSEXT_IDX_early_data, 42, "early_data" },
    { TLSEXT_IDX_certificate_authorities, 47, "certificate_authorities" },
    { TLSEXT_IDX_padding, 21, "padding" },
    { TLSEXT_IDX_psk, 41, "psk" },
    { TLSEXT_IDX_num_builtins, 0x10001, "num_builtins" }
};

static int test_extension_list(void)
{
    size_t n = (sizeof(ext_list)/sizeof((ext_list)[0]));
    size_t i;
    unsigned int type;
    int retval = 1;

    for (i = 0; i < n; i++) {
        if (!test_size_t_eq("../test/ext_internal_test.c", 88, "i", "ext_list[i].idx", i, ext_list[i].idx)) {
            retval = 0;
            test_error("../test/ext_internal_test.c", 90, "TLSEXT_IDX_%s=%zd, found at=%zd\n", ext_list[i].name, ext_list[i].idx, i)
                                                            ;
        }
        type = ossl_get_extension_type(ext_list[i].idx);
        if (!test_uint_eq("../test/ext_internal_test.c", 94, "type", "ext_list[i].type", type, ext_list[i].type)) {
            retval = 0;
            test_error("../test/ext_internal_test.c", 96, "TLSEXT_IDX_%s=%zd expected=0x%05X got=0x%05X", ext_list[i].name, ext_list[i].idx, ext_list[i].type, type)
...
int setup_tests(void)
{
    add_test("test_extension_list", test_extension_list);
    return 1;
}
