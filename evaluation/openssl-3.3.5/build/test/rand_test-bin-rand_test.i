...
int RAND_poll(void);
# 12 "../test/rand_test.c" 2

# 1 "include/openssl/core_names.h" 1
# 16 "include/openssl/core_names.h"
        
# 14 "../test/rand_test.c" 2
...
static int test_rand(void)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!test_ptr("../test/rand_test.c", 30, "privctx = RAND_get0_private(NULL)", privctx = RAND_get0_private(
# 30 "../test/rand_test.c" 3 4
        ((void *)0)
# 30 "../test/rand_test.c"
        ))
            || !test_true("../test/rand_test.c", 31, "EVP_RAND_CTX_set_params(privctx, params)", (EVP_RAND_CTX_set_params(privctx, params)) != 0)
            || !test_int_gt("../test/rand_test.c", 32, "RAND_priv_bytes(outbuf, sizeof(outbuf))", "0", RAND_priv_bytes(outbuf, sizeof(outbuf)), 0)
            || !test_mem_eq("../test/rand_test.c", 33, "outbuf", "entropy1", outbuf, sizeof(outbuf), entropy1, sizeof(outbuf))
            || !test_int_le("../test/rand_test.c", 34, "RAND_priv_bytes(outbuf, sizeof(outbuf) + 1)", "0", RAND_priv_bytes(outbuf, sizeof(outbuf) + 1), 0)
            || !test_int_gt("../test/rand_test.c", 35, "RAND_priv_bytes(outbuf, sizeof(outbuf))", "0", RAND_priv_bytes(outbuf, sizeof(outbuf)), 0)
            || !test_mem_eq("../test/rand_test.c", 36, "outbuf", "entropy1 + sizeof(outbuf)", outbuf, sizeof(outbuf), entropy1 + sizeof(outbuf), sizeof(outbuf))
                                                                      )
        return 0;

    *params = OSSL_PARAM_construct_octet_string("test_entropy",
                                                entropy2, sizeof(entropy2));
    if (!test_true("../test/rand_test.c", 42, "EVP_RAND_CTX_set_params(privctx, params)", (EVP_RAND_CTX_set_params(privctx, params)) != 0)
            || !test_int_gt("../test/rand_test.c", 43, "RAND_priv_bytes(outbuf, sizeof(outbuf))", "0", RAND_priv_bytes(outbuf, sizeof(outbuf)), 0)
            || !test_mem_eq("../test/rand_test.c", 44, "outbuf", "entropy2", outbuf, sizeof(outbuf), entropy2, sizeof(outbuf)))
        return 0;

    *params = OSSL_PARAM_construct_octet_string("test_nonce",
                                                nonce, sizeof(nonce));
    if (!test_true("../test/rand_test.c", 49, "EVP_RAND_CTX_set_params(privctx, params)", (EVP_RAND_CTX_set_params(privctx, params)) != 0)
            || !test_true("../test/rand_test.c", 50, "EVP_RAND_nonce(privctx, outbuf, sizeof(outbuf))", (EVP_RAND_nonce(privctx, outbuf, sizeof(outbuf))) != 0)
            || !test_mem_eq("../test/rand_test.c", 51, "outbuf", "nonce", outbuf, sizeof(outbuf), nonce, sizeof(outbuf)))
        return 0;
    return 1;
}
...
static int test_rand_uniform(void)
{
    ...
    if (!test_get_libctx(&ctx, 
# 62 "../test/rand_test.c" 3 4
                              ((void *)0)
# 62 "../test/rand_test.c"
                                  , 
# 62 "../test/rand_test.c" 3 4
...
# 62 "../test/rand_test.c"
    ...
    for (i = 1; i < 100; i += 13) {
        ...
        if (!test_int_eq("../test/rand_test.c", 67, "err", "0", err, 0)
                || !test_uint_ge("../test/rand_test.c", 68, "x", "0", x, 0)
                || !test_uint_lt("../test/rand_test.c", 69, "x", "i", x, i))
            return 0;
    }
    for (i = 1; i < 100; i += 17)
        for (j = i + 1; j < 150; j += 11) {
            x = ossl_rand_range_uint32(ctx, i, j, &err);
            if (!test_int_eq("../test/rand_test.c", 75, "err", "0", err, 0)
                    || !test_uint_ge("../test/rand_test.c", 76, "x", "i", x, i)
                    || !test_uint_lt("../test/rand_test.c", 77, "x", "j", x, j))
                return 0;
        }
...
int setup_tests(void)
{
    if (!test_true("../test/rand_test.c", 89, "RAND_set_DRBG_type(NULL, \"TEST-RAND\", NULL, NULL, NULL)", (RAND_set_DRBG_type(
# 89 "../test/rand_test.c" 3 4
        ((void *)0)
# 89 "../test/rand_test.c"
        , "TEST-RAND", 
# 89 "../test/rand_test.c" 3 4
