# 0 "../test/clienthellotest.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../test/clienthellotest.c"
# 10 "../test/clienthellotest.c"
...
# 23 "../test/clienthellotest.c" 2
# 46 "../test/clienthellotest.c"
static const char *sessionfile = 
# 46 "../test/clienthellotest.c" 3 4
                                ((void *)0)
# 46 "../test/clienthellotest.c"
                                    ;
...
static int test_client_hello(int currtest)
{
    SSL_CTX *ctx;
    SSL *con = 
# 62 "../test/clienthellotest.c" 3 4
              ((void *)0)
# 62 "../test/clienthellotest.c"
                  ;
    BIO *rbio;
    BIO *wbio;
    long len;
    ...
    size_t msglen;
    BIO *sessbio = 
# 72 "../test/clienthellotest.c" 3 4
                  ((void *)0)
# 72 "../test/clienthellotest.c"
                      ;
    SSL_SESSION *sess = 
# 73 "../test/clienthellotest.c" 3 4
                       ((void *)0)
# 73 "../test/clienthellotest.c"
                           ;
    ...
    if (!test_ptr("../test/clienthellotest.c", 89, "ctx", ctx))
        goto end;
    if (!test_true("../test/clienthellotest.c", 91, "SSL_CTX_set_max_proto_version(ctx, 0)", (SSL_CTX_ctrl(ctx, 124, 0, 
# 91 "../test/clienthellotest.c" 3 4
        ((void *)0)
# 91 "../test/clienthellotest.c"
        )) != 0))
        goto end;
    ...
    case 0:
        ...
        if (!test_true("../test/clienthellotest.c", 102, "SSL_CTX_set_max_proto_version(ctx, TLS1_2_VERSION)", (SSL_CTX_ctrl(ctx, 124, 0x0303, 
# 102 "../test/clienthellotest.c" 3 4
            ((void *)0)
# 102 "../test/clienthellotest.c"
            )) != 0))
            goto end;
    ...
    case 3:
        ...
        if (!test_false("../test/clienthellotest.c", 113, "SSL_CTX_set_cipher_list(ctx, \"\")", (SSL_CTX_set_cipher_list(ctx, "")) != 0))
            goto end;
        ERR_clear_error();

    case 1:
    case 2:
        SSL_CTX_set_options(ctx, ((uint64_t)1 << (uint64_t)4));

        SSL_CTX_clear_options(ctx, ((uint64_t)1 << (uint64_t)20));
        ...
        if (currtest == 1) {
             if (!test_false("../test/clienthellotest.c", 128, "SSL_CTX_set_alpn_protos(ctx, (unsigned char *)alpn_prots, sizeof(alpn_prots) - 1)", (SSL_CTX_set_alpn_protos(ctx, (unsigned char *)alpn_prots, sizeof(alpn_prots) - 1)) != 0)
        ...
        } else if (!test_true("../test/clienthellotest.c", 136, "SSL_CTX_set_cipher_list(ctx, \"AES128-SHA\")", (SSL_CTX_set_cipher_list(ctx, "AES128-SHA")) != 0)

                   || !test_true("../test/clienthellotest.c", 138, "SSL_CTX_set_ciphersuites(ctx, \"TLS_AES_128_GCM_SHA256\")", (SSL_CTX_set_ciphersuites(ctx, "TLS_AES_128_GCM_SHA256")) != 0)
                                                           ) {
            goto end;
        }
        break;
    ...
    con = SSL_new(ctx);
    if (!test_ptr("../test/clienthellotest.c", 149, "con", con))
        goto end;

    if (currtest == 3) {
        sessbio = BIO_new_file(sessionfile, "r");
        if (!test_ptr("../test/clienthellotest.c", 154, "sessbio", sessbio)) {
            test_info("../test/clienthellotest.c", 155, "Unable to open session.pem");
            goto end;
        }
        sess = PEM_read_bio_SSL_SESSION(sessbio, 
# 158 "../test/clienthellotest.c" 3 4
                                                ((void *)0)
# 158 "../test/clienthellotest.c"
                                                    , 
# 158 "../test/clienthellotest.c" 3 4
                                                      ((void *)0)
# 158 "../test/clienthellotest.c"
...
# 158 "../test/clienthellotest.c"
        ...
        if (!test_ptr("../test/clienthellotest.c", 159, "sess", sess)) {
            test_info("../test/clienthellotest.c", 160, "Unable to load SSL_SESSION");
            goto end;
        }
        ...
        if (!test_true("../test/clienthellotest.c", 167, "SSL_SESSION_set_time(sess, (long)time(NULL))", (SSL_SESSION_set_time(sess, (long)time(
# 167 "../test/clienthellotest.c" 3 4
            ((void *)0)
# 167 "../test/clienthellotest.c"
            ))) != 0)
                || !test_true("../test/clienthellotest.c", 168, "SSL_set_session(con, sess)", (SSL_set_session(con, sess)) != 0))
            goto end;
    }

    rbio = BIO_new(BIO_s_mem());
    wbio = BIO_new(BIO_s_mem());
    if (!test_ptr("../test/clienthellotest.c", 174, "rbio", rbio)|| !test_ptr("../test/clienthellotest.c", 174, "wbio", wbio)) {
        BIO_free(rbio);
        BIO_free(wbio);
        goto end;
    }
    ...
    if (currtest == 0) {
        if (!test_true("../test/clienthellotest.c", 184, "SSL_set_session_ticket_ext(con, dummytick, strlen(dummytick))", (SSL_set_session_ticket_ext(con, dummytick, strlen(dummytick))) != 0)
                                                                     )
            goto end;
    }

    if (!test_int_le("../test/clienthellotest.c", 189, "SSL_connect(con)", "0", SSL_connect(con), 0)) {

        goto end;
    }

    if (!test_long_ge("../test/clienthellotest.c", 194, "len = BIO_get_mem_data(wbio, (char **)&data)", "0", len = BIO_ctrl(wbio,3,0,(char *)((char **)&data)), 0)
            || !test_true("../test/clienthellotest.c", 195, "PACKET_buf_init(&pkt, data, len)", (PACKET_buf_init(&pkt, data, len)) != 0)
    ...
    if (!test_true("../test/clienthellotest.c", 203, "PACKET_forward(&pkt, SSL3_HM_HEADER_LENGTH)", (PACKET_forward(&pkt, 4)) != 0)

            || !test_true("../test/clienthellotest.c", 205, "PACKET_forward(&pkt, CLIENT_VERSION_LEN + SSL3_RANDOM_SIZE)", (PACKET_forward(&pkt, 2 + 32)) != 0)


            || !test_true("../test/clienthellotest.c", 208, "PACKET_get_length_prefixed_1(&pkt, &pkt2)", (PACKET_get_length_prefixed_1(&pkt, &pkt2)) != 0)

            || !test_true("../test/clienthellotest.c", 210, "PACKET_get_length_prefixed_2(&pkt, &pkt2)", (PACKET_get_length_prefixed_2(&pkt, &pkt2)) != 0)

            || !test_true("../test/clienthellotest.c", 212, "PACKET_get_length_prefixed_1(&pkt, &pkt2)", (PACKET_get_length_prefixed_1(&pkt, &pkt2)) != 0)

            || !test_true("../test/clienthellotest.c", 214, "PACKET_as_length_prefixed_2(&pkt, &pkt2)", (PACKET_as_length_prefixed_2(&pkt, &pkt2)) != 0))
        goto end;


    while (PACKET_remaining(&pkt2)) {

        if (!test_true("../test/clienthellotest.c", 220, "PACKET_get_net_2(&pkt2, &type)", (PACKET_get_net_2(&pkt2, &type)) != 0)
                || !test_true("../test/clienthellotest.c", 221, "PACKET_get_length_prefixed_2(&pkt2, &pkt3)", (PACKET_get_length_prefixed_2(&pkt2, &pkt3)) != 0))
            goto end;

        if (type == 35) {
            if (currtest == 0) {
                if (test_true("../test/clienthellotest.c", 226, "PACKET_equal(&pkt3, dummytick, strlen(dummytick))", (PACKET_equal(&pkt3, dummytick, strlen(dummytick))) != 0)
                                                              ) {
        ...
        if (type == 21) {
            if (!test_false("../test/clienthellotest.c", 235, "currtest == TEST_PADDING_NOT_NEEDED", (currtest == 2) != 0))
                goto end;
            else if (test_true("../test/clienthellotest.c", 237, "currtest == TEST_ADD_PADDING || currtest == TEST_ADD_PADDING_AND_PSK", (currtest == 1 || currtest == 3) != 0)
                                                            )
                testresult = test_true("../test/clienthellotest.c", 239, "msglen == F5_WORKAROUND_MAX_MSG_LEN", (msglen == 0x200) != 0);
        }
    }
...
const OPTIONS *test_get_options(void) { enum { OPT_TEST_HELP = 500, OPT_TEST_LIST, OPT_TEST_SINGLE, OPT_TEST_ITERATION, OPT_TEST_INDENT, OPT_TEST_SEED }; static const OPTIONS options[] = { { OPT_HELP_STR, 1, '-', "Usage: %s [options] " "sessionfile\n" }, { OPT_HELP_STR, 1, '-', "Valid options are:\n" }, { "help", OPT_TEST_HELP, '-', "Display this summary" }, { "list", OPT_TEST_LIST, '-', "Display the list of tests available" }, { "test", OPT_TEST_SINGLE, 's', "Run a single test by id or name" }, { "iter", OPT_TEST_ITERATION, 'n', "Run a single iteration of a test" }, { "indent", OPT_TEST_INDENT,'p', "Number of tabs added to output" }, { "seed", OPT_TEST_SEED, 'n', "Seed value to randomize tests with" }, { 
# 255 "../test/clienthellotest.c" 3 4
((void *)0) 
# 255 "../test/clienthellotest.c"
} }; return options; }

int setup_tests(void)
{
    if (!test_skip_common_options()) {
        test_error("../test/clienthellotest.c", 260, "Error parsing test options\n");
        return 0;
    }

    if (!test_ptr("../test/clienthellotest.c", 264, "sessionfile = test_get_argument(0)", sessionfile = test_get_argument(0)))
        return 0;

    add_all_tests("test_client_hello", test_client_hello, 4, 1);
    return 1;
}
