...
static int test_sec_mem(void)
{
    ...
    int testresult = 0;
    char *p = 
# 19 "../test/secmemtest.c" 3 4
             ((void *)0)
# 19 "../test/secmemtest.c"
                 , *q = 
# 19 "../test/secmemtest.c" 3 4
                        ((void *)0)
# 19 "../test/secmemtest.c"
...
# 19 "../test/secmemtest.c"
    ...
    test_info("../test/secmemtest.c", 21, "Secure memory is implemented.");

    s = CRYPTO_secure_malloc(20, "../test/secmemtest.c", 23);

    if (!test_ptr("../test/secmemtest.c", 25, "s", s)
        || !test_false("../test/secmemtest.c", 26, "CRYPTO_secure_allocated(s)", (CRYPTO_secure_allocated(s)) != 0))
        goto end;
    r = CRYPTO_secure_malloc(20, "../test/secmemtest.c", 28);

    if (!test_ptr("../test/secmemtest.c", 30, "r", r)
        || !test_true("../test/secmemtest.c", 31, "CRYPTO_secure_malloc_init(4096, 32)", (CRYPTO_secure_malloc_init(4096, 32)) != 0)
        || !test_false("../test/secmemtest.c", 32, "CRYPTO_secure_allocated(r)", (CRYPTO_secure_allocated(r)) != 0))
        goto end;
    p = CRYPTO_secure_malloc(20, "../test/secmemtest.c", 34);
    if (!test_ptr("../test/secmemtest.c", 35, "p", p)

        || !test_true("../test/secmemtest.c", 37, "CRYPTO_secure_allocated(p)", (CRYPTO_secure_allocated(p)) != 0)

        || !test_size_t_eq("../test/secmemtest.c", 39, "CRYPTO_secure_used()", "32", CRYPTO_secure_used(), 32))
        goto end;
    q = CRYPTO_malloc(20, "../test/secmemtest.c", 41);
    if (!test_ptr("../test/secmemtest.c", 42, "q", q))
        goto end;

    if (!test_false("../test/secmemtest.c", 45, "CRYPTO_secure_allocated(q)", (CRYPTO_secure_allocated(q)) != 0))
        goto end;
    CRYPTO_secure_clear_free(s, 20, "../test/secmemtest.c", 47);
    s = CRYPTO_secure_malloc(20, "../test/secmemtest.c", 48);
    if (!test_ptr("../test/secmemtest.c", 49, "s", s)

        || !test_true("../test/secmemtest.c", 51, "CRYPTO_secure_allocated(s)", (CRYPTO_secure_allocated(s)) != 0)

        || !test_size_t_eq("../test/secmemtest.c", 53, "CRYPTO_secure_used()", "64", CRYPTO_secure_used(), 64))
        goto end;
    CRYPTO_secure_clear_free(p, 20, "../test/secmemtest.c", 55);
    p = 
# 56 "../test/secmemtest.c" 3 4
       ((void *)0)
# 56 "../test/secmemtest.c"
           ;

    if (!test_size_t_eq("../test/secmemtest.c", 58, "CRYPTO_secure_used()", "32", CRYPTO_secure_used(), 32))
        goto end;
    CRYPTO_free(q, "../test/secmemtest.c", 60);
    q = 
# 61 "../test/secmemtest.c" 3 4
       ((void *)0)
# 61 "../test/secmemtest.c"
           ;

    if (!test_false("../test/secmemtest.c", 63, "CRYPTO_secure_malloc_done()", (CRYPTO_secure_malloc_done()) != 0)
        || !test_true("../test/secmemtest.c", 64, "CRYPTO_secure_malloc_initialized()", (CRYPTO_secure_malloc_initialized()) != 0))
        goto end;
    CRYPTO_secure_free(s, "../test/secmemtest.c", 66);
    s = 
# 67 "../test/secmemtest.c" 3 4
       ((void *)0)
# 67 "../test/secmemtest.c"
           ;

    if (!test_size_t_eq("../test/secmemtest.c", 69, "CRYPTO_secure_used()", "0", CRYPTO_secure_used(), 0)
        || !test_true("../test/secmemtest.c", 70, "CRYPTO_secure_malloc_done()", (CRYPTO_secure_malloc_done()) != 0)
        || !test_false("../test/secmemtest.c", 71, "CRYPTO_secure_malloc_initialized()", (CRYPTO_secure_malloc_initialized()) != 0))
        goto end;

    test_info("../test/secmemtest.c", 74, "Possible infinite loop: allocate more than available");
    if (!test_true("../test/secmemtest.c", 75, "CRYPTO_secure_malloc_init(32768, 16)", (CRYPTO_secure_malloc_init(32768, 16)) != 0))
        goto end;
    test_ptr_null("../test/secmemtest.c", 77, "OPENSSL_secure_malloc((size_t)-1)", CRYPTO_secure_malloc((size_t)-1, "../test/secmemtest.c", 77));
    test_true("../test/secmemtest.c", 78, "CRYPTO_secure_malloc_done()", (CRYPTO_secure_malloc_done()) != 0);





    if (test_false("../test/secmemtest.c", 84, "CRYPTO_secure_malloc_init(16, 16)", (CRYPTO_secure_malloc_init(16, 16)) != 0) &&
        !test_false("../test/secmemtest.c", 85, "CRYPTO_secure_malloc_initialized()", (CRYPTO_secure_malloc_initialized()) != 0)) {
        test_true("../test/secmemtest.c", 86, "CRYPTO_secure_malloc_done()", (CRYPTO_secure_malloc_done()) != 0);
        goto end;
    }
# 122 "../test/secmemtest.c"
    testresult = 1;
 end:
    CRYPTO_secure_free(p, "../test/secmemtest.c", 124);
    CRYPTO_free(q, "../test/secmemtest.c", 125);
    CRYPTO_secure_free(r, "../test/secmemtest.c", 126);
    CRYPTO_secure_free(s, "../test/secmemtest.c", 127);
    return testresult;
...
static int test_sec_mem_clear(void)
{
    ...
    const int size = 64;
    unsigned char *p = 
# 140 "../test/secmemtest.c" 3 4
                      ((void *)0)
# 140 "../test/secmemtest.c"
                          ;
    int i, res = 0;

    if (!test_true("../test/secmemtest.c", 143, "CRYPTO_secure_malloc_init(4096, 32)", (CRYPTO_secure_malloc_init(4096, 32)) != 0)
            || !test_ptr("../test/secmemtest.c", 144, "p = OPENSSL_secure_malloc(size)", p = CRYPTO_secure_malloc(size, "../test/secmemtest.c", 144)))
        goto err;

    for (i = 0; i < size; i++)
        if (!test_uchar_eq("../test/secmemtest.c", 148, "p[i]", "0", p[i], 0))
            goto err;

    for (i = 0; i < size; i++)
        p[i] = (unsigned char)(i + ' ' + 1);

    CRYPTO_secure_free(p, "../test/secmemtest.c", 154);
# 163 "../test/secmemtest.c"
    for (i = sizeof(void *) * 2; i < size; i++)
        if (!test_uchar_eq("../test/secmemtest.c", 164, "p[i]", "0", p[i], 0))
            return 0;

    res = 1;
    p = 
# 168 "../test/secmemtest.c" 3 4
       ((void *)0)
# 168 "../test/secmemtest.c"
           ;
err:
    CRYPTO_secure_free(p, "../test/secmemtest.c", 170);
...
int setup_tests(void)
{
    ...
    add_test("test_sec_mem_clear", test_sec_mem_clear);
    return 1;
}
