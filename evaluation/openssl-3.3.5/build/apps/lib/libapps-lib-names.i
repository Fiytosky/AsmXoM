...
void collect_names(const char *name, void *vdata)
{
    struct stack_st_OPENSSL_CSTRING *names = vdata;

    OPENSSL_sk_push(ossl_check_OPENSSL_CSTRING_sk_type(names), ossl_check_OPENSSL_CSTRING_type(name));
}

void print_names(BIO *out, struct stack_st_OPENSSL_CSTRING *names)
{
    int i = OPENSSL_sk_num(ossl_check_const_OPENSSL_CSTRING_sk_type(names));
    int j;
    ...
    if (i > 1)
        BIO_printf(out, "{ ");
    for (j = 0; j < i; j++) {
        const char *name = ((const char *)OPENSSL_sk_value(ossl_check_const_OPENSSL_CSTRING_sk_type(names), (j)));
