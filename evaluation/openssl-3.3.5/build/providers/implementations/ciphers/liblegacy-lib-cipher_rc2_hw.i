# 0 "../providers/implementations/ciphers/cipher_rc2_hw.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../providers/implementations/ciphers/cipher_rc2_hw.c"
# 14 "../providers/implementations/ciphers/cipher_rc2_hw.c"
...
# 15 "../providers/implementations/ciphers/cipher_rc2_hw.c" 2
...
# 17 "../providers/implementations/ciphers/cipher_rc2_hw.c" 2
...
# 40 "../providers/implementations/ciphers/cipher_rc2_hw.c"
static int cipher_hw_rc2_cbc_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len) { RC2_KEY *key = &(((PROV_RC2_CTX *)ctx)->ks.ks); while (len >= ((size_t)1 << 30)) { RC2_cbc_encrypt(in, out, ((size_t)1 << 30), key, ctx->iv, ctx->enc); len -= ((size_t)1 << 30); in += ((size_t)1 << 30); out += ((size_t)1 << 30); } if (len > 0) RC2_cbc_encrypt(in, out, (long)len, key, ctx->iv, ctx->enc); return 1; } static const PROV_CIPHER_HW rc2_cbc = { cipher_hw_rc2_initkey, cipher_hw_rc2_cbc_cipher }; const PROV_CIPHER_HW *ossl_prov_cipher_hw_rc2_cbc(size_t keybits) { return &rc2_cbc; }
static int cipher_hw_rc2_ecb_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len) { size_t i, bl = ctx->blocksize; RC2_KEY *key = &(((PROV_RC2_CTX *)ctx)->ks.ks); if (len < bl) return 1; for (i = 0, len -= bl; i <= len; i += bl) RC2_ecb_encrypt(in + i, out + i, key, ctx->enc); return 1; } static const PROV_CIPHER_HW rc2_ecb = { cipher_hw_rc2_initkey, cipher_hw_rc2_ecb_cipher }; const PROV_CIPHER_HW *ossl_prov_cipher_hw_rc2_ecb(size_t keybits) { return &rc2_ecb; }
static int cipher_hw_rc2_ofb64_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len) { int num = ctx->num; RC2_KEY *key = &(((PROV_RC2_CTX *)ctx)->ks.ks); while (len >= ((size_t)1 << 30)) { RC2_ofb64_encrypt(in, out, ((size_t)1 << 30), key, ctx->iv, &num); len -= ((size_t)1 << 30); in += ((size_t)1 << 30); out += ((size_t)1 << 30); } if (len > 0) { RC2_ofb64_encrypt(in, out, (long)len, key, ctx->iv, &num); } ctx->num = num; return 1; } static const PROV_CIPHER_HW rc2_ofb64 = { cipher_hw_rc2_initkey, cipher_hw_rc2_ofb64_cipher }; const PROV_CIPHER_HW *ossl_prov_cipher_hw_rc2_ofb64(size_t keybits) { return &rc2_ofb64; }
static int cipher_hw_rc2_cfb64_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len) { size_t chunk = ((size_t)1 << 30); RC2_KEY *key = &(((PROV_RC2_CTX *)ctx)->ks.ks); int num = ctx->num; if (len < chunk) chunk = len; while (len > 0 && len >= chunk) { RC2_cfb64_encrypt(in, out, (long)chunk, key, ctx->iv, &num, ctx->enc); len -= chunk; in += chunk; out += chunk; if (len < chunk) chunk = len; } ctx->num = num; return 1; } static const PROV_CIPHER_HW rc2_cfb64 = { cipher_hw_rc2_initkey, cipher_hw_rc2_cfb64_cipher }; const PROV_CIPHER_HW *ossl_prov_cipher_hw_rc2_cfb64(size_t keybits) { return &rc2_cfb64; }
