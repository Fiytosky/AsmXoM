# 0 "../providers/implementations/ciphers/cipher_idea_hw.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../providers/implementations/ciphers/cipher_idea_hw.c"
# 15 "../providers/implementations/ciphers/cipher_idea_hw.c"
...
# 16 "../providers/implementations/ciphers/cipher_idea_hw.c" 2
...
# 18 "../providers/implementations/ciphers/cipher_idea_hw.c" 2
...
# 54 "../providers/implementations/ciphers/cipher_idea_hw.c"
static int cipher_hw_idea_cbc_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len) { IDEA_KEY_SCHEDULE *key = &(((PROV_IDEA_CTX *)ctx)->ks.ks); while (len >= ((size_t)1 << 30)) { IDEA_cbc_encrypt(in, out, ((size_t)1 << 30), key, ctx->iv, ctx->enc); len -= ((size_t)1 << 30); in += ((size_t)1 << 30); out += ((size_t)1 << 30); } if (len > 0) IDEA_cbc_encrypt(in, out, (long)len, key, ctx->iv, ctx->enc); return 1; } static const PROV_CIPHER_HW idea_cbc = { cipher_hw_idea_initkey, cipher_hw_idea_cbc_cipher }; const PROV_CIPHER_HW *ossl_prov_cipher_hw_idea_cbc(size_t keybits) { return &idea_cbc; }
static int cipher_hw_idea_ofb64_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len) { int num = ctx->num; IDEA_KEY_SCHEDULE *key = &(((PROV_IDEA_CTX *)ctx)->ks.ks); while (len >= ((size_t)1 << 30)) { IDEA_ofb64_encrypt(in, out, ((size_t)1 << 30), key, ctx->iv, &num); len -= ((size_t)1 << 30); in += ((size_t)1 << 30); out += ((size_t)1 << 30); } if (len > 0) { IDEA_ofb64_encrypt(in, out, (long)len, key, ctx->iv, &num); } ctx->num = num; return 1; } static const PROV_CIPHER_HW idea_ofb64 = { cipher_hw_idea_initkey, cipher_hw_idea_ofb64_cipher }; const PROV_CIPHER_HW *ossl_prov_cipher_hw_idea_ofb64(size_t keybits) { return &idea_ofb64; }
static int cipher_hw_idea_cfb64_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len) { size_t chunk = ((size_t)1 << 30); IDEA_KEY_SCHEDULE *key = &(((PROV_IDEA_CTX *)ctx)->ks.ks); int num = ctx->num; if (len < chunk) chunk = len; while (len > 0 && len >= chunk) { IDEA_cfb64_encrypt(in, out, (long)chunk, key, ctx->iv, &num, ctx->enc); len -= chunk; in += chunk; out += chunk; if (len < chunk) chunk = len; } ctx->num = num; return 1; } static const PROV_CIPHER_HW idea_cfb64 = { cipher_hw_idea_initkey, cipher_hw_idea_cfb64_cipher }; const PROV_CIPHER_HW *ossl_prov_cipher_hw_idea_cfb64(size_t keybits) { return &idea_cfb64; }
...
static int cipher_hw_idea_ecb_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len) { size_t i, bl = ctx->blocksize; IDEA_KEY_SCHEDULE *key = &(((PROV_IDEA_CTX *)ctx)->ks.ks); if (len < bl) return 1; for (i = 0, len -= bl; i <= len; i += bl) IDEA_ecb_encrypt(in + i, out + i, key); return 1; } static const PROV_CIPHER_HW idea_ecb = { cipher_hw_idea_initkey, cipher_hw_idea_ecb_cipher }; const PROV_CIPHER_HW *ossl_prov_cipher_hw_idea_ecb(size_t keybits) { return &idea_ecb; }
