# 0 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c"
# 15 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c"
...
# 16 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c" 2
...
# 18 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c" 2
# 31 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c"
...
int RAND_poll(void);
# 32 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c" 2
...
int evp_pkey_decrypt_alloc(EVP_PKEY_CTX *ctx, unsigned char **outp,
                           ...
                           const unsigned char *in, size_t inlen);
# 33 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c" 2
...
# 34 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c" 2
...
# 43 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c" 3 4
                                  ((void *)0)
# 43 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c"
                                      , 
# 43 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c" 3 4
                                        ((void *)0)
# 43 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c"
                                          ...
                                          size_t keylen)
{
    ...
    SHA256_Init(&sctx->head);
    sctx->tail = sctx->head;
    sctx->md = sctx->head;

    ctx->payload_length = ((size_t)-1);

    vctx->removetlspad = 1;
    vctx->removetlsfixed = 32 + 16;
...
# 236 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c"
    ...
    for (i = 0; i < x4; i++) {
        ...
        blocks[i].d[0] = ({ u32 ret_=(mctx->A[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        mctx->A[i] = sctx->tail.h[0];
        blocks[i].d[1] = ({ u32 ret_=(mctx->B[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        mctx->B[i] = sctx->tail.h[1];
        blocks[i].d[2] = ({ u32 ret_=(mctx->C[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        mctx->C[i] = sctx->tail.h[2];
        blocks[i].d[3] = ({ u32 ret_=(mctx->D[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        mctx->D[i] = sctx->tail.h[3];
        blocks[i].d[4] = ({ u32 ret_=(mctx->E[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        mctx->E[i] = sctx->tail.h[4];
        blocks[i].d[5] = ({ u32 ret_=(mctx->F[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        mctx->F[i] = sctx->tail.h[5];
        blocks[i].d[6] = ({ u32 ret_=(mctx->G[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        mctx->G[i] = sctx->tail.h[6];
        blocks[i].d[7] = ({ u32 ret_=(mctx->H[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        mctx->H[i] = sctx->tail.h[7];
        blocks[i].c[32] = 0x80;
        blocks[i].d[15] = ({ u32 ret_=((64 + 32) * 8); asm ("bswapl %0" : "+r"(ret_)); ret_; });
# 337 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c"
        edges[i].ptr = blocks[i].c;
        edges[i].blocks = 1;
    }
    ...
    for (i = 0; i < x4; i++) {
        ...
        *(u32 *)(out + 0) = ({ u32 ret_=(mctx->A[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        *(u32 *)(out + 4) = ({ u32 ret_=(mctx->B[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        *(u32 *)(out + 8) = ({ u32 ret_=(mctx->C[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        *(u32 *)(out + 12) = ({ u32 ret_=(mctx->D[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        *(u32 *)(out + 16) = ({ u32 ret_=(mctx->E[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        *(u32 *)(out + 20) = ({ u32 ret_=(mctx->F[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        *(u32 *)(out + 24) = ({ u32 ret_=(mctx->G[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        *(u32 *)(out + 28) = ({ u32 ret_=(mctx->H[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        out += 32;
        len += 32;
                                        ...
                                        const unsigned char *in, size_t len)
{
    ...
    size_t sha_off = (16*4) - sctx->md.num;
        ...
        if (plen == ((size_t)-1))
            plen = len;
                 ...
                 ((plen + 32 +
                   16) & -16))
            return 0;
        else if (ctx->aux.tls_ver >= 0x0302)
            iv = 16;
# 432 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c"
        ...
        if (plen != len) {
            ...
            sctx->md = sctx->tail;
            sha256_update(&sctx->md, out + plen, 32);
            SHA256_Final(out + plen, &sctx->md);
        ...
        if (plen != ((size_t)-1)) {
            ...
            bitlen = sctx->md.Nl + (inp_len << 3);

            bitlen = ({ u32 ret_=(bitlen); asm ("bswapl %0" : "+r"(ret_)); ret_; });
# 558 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c"
            pmac->u[0] = 0;
            pmac->u[1] = 0;
            pmac->u[2] = 0;
            pmac->u[3] = 0;
...
# 643 "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c"
            len += 32;
            sctx->md = sctx->tail;
            sha256_update(&sctx->md, pmac->c, 32);
                                              ...
                                              unsigned char *aad_rec, int aad_len)
{
    ...
    if (ctx->base.enc) {
        ...
        if ((ctx->aux.tls_ver =
             p[aad_len - 4] << 8 | p[aad_len - 3]) >= 0x0302) {
            if (len < 16)
                return 0;
            len -= 16;
            p[aad_len - 2] = len >> 8;
    ...
    void *vctx)
{
    ...
    (void)((ctx->multiblock_max_send_fragment != 0) ? 0 : (OPENSSL_die("assertion failed: " "ctx->multiblock_max_send_fragment != 0", "../providers/implementations/ciphers/cipher_aes_cbc_hmac_sha256_hw.c", 760), 1));
    return (int)(5 + 16
