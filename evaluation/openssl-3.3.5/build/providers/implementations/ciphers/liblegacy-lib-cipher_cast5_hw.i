# 0 "../providers/implementations/ciphers/cipher_cast5_hw.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../providers/implementations/ciphers/cipher_cast5_hw.c"
# 14 "../providers/implementations/ciphers/cipher_cast5_hw.c"
...
# 15 "../providers/implementations/ciphers/cipher_cast5_hw.c" 2
...
# 17 "../providers/implementations/ciphers/cipher_cast5_hw.c" 2
...
# 39 "../providers/implementations/ciphers/cipher_cast5_hw.c"
static int cipher_hw_cast5_cbc_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len) { CAST_KEY *key = &(((PROV_CAST_CTX *)ctx)->ks.ks); while (len >= ((size_t)1 << 30)) { CAST_cbc_encrypt(in, out, ((size_t)1 << 30), key, ctx->iv, ctx->enc); len -= ((size_t)1 << 30); in += ((size_t)1 << 30); out += ((size_t)1 << 30); } if (len > 0) CAST_cbc_encrypt(in, out, (long)len, key, ctx->iv, ctx->enc); return 1; } static const PROV_CIPHER_HW cast5_cbc = { cipher_hw_cast5_initkey, cipher_hw_cast5_cbc_cipher }; const PROV_CIPHER_HW *ossl_prov_cipher_hw_cast5_cbc(size_t keybits) { return &cast5_cbc; }
static int cipher_hw_cast5_ecb_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len) { size_t i, bl = ctx->blocksize; CAST_KEY *key = &(((PROV_CAST_CTX *)ctx)->ks.ks); if (len < bl) return 1; for (i = 0, len -= bl; i <= len; i += bl) CAST_ecb_encrypt(in + i, out + i, key, ctx->enc); return 1; } static const PROV_CIPHER_HW cast5_ecb = { cipher_hw_cast5_initkey, cipher_hw_cast5_ecb_cipher }; const PROV_CIPHER_HW *ossl_prov_cipher_hw_cast5_ecb(size_t keybits) { return &cast5_ecb; }
static int cipher_hw_cast5_ofb64_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len) { int num = ctx->num; CAST_KEY *key = &(((PROV_CAST_CTX *)ctx)->ks.ks); while (len >= ((size_t)1 << 30)) { CAST_ofb64_encrypt(in, out, ((size_t)1 << 30), key, ctx->iv, &num); len -= ((size_t)1 << 30); in += ((size_t)1 << 30); out += ((size_t)1 << 30); } if (len > 0) { CAST_ofb64_encrypt(in, out, (long)len, key, ctx->iv, &num); } ctx->num = num; return 1; } static const PROV_CIPHER_HW cast5_ofb64 = { cipher_hw_cast5_initkey, cipher_hw_cast5_ofb64_cipher }; const PROV_CIPHER_HW *ossl_prov_cipher_hw_cast5_ofb64(size_t keybits) { return &cast5_ofb64; }
static int cipher_hw_cast5_cfb64_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len) { size_t chunk = ((size_t)1 << 30); CAST_KEY *key = &(((PROV_CAST_CTX *)ctx)->ks.ks); int num = ctx->num; if (len < chunk) chunk = len; while (len > 0 && len >= chunk) { CAST_cfb64_encrypt(in, out, (long)chunk, key, ctx->iv, &num, ctx->enc); len -= chunk; in += chunk; out += chunk; if (len < chunk) chunk = len; } ctx->num = num; return 1; } static const PROV_CIPHER_HW cast5_cfb64 = { cipher_hw_cast5_initkey, cipher_hw_cast5_cfb64_cipher }; const PROV_CIPHER_HW *ossl_prov_cipher_hw_cast5_cfb64(size_t keybits) { return &cast5_cfb64; }
