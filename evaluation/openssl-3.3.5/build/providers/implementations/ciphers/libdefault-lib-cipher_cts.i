# 0 "../providers/implementations/ciphers/cipher_cts.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../providers/implementations/ciphers/cipher_cts.c"
# 49 "../providers/implementations/ciphers/cipher_cts.c"
# 1 "include/openssl/core_names.h" 1
# 16 "include/openssl/core_names.h"
        
# 50 "../providers/implementations/ciphers/cipher_cts.c" 2
...
int ossl_cipher_trailingdata(unsigned char *buf, size_t *buflen,
                             size_t blocksize,
                             const unsigned char **in, size_t *inlen);
# 51 "../providers/implementations/ciphers/cipher_cts.c" 2
...
# 53 "../providers/implementations/ciphers/cipher_cts.c" 2
# 61 "../providers/implementations/ciphers/cipher_cts.c"
typedef union {
    size_t align;
    unsigned char c[16];
} aligned_16bytes;
...
const char *ossl_cipher_cbc_cts_mode_id2name(unsigned int id)
{
    ...
    for (i = 0; i < (sizeof(cts_modes)/sizeof((cts_modes)[0])); ++i) {
        if (cts_modes[i].id == id)
            return cts_modes[i].name;
    }
    return 
# 86 "../providers/implementations/ciphers/cipher_cts.c" 3 4
          ((void *)0)
# 86 "../providers/implementations/ciphers/cipher_cts.c"
              ;
}
                                 ...
                                 unsigned char *out, size_t len)
{
    ...
    if (!ctx->hw->cipher(ctx, out - 16 + residue, tmp_in.c,
                         16))
        return 0;
    return len + residue;
}
                                 ...
                                 unsigned char *out, size_t len)
{
    ...
    memcpy(mid_iv.c, ctx->iv, 16);

    memcpy(cn.c, in + residue, 16);


    memset(ctx->iv, 0, 16);
    if (!ctx->hw->cipher(ctx, pt_last.c, in + residue, 16))
        return 0;
    ...
    memcpy(ct_mid.c + residue, pt_last.c + residue, 16 - residue);
    ...
    do_xor(ct_mid.c, pt_last.c, residue, out + 16);


    memcpy(ctx->iv, mid_iv.c, 16);





    if (!ctx->hw->cipher(ctx, out, ct_mid.c, 16))
        return 0;


    memcpy(ctx->iv, cn.c, 16);
    return len + 16 + residue;
}
...
# 236 "../providers/implementations/ciphers/cipher_cts.c"
                                 ...
                                 unsigned char *out, size_t len)
{
    ...
    memset(ctx->iv, 0, 16);
    if (!ctx->hw->cipher(ctx, pt_last.c, in, 16))
        return 0;
                                     ...
                                     size_t inl)
{
    ...
    if (outsize < inl)
        return 0;
    if (out == 
# 339 "../providers/implementations/ciphers/cipher_cts.c" 3 4
              ((void *)0)
# 339 "../providers/implementations/ciphers/cipher_cts.c"
                  ) {
        *outl = inl;
        return 1;
    }
    ...
    if (ctx->enc) {
        ...
        else if (ctx->cts_mode == 1)
            sz = cts128_cs2_encrypt(ctx, in, out, inl);
        else if (ctx->cts_mode == 2)
