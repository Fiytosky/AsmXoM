# 0 "../crypto/rand/rand_uniform.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../crypto/rand/rand_uniform.c"
# 10 "../crypto/rand/rand_uniform.c"
...
uint32_t ossl_rand_range_uint32(OSSL_LIB_CTX *ctx, uint32_t lower, uint32_t upper,
                                int *err);
# 11 "../crypto/rand/rand_uniform.c" 2
...
# 12 "../crypto/rand/rand_uniform.c" 2
# 25 "../crypto/rand/rand_uniform.c"
uint32_t ossl_rand_uniform_uint32(OSSL_LIB_CTX *ctx, uint32_t upper, int *err)
{
    uint32_t i, f;
    uint32_t f2, rand;
    ...
    if (!__builtin_expect(!!((upper > 0) != 0), 1)) {
        *err = 0;
        return 0;
    }
    if (__builtin_expect(!!(upper == 1), 0))
        return 0;
...
# 56 "../crypto/rand/rand_uniform.c"
    prod = (uint64_t)upper * rand;
    i = prod >> 32;
    f = prod & 0xffffffff;
    if (__builtin_expect(!!(f <= 1 + ~upper), 1))
        return i;
# 76 "../crypto/rand/rand_uniform.c"
    for (j = 0; j < max_followup_iterations; j++) {
        ...
        if (__builtin_expect(!!(f != 0xffffffff), 1))
            return i;
...
uint32_t ossl_rand_range_uint32(OSSL_LIB_CTX *ctx, uint32_t lower, uint32_t upper,
                                int *err)
{
    if (!__builtin_expect(!!((lower < upper) != 0), 1)) {
        *err = 1;
        return 0;
    }
