...
void OSSL_sleep(uint64_t millis)
{
    ...
    OSSL_TIME finish = ossl_time_add(now, ossl_ticks2time((millis) * (((uint64_t)1000000000) / 1000)));
    uint64_t left = millis;
# 102 "../crypto/sleep.c"
    do {
        ossl_sleep_millis(left);
        now = ossl_time_now();
        left = (ossl_time2ticks(ossl_time_subtract(finish, now)) / (((uint64_t)1000000000) / 1000));
