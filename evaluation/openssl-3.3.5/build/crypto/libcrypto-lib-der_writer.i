# 0 "../crypto/der_writer.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../crypto/der_writer.c"
# 10 "../crypto/der_writer.c"
...
# 15 "../crypto/der_writer.c" 2

static int int_start_context(WPACKET *pkt, int tag)
{
    if (tag < 0)
        return 1;
    if (!__builtin_expect(!!((tag <= 30) != 0), 1))
        return 0;
...
# 35 "../crypto/der_writer.c"
    ...
    return WPACKET_get_total_written(pkt, &size1)
        ...
        && (size1 == size2 || WPACKET_put_bytes__((pkt), (tag), 1));
}
...
int ossl_DER_w_boolean(WPACKET *pkt, int tag, int b)
{
    return int_start_context(pkt, tag)
        ...
        && (!b || WPACKET_put_bytes__((pkt), (0xFF), 1))
        && !WPACKET_close(pkt)
                             ...
                             const void *v)
{
    ...
    return int_start_context(pkt, tag)
        ...
        && ((top_byte & 0x80) == 0 || WPACKET_put_bytes__((pkt), (0), 1))
        && WPACKET_close(pkt)
...
{
    unsigned char *p = 
# 136 "../crypto/der_writer.c" 3 4
                      ((void *)0)
# 136 "../crypto/der_writer.c"
                          ;
    size_t n = ((BN_num_bits(v)+7)/8);


    *top_byte =
        ((bn_get_words(v) [(n - 1) / 8]) >> (8 * ((n - 1) % 8)))
        & 0xFF;

    if (!WPACKET_allocate_bytes(pkt, n, &p))
        return 0;
    if (p != 
# 146 "../crypto/der_writer.c" 3 4
            ((void *)0)
# 146 "../crypto/der_writer.c"
                )
        BN_bn2bin(v, p);
    return 1;
}
...
{
    if (v == 
# 153 "../crypto/der_writer.c" 3 4
            ((void *)0) 
# 153 "../crypto/der_writer.c"
                 || BN_is_negative(v))
        return 0;
    if (BN_is_zero(v))
...
# 190 "../crypto/der_writer.c"
    ...
    return WPACKET_get_total_written(pkt, &size1)
        ...
        && WPACKET_get_total_written(pkt, &size2)
        && (size1 == size2
            ? WPACKET_set_flags(pkt, 2)
            : WPACKET_put_bytes__((pkt), (0x20 | 16), 1))
        && int_end_context(pkt, tag);
}
