...
int ossl_a2ulabel(const char *in, char *out, size_t outlen)
{
    ...
    if (!__builtin_expect(!!((out != 
# 263 "../crypto/punycode.c" 3 4
        ((void *)0)
# 263 "../crypto/punycode.c"
        ) != 0), 1))
        return -1;
    ...
    while (1) {
        ...
        size_t delta = tmpptr != 
# 271 "../crypto/punycode.c" 3 4
                                ((void *)0) 
# 271 "../crypto/punycode.c"
                                     ? (size_t)(tmpptr - inptr) : strlen(inptr);

        if (!(strncmp(inptr, "xn--" "", sizeof("xn--") - 1) == 0)) {
        ...
        if (tmpptr == 
# 298 "../crypto/punycode.c" 3 4
                     ((void *)0)
# 298 "../crypto/punycode.c"
                         )
            break;

        if (!WPACKET_put_bytes__((&pkt), ('.'), 1))
            result = 0;

        inptr = tmpptr + 1;
    }

    if (!WPACKET_put_bytes__((&pkt), ('\0'), 1))
        result = 0;
 end:
    WPACKET_cleanup(&pkt);
    return result;
