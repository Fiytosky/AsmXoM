...
# 15 "../include/internal/thread_once.h" 2
# 14 "../crypto/trace.c" 2
...
const char *OSSL_trace_get_category_name(int num)
{
    if (num < 0 || (size_t)num >= (sizeof(trace_categories)/sizeof((trace_categories)[0])))
        return 
# 148 "../crypto/trace.c" 3 4
              ((void *)0)
# 148 "../crypto/trace.c"
                  ;
    ...
    if (!__builtin_expect(!!((trace_categories[num].name != 
# 153 "../crypto/trace.c" 3 4
        ((void *)0)
# 153 "../crypto/trace.c"
        ) != 0), 1)
        || !__builtin_expect(!!((trace_categories[num].num == num) != 0), 1))
        return 
# 155 "../crypto/trace.c" 3 4
              ((void *)0)
# 155 "../crypto/trace.c"
                  ;
...
int OSSL_trace_get_category_num(const char *name)
{
    ...
    if (name == 
# 163 "../crypto/trace.c" 3 4
               ((void *)0)
# 163 "../crypto/trace.c"
                   )
        return -1;

    for (i = 0; i < (sizeof(trace_categories)/sizeof((trace_categories)[0])); i++)
                      ...
                      const unsigned char *data, size_t size)
{
    ...
    if (!text) {
        for (i = 0; i < len; i++, data++)
            buf[i] = (char)*data != '\n' && (ossl_ctype_check(((int)*data), 0x40))
                ? ' ' : *data;
        if (len == 0 || data[-1] != '\n')
            buf[len++] = '\n';
        data = buf;
