                                               ...
                                               void *data, int joinable)
{
    ...
    if (routine == 
# 18 "../crypto/thread/arch.c" 3 4
                  ((void *)0)
# 18 "../crypto/thread/arch.c"
                      )
        return 
# 19 "../crypto/thread/arch.c" 3 4
              ((void *)0)
# 19 "../crypto/thread/arch.c"
                  ;

    handle = CRYPTO_zalloc(sizeof(*handle), "../crypto/thread/arch.c", 21);
    if (handle == 
# 22 "../crypto/thread/arch.c" 3 4
                 ((void *)0)
# 22 "../crypto/thread/arch.c"
                     )
        return 
# 23 "../crypto/thread/arch.c" 3 4
              ((void *)0)
# 23 "../crypto/thread/arch.c"
                  ;

    if ((handle->lock = ossl_crypto_mutex_new()) == 
# 25 "../crypto/thread/arch.c" 3 4
                                                   ((void *)0)
# 25 "../crypto/thread/arch.c"
                                                       )
        goto fail;
    if ((handle->statelock = ossl_crypto_mutex_new()) == 
# 27 "../crypto/thread/arch.c" 3 4
                                                        ((void *)0)
# 27 "../crypto/thread/arch.c"
                                                            )
        goto fail;
    if ((handle->condvar = ossl_crypto_condvar_new()) == 
# 29 "../crypto/thread/arch.c" 3 4
                                                        ((void *)0)
# 29 "../crypto/thread/arch.c"
                                                            )
        goto fail;
...
fail:
    ...
    return 
# 44 "../crypto/thread/arch.c" 3 4
          ((void *)0)
# 44 "../crypto/thread/arch.c"
              ;
}
...
int ossl_crypto_thread_native_join(CRYPTO_THREAD *thread, CRYPTO_THREAD_RETVAL *retval)
{
    ...
    if (thread == 
# 51 "../crypto/thread/arch.c" 3 4
                 ((void *)0)
# 51 "../crypto/thread/arch.c"
                     )
        return 0;

    ossl_crypto_mutex_lock(thread->statelock);
    req_state_mask = (1UL << 0) | (1UL << 2);
    while (!((thread)->state & (req_state_mask)))
        ossl_crypto_condvar_wait(thread->condvar, thread->statelock);

    if (((thread)->state & ((1UL << 2))))
        goto pass;


    while (((thread)->state & ((1UL << 1)))) {
        if (!((thread)->state & ((1UL << 2))))
            ossl_crypto_condvar_wait(thread->condvar, thread->statelock);
        if (((thread)->state & ((1UL << 2))))
            goto pass;
    }
    do { (thread)->state |= ((1UL << 1)); } while ((void)0, 0);
    ...
    ossl_crypto_mutex_lock(thread->statelock);
pass:
    do { (thread)->state &= ~(((1UL << 2)) << 16); } while ((void)0, 0);
    do { (thread)->state |= ((1UL << 2)); } while ((void)0, 0);
    ...
    if (retval != 
# 89 "../crypto/thread/arch.c" 3 4
                 ((void *)0)
# 89 "../crypto/thread/arch.c"
                     )
...
int ossl_crypto_thread_native_clean(CRYPTO_THREAD *handle)
{
    ...
    if (handle == 
# 110 "../crypto/thread/arch.c" 3 4
                 ((void *)0)
# 110 "../crypto/thread/arch.c"
                     )
        return 0;

    req_state_mask = 0;
    req_state_mask |= (1UL << 0);
    req_state_mask |= (1UL << 2);

    ossl_crypto_mutex_lock(handle->statelock);
    if (((handle)->state & (req_state_mask)) == 0) {
    ...
    CRYPTO_free(handle->handle, "../crypto/thread/arch.c", 128);
    CRYPTO_free(handle, "../crypto/thread/arch.c", 129);
