# 0 "../crypto/modes/ocb128.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../crypto/modes/ocb128.c"
# 10 "../crypto/modes/ocb128.c"
...
static OCB_BLOCK *ocb_lookup_l(OCB128_CONTEXT *ctx, size_t idx)
{
    ...
    if (idx >= ctx->max_l_index) {
        void *tmp_ptr;
# 112 "../crypto/modes/ocb128.c"
        ctx->max_l_index += (idx - ctx->max_l_index + 4) & ~3;
        tmp_ptr = CRYPTO_realloc(ctx->l, ctx->max_l_index * sizeof(OCB_BLOCK), "../crypto/modes/ocb128.c", 113);
        if (tmp_ptr == 
# 114 "../crypto/modes/ocb128.c" 3 4
                      ((void *)0)
# 114 "../crypto/modes/ocb128.c"
                          )
            return 
# 115 "../crypto/modes/ocb128.c" 3 4
                  ((void *)0)
# 115 "../crypto/modes/ocb128.c"
                      ;
        ctx->l = tmp_ptr;
    }
    while (l_index < idx) {
                                  ...
                                  ocb128_f stream)
{
    ...
    if ((octx = CRYPTO_malloc(sizeof(*octx), "../crypto/modes/ocb128.c", 137)) != 
# 137 "../crypto/modes/ocb128.c" 3 4
                                                 ((void *)0)
# 137 "../crypto/modes/ocb128.c"
                                                     ) {
        ret = CRYPTO_ocb128_init(octx, keyenc, keydec, encrypt, decrypt,
                                 stream);
        if (ret)
            return octx;
        CRYPTO_free(octx, "../crypto/modes/ocb128.c", 142);
    }

    return 
# 145 "../crypto/modes/ocb128.c" 3 4
          ((void *)0)
# 145 "../crypto/modes/ocb128.c"
              ;
}
                       ...
                       ocb128_f stream)
{
    ...
    ctx->l_index = 0;
    ctx->max_l_index = 5;
    if ((ctx->l = CRYPTO_malloc(ctx->max_l_index * 16, "../crypto/modes/ocb128.c", 158)) == 
# 158 "../crypto/modes/ocb128.c" 3 4
                                                           ((void *)0)
# 158 "../crypto/modes/ocb128.c"
                                                               )
        return 0;
                           ...
                           void *keyenc, void *keydec)
{
    ...
    if (keydec)
        dest->keydec = keydec;
    if (src->l) {
        if ((dest->l = CRYPTO_malloc(src->max_l_index * 16, "../crypto/modes/ocb128.c", 203)) == 
# 203 "../crypto/modes/ocb128.c" 3 4
                                                                ((void *)0)
# 203 "../crypto/modes/ocb128.c"
                                                                    )
            return 0;
                      ...
                      size_t len)
{
    ...
    for (i = ctx->sess.blocks_hashed + 1; i <= all_num_blocks; i++) {
        ...
        if (lookup == 
# 281 "../crypto/modes/ocb128.c" 3 4
                     ((void *)0)
# 281 "../crypto/modes/ocb128.c"
                         )
            return 0;
        ( (&ctx->sess.offset_aad)->a[0]=(&ctx->sess.offset_aad)->a[0]^(lookup)->a[0], (&ctx->sess.offset_aad)->a[1]=(&ctx->sess.offset_aad)->a[1]^(lookup)->a[1] );

        memcpy(tmp.c, aad, 16);
        aad += 16;


        ( (&tmp)->a[0]=(&ctx->sess.offset_aad)->a[0]^(&tmp)->a[0], (&tmp)->a[1]=(&ctx->sess.offset_aad)->a[1]^(&tmp)->a[1] );
        ctx->encrypt(tmp.c, tmp.c, ctx->keyenc);
        ( (&ctx->sess.sum)->a[0]=(&tmp)->a[0]^(&ctx->sess.sum)->a[0], (&ctx->sess.sum)->a[1]=(&tmp)->a[1]^(&ctx->sess.sum)->a[1] );
    }
    ...
    if (last_len > 0) {
        ...
        ( (&ctx->sess.offset_aad)->a[0]=(&ctx->sess.offset_aad)->a[0]^(&ctx->l_star)->a[0], (&ctx->sess.offset_aad)->a[1]=(&ctx->sess.offset_aad)->a[1]^(&ctx->l_star)->a[1] )
                                              ;
                          ...
                          size_t len)
{
    ...
    if (num_blocks && all_num_blocks == (size_t)all_num_blocks
        && ctx->stream != 
# 340 "../crypto/modes/ocb128.c" 3 4
                         ((void *)0)
# 340 "../crypto/modes/ocb128.c"
                             ) {
        ...
        while (top >>= 1)
            max_idx++;
        if (ocb_lookup_l(ctx, max_idx) == 
# 349 "../crypto/modes/ocb128.c" 3 4
                                         ((void *)0)
# 349 "../crypto/modes/ocb128.c"
                                             )
            return 0;
        ...
        for (i = ctx->sess.blocks_processed + 1; i <= all_num_blocks; i++) {
            ...
            lookup = ocb_lookup_l(ctx, ocb_ntz(i));
            if (lookup == 
# 363 "../crypto/modes/ocb128.c" 3 4
                         ((void *)0)
# 363 "../crypto/modes/ocb128.c"
                             )
                return 0;
            ( (&ctx->sess.offset)->a[0]=(&ctx->sess.offset)->a[0]^(lookup)->a[0], (&ctx->sess.offset)->a[1]=(&ctx->sess.offset)->a[1]^(lookup)->a[1] );

            memcpy(tmp.c, in, 16);
            in += 16;


            ( (&ctx->sess.checksum)->a[0]=(&tmp)->a[0]^(&ctx->sess.checksum)->a[0], (&ctx->sess.checksum)->a[1]=(&tmp)->a[1]^(&ctx->sess.checksum)->a[1] );


            ( (&tmp)->a[0]=(&ctx->sess.offset)->a[0]^(&tmp)->a[0], (&tmp)->a[1]=(&ctx->sess.offset)->a[1]^(&tmp)->a[1] );
            ctx->encrypt(tmp.c, tmp.c, ctx->keyenc);
            ( (&tmp)->a[0]=(&ctx->sess.offset)->a[0]^(&tmp)->a[0], (&tmp)->a[1]=(&ctx->sess.offset)->a[1]^(&tmp)->a[1] );
    ...
    if (last_len > 0) {
        ...
        ( (&ctx->sess.offset)->a[0]=(&ctx->sess.offset)->a[0]^(&ctx->l_star)->a[0], (&ctx->sess.offset)->a[1]=(&ctx->sess.offset)->a[1]^(&ctx->l_star)->a[1] );
        ...
        pad.c[last_len] = 0x80;
        ( (&ctx->sess.checksum)->a[0]=(&pad)->a[0]^(&ctx->sess.checksum)->a[0], (&ctx->sess.checksum)->a[1]=(&pad)->a[1]^(&ctx->sess.checksum)->a[1] );
    }
                          ...
                          size_t len)
{
    ...
    if (num_blocks && all_num_blocks == (size_t)all_num_blocks
        && ctx->stream != 
# 432 "../crypto/modes/ocb128.c" 3 4
                         ((void *)0)
# 432 "../crypto/modes/ocb128.c"
                             ) {
        size_t max_idx = 0, top = (size_t)all_num_blocks;
        ...
        while (top >>= 1)
            max_idx++;
        if (ocb_lookup_l(ctx, max_idx) == 
# 441 "../crypto/modes/ocb128.c" 3 4
                                         ((void *)0)
# 441 "../crypto/modes/ocb128.c"
                                             )
            return 0;
        ...
        for (i = ctx->sess.blocks_processed + 1; i <= all_num_blocks; i++) {
            ...
            if (lookup == 
# 455 "../crypto/modes/ocb128.c" 3 4
                         ((void *)0)
# 455 "../crypto/modes/ocb128.c"
                             )
                return 0;
            ( (&ctx->sess.offset)->a[0]=(&ctx->sess.offset)->a[0]^(lookup)->a[0], (&ctx->sess.offset)->a[1]=(&ctx->sess.offset)->a[1]^(lookup)->a[1] );
                      ...
                      int write)
{
    ...
    ( (&tmp)->a[0]=(&ctx->sess.checksum)->a[0]^(&ctx->sess.offset)->a[0], (&tmp)->a[1]=(&ctx->sess.checksum)->a[1]^(&ctx->sess.offset)->a[1] );
    ( (&tmp)->a[0]=(&ctx->l_dollar)->a[0]^(&tmp)->a[0], (&tmp)->a[1]=(&ctx->l_dollar)->a[1]^(&tmp)->a[1] );
    ctx->encrypt(tmp.c, tmp.c, ctx->keyenc);
    ( (&tmp)->a[0]=(&tmp)->a[0]^(&ctx->sess.sum)->a[0], (&tmp)->a[1]=(&tmp)->a[1]^(&ctx->sess.sum)->a[1] );
...
{
    if (ctx) {
        CRYPTO_clear_free(ctx->l, ctx->max_l_index * 16, "../crypto/modes/ocb128.c", 553);
