# 0 "../crypto/modes/gcm128.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../crypto/modes/gcm128.c"
# 10 "../crypto/modes/gcm128.c"
...
# 15 "../crypto/modes/gcm128.c" 2


typedef size_t size_t_aX __attribute((__aligned__(1)));
# 88 "../crypto/modes/gcm128.c"
static void gcm_init_4bit(u128 Htable[16], const u64 H[2])
{
    ...
    V.hi = H[0];
    V.lo = H[1];
# 115 "../crypto/modes/gcm128.c"
    Htable[8] = V;
    do { if (sizeof(size_t)==8) { u64 T = 0xe100000000000000ULL & (0-(V.lo&1)); V.lo = (V.hi<<63)|(V.lo>>1); V.hi = (V.hi>>1 )^T; } else { u32 T = 0xe1000000U & (0-(u32)(V.lo&1)); V.lo = (V.hi<<63)|(V.lo>>1); V.hi = (V.hi>>1 )^((u64)T<<32); } } while(0);
    Htable[4] = V;
    do { if (sizeof(size_t)==8) { u64 T = 0xe100000000000000ULL & (0-(V.lo&1)); V.lo = (V.hi<<63)|(V.lo>>1); V.hi = (V.hi>>1 )^T; } else { u32 T = 0xe1000000U & (0-(u32)(V.lo&1)); V.lo = (V.hi<<63)|(V.lo>>1); V.hi = (V.hi>>1 )^((u64)T<<32); } } while(0);
    Htable[2] = V;
    do { if (sizeof(size_t)==8) { u64 T = 0xe100000000000000ULL & (0-(V.lo&1)); V.lo = (V.hi<<63)|(V.lo>>1); V.hi = (V.hi>>1 )^T; } else { u32 T = 0xe1000000U & (0-(u32)(V.lo&1)); V.lo = (V.hi<<63)|(V.lo>>1); V.hi = (V.hi>>1 )^((u64)T<<32); } } while(0);
...
# 157 "../crypto/modes/gcm128.c"
}
# 323 "../crypto/modes/gcm128.c"
void gcm_gmult_4bit(u64 Xi[2], const u128 Htable[16]);
void gcm_ghash_4bit(u64 Xi[2], const u128 Htable[16], const u8 *inp,
                    size_t len);
# 346 "../crypto/modes/gcm128.c"
...
static void gcm_get_funcs(struct gcm_funcs_st *ctx)
{
    ...
    ctx->gmult = 
# 437 "../crypto/modes/gcm128.c" 3 4
                ((void *)0)
# 437 "../crypto/modes/gcm128.c"
                    ;
    ...
    ctx->ghash = 
# 442 "../crypto/modes/gcm128.c" 3 4
                ((void *)0)
# 442 "../crypto/modes/gcm128.c"
                    ;
...
# 481 "../crypto/modes/gcm128.c"
    ctx->gmult = gcm_gmult_4bit;
    ctx->ghash = gcm_ghash_4bit;
    return;
# 563 "../crypto/modes/gcm128.c"
}
                         ...
                         const u8 *inp, size_t len)
{
    ...
    gcm_get_funcs(&funcs);
    if (funcs.ghash != 
# 589 "../crypto/modes/gcm128.c" 3 4
                      ((void *)0)
# 589 "../crypto/modes/gcm128.c"
                          ) {
...
void CRYPTO_gcm128_init(GCM128_CONTEXT *ctx, void *key, block128_f block)
{
    ...
    if ((ossl_is_little_endian)) {
        ...
        ctx->H.u[0] = ({ u64 ret_=(ctx->H.u[0]); asm ("bswapq %0" : "+r"(ret_)); ret_; });
        ctx->H.u[1] = ({ u64 ret_=(ctx->H.u[1]); asm ("bswapq %0" : "+r"(ret_)); ret_; });
# 625 "../crypto/modes/gcm128.c"
    }
                         ...
                         size_t len)
{
    ...
    if (len == 12) {
        ...
        while (len >= 16) {
            for (i = 0; i < 16; ++i)
                ctx->Xi.c[i] ^= iv[i];
            ctx->funcs.gmult(ctx->Xi.u,ctx->Htable);
            iv += 16;
            len -= 16;
        }
        if (len) {
        ...
        if ((ossl_is_little_endian)) {
            ...
            ctx->Xi.u[1] ^= ({ u64 ret_=(len0); asm ("bswapq %0" : "+r"(ret_)); ret_; });
# 683 "../crypto/modes/gcm128.c"
        } else {
            ctx->Xi.u[1] ^= len0;
        }
        ...
        if ((ossl_is_little_endian))
            ...
            ctr = ({ u32 ret_=(ctx->Xi.d[3]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
    ...
    if ((ossl_is_little_endian))
        ...
        ctx->Yi.d[3] = ({ u32 ret_=(ctr); asm ("bswapl %0" : "+r"(ret_)); ret_; });
                      ...
                      size_t len)
{
    ...
    if ((i = (len & (size_t)-16))) {
        ctx->funcs.ghash((ctx)->Xi.u,(ctx)->Htable,aad,i);
        aad += i;
        len -= i;
    }
# 762 "../crypto/modes/gcm128.c"
    if (len) {
        n = (unsigned int)len;
        for (i = 0; i < len; ++i)
            ctx->Xi.c[i] ^= aad[i];
                          ...
                          size_t len)
{
    ...
    if ((ossl_is_little_endian))
        ...
        ctr = ({ u32 ret_=(ctx->Yi.d[3]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
    ...
    if (16 % sizeof(size_t) == 0) {
        do {
            if (n) {
                ...
                if (n == 0) {
                    ctx->funcs.ghash((ctx)->Xi.u,(ctx)->Htable,ctx->Xn,mres);
                    mres = 0;
                } else {
                    ctx->mres = mres;
                    return 0;
                }
# 849 "../crypto/modes/gcm128.c"
            }
            ...
            while (len >= (3*1024)) {
                ...
                ctx->funcs.ghash((ctx)->Xi.u,(ctx)->Htable,out - (3*1024),(3*1024));
                len -= (3*1024);
            }
            ...
            if ((i = (len & (size_t)-16))) {
                ...
                ctx->funcs.ghash((ctx)->Xi.u,(ctx)->Htable,out - j,j);
            }
# 935 "../crypto/modes/gcm128.c"
            if (len) {
                (*block) (ctx->Yi.c, ctx->EKi.c, key);
                ++ctr;
                if ((ossl_is_little_endian))
    ...
    for (i = 0; i < len; ++i) {
        ...
        if (mres == sizeof(ctx->Xn)) {
            ctx->funcs.ghash((ctx)->Xi.u,(ctx)->Htable,ctx->Xn,sizeof(ctx->Xn));
            mres = 0;
        }
...
# 1076 "../crypto/modes/gcm128.c"
            ...
            while (len >= (3*1024)) {
                ...
                ctx->funcs.ghash((ctx)->Xi.u,(ctx)->Htable,in,(3*1024));
                while (j) {
                    size_t_aX *out_t = (size_t_aX *)out;
                    const size_t_aX *in_t = (const size_t_aX *)in;
            ...
            if ((i = (len & (size_t)-16))) {
                ctx->funcs.ghash((ctx)->Xi.u,(ctx)->Htable,in,i);
                while (len >= 16) {
                    size_t_aX *out_t = (size_t_aX *)out;
                    const size_t_aX *in_t = (const size_t_aX *)in;
...
# 1306 "../crypto/modes/gcm128.c"
    ...
    while (len >= (3*1024)) {
        ...
        if ((ossl_is_little_endian))
            ...
            ctx->Yi.d[3] = ctr;
        ctx->funcs.ghash((ctx)->Xi.u,(ctx)->Htable,out,(3*1024));
        out += (3*1024);
        in += (3*1024);
        len -= (3*1024);
    }
    ...
    if ((i = (len & (size_t)-16))) {
        ...
        ctx->funcs.ghash((ctx)->Xi.u,(ctx)->Htable,out,i);
        out += i;
# 1357 "../crypto/modes/gcm128.c"
    }
    if (len) {
                         ...
                         size_t len)
{
    ...
    if ((ossl_is_little_endian)) {
        ...
        alen = ({ u64 ret_=(alen); asm ("bswapq %0" : "+r"(ret_)); ret_; });
        clen = ({ u64 ret_=(clen); asm ("bswapq %0" : "+r"(ret_)); ret_; });
# 1586 "../crypto/modes/gcm128.c"
    }
...
# 1611 "../crypto/modes/gcm128.c" 3 4
                             ((void *)0)
# 1611 "../crypto/modes/gcm128.c"
                                 , 0);
    memcpy(tag, ctx->Xi.c,
...
GCM128_CONTEXT *CRYPTO_gcm128_new(void *key, block128_f block)
{
    ...
    if ((ret = CRYPTO_malloc(sizeof(*ret), "../crypto/modes/gcm128.c", 1620)) != 
# 1620 "../crypto/modes/gcm128.c" 3 4
                                               ((void *)0)
# 1620 "../crypto/modes/gcm128.c"
                                                   )
...
void CRYPTO_gcm128_release(GCM128_CONTEXT *ctx)
{
    CRYPTO_clear_free(ctx, sizeof(*ctx), "../crypto/modes/gcm128.c", 1628);
}
