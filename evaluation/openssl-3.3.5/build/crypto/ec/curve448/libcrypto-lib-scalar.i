# 0 "../crypto/ec/curve448/scalar.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../crypto/ec/curve448/scalar.c"
# 12 "../crypto/ec/curve448/scalar.c"
...
# 13 "../crypto/ec/curve448/scalar.c" 2
...
# 15 "../crypto/ec/curve448/scalar.c" 2
...
# 16 "../crypto/ec/curve448/scalar.c" 2

static const c448_word_t MONTGOMERY_FACTOR = (c448_word_t) 0x3bd440fae918bc5ULL;
static const curve448_scalar_t sc_p = {
    {
        {
            (0x2378c292ab5844f3ULL), (0x216cc2728dc58f55ULL),
            (0xc44edb49aed63690ULL), (0xffffffff7cca23e9ULL),
    ...
    {
        {
            ...
            (0xe3539257049b9b60ULL), (0x7af32c4bc1b195d9ULL),
            (0x0d66de2388ea1859ULL), (0xae17cf725ee4d838ULL),
            (0x1a9cc14ba3c47c44ULL), (0x2052bcb7e4d070afULL),
...
static void sc_subx(curve448_scalar_t out,
                    const c448_word_t accum[((446-1)/64 +1)],
...
{
    unsigned int i, j;
    c448_word_t accum[((446-1)/64 +1) + 1] = { 0 };
                            ...
                            const unsigned char ser[56])
{
    ...
    scalar_decode_short(s, ser, 56);
    for (i = 0; i < ((446-1)/64 +1); i++)
        accum = (accum + s->limb[i] - sc_p->limb[i]) >> 64;


    ossl_curve448_scalar_mul(s, s, ossl_curve448_scalar_one);

    return c448_succeed_if(~constant_time_is_zero_64((uint32_t)accum));
}
...
# 194 "../crypto/ec/curve448/scalar.c" 3 4
       ((void) (0))
# 194 "../crypto/ec/curve448/scalar.c"
                     ;
...
ossl_curve448_scalar_halve(curve448_scalar_t out, const curve448_scalar_t a)
{
    ...
    for (i = 0; i < ((446-1)/64 +1) - 1; i++)
        out->limb[i] = out->limb[i] >> 1 | out->limb[i + 1] << (64 - 1);
    out->limb[i] = out->limb[i] >> 1 | (c448_word_t)(chain << (64 - 1));
}
