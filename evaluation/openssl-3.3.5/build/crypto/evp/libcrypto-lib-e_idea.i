# 0 "../crypto/evp/e_idea.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../crypto/evp/e_idea.c"
# 15 "../crypto/evp/e_idea.c"
                           ...
                           const unsigned char *in, size_t inl)
{
    size_t i, bl; bl = EVP_CIPHER_CTX_get0_cipher(ctx)->block_size; if (inl < bl) return 1; inl -= bl; for (i=0; i <= inl; i+=bl)
        IDEA_ecb_encrypt(in + i, out + i, &((EVP_IDEA_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks);
    return 1;
}

static int idea_cbc_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl) { while(inl>=((size_t)1 << 30)) { IDEA_cbc_encrypt(in, out, (long)((size_t)1 << 30), &((EVP_IDEA_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, ctx->iv, EVP_CIPHER_CTX_is_encrypting(ctx)); inl-=((size_t)1 << 30); in +=((size_t)1 << 30); out+=((size_t)1 << 30); } if (inl) IDEA_cbc_encrypt(in, out, (long)inl, &((EVP_IDEA_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, ctx->iv, EVP_CIPHER_CTX_is_encrypting(ctx)); return 1;}
static int idea_ofb_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl) { while(inl>=((size_t)1 << 30)) { int num = EVP_CIPHER_CTX_get_num(ctx); IDEA_ofb64_encrypt(in, out, (long)((size_t)1 << 30), &((EVP_IDEA_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, ctx->iv, &num); EVP_CIPHER_CTX_set_num(ctx, num); inl-=((size_t)1 << 30); in +=((size_t)1 << 30); out+=((size_t)1 << 30); } if (inl) { int num = EVP_CIPHER_CTX_get_num(ctx); IDEA_ofb64_encrypt(in, out, (long)inl, &((EVP_IDEA_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, ctx->iv, &num); EVP_CIPHER_CTX_set_num(ctx, num); } return 1;}
static int idea_cfb64_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl) { size_t chunk = ((size_t)1 << 30); if (64 == 1) chunk >>= 3; if (inl < chunk) chunk = inl; while (inl && inl >= chunk) { int num = EVP_CIPHER_CTX_get_num(ctx); IDEA_cfb64_encrypt(in, out, (long) ((64 == 1) && !EVP_CIPHER_CTX_test_flags(ctx, 0x2000) ? chunk*8 : chunk), &((EVP_IDEA_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, ctx->iv, &num, EVP_CIPHER_CTX_is_encrypting(ctx)); EVP_CIPHER_CTX_set_num(ctx, num); inl -= chunk; in += chunk; out += chunk; if (inl < chunk) chunk = inl; } return 1;}

static const EVP_CIPHER idea_cbc = { 34, 8, 16, 8, 0 | 0x2, 1, idea_init_key, idea_cbc_cipher, 
# 53 "../crypto/evp/e_idea.c" 3 4
((void *)0)
# 53 "../crypto/evp/e_idea.c"
...
((void *)0) 
# 53 "../crypto/evp/e_idea.c"
}; const EVP_CIPHER *EVP_idea_cbc(void) { return &idea_cbc; } static const EVP_CIPHER idea_cfb64 = { 35, 1, 16, 8, 0 | 0x3, 1, idea_init_key, idea_cfb64_cipher, 
# 53 "../crypto/evp/e_idea.c" 3 4
((void *)0)
# 53 "../crypto/evp/e_idea.c"
, sizeof(IDEA_KEY_SCHEDULE), EVP_CIPHER_set_asn1_iv, EVP_CIPHER_get_asn1_iv, 
...
((void *)0) 
# 53 "../crypto/evp/e_idea.c"
}; const EVP_CIPHER *EVP_idea_cfb64(void) { return &idea_cfb64; } static const EVP_CIPHER idea_ofb = { 46, 1, 16, 8, 0 | 0x4, 1, idea_init_key, idea_ofb_cipher, 
# 53 "../crypto/evp/e_idea.c" 3 4
((void *)0)
# 53 "../crypto/evp/e_idea.c"
, sizeof(IDEA_KEY_SCHEDULE), EVP_CIPHER_set_asn1_iv, EVP_CIPHER_get_asn1_iv, 
...
((void *)0) 
# 53 "../crypto/evp/e_idea.c"
}; const EVP_CIPHER *EVP_idea_ofb(void) { return &idea_ofb; } static const EVP_CIPHER idea_ecb = { 36, 8, 16, 0, 0 | 0x1, 1, idea_init_key, idea_ecb_cipher, 
# 53 "../crypto/evp/e_idea.c" 3 4
((void *)0)
# 53 "../crypto/evp/e_idea.c"
, sizeof(IDEA_KEY_SCHEDULE), EVP_CIPHER_set_asn1_iv, EVP_CIPHER_get_asn1_iv, 
...
{
    if (!enc) {
        if (EVP_CIPHER_get_mode(EVP_CIPHER_CTX_get0_cipher(ctx)) == 0x4)
            enc = 1;
        else if (EVP_CIPHER_get_mode(EVP_CIPHER_CTX_get0_cipher(ctx)) == 0x3)
            enc = 1;
    }
    if (enc)
        IDEA_set_encrypt_key(key, &((EVP_IDEA_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks);
    else {
        IDEA_KEY_SCHEDULE tmp;

        IDEA_set_encrypt_key(key, &tmp);
        IDEA_set_decrypt_key(&tmp, &((EVP_IDEA_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks);
