# 0 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
# 15 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
...
# 16 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 2
...
# 18 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 2
...
# 19 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 2
...
# 21 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 2
...
int AES_unwrap_key(AES_KEY *key, const unsigned char *iv,
                   unsigned char *out, const unsigned char *in,
                   unsigned int inlen);
# 23 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 2
...
# 24 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 2
...
int RAND_poll(void);
# 25 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 2
...
unsigned char *ossl_hexstr2buf_sep(const char *str, long *buflen,
                                   const char sep);
# 26 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 2
...
# 27 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 2
...
static inline unsigned long constant_time_msb_bn(unsigned long a)
{
    return 0 - (a >> (sizeof(a) * 8 - 1));
}

static inline unsigned long constant_time_lt_bn(unsigned long a, unsigned long b)
{
    return constant_time_msb_bn(a ^ ((a ^ b) | ((a - b) ^ b)));
}

static inline unsigned long constant_time_is_zero_bn(unsigned long a)
{
    return constant_time_msb_bn(~a & (a - 1));
}

static inline unsigned long constant_time_eq_bn(unsigned long a,
                                                unsigned long b)
{
...
# 28 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 2
...
int evp_pkey_decrypt_alloc(EVP_PKEY_CTX *ctx, unsigned char **outp,
                           ...
                           const unsigned char *in, size_t inlen);
# 29 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 2
...
# 30 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 2
...
# 49 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
...
static void sha256_update(SHA256_CTX *c, const void *data, size_t len)
{
    ...
    if (len) {
        sha256_block_data_order(c, ptr, len / (16*4));
...
# 136 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
typedef struct {
                                         ...
                                         size_t inp_len, int n4x)
{
    ...
    seqnum = ({ u64 ret_=(blocks[0].q[0]); asm ("bswapq %0" : "+r"(ret_)); ret_; });
    ...
    for (i = 0; i < x4; i++) {
        ...
        blocks[i].q[0] = ({ u64 ret_=(seqnum + i); asm ("bswapq %0" : "+r"(ret_)); ret_; });
...
# 264 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
    minblocks = ((frag <= last ? frag : last) - (64 - 13)) / 64;
    if (minblocks > 2048 / 64) {
        for (i = 0; i < x4; i++) {
            edges[i].ptr = hash_d[i].ptr;
            edges[i].blocks = 2048 / 64;
            ciph_d[i].blocks = 2048 / 16;
        }
        do {
            sha256_multi_block(ctx, edges, n4x);
            ...
            for (i = 0; i < x4; i++) {
                edges[i].ptr = hash_d[i].ptr += 2048;
                hash_d[i].blocks -= 2048 / 64;
                edges[i].blocks = 2048 / 64;
                ciph_d[i].inp += 2048;
                ciph_d[i].out += 2048;
                ciph_d[i].blocks = 2048 / 16;
    ...
    for (i = 0; i < x4; i++) {
        ...
        if (off < (64 - 8)) {
            ...
            blocks[i].d[15] = ({ u32 ret_=(len); asm ("bswapl %0" : "+r"(ret_)); ret_; });
            ...
            blocks[i].d[31] = ({ u32 ret_=(len); asm ("bswapl %0" : "+r"(ret_)); ret_; });
    ...
    for (i = 0; i < x4; i++) {
        ...
        blocks[i].d[0] = ({ u32 ret_=(ctx->A[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        ctx->A[i] = key->tail.h[0];
        blocks[i].d[1] = ({ u32 ret_=(ctx->B[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        ctx->B[i] = key->tail.h[1];
        blocks[i].d[2] = ({ u32 ret_=(ctx->C[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        ctx->C[i] = key->tail.h[2];
        blocks[i].d[3] = ({ u32 ret_=(ctx->D[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        ctx->D[i] = key->tail.h[3];
        blocks[i].d[4] = ({ u32 ret_=(ctx->E[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        ctx->E[i] = key->tail.h[4];
        blocks[i].d[5] = ({ u32 ret_=(ctx->F[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        ctx->F[i] = key->tail.h[5];
        blocks[i].d[6] = ({ u32 ret_=(ctx->G[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        ctx->G[i] = key->tail.h[6];
        blocks[i].d[7] = ({ u32 ret_=(ctx->H[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        ctx->H[i] = key->tail.h[7];
        blocks[i].c[32] = 0x80;
        blocks[i].d[15] = ({ u32 ret_=((64 + 32) * 8); asm ("bswapl %0" : "+r"(ret_)); ret_; });
# 365 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
        edges[i].ptr = blocks[i].c;
        edges[i].blocks = 1;
    }
    ...
    for (i = 0; i < x4; i++) {
        ...
        *(u32 *)(out + 0) = ({ u32 ret_=(ctx->A[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        *(u32 *)(out + 4) = ({ u32 ret_=(ctx->B[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        *(u32 *)(out + 8) = ({ u32 ret_=(ctx->C[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        *(u32 *)(out + 12) = ({ u32 ret_=(ctx->D[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        *(u32 *)(out + 16) = ({ u32 ret_=(ctx->E[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        *(u32 *)(out + 20) = ({ u32 ret_=(ctx->F[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        *(u32 *)(out + 24) = ({ u32 ret_=(ctx->G[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        *(u32 *)(out + 28) = ({ u32 ret_=(ctx->H[i]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
        out += 32;
        len += 32;
                                        ...
                                        const unsigned char *in, size_t len)
{
    ...
    if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
        if (plen == ((size_t)-1))
            ...
            iv = 16;
# 463 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
        if (((OPENSSL_ia32cap_P[2] & (1 << 29)) ||
        ...
        if (plen != len) {
            ...
            key->md = key->tail;
            sha256_update(&key->md, out + plen, 32);
            SHA256_Final(out + plen, &key->md);
        ...
        union {
            unsigned int u[32 / sizeof(unsigned int)];
            unsigned char c[64 + 32];
        } mac, *pmac;
        ...
        if (plen != ((size_t)-1)) {
            ...
            bitlen = ({ u32 ret_=(bitlen); asm ("bswapl %0" : "+r"(ret_)); ret_; });
# 589 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
            pmac->u[0] = 0;
            pmac->u[1] = 0;
            pmac->u[2] = 0;
            pmac->u[3] = 0;
            ...
            pmac->u[0] = ({ u32 ret_=(pmac->u[0]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
            pmac->u[1] = ({ u32 ret_=(pmac->u[1]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
            pmac->u[2] = ({ u32 ret_=(pmac->u[2]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
            pmac->u[3] = ({ u32 ret_=(pmac->u[3]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
            pmac->u[4] = ({ u32 ret_=(pmac->u[4]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
            pmac->u[5] = ({ u32 ret_=(pmac->u[5]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
            pmac->u[6] = ({ u32 ret_=(pmac->u[6]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
            pmac->u[7] = ({ u32 ret_=(pmac->u[7]); asm ("bswapl %0" : "+r"(ret_)); ret_; });
# 674 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
            len += 32;
# 695 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
            key->md = key->tail;
            sha256_update(&key->md, pmac->c, 32);
...
# 739 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
            return ret;
        } else {
    ...
    case 0x19:
        {
            ...
            if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
                if ((param->inp[9] << 8 | param->inp[10]) < 0x0302)
                    return -1;
...
# 904 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 3 4
   ((void *)0)
# 904 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
       ,
    sizeof(EVP_AES_HMAC_SHA256),
    0 ? 
# 906 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 3 4
                                ((void *)0) 
# 906 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
                                     : EVP_CIPHER_set_asn1_iv,
    0 ? 
# 907 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 3 4
                                ((void *)0) 
# 907 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
                                     : EVP_CIPHER_get_asn1_iv,
    aesni_cbc_hmac_sha256_ctrl,
    
# 909 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 3 4
   ((void *)0)

# 910 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
};
...
# 924 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 3 4
   ((void *)0)
# 924 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
       ,
    sizeof(EVP_AES_HMAC_SHA256),
    0 ? 
# 926 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 3 4
                                ((void *)0) 
# 926 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
                                     : EVP_CIPHER_set_asn1_iv,
    0 ? 
# 927 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 3 4
                                ((void *)0) 
# 927 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
                                     : EVP_CIPHER_get_asn1_iv,
    aesni_cbc_hmac_sha256_ctrl,
    
# 929 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 3 4
   ((void *)0)

# 930 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
};
...
# 935 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 3 4
                                ((void *)0)
# 935 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
                                    , 
# 935 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 3 4
                                      ((void *)0)
# 935 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
...
# 936 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 3 4
                                               ((void *)0)
# 936 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
                                                   );
}
...
const EVP_CIPHER *EVP_aes_256_cbc_hmac_sha256(void)
{
    return ((OPENSSL_ia32cap_P[1] & (1<<(57-32))) &&
            aesni_cbc_sha256_enc(
# 942 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 3 4
                                ((void *)0)
# 942 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
                                    , 
# 942 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 3 4
                                      ((void *)0)
# 942 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
...
# 943 "../crypto/evp/e_aes_cbc_hmac_sha256.c" 3 4
                                               ((void *)0)
# 943 "../crypto/evp/e_aes_cbc_hmac_sha256.c"
                                                   );
}
