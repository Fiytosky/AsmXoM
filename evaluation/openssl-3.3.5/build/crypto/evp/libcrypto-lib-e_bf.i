...
static int bf_cbc_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl) { while(inl>=((size_t)1 << 30)) { BF_cbc_encrypt(in, out, (long)((size_t)1 << 30), &((EVP_BF_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, ctx->iv, EVP_CIPHER_CTX_is_encrypting(ctx)); inl-=((size_t)1 << 30); in +=((size_t)1 << 30); out+=((size_t)1 << 30); } if (inl) BF_cbc_encrypt(in, out, (long)inl, &((EVP_BF_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, ctx->iv, EVP_CIPHER_CTX_is_encrypting(ctx)); return 1;} static int bf_cfb64_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl) { size_t chunk = ((size_t)1 << 30); if (64 == 1) chunk >>= 3; if (inl < chunk) chunk = inl; while (inl && inl >= chunk) { int num = EVP_CIPHER_CTX_get_num(ctx); BF_cfb64_encrypt(in, out, (long) ((64 == 1) && !EVP_CIPHER_CTX_test_flags(ctx, 0x2000) ? chunk*8 : chunk), &((EVP_BF_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, ctx->iv, &num, EVP_CIPHER_CTX_is_encrypting(ctx)); EVP_CIPHER_CTX_set_num(ctx, num); inl -= chunk; in += chunk; out += chunk; if (inl < chunk) chunk = inl; } return 1;} static int bf_ecb_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl) { size_t i, bl; bl = EVP_CIPHER_CTX_get0_cipher(ctx)->block_size; if (inl < bl) return 1; inl -= bl; for (i=0; i <= inl; i+=bl) BF_ecb_encrypt(in + i, out + i, &((EVP_BF_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, EVP_CIPHER_CTX_is_encrypting(ctx)); return 1;} static int bf_ofb_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl) { while(inl>=((size_t)1 << 30)) { int num = EVP_CIPHER_CTX_get_num(ctx); BF_ofb64_encrypt(in, out, (long)((size_t)1 << 30), &((EVP_BF_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, ctx->iv, &num); EVP_CIPHER_CTX_set_num(ctx, num); inl-=((size_t)1 << 30); in +=((size_t)1 << 30); out+=((size_t)1 << 30); } if (inl) { int num = EVP_CIPHER_CTX_get_num(ctx); BF_ofb64_encrypt(in, out, (long)inl, &((EVP_BF_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, ctx->iv, &num); EVP_CIPHER_CTX_set_num(ctx, num); } return 1;} static const EVP_CIPHER bf_cbc = { 91, 8, 16, 8, 0x8 | 0x2, 1, bf_init_key, bf_cbc_cipher, 
# 34 "../crypto/evp/e_bf.c" 3 4
((void *)0)
# 34 "../crypto/evp/e_bf.c"
...
((void *)0) 
# 34 "../crypto/evp/e_bf.c"
}; const EVP_CIPHER *EVP_bf_cbc(void) { return &bf_cbc; } static const EVP_CIPHER bf_cfb64 = { 93, 1, 16, 8, 0x8 | 0x3, 1, bf_init_key, bf_cfb64_cipher, 
# 34 "../crypto/evp/e_bf.c" 3 4
((void *)0)
# 34 "../crypto/evp/e_bf.c"
, sizeof(EVP_BF_KEY), EVP_CIPHER_set_asn1_iv, EVP_CIPHER_get_asn1_iv, 
...
((void *)0) 
# 34 "../crypto/evp/e_bf.c"
}; const EVP_CIPHER *EVP_bf_cfb64(void) { return &bf_cfb64; } static const EVP_CIPHER bf_ofb = { 94, 1, 16, 8, 0x8 | 0x4, 1, bf_init_key, bf_ofb_cipher, 
# 34 "../crypto/evp/e_bf.c" 3 4
((void *)0)
# 34 "../crypto/evp/e_bf.c"
, sizeof(EVP_BF_KEY), EVP_CIPHER_set_asn1_iv, EVP_CIPHER_get_asn1_iv, 
...
((void *)0) 
# 34 "../crypto/evp/e_bf.c"
}; const EVP_CIPHER *EVP_bf_ofb(void) { return &bf_ofb; } static const EVP_CIPHER bf_ecb = { 92, 8, 16, 0, 0x8 | 0x1, 1, bf_init_key, bf_ecb_cipher, 
# 34 "../crypto/evp/e_bf.c" 3 4
((void *)0)
# 34 "../crypto/evp/e_bf.c"
, sizeof(EVP_BF_KEY), EVP_CIPHER_set_asn1_iv, EVP_CIPHER_get_asn1_iv, 
                       ...
                       const unsigned char *iv, int enc)
{
    ...
    if (len < 0)
        return 0;
    BF_set_key(&((EVP_BF_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, len, key);
    return 1;
}
