# 0 "../crypto/evp/e_seed.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../crypto/evp/e_seed.c"
# 14 "../crypto/evp/e_seed.c"
...
# 20 "../crypto/evp/e_seed.c" 2
# 1 "/usr/include/assert.h" 1 3 4
# 21 "../crypto/evp/e_seed.c" 2
# 1 "../include/openssl/seed.h" 1
# 37 "../include/openssl/seed.h"
        
# 67 "../include/openssl/seed.h"
...
static int seed_cbc_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl) { while(inl>=((size_t)1 << 30)) { SEED_cbc_encrypt(in, out, (long)((size_t)1 << 30), &((EVP_SEED_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, ctx->iv, EVP_CIPHER_CTX_is_encrypting(ctx)); inl-=((size_t)1 << 30); in +=((size_t)1 << 30); out+=((size_t)1 << 30); } if (inl) SEED_cbc_encrypt(in, out, (long)inl, &((EVP_SEED_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, ctx->iv, EVP_CIPHER_CTX_is_encrypting(ctx)); return 1;} static int seed_cfb128_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl) { size_t chunk = ((size_t)1 << 30); if (128 == 1) chunk >>= 3; if (inl < chunk) chunk = inl; while (inl && inl >= chunk) { int num = EVP_CIPHER_CTX_get_num(ctx); SEED_cfb128_encrypt(in, out, (long) ((128 == 1) && !EVP_CIPHER_CTX_test_flags(ctx, 0x2000) ? chunk*8 : chunk), &((EVP_SEED_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, ctx->iv, &num, EVP_CIPHER_CTX_is_encrypting(ctx)); EVP_CIPHER_CTX_set_num(ctx, num); inl -= chunk; in += chunk; out += chunk; if (inl < chunk) chunk = inl; } return 1;} static int seed_ecb_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl) { size_t i, bl; bl = EVP_CIPHER_CTX_get0_cipher(ctx)->block_size; if (inl < bl) return 1; inl -= bl; for (i=0; i <= inl; i+=bl) SEED_ecb_encrypt(in + i, out + i, &((EVP_SEED_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, EVP_CIPHER_CTX_is_encrypting(ctx)); return 1;} static int seed_ofb_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl) { while(inl>=((size_t)1 << 30)) { int num = EVP_CIPHER_CTX_get_num(ctx); SEED_ofb128_encrypt(in, out, (long)((size_t)1 << 30), &((EVP_SEED_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, ctx->iv, &num); EVP_CIPHER_CTX_set_num(ctx, num); inl-=((size_t)1 << 30); in +=((size_t)1 << 30); out+=((size_t)1 << 30); } if (inl) { int num = EVP_CIPHER_CTX_get_num(ctx); SEED_ofb128_encrypt(in, out, (long)inl, &((EVP_SEED_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, ctx->iv, &num); EVP_CIPHER_CTX_set_num(ctx, num); } return 1;} static const EVP_CIPHER seed_cbc = { 777, 16, 16, 16, 0 | 0x2, 1, seed_init_key, seed_cbc_cipher, 0, sizeof(EVP_SEED_KEY), 0, 0, 0, 
# 32 "../crypto/evp/e_seed.c" 3 4
((void *)0) 
# 32 "../crypto/evp/e_seed.c"
}; const EVP_CIPHER *EVP_seed_cbc(void) { return &seed_cbc; } static const EVP_CIPHER seed_cfb128 = { 779, 1, 16, 16, 0 | 0x3, 1, seed_init_key, seed_cfb128_cipher, 0, sizeof(EVP_SEED_KEY), 0, 0, 0, 
# 32 "../crypto/evp/e_seed.c" 3 4
((void *)0) 
# 32 "../crypto/evp/e_seed.c"
}; const EVP_CIPHER *EVP_seed_cfb128(void) { return &seed_cfb128; } static const EVP_CIPHER seed_ofb = { 778, 1, 16, 16, 0 | 0x4, 1, seed_init_key, seed_ofb_cipher, 0, sizeof(EVP_SEED_KEY), 0, 0, 0, 
# 32 "../crypto/evp/e_seed.c" 3 4
((void *)0) 
# 32 "../crypto/evp/e_seed.c"
}; const EVP_CIPHER *EVP_seed_ofb(void) { return &seed_ofb; } static const EVP_CIPHER seed_ecb = { 776, 16, 16, 0, 0 | 0x1, 1, seed_init_key, seed_ecb_cipher, 0, sizeof(EVP_SEED_KEY), 0, 0, 0, 
# 32 "../crypto/evp/e_seed.c" 3 4
((void *)0) 
# 32 "../crypto/evp/e_seed.c"
}; const EVP_CIPHER *EVP_seed_ecb(void) { return &seed_ecb; }
