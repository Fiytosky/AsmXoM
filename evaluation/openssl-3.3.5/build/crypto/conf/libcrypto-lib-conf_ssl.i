# 0 "../crypto/conf/conf_ssl.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../crypto/conf/conf_ssl.c"
# 10 "../crypto/conf/conf_ssl.c"
...
static void ssl_module_free(CONF_IMODULE *md)
{
    size_t i, j;
    if (ssl_names == 
# 44 "../crypto/conf/conf_ssl.c" 3 4
                    ((void *)0)
# 44 "../crypto/conf/conf_ssl.c"
                        )
        return;
    for (i = 0; i < ssl_names_count; i++) {
        struct ssl_conf_name_st *tname = ssl_names + i;

        CRYPTO_free(tname->name, "../crypto/conf/conf_ssl.c", 49);
        for (j = 0; j < tname->cmd_count; j++) {
            CRYPTO_free(tname->cmds[j].cmd, "../crypto/conf/conf_ssl.c", 51);
            CRYPTO_free(tname->cmds[j].arg, "../crypto/conf/conf_ssl.c", 52);
        }
        CRYPTO_free(tname->cmds, "../crypto/conf/conf_ssl.c", 54);
    }
    CRYPTO_free(ssl_names, "../crypto/conf/conf_ssl.c", 56);
    ssl_names = 
# 57 "../crypto/conf/conf_ssl.c" 3 4
               ((void *)0)
# 57 "../crypto/conf/conf_ssl.c"
                   ;
    ssl_names_count = 0;
}
...
static int ssl_module_init(CONF_IMODULE *md, const CONF *cnf)
{
    ...
    struct stack_st_CONF_VALUE *cmd_lists;

    ssl_conf_section = CONF_imodule_get_value(md);
    cmd_lists = NCONF_get_section(cnf, ssl_conf_section);
    if (OPENSSL_sk_num(ossl_check_const_CONF_VALUE_sk_type(cmd_lists)) <= 0) {
        int rcode =
            cmd_lists == 
# 72 "../crypto/conf/conf_ssl.c" 3 4
                        ((void *)0)
            
# 73 "../crypto/conf/conf_ssl.c"
           ? 120
            : 119;

        (ERR_new(), ERR_set_debug("../crypto/conf/conf_ssl.c",76,__func__), ERR_set_error)(14, rcode, "section=%s", ssl_conf_section);
        goto err;
    }
    cnt = OPENSSL_sk_num(ossl_check_const_CONF_VALUE_sk_type(cmd_lists));
    ssl_module_free(md);
    ssl_names = CRYPTO_zalloc(sizeof(*ssl_names) * cnt, "../crypto/conf/conf_ssl.c", 81);
    if (ssl_names == 
# 82 "../crypto/conf/conf_ssl.c" 3 4
                    ((void *)0)
# 82 "../crypto/conf/conf_ssl.c"
                        )
        goto err;
    ssl_names_count = cnt;
    for (i = 0; i < ssl_names_count; i++) {
        struct ssl_conf_name_st *ssl_name = ssl_names + i;
        CONF_VALUE *sect = ((CONF_VALUE *)OPENSSL_sk_value(ossl_check_const_CONF_VALUE_sk_type(cmd_lists), ((int)i)));
        struct stack_st_CONF_VALUE *cmds = NCONF_get_section(cnf, sect->value);

        if (OPENSSL_sk_num(ossl_check_const_CONF_VALUE_sk_type(cmds)) <= 0) {
            int rcode =
                cmds == 
# 92 "../crypto/conf/conf_ssl.c" 3 4
                       ((void *)0)
                
# 93 "../crypto/conf/conf_ssl.c"
               ? 118
                : 117;

            (ERR_new(), ERR_set_debug("../crypto/conf/conf_ssl.c",96,__func__), ERR_set_error)(14, rcode,
                           "name=%s, value=%s", sect->name, sect->value);
            goto err;
        }
        ssl_name->name = CRYPTO_strdup(sect->name, "../crypto/conf/conf_ssl.c", 100);
        if (ssl_name->name == 
# 101 "../crypto/conf/conf_ssl.c" 3 4
                             ((void *)0)
# 101 "../crypto/conf/conf_ssl.c"
                                 )
            goto err;
        cnt = OPENSSL_sk_num(ossl_check_const_CONF_VALUE_sk_type(cmds));
        ssl_name->cmds = CRYPTO_zalloc(cnt * sizeof(struct ssl_conf_cmd_st), "../crypto/conf/conf_ssl.c", 104);
        if (ssl_name->cmds == 
# 105 "../crypto/conf/conf_ssl.c" 3 4
                             ((void *)0)
# 105 "../crypto/conf/conf_ssl.c"
                                 )
            goto err;
        ssl_name->cmd_count = cnt;
        for (j = 0; j < cnt; j++) {
            const char *name;
            CONF_VALUE *cmd_conf = ((CONF_VALUE *)OPENSSL_sk_value(ossl_check_const_CONF_VALUE_sk_type(cmds), ((int)j)));
            struct ssl_conf_cmd_st *cmd = ssl_name->cmds + j;


            name = strchr(cmd_conf->name, '.');
            if (name != 
# 115 "../crypto/conf/conf_ssl.c" 3 4
                       ((void *)0)
# 115 "../crypto/conf/conf_ssl.c"
                           )
                name++;
            else
                name = cmd_conf->name;
            cmd->cmd = CRYPTO_strdup(name, "../crypto/conf/conf_ssl.c", 119);
            cmd->arg = CRYPTO_strdup(cmd_conf->value, "../crypto/conf/conf_ssl.c", 120);
            if (cmd->cmd == 
# 121 "../crypto/conf/conf_ssl.c" 3 4
                           ((void *)0) 
# 121 "../crypto/conf/conf_ssl.c"
                                || cmd->arg == 
# 121 "../crypto/conf/conf_ssl.c" 3 4
                                               ((void *)0)
# 121 "../crypto/conf/conf_ssl.c"
...
int conf_ssl_name_find(const char *name, size_t *idx)
{
    ...
    if (name == 
# 155 "../crypto/conf/conf_ssl.c" 3 4
               ((void *)0)
# 155 "../crypto/conf/conf_ssl.c"
                   )
        return 0;
