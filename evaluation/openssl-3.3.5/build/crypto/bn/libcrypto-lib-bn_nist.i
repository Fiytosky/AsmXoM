# 0 "../crypto/bn/bn_nist.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../crypto/bn/bn_nist.c"
# 10 "../crypto/bn/bn_nist.c"
...
int ossl_bn_check_prime(const BIGNUM *w, int checks, BN_CTX *ctx,
                        int do_trial_division, BN_GENCB *cb);
# 11 "../crypto/bn/bn_nist.c" 2
# 21 "../crypto/bn/bn_nist.c"
static const unsigned long _nist_p_192[][(192+(8 * 8)-1)/(8 * 8)] = {
...
static const unsigned long _nist_p_224[][(224+(8 * 8)-1)/(8 * 8)] = {
...
static const unsigned long _nist_p_256[][(256+(8 * 8)-1)/(8 * 8)] = {
    {0xFFFFFFFFFFFFFFFFULL, 0x00000000FFFFFFFFULL,
...
static const unsigned long _nist_p_384[][(384+(8 * 8)-1)/(8 * 8)] = {
...
static inline void store_lo32(void *ptr, long long val)
{
...
# 351 "../crypto/bn/bn_nist.c"
int BN_nist_mod_192(BIGNUM *r, const BIGNUM *a, const BIGNUM *field,
                    BN_CTX *ctx)
{
    int top = a->top, i;
    int carry;
    register unsigned long *r_d, *a_d = a->d;
    union {
        unsigned long bn[(192+(8 * 8)-1)/(8 * 8)];
        unsigned int ui[(192+(8 * 8)-1)/(8 * 8) * sizeof(unsigned long) /
                        sizeof(unsigned int)];
    } buf;
    unsigned long c_d[(192+(8 * 8)-1)/(8 * 8)], *res;
...
# 380 "../crypto/bn/bn_nist.c" 3 4
                                               ((void *)0)
# 380 "../crypto/bn/bn_nist.c"
                                                   );

    if (r != a) {
        if (!bn_wexpand(r, (192+(8 * 8)-1)/(8 * 8)))
            return 0;
        r_d = r->d;
        nist_cp_bn(r_d, a_d, (192+(8 * 8)-1)/(8 * 8));
    } else
        r_d = a_d;

    { int ii; const unsigned long *src = a_d + (192+(8 * 8)-1)/(8 * 8); for (ii = 0; ii < top - (192+(8 * 8)-1)/(8 * 8); ii++) (buf.bn)[ii] = src[ii]; for (; ii < (192+(8 * 8)-1)/(8 * 8); ii++) (buf.bn)[ii] = 0; }
                                 ;
...
# 450 "../crypto/bn/bn_nist.c"
    ...
    res = (bn_sub_words(c_d, r_d, _nist_p_192[0], (192+(8 * 8)-1)/(8 * 8)) && carry)
        ? r_d
        : c_d;
    nist_cp_bn(r_d, res, (192+(8 * 8)-1)/(8 * 8));
    r->top = (192+(8 * 8)-1)/(8 * 8);
    bn_correct_top(r);
                    ...
                    BN_CTX *ctx)
{
    ...
    union {
        unsigned long bn[(224+(8 * 8)-1)/(8 * 8)];
        unsigned int ui[(224+(8 * 8)-1)/(8 * 8) * sizeof(unsigned long) /
                        sizeof(unsigned int)];
    } buf;
    unsigned long c_d[(224+(8 * 8)-1)/(8 * 8)], *res;
    bn_addsub_f adjust;
    ...
    if (i == 0) {
        ...
        return (r == a) ? 1 : (BN_copy(r, a) != 
# 517 "../crypto/bn/bn_nist.c" 3 4
                                               ((void *)0)
# 517 "../crypto/bn/bn_nist.c"
                                                   );

    if (r != a) {
        if (!bn_wexpand(r, (224+(8 * 8)-1)/(8 * 8)))
            return 0;
        r_d = r->d;
        nist_cp_bn(r_d, a_d, (224+(8 * 8)-1)/(8 * 8));
    } else
        r_d = a_d;



    { int ii; const unsigned long *src = a_d + ((224+(8 * 8)-1)/(8 * 8) - 1); for (ii = 0; ii < top - ((224+(8 * 8)-1)/(8 * 8) - 1); ii++) (c_d)[ii] = src[ii]; for (; ii < (224+(8 * 8)-1)/(8 * 8); ii++) (c_d)[ii] = 0; }
                                                              ;
    ...
    r_d[(224+(8 * 8)-1)/(8 * 8) - 1] &= (0xffffffffL);
# 596 "../crypto/bn/bn_nist.c"
    {
        unsigned long t_d[(224+(8 * 8)-1)/(8 * 8)];
        ...
        carry = (int)bn_add_words(r_d, r_d, t_d, (224+(8 * 8)-1)/(8 * 8));
        ...
        carry -= (int)bn_sub_words(r_d, r_d, t_d, (224+(8 * 8)-1)/(8 * 8));
        ...
        carry = (int)(r_d[(224+(8 * 8)-1)/(8 * 8) - 1] >> 32);
    ...
    if (carry > 0) {
        ...
        carry = (int)(~(r_d[(224+(8 * 8)-1)/(8 * 8) - 1] >> 32)) & 1;
    ...
    res = ((*adjust) (c_d, r_d, _nist_p_224[0], (224+(8 * 8)-1)/(8 * 8)) && carry)
        ? r_d
        : c_d;
    nist_cp_bn(r_d, res, (224+(8 * 8)-1)/(8 * 8));
    r->top = (224+(8 * 8)-1)/(8 * 8);
    bn_correct_top(r);

    return 1;
}
# 659 "../crypto/bn/bn_nist.c"
int BN_nist_mod_256(BIGNUM *r, const BIGNUM *a, const BIGNUM *field,
                    BN_CTX *ctx)
{
    int i, top = a->top;
    int carry = 0;
    register unsigned long *a_d = a->d, *r_d;
    union {
        unsigned long bn[(256+(8 * 8)-1)/(8 * 8)];
        unsigned int ui[(256+(8 * 8)-1)/(8 * 8) * sizeof(unsigned long) /
                        sizeof(unsigned int)];
    } buf;
    unsigned long c_d[(256+(8 * 8)-1)/(8 * 8)], *res;
    bn_addsub_f adjust;
    ...
    if (i == 0) {
        ...
        return (r == a) ? 1 : (BN_copy(r, a) != 
# 689 "../crypto/bn/bn_nist.c" 3 4
                                               ((void *)0)
# 689 "../crypto/bn/bn_nist.c"
                                                   );

    if (r != a) {
        if (!bn_wexpand(r, (256+(8 * 8)-1)/(8 * 8)))
            return 0;
        r_d = r->d;
        nist_cp_bn(r_d, a_d, (256+(8 * 8)-1)/(8 * 8));
    } else
        r_d = a_d;

    { int ii; const unsigned long *src = a_d + (256+(8 * 8)-1)/(8 * 8); for (ii = 0; ii < top - (256+(8 * 8)-1)/(8 * 8); ii++) (buf.bn)[ii] = src[ii]; for (; ii < (256+(8 * 8)-1)/(8 * 8); ii++) (buf.bn)[ii] = 0; }
                                 ;
...
# 857 "../crypto/bn/bn_nist.c"
    ...
    res = ((*adjust) (c_d, r_d, _nist_p_256[0], (256+(8 * 8)-1)/(8 * 8)) && carry)
        ? r_d
        : c_d;
    nist_cp_bn(r_d, res, (256+(8 * 8)-1)/(8 * 8));
    r->top = (256+(8 * 8)-1)/(8 * 8);
    bn_correct_top(r);

    return 1;
}
# 896 "../crypto/bn/bn_nist.c"
                    ...
                    BN_CTX *ctx)
{
    ...
    register unsigned long *r_d, *a_d = a->d;
    union {
        unsigned long bn[(384+(8 * 8)-1)/(8 * 8)];
        unsigned int ui[(384+(8 * 8)-1)/(8 * 8) * sizeof(unsigned long) /
                        sizeof(unsigned int)];
    } buf;
    unsigned long c_d[(384+(8 * 8)-1)/(8 * 8)], *res;
    bn_addsub_f adjust;
    ...
    if (i == 0) {
        ...
        return (r == a) ? 1 : (BN_copy(r, a) != 
# 926 "../crypto/bn/bn_nist.c" 3 4
                                               ((void *)0)
# 926 "../crypto/bn/bn_nist.c"
                                                   );

    if (r != a) {
        if (!bn_wexpand(r, (384+(8 * 8)-1)/(8 * 8)))
            return 0;
        r_d = r->d;
        nist_cp_bn(r_d, a_d, (384+(8 * 8)-1)/(8 * 8));
    } else
        r_d = a_d;

    { int ii; const unsigned long *src = a_d + (384+(8 * 8)-1)/(8 * 8); for (ii = 0; ii < top - (384+(8 * 8)-1)/(8 * 8); ii++) (buf.bn)[ii] = src[ii]; for (; ii < (384+(8 * 8)-1)/(8 * 8); ii++) (buf.bn)[ii] = 0; }
                                 ;
...
# 1129 "../crypto/bn/bn_nist.c"
    ...
    res = ((*adjust) (c_d, r_d, _nist_p_384[0], (384+(8 * 8)-1)/(8 * 8)) && carry)
        ? r_d
        : c_d;
    nist_cp_bn(r_d, res, (384+(8 * 8)-1)/(8 * 8));
    r->top = (384+(8 * 8)-1)/(8 * 8);
    bn_correct_top(r);
...
{
    int top = a->top, i;
    unsigned long *r_d, *a_d = a->d, t_d[(521+(8 * 8)-1)/(8 * 8)], val, tmp, *res;
    ...
    if (i == 0) {
        ...
        return (r == a) ? 1 : (BN_copy(r, a) != 
# 1178 "../crypto/bn/bn_nist.c" 3 4
                                               ((void *)0)
# 1178 "../crypto/bn/bn_nist.c"
                                                   );

    if (r != a) {
        if (!bn_wexpand(r, (521+(8 * 8)-1)/(8 * 8)))
            return 0;
        r_d = r->d;
        nist_cp_bn(r_d, a_d, (521+(8 * 8)-1)/(8 * 8));
    } else
        r_d = a_d;


    { int ii; const unsigned long *src = a_d + ((521+(8 * 8)-1)/(8 * 8) - 1); for (ii = 0; ii < top - ((521+(8 * 8)-1)/(8 * 8) - 1); ii++) (t_d)[ii] = src[ii]; for (; ii < (521+(8 * 8)-1)/(8 * 8); ii++) (t_d)[ii] = 0; }
                                                              ;

    for (val = t_d[0], i = 0; i < (521+(8 * 8)-1)/(8 * 8) - 1; i++) {
# 1203 "../crypto/bn/bn_nist.c"
        t_d[i] = (val >> (521%(8 * 8)) |
                  (tmp = t_d[i + 1]) << ((8 * 8)-(521%(8 * 8)))) & (0xffffffffffffffffL);
        val = tmp;

    }
    t_d[i] = val >> (521%(8 * 8));

    r_d[i] &= ((unsigned long)(0xffffffffffffffffL)>>((8 * 8)-(521%(8 * 8))));

    bn_add_words(r_d, r_d, t_d, (521+(8 * 8)-1)/(8 * 8));
    res = bn_sub_words(t_d, r_d, _nist_p_521,
                       (521+(8 * 8)-1)/(8 * 8))
        ? r_d
        : t_d;
    nist_cp_bn(r_d, res, (521+(8 * 8)-1)/(8 * 8));
    r->top = (521+(8 * 8)-1)/(8 * 8);
    bn_correct_top(r);
