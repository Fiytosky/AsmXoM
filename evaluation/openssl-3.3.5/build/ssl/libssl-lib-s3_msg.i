...
# 23 "../ssl/s3_msg.c"
            ...
            (ERR_new(), ERR_set_debug("../ssl/s3_msg.c",25,__func__), ERR_set_error)((20),(133),
# 25 "../ssl/s3_msg.c" 3 4
           ((void *)0)
# 25 "../ssl/s3_msg.c"
           );
...
# 88 "../ssl/s3_msg.c"
    ...
    templ.type = 21;
    templ.version = (sc->version == 0x0304) ? 0x0303
                                                    : sc->version;
    if (SSL_get_state(s) == TLS_ST_CW_CLNT_HELLO
            && !sc->renegotiate
            && ((SSL_version(s) >> 8) == 0x03 ? SSL_version(s) : 0) > 0x0301
            && sc->hello_retry_request == SSL_HRR_NONE) {
        templ.version = 0x0301;
    }
    templ.buf = &sc->s3.send_alert[0];
    templ.buflen = 2;

    if (RECORD_LAYER_write_pending(&sc->rlayer)) {
        if (sc->s3.alert_dispatch != 2) {
        ...
        i = ossl_tls_handle_rlayer_return(sc, 1, sc->rlayer.wrlmethod->retry_write_records(sc->rlayer.wrl), "../ssl/s3_msg.c", 117)
                                                                          ;
        if (i <= 0) {
    ...
    i = ossl_tls_handle_rlayer_return(sc, 1, sc->rlayer.wrlmethod->write_records(sc->rlayer.wrl, &templ, 1), "../ssl/s3_msg.c", 128)
                                                                           ;

    if (i <= 0) {
        sc->s3.alert_dispatch = 2;
        ...
        (void)(int)BIO_ctrl(sc->wbio,11,0,
# 141 "../ssl/s3_msg.c" 3 4
             ((void *)0)
# 141 "../ssl/s3_msg.c"
             );
        sc->s3.alert_dispatch = 0;
        ...
        if (sc->msg_callback)
            sc->msg_callback(1, sc->version, 21, sc->s3.send_alert,
                             2, s, sc->msg_callback_arg);

        if (sc->info_callback != 
# 148 "../ssl/s3_msg.c" 3 4
                                ((void *)0)
# 148 "../ssl/s3_msg.c"
                                    )
            cb = sc->info_callback;
        else if (s->ctx->info_callback != 
# 150 "../ssl/s3_msg.c" 3 4
                                         ((void *)0)
# 150 "../ssl/s3_msg.c"
                                             )
