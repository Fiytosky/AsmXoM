# 0 "../ssl/quic/quic_rstream.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "../ssl/quic/quic_rstream.c"
# 9 "../ssl/quic/quic_rstream.c"
...
# 13 "../ssl/quic/quic_rstream.c" 2
# 1 "../include/internal/quic_sf_list.h" 1
# 14 "../include/internal/quic_sf_list.h"
...
# 15 "../include/internal/quic_sf_list.h" 2
# 40 "../include/internal/quic_sf_list.h"
...
static inline __attribute__((unused)) uint64_t safe_add_u64(uint64_t a, uint64_t b, int *err) { uint64_t r; if (!__builtin_add_overflow(a, b, &r)) return r; *err |= 1; return a + b; } static inline __attribute__((unused)) uint64_t safe_sub_u64(uint64_t a, uint64_t b, int *err) { if (b > a) *err |= 1; return a - b; } static inline __attribute__((unused)) uint64_t safe_mul_u64(uint64_t a, uint64_t b, int *err) { uint64_t r; if (!__builtin_mul_overflow(a, b, &r)) return r; *err |= 1; return a * b; } static inline __attribute__((unused)) uint64_t safe_div_u64(uint64_t a, uint64_t b, int *err) { if (b != 0) return a / b; *err |= 1; return (~(uint64_t)0); } static inline __attribute__((unused)) uint64_t safe_mod_u64(uint64_t a, uint64_t b, int *err) { if (b != 0) return a % b; *err |= 1; return 0; } static inline __attribute__((unused)) uint64_t safe_div_round_up_u64 (uint64_t a, uint64_t b, int *errp) { uint64_t x; int *err, err_local = 0; err = errp != 
# 43 "../include/internal/ring_buf.h" 3 4
((void *)0) 
# 43 "../include/internal/ring_buf.h"
? errp : &err_local; if (b > 0 && a > 0) { if (a < (~(uint64_t)0) - b) return (a + b - 1) / b; return a / b + (a % b != 0); } if (b == 0) { *err |= 1; return a == 0 ? 0 : (~(uint64_t)0); } if (a == 0) return 0; x = safe_mod_u64(a, b, err); return safe_add_u64(safe_div_u64(a, b, err), x != 0, err); } static inline __attribute__((unused)) uint64_t safe_muldiv_u64(uint64_t a, uint64_t b, uint64_t c, int *err) { int e2 = 0; uint64_t x, y; if (c == 0) { *err |= 1; return a == 0 || b == 0 ? 0 : (~(uint64_t)0); } x = safe_mul_u64(a, b, &e2); if (!e2) return x / c; if (b > a) { x = b; b = a; a = x; } x = safe_mul_u64(a % c, b, err); y = safe_mul_u64(a / c, b, err); return safe_add_u64(y, x / c, err); } static inline __attribute__((unused)) uint64_t safe_neg_u64(uint64_t a, int *err) { if (a == 0) return a; *err |= 1; return 1 + ~a; } static inline __attribute__((unused)) uint64_t safe_abs_u64(uint64_t a, int *err) { return a; }
...
{
    r->start = 
# 49 "../include/internal/ring_buf.h" 3 4
              ((void *)0)
# 49 "../include/internal/ring_buf.h"
                  ;
    r->alloc = 0;
...
{
    if (cleanse)
        CRYPTO_clear_free(r->start, r->alloc, "../include/internal/ring_buf.h", 58);
    else
        CRYPTO_free(r->start, "../include/internal/ring_buf.h", 60);
    r->start = 
# 61 "../include/internal/ring_buf.h" 3 4
              ((void *)0)
# 61 "../include/internal/ring_buf.h"
                  ;
    r->alloc = 0;
}
...
# 108 "../include/internal/ring_buf.h" 3 4
   ((void) (0))
# 108 "../include/internal/ring_buf.h"
                       ;
                                        ...
                                        size_t buf_len)
{
    ...
    for (;;) {
        ...
        if (buf_len > (((uint64_t)1) << 62) - r->head_offset)
            buf_len = (size_t)((((uint64_t)1) << 62) - r->head_offset);
                                                         ...
                                                         size_t *max_len)
{
    ...
    if (logical_offset >= r->head_offset || logical_offset < r->ctail_offset)
        return 
# 154 "../include/internal/ring_buf.h" 3 4
              ((void *)0)
# 154 "../include/internal/ring_buf.h"
                  ;
    idx = logical_offset % r->alloc;
    *max_len = r->alloc - idx;
    return start + idx;
}
# 172 "../include/internal/ring_buf.h"
                                           ...
                                           size_t *buf_len)
{
    ...
    if (r->alloc == 0) {
        *buf = 
# 184 "../include/internal/ring_buf.h" 3 4
                     ((void *)0)
# 184 "../include/internal/ring_buf.h"
                         ;
        *buf_len = 0;
        return 1;
    }
...
# 203 "../include/internal/ring_buf.h" 3 4
   ((void) (0))
# 203 "../include/internal/ring_buf.h"
                       ;

    if (start > r->ctail_offset || end >= (((uint64_t)1) << 62))
        return;
...
# 235 "../include/internal/ring_buf.h" 3 4
                              ((void *)0)
# 235 "../include/internal/ring_buf.h"
                                  ;
    ...
    rnew.start = CRYPTO_malloc(num_bytes, "../include/internal/ring_buf.h", 244);
    if (rnew.start == 
# 245 "../include/internal/ring_buf.h" 3 4
                     ((void *)0)
# 245 "../include/internal/ring_buf.h"
                         )
        return 0;
    ...
    for (;;) {
        if (!ring_buf_get_buf_at(r, r->ctail_offset + copied, &src, &src_len)) {
            CRYPTO_free(rnew.start, "../include/internal/ring_buf.h", 254);
            return 0;
        }
        ...
        if (ring_buf_push(&rnew, src, src_len) != src_len) {
            CRYPTO_free(rnew.start, "../include/internal/ring_buf.h", 262);
            return 0;
        }
...
# 269 "../include/internal/ring_buf.h" 3 4
   ((void) (0))
# 269 "../include/internal/ring_buf.h"
                                             ;
...
QUIC_RSTREAM *ossl_quic_rstream_new(QUIC_RXFC *rxfc,
                                    OSSL_STATM *statm, size_t rbuf_size)
{
    QUIC_RSTREAM *ret = CRYPTO_zalloc(sizeof(*ret), "../ssl/quic/quic_rstream.c", 27);

    if (ret == 
# 29 "../ssl/quic/quic_rstream.c" 3 4
              ((void *)0)
# 29 "../ssl/quic/quic_rstream.c"
                  )
        return 
# 30 "../ssl/quic/quic_rstream.c" 3 4
              ((void *)0)
# 30 "../ssl/quic/quic_rstream.c"
                  ;

    ring_buf_init(&ret->rbuf);
    if (!ring_buf_resize(&ret->rbuf, rbuf_size, 0)) {
        CRYPTO_free(ret, "../ssl/quic/quic_rstream.c", 34);
        return 
# 35 "../ssl/quic/quic_rstream.c" 3 4
              ((void *)0)
# 35 "../ssl/quic/quic_rstream.c"
                  ;
    }
...
void ossl_quic_rstream_free(QUIC_RSTREAM *qrs)
{
    ...
    if (qrs == 
# 48 "../ssl/quic/quic_rstream.c" 3 4
              ((void *)0)
# 48 "../ssl/quic/quic_rstream.c"
                  )
        return;
    ...
    CRYPTO_free(qrs, "../ssl/quic/quic_rstream.c", 54);
}
                                 ...
                                 int fin)
{
    ...
    if ((data == 
# 64 "../ssl/quic/quic_rstream.c" 3 4
                ((void *)0) 
# 64 "../ssl/quic/quic_rstream.c"
                     && data_len != 0) || (data_len == 0 && fin == 0)) {

        (ERR_new(), ERR_set_debug("../ssl/quic/quic_rstream.c",66,__func__), ERR_set_error)((20),((259|((0x1 << 18L)|(0x2 << 18L)))),
# 66 "../ssl/quic/quic_rstream.c" 3 4
       ((void *)0)
# 66 "../ssl/quic/quic_rstream.c"
       );
        return 0;
    }
...
{
    void *iter = 
# 79 "../ssl/quic/quic_rstream.c" 3 4
                ((void *)0)
# 79 "../ssl/quic/quic_rstream.c"
                    ;
    UINT_RANGE range;
    const unsigned char *data;
    uint64_t offset = 0;
    ...
    while (ossl_sframe_list_peek(&qrs->fl, &iter, &range, &data, &fin_)) {
        ...
        if (data == 
# 97 "../ssl/quic/quic_rstream.c" 3 4
                   ((void *)0)
# 97 "../ssl/quic/quic_rstream.c"
                       ) {
            size_t max_len;

            data = ring_buf_get_ptr(&qrs->rbuf, range.start, &max_len);
            if (!__builtin_expect(!!((data != 
# 101 "../ssl/quic/quic_rstream.c" 3 4
                ((void *)0)
# 101 "../ssl/quic/quic_rstream.c"
                ) != 0), 1))
                return 0;
            if (max_len < l) {
                ...
                data = ring_buf_get_ptr(&qrs->rbuf, range.start + max_len,
                                        &max_len);
                if (!__builtin_expect(!!((data != 
# 111 "../ssl/quic/quic_rstream.c" 3 4
                    ((void *)0)
# 111 "../ssl/quic/quic_rstream.c"
                    ) != 0), 1) || !__builtin_expect(!!((max_len > l) != 0), 1))
                    return 0;
            }
        }
...
static OSSL_TIME get_rtt(QUIC_RSTREAM *qrs)
{
    ...
    if (qrs->statm != 
# 141 "../ssl/quic/quic_rstream.c" 3 4
                     ((void *)0)
# 141 "../ssl/quic/quic_rstream.c"
                         ) {
                           ...
                           size_t *readbytes, int *fin)
{
    ...
    if (qrs->rxfc != 
# 160 "../ssl/quic/quic_rstream.c" 3 4
                    ((void *)0)
        
# 161 "../ssl/quic/quic_rstream.c"
...
{
    void *iter = 
# 175 "../ssl/quic/quic_rstream.c" 3 4
                ((void *)0)
# 175 "../ssl/quic/quic_rstream.c"
                    ;
    UINT_RANGE range;
    const unsigned char *data;
    uint64_t avail_ = 0;
...
int ossl_quic_rstream_get_record(QUIC_RSTREAM *qrs,
                                 ...
                                 int *fin)
{
    const unsigned char *record_ = 
# 195 "../ssl/quic/quic_rstream.c" 3 4
                                  ((void *)0)
# 195 "../ssl/quic/quic_rstream.c"
                                      ;
    ...
    if (!ossl_sframe_list_lock_head(&qrs->fl, &qrs->head_range, &record_, fin)) {
        ...
        *record = 
# 200 "../ssl/quic/quic_rstream.c" 3 4
                 ((void *)0)
# 200 "../ssl/quic/quic_rstream.c"
                     ;
        *rec_len = 0;
        return 1;
    }
    ...
    if (record_ == 
# 215 "../ssl/quic/quic_rstream.c" 3 4
                  ((void *)0) 
# 215 "../ssl/quic/quic_rstream.c"
                       && rec_len_ != 0) {
        record_ = ring_buf_get_ptr(&qrs->rbuf, qrs->head_range.start,
                                   &max_len);
        if (!__builtin_expect(!!((record_ != 
# 218 "../ssl/quic/quic_rstream.c" 3 4
            ((void *)0)
# 218 "../ssl/quic/quic_rstream.c"
            ) != 0), 1))
            return 0;
        if (max_len < rec_len_) {
            rec_len_ = max_len;
...
int ossl_quic_rstream_release_record(QUIC_RSTREAM *qrs, size_t read_len)
{
    ...
    if (read_len > qrs->head_range.end - qrs->head_range.start) {
        if (read_len != 
# 240 "../ssl/quic/quic_rstream.c" 3 4
                       (18446744073709551615UL)
# 240 "../ssl/quic/quic_rstream.c"
                               )
            return 0;
    ...
    if (qrs->rxfc != 
# 253 "../ssl/quic/quic_rstream.c" 3 4
                    ((void *)0)
# 253 "../ssl/quic/quic_rstream.c"
                        ) {
        OSSL_TIME rtt = get_rtt(qrs);
