...
LD="$BASE_ROOT/binutils-2.44/build/bin/ld"
...
usage() {
    cat <<EOF
Usage: $SCRIPT_NAME -p <target> [-j <jobs>] <-c | -b | -f > [-s <mode>]
...
Options:
  ...
  -s           Security test. (-s1 for xom compatibility test, -s2 for code disclosure test)
...
set_mode() {
    ...
    if [[ -n "$MODE" ]]; then
        die "options -c -b -f and -s are mutually exclusive"
    fi
...
build_gnutls() {
    local target="$1"
    local config_log="config_process.log"
    local -a library_options=(--disable-shared --enable-static)

    # Some tests override exported GnuTLS symbols and therefore must link
    # against the shared library rather than libgnutls.a.
    if [[ "$MODE" == "functional" ]]; then
        library_options=(--enable-shared --disable-static)
    fi
    ...
    # Keep the matching Nettle library ahead of any system library paths added by dependencies.
    ../configure --prefix="$PROJECT_DIR/$target/build-install"  --with-included-libtasn1 --with-included-unistring --without-p11-kit --disable-doc "${library_options[@]}" CC="$CC" CXX="$CXX" LD="$LD" CFLAGS="$EXTRA_CFLAGS -std=gnu11 -fPIC" LDFLAGS="-L$NETTLE_LIB $EXTRA_LDFLAGS -fPIC" > "$config_log" 2>&1
    check_config "$config_log"
...
while getopts ':p:j:cbfs:h' option; do
    case "$option" in
        p)
            TARGET="${OPTARG,,}"
            ;;
...
if [[ -z "$MODE" ]]; then
    die "one of -c, -b, -f or -s is required"
fi
...
run_standard_test() {
    ...
    case "$target" in
        ...
        libgcrypt-*)
            ...
            ;;
        boringssl-*)
            # before run test, go version should be correctted.
            make run_tests
            ;;
        gnutls-*)
            # env -u DEBUG LD_LIBRARY_PATH="/root/AsmXoM/workspace/projects/nettle/nettle-3.10.2/build/lib64" make check -j4
            env -u DEBUG LD_LIBRARY_PATH="$PROJECT_DIR/nettle/nettle-3.10.2/build/lib64" make check -j4
            ;;
        *)
            return 1
            ;;
...
run_compatibility_test() {
    ...
    case "$target" in
        openssl-*)
            $XOM_RUN ./apps/openssl speed sha1
            ;;
        tongsuo-*)
            $XOM_RUN ./apps/openssl speed sha1
            ;;
        libressl-*)
            $XOM_RUN make check TESTS=asn1test
            ;;
        libgcrypt-*)
            $XOM_RUN ./tests/benchmark md SHA256
            ;;
        boringssl-*)
            # before run test, go version should be correctted.
            $XOM_RUN ./tool/bssl speed -filter AES-256-GCM
            ;;
        gnutls-*)
            # LD_LIBRARY_PATH="/root/AsmXoM/workspace/projects/nettle/nettle-3.10.2/build/lib64"
            LD_LIBRARY_PATH="$PROJECT_DIR/nettle/nettle-3.10.2/build/lib64" $XOM_RUN ./src/gnutls-cli --benchmark-ciphers
            ;;
        *)
            return 1
            ;;
...
case "$MODE" in
    ...
    functional)
        ...
        log_step "Run standard tests for $PROJECT_FULL ..."
        run_standard_test "$PROJECT_FULL"
        ;;
    compatibility)
        EXTRA_CFLAGS="-O0 -Wa,--keep-locals -save-temps=obj"
        EXTRA_LDFLAGS="-Wl,-T,$LD_SCRIPT"
        EXTRA_ASM_FLAGS="-O0 -Wa,--keep-locals -save-temps=obj"
        compile_project "$PROJECT_FULL" "$JOBS"

        log_step "Run $PROJECT_FULL in XoM environment compiled by native GCC..."
        run_compatibility_test "$PROJECT_FULL"
        ...
        log_step "Run $PROJECT_FULL in XoM environment compiled by native AsmXoM..."
        run_compatibility_test "$PROJECT_FULL"
        ;;
esac
